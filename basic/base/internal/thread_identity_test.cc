...
static basic::base_internal::SpinLock map_lock(
...
static void TestThreadIdentityCurrent(const void* assert_no_identity) {
  ...
  // on Basic, then some previous action may have already allocated an
  // identity.
  ...
  basic::base_internal::SpinLockHolder l(&map_lock);
