...
TEST(EndianessTest, ghtonll_gntohll) {
  // Test that basic::ghtonl compiles correctly
  uint32_t test = 0x01234567;
  EXPECT_EQ(basic::gntohl(basic::ghtonl(test)), test);

  uint64_t comp = basic::ghtonll(kInitialNumber);
  EXPECT_EQ(comp, kInitialInNetworkOrder);
  comp = basic::gntohll(kInitialInNetworkOrder);
  ...
  uint64_t value = 1;
  for (int i = 0; i < 100; ++i) {
    comp = basic::ghtonll(basic::gntohll(value));
    EXPECT_EQ(value, comp);
    comp = basic::gntohll(basic::ghtonll(value));
    EXPECT_EQ(value, comp);
    value *= 37;
  }
}
