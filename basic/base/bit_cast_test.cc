...
void TestMarshall(const T values[], int num_values) {
  for (int i = 0; i < num_values; ++i) {
    T t0 = values[i];
    marshall<sizeof(T)> m0 = basic::bit_cast<marshall<sizeof(T)> >(t0);
    T t1 = basic::bit_cast<T>(m0);
    marshall<sizeof(T)> m1 = basic::bit_cast<marshall<sizeof(T)> >(t1);
...
void TestIntegral(const T values[], int num_values) {
  for (int i = 0; i < num_values; ++i) {
    T t0 = values[i];
    I i0 = basic::bit_cast<I>(t0);
    T t1 = basic::bit_cast<T>(i0);
    I i1 = basic::bit_cast<I>(t1);
    ASSERT_EQ(0, memcmp(&t0, &t1, sizeof(T)));
    ASSERT_EQ(i0, i1);
  }
}
