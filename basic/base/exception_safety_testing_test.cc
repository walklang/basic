...
TEST(ThrowingValueTest, ThrowingAllocatingOps) {
  ...
  TestAllocatingOp([]() { return basic::make_unique<ThrowingValue<>>(1); });
  TestAllocatingOp([]() { return basic::make_unique<ThrowingValue<>[]>(2); });
}
...
using Storage =
    basic::aligned_storage_t<sizeof(ThrowingValue<>), alignof(ThrowingValue<>)>;
...
struct NullaryTestValidator<
    TesterInstance,
    basic::void_t<decltype(std::declval<TesterInstance>().Test())>>
...
struct UnaryTestValidator<
    TesterInstance,
    basic::void_t<decltype(std::declval<TesterInstance>().Test(DummyOp))>>
    : public std::true_type {};
...
TEST(ExceptionSafetyTesterTest, IncompleteTypesAreNotTestable) {
  ...
  auto fac = []() { return basic::make_unique<T>(); };
...
std::unique_ptr<ExampleStruct> ExampleFunctionFactory() {
  return basic::make_unique<ExampleStruct>();
}
...
TEST(ExceptionCheckTest, NonCopyable) {
  auto factory = []() { return basic::make_unique<NonCopyable>(); };
...
TEST(ConstructorTrackerTest, NotDestroyedAfter) {
  basic::aligned_storage_t<sizeof(Tracked), alignof(Tracked)> storage;
  EXPECT_NONFATAL_FAILURE(
      {
...
TEST(ThrowingAllocatorTraitsTest, Assignablility) {
  EXPECT_TRUE(basic::is_move_assignable<ThrowingAllocator<int>>::value);
  EXPECT_TRUE(basic::is_copy_assignable<ThrowingAllocator<int>>::value);
