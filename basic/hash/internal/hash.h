...
// for `basic::Hash` should not rely on any parts of the state object other than
...
class HashStateBase {
 public:
  ...
  // updated state.
  //
  // Each of the value types `T` must be separately hashable by the Basic
  // hashing framework.
  //
  // NOTE:
  //
...
// is_hashable()
//
// Trait class which returns true if T is hashable by the basic::Hash framework.
...
H hash_tuple(H hash_state, const Tuple& t, basic::index_sequence<Is...>) {
...
H
#else  // _MSC_VER
typename std::enable_if<basic::conjunction<is_hashable<Ts>...>::value, H>::type
#endif  // _MSC_VER
AbslHashValue(H hash_state, const std::tuple<Ts...>& t) {
  return hash_internal::hash_tuple(std::move(hash_state), t,
                                   basic::make_index_sequence<sizeof...(Ts)>());
}
...
//      any allocator A)
//  - `basic::string_view` and `std::string_view`
//
...
template <typename H>
H AbslHashValue(H hash_state, basic::string_view str) {
  return H::combine(
...
// AbslHashValue for hashing basic::optional
template <typename H, typename T>
typename std::enable_if<is_hashable<T>::value, H>::type AbslHashValue(
    H hash_state, const basic::optional<T>& opt) {
...
// AbslHashValue for hashing basic::variant
template <typename H, typename... T>
typename std::enable_if<conjunction<is_hashable<T>...>::value, H>::type
AbslHashValue(H hash_state, const basic::variant<T...>& v) {
  if (!v.valueless_by_exception()) {
    hash_state = basic::visit(VariantVisitor<H>{std::move(hash_state)}, v);
  }
 ...
 private:
  ...
  struct UniquelyRepresentedProbe {
    ...
    static auto Invoke(H state, const T& value)
        -> basic::enable_if_t<is_uniquely_represented<T>::value, H> {
  ...
  struct HashValueProbe {
    template <typename H, typename T>
    static auto Invoke(H state, const T& value) -> basic::enable_if_t<
        std::is_same<H,
  ...
  struct StdHashProbe {
    template <typename H, typename T>
    static auto Invoke(H state, const T& value)
        -> basic::enable_if_t<type_traits_internal::IsHashable<T>::value, H> {
...
class CityHashState : public HashStateBase<CityHashState> {
  // basic::uint128 is not an alias or a thin wrapper around the intrinsic.
  // We use the intrinsic when available to improve performance.
#ifdef ABSL_HAVE_INTRINSIC_INT128
  using uint128 = __uint128_t;
#else   // ABSL_HAVE_INTRINSIC_INT128
  using uint128 = basic::uint128;
 ...
 public:
  ...
  template <typename T, basic::enable_if_t<IntegralFastPath<T>::value, int> = 0>
 ...
 private:
  ...
  ABSL_ATTRIBUTE_ALWAYS_INLINE static uint64_t Mix(uint64_t state, uint64_t v) {
    using MultType =
        basic::conditional_t<sizeof(size_t) == 4, uint64_t, uint128>;
...
inline uint64_t CityHashState::CombineContiguousImpl(
  ...
  uint64_t v;
  if (len > 8) {
    v = basic::hash_internal::CityHash32(reinterpret_cast<const char*>(first), len);
  } else if (len >= 4) {
...
inline uint64_t CityHashState::CombineContiguousImpl(
  ...
  uint64_t v;
  if (len > 16) {
    v = basic::hash_internal::CityHash64(reinterpret_cast<const char*>(first), len);
  } else if (len > 8) {
...
template <typename T>
struct Hash
    : basic::conditional_t<is_hashable<T>::value, HashImpl<T>, PoisonedHash> {};
