...
class SpyHashStateImpl : public HashStateBase<SpyHashStateImpl<T>> {
 public:
  SpyHashStateImpl() : error_(std::make_shared<basic::optional<std::string>>()) {
  ...
  static SpyHashStateImpl combine(SpyHashStateImpl s) {
    if (direct_basic_hash_value_error_) {
  ...
  static void SetDirectAbslHashValueError() {
    direct_basic_hash_value_error_ = true;
  }
  ...
  static CompareResult Compare(const SpyHashStateImpl& a,
    ...
    const std::string a_flat = basic::StrJoin(a.hash_representation_, "");
    const std::string b_flat = basic::StrJoin(b.hash_representation_, "");
    if (a_flat == b_flat) return CompareResult::kEqual;
    if (basic::EndsWith(a_flat, b_flat)) return CompareResult::kBSuffixA;
    if (basic::EndsWith(b_flat, a_flat)) return CompareResult::kASuffixB;
  ...
  friend std::ostream& operator<<(std::ostream& out,
    ...
    for (auto& s : hash_state.hash_representation_) {
      ...
      for (char c : s) {
        if (offset % 16 == 0) {
          out << basic::StreamFormat("\n0x%04x: ", offset);
        }
        if (offset % 2 == 0) {
          out << " ";
        }
        out << basic::StreamFormat("%02x", c);
        ++offset;
      }
      out << "\n";
    }
  ...
  basic::optional<std::string> error() const {
    if (moved_from_) {
 ...
 private:
  ...
  static bool direct_basic_hash_value_error_;
  ...
  std::shared_ptr<basic::optional<std::string>> error_;
  bool moved_from_ = false;
};

template <typename T>
bool SpyHashStateImpl<T>::direct_basic_hash_value_error_;
...
template <
    ...
    typename = basic::enable_if_t<!std::is_same<T, U>::value>,
