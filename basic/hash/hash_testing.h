...
// Run the basic::Hash algorithm over all the elements passed in and verify that
...
//     return basic::equal(x.p, x.p + x.size, y.p, y.p + y.size);
//   }
// };
//
...
struct PrintVisitor {
  ...
  std::string operator()(const T* value) const {
    return basic::StrCat("#", index, "(", testing::PrintToString(*value), ")");
  }
};
...
VerifyTypeImplementsAbslHashCorrectly(const Container& values, Eq equals) {
  ...
  struct Info {
    ...
    std::string ToString() const {
      return basic::visit(PrintVisitor{index}, value);
    }
    SpyHashState expand() const { return basic::visit(ExpandVisitor{}, value); }
  };
  ...
  for (const auto& value : values) {
    ...
    for (auto& eqclass : classes) {
      if (basic::visit(EqVisitor<Eq>{equals}, value, eqclass[0].value)) {
        c = &eqclass;
        break;
      }
    }
...
using VariantForTypes = typename MakeTypeSet<
    const typename std::decay<T>::type*...>::template apply<basic::variant>;

template <typename Container>
struct ContainerAsVector {
  using V = basic::variant<const typename Container::value_type*>;
...
struct ContainerAsVector<std::tuple<T...>> {
  ...
  template <size_t... I>
  static Out DoImpl(const std::tuple<T...>& tuple, basic::index_sequence<I...>) {
    return Out{&std::get<I>(tuple)...};
  }

  static Out Do(const std::tuple<T...>& values) {
    return DoImpl(values, basic::index_sequence_for<T...>());
  }
};
