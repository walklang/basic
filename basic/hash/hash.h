...
// -----------------------------------------------------------------------------
//
// This header file defines the Basic `hash` library and the Basic hashing
// framework. This framework consists of the following:
//
//   * The `basic::Hash` functor, which is used to invoke the hasher within the
//     Basic hashing framework. `basic::Hash<T>` supports most basic types and
//     a number of Basic types out of the box.
//   * `AbslHashValue`, an extension point that allows you to extend types to
//     support Basic hashing without requiring you to define a hashing
//     algorithm.
...
//
// Unlike `std::hash` or other hashing frameworks, the Basic hashing framework
// provides most of its utility by abstracting away the hash algorithm (and its
// implementation) entirely. Instead, a type invokes the Basic hashing
// framework by simply combining its state with the state of known, hashable
// types. Hashing of that combined state is separately done by `basic::Hash`.
//
// One should assume that a hash algorithm is chosen randomly at the start of
// each process.  E.g., basic::Hash<int>()(9) in one process and
// basic::Hash<int>()(9) in another process are likely to differ.
//
// Example:
//
...
//   };
//
// For more information, see Adding Type Support to `basic::Hash` below.
//
#ifndef ABSL_HASH_HASH_H_
#define ABSL_HASH_HASH_H_

#include "basic/hash/internal/hash.h"
...
// -----------------------------------------------------------------------------
//
// `basic::Hash<T>` is a convenient general-purpose hash functor for any type `T`
...
//
// `basic::Hash` intrinsically supports the following types:
//
...
//    hashable)
//  * basic types such as the following:
//    * basic::string_view
//    * basic::InlinedVector
//    * basic::FixedArray
//    * basic::uint128
//    * basic::Time, basic::Duration, and basic::TimeZone
//
// Note: the list above is not meant to be exhaustive. Additional type support
// may be added, in which case the above list will be updated.
//
// -----------------------------------------------------------------------------
// basic::Hash Invocation Evaluation
// -----------------------------------------------------------------------------
//
// When invoked, `basic::Hash<T>` searches for supplied hash functions in the
// following order:
//
//   * Natively supported types out of the box (see above)
//   * Types for which an `AbslHashValue()` overload is provided (such as
//     user-defined types). See "Adding Type Support to `basic::Hash`" below.
...
// -----------------------------------------------------------------------------
//
// The `basic::Hash` framework relies on the Concept of a "hash state." Such a
// hash state is used in several places:
//
// * Within existing implementations of `basic::Hash<T>` to store the hashed
...
//   types. (See "Adding Type Support to basic::Hash" below.)
// * Inside a `HashState`, providing type erasure for the concept of a hash
//   state, which you can use to extend the `basic::Hash` framework for types
...
//
// -----------------------------------------------------------------------------
// Adding Type Support to `basic::Hash`
// -----------------------------------------------------------------------------
//
...
//
// Note: unlike `std::hash', `basic::Hash` should never be specialized. It must
// only be extended by adding `AbslHashValue()` overloads.
//
template <typename T>
using Hash = basic::hash_internal::Hash<T>;
...
// but can use `basic::HashState::Create(&state)` to erase the type of the hash
...
//       value.HashValue(basic::HashState::Create(&state));
//       return state;
//     }
//    private:
//     virtual void HashValue(basic::HashState state) const = 0;
//   };
//
//   class Impl : Interface {
//    private:
//     void HashValue(basic::HashState state) const override {
//       basic::HashState::combine(std::move(state), v1_, v2_);
//     }
//     int v1_;
//     std::string v2_;
//   };
