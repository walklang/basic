...
#include "basic/container/flat_hash_set.h"
#include "basic/hash/hash_testing.h"
#include "basic/hash/internal/spy_hash_state.h"
...
using basic::Hash;
using basic::hash_internal::SpyHashState;
...
// Helper trait to verify if T is hashable. We use basic::Hash's poison status to
// detect it.
template <typename T>
using is_hashable = std::is_default_constructible<basic::Hash<T>>;
...
TYPED_TEST_P(HashValueIntTest, FastPath) {
  ...
  TypeParam n = 42;
  EXPECT_EQ(basic::Hash<TypeParam>{}(n),
            basic::Hash<std::tuple<TypeParam>>{}(std::tuple<TypeParam>(n)));
}
...
TEST(HashValueTest, EnumAndBool) {
  ...
  EXPECT_TRUE(basic::VerifyTypeImplementsAbslHashCorrectly(std::make_tuple(
...
TEST(HashValueTest, PointerAlignment) {
  ...
  for (size_t align = 1; align < kTotalSize / kNumValues;
    ...
    for (size_t i = 0; i < kNumValues; ++i) {
      size_t hash = basic::Hash<void*>()(data.get() + i * align);
      bits_or |= hash;
      bits_and &= hash;
    }
...
TEST(HashValueTest, Strings) {
  ...
  EXPECT_TRUE(basic::VerifyTypeImplementsAbslHashCorrectly(std::make_tuple(
      std::string(), basic::string_view(),
      std::string(""), basic::string_view(""),
      std::string(small), basic::string_view(small),
      std::string(dup), basic::string_view(dup),
      std::string(large), basic::string_view(large),
      std::string(huge), basic::string_view(huge))));

  // Also check that nested types maintain the same hash.
  const WrapInTuple t{};
  EXPECT_TRUE(basic::VerifyTypeImplementsAbslHashCorrectly(std::make_tuple(
      t(std::string()), t(basic::string_view()),
      t(std::string("")), t(basic::string_view("")),
      t(std::string(small)), t(basic::string_view(small)),
      t(std::string(dup)), t(basic::string_view(dup)),
      t(std::string(large)), t(basic::string_view(large)),
      t(std::string(huge)), t(basic::string_view(huge)))));

  // Make sure that hashing a `const char*` does not use its std::string-value.
  EXPECT_NE(SpyHash(static_cast<const char*>("ABC")),
            SpyHash(basic::string_view("ABC")));
}
...
// implentation is recursive within basic::Hash.
...
TEST(HashValueTest, Optional) {
  EXPECT_TRUE(is_hashable<basic::optional<Private>>::value);

  using O = basic::optional<Private>;
  EXPECT_TRUE(basic::VerifyTypeImplementsAbslHashCorrectly(
      std::make_tuple(O{}, O{{1}}, O{{-1}}, O{{10}})));
}

TEST(HashValueTest, Variant) {
  using V = basic::variant<Private, std::string>;
  ...
  struct S {};
  EXPECT_FALSE(is_hashable<basic::variant<S>>::value);
#endif
}
...
template <typename T>
struct IsHashCallable<T, basic::void_t<decltype(std::declval<basic::Hash<T>>()(
...
template <typename T>
struct IsAggregateInitializable<T, basic::void_t<decltype(T{})>>
    : std::true_type {};

TEST(IsHashableTest, ValidHash) {
  EXPECT_TRUE((is_hashable<int>::value));
  EXPECT_TRUE(std::is_default_constructible<basic::Hash<int>>::value);
  EXPECT_TRUE(std::is_copy_constructible<basic::Hash<int>>::value);
  EXPECT_TRUE(std::is_move_constructible<basic::Hash<int>>::value);
  EXPECT_TRUE(basic::is_copy_assignable<basic::Hash<int>>::value);
  EXPECT_TRUE(basic::is_move_assignable<basic::Hash<int>>::value);
  EXPECT_TRUE(IsHashCallable<int>::value);
  EXPECT_TRUE(IsAggregateInitializable<basic::Hash<int>>::value);
}
...
TEST(IsHashableTest, PoisonHash) {
  ...
  EXPECT_FALSE(std::is_default_constructible<basic::Hash<X>>::value);
  EXPECT_FALSE(std::is_copy_constructible<basic::Hash<X>>::value);
  EXPECT_FALSE(std::is_move_constructible<basic::Hash<X>>::value);
  EXPECT_FALSE(basic::is_copy_assignable<basic::Hash<X>>::value);
  EXPECT_FALSE(basic::is_move_assignable<basic::Hash<X>>::value);
  EXPECT_FALSE(IsHashCallable<X>::value);
  EXPECT_FALSE(IsAggregateInitializable<basic::Hash<X>>::value);
}
...
struct EnableIfContained
    : std::enable_if<basic::disjunction<
...
TEST(IsUniquelyRepresentedTest, SanityTest) {
  using basic::hash_internal::is_uniquely_represented;
...
struct TypeErased {
  ...
  friend H AbslHashValue(H hash_state, const TypeErased& v) {
    v.HashValue(basic::HashState::Create(&hash_state));
    return hash_state;
  }

  void HashValue(basic::HashState state) const {
    basic::HashState::combine(std::move(state), n);
  }
};
...
TEST(HashTest, DoesNotUseImplicitConversionsToBool) {
  EXPECT_NE(basic::Hash<ValueWithBoolConversion>()(ValueWithBoolConversion{0}),
            basic::Hash<ValueWithBoolConversion>()(ValueWithBoolConversion{1}));
}
