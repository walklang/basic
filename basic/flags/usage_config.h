...
// documents Basic's supported built-in usage flags. If these flags are found
// when parsing a command-line, Basic will exit the program and display
...
// -----------------------------------------------------------------------------
//
// Basic supports the following built-in usage flags. When passed, these flags
...
using FlagKindFilter = std::function<bool (basic::string_view)>;
}  // namespace flags_internal

// FlagsUsageConfig
//
// This structure contains the collection of callbacks for changing the behavior
// of the usage reporting routines in Basic Flags.
struct FlagsUsageConfig {
  ...
  // might produce
  //   "project/file.cc".
  std::function<std::string (basic::string_view)> normalize_filename;
};
...
void AbslInternalReportFatalUsageError(basic::string_view);
