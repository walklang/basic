...
TEST(MarshallingTest, TestBoolParsing) {
  ...
  // True values.
  EXPECT_TRUE(basic::ParseFlag("True", &value, &err));
  EXPECT_TRUE(value);
  EXPECT_TRUE(basic::ParseFlag("true", &value, &err));
  EXPECT_TRUE(value);
  EXPECT_TRUE(basic::ParseFlag("TRUE", &value, &err));
  ...
  EXPECT_TRUE(basic::ParseFlag("Yes", &value, &err));
  EXPECT_TRUE(value);
  EXPECT_TRUE(basic::ParseFlag("yes", &value, &err));
  EXPECT_TRUE(value);
  EXPECT_TRUE(basic::ParseFlag("YES", &value, &err));
  ...
  EXPECT_TRUE(basic::ParseFlag("t", &value, &err));
  EXPECT_TRUE(value);
  EXPECT_TRUE(basic::ParseFlag("T", &value, &err));
  EXPECT_TRUE(value);

  EXPECT_TRUE(basic::ParseFlag("y", &value, &err));
  EXPECT_TRUE(value);
  EXPECT_TRUE(basic::ParseFlag("Y", &value, &err));
  EXPECT_TRUE(value);

  EXPECT_TRUE(basic::ParseFlag("1", &value, &err));
  EXPECT_TRUE(value);

  // False values.
  EXPECT_TRUE(basic::ParseFlag("False", &value, &err));
  EXPECT_FALSE(value);
  EXPECT_TRUE(basic::ParseFlag("false", &value, &err));
  EXPECT_FALSE(value);
  EXPECT_TRUE(basic::ParseFlag("FALSE", &value, &err));
  ...
  EXPECT_TRUE(basic::ParseFlag("No", &value, &err));
  EXPECT_FALSE(value);
  EXPECT_TRUE(basic::ParseFlag("no", &value, &err));
  EXPECT_FALSE(value);
  EXPECT_TRUE(basic::ParseFlag("NO", &value, &err));
  ...
  EXPECT_TRUE(basic::ParseFlag("f", &value, &err));
  EXPECT_FALSE(value);
  EXPECT_TRUE(basic::ParseFlag("F", &value, &err));
  EXPECT_FALSE(value);

  EXPECT_TRUE(basic::ParseFlag("n", &value, &err));
  EXPECT_FALSE(value);
  EXPECT_TRUE(basic::ParseFlag("N", &value, &err));
  EXPECT_FALSE(value);

  EXPECT_TRUE(basic::ParseFlag("0", &value, &err));
  EXPECT_FALSE(value);
  ...
  // Invalid input.
  EXPECT_FALSE(basic::ParseFlag("", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("  ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\n", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\t", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("2", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("11", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("tt", &value, &err));
}
...
TEST(MarshallingTest, TestInt16Parsing) {
  ...
  EXPECT_TRUE(basic::ParseFlag("-1", &value, &err));
  EXPECT_EQ(value, -1);
  EXPECT_TRUE(basic::ParseFlag("123", &value, &err));
  EXPECT_EQ(value, 123);
  EXPECT_TRUE(basic::ParseFlag("-18765", &value, &err));
  EXPECT_EQ(value, -18765);
  EXPECT_TRUE(basic::ParseFlag("+3", &value, &err));
  EXPECT_EQ(value, 3);

  // Leading zero values.
  EXPECT_TRUE(basic::ParseFlag("01", &value, &err));
  EXPECT_EQ(value, 1);
  EXPECT_TRUE(basic::ParseFlag("-001", &value, &err));
  EXPECT_EQ(value, -1);
  EXPECT_TRUE(basic::ParseFlag("0000100", &value, &err));
  EXPECT_EQ(value, 100);

  // Hex values.
  EXPECT_TRUE(basic::ParseFlag("0x10", &value, &err));
  EXPECT_EQ(value, 16);
  EXPECT_TRUE(basic::ParseFlag("0X234", &value, &err));
  EXPECT_EQ(value, 564);
  // TODO(rogeeff): fix below validations
  EXPECT_FALSE(basic::ParseFlag("-0x7FFD", &value, &err));
  EXPECT_NE(value, -3);
  EXPECT_FALSE(basic::ParseFlag("+0x31", &value, &err));
  EXPECT_NE(value, 49);

  // Whitespace handling
  EXPECT_TRUE(basic::ParseFlag("10  ", &value, &err));
  EXPECT_EQ(value, 10);
  EXPECT_TRUE(basic::ParseFlag("  11", &value, &err));
  EXPECT_EQ(value, 11);
  EXPECT_TRUE(basic::ParseFlag("  012  ", &value, &err));
  EXPECT_EQ(value, 12);
  EXPECT_TRUE(basic::ParseFlag(" 0x22    ", &value, &err));
  EXPECT_EQ(value, 34);
  ...
  EXPECT_FALSE(basic::ParseFlag(" ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("  ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("40000", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("--1", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\n", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\t", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("2U", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("FFF", &value, &err));
}
...
TEST(MarshallingTest, TestUint16Parsing) {
  ...
  EXPECT_FALSE(basic::ParseFlag(" ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("  ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("70000", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("-1", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("--1", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\n", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\t", &value, &err));
...
TEST(MarshallingTest, TestInt32Parsing) {
  ...
  EXPECT_TRUE(basic::ParseFlag("123", &value, &err));
  EXPECT_EQ(value, 123);
  EXPECT_TRUE(basic::ParseFlag("-98765", &value, &err));
  ...
  EXPECT_EQ(value, 564);
  // TODO(rogeeff): fix below validations
  EXPECT_FALSE(basic::ParseFlag("-0x7FFFFFFD", &value, &err));
  EXPECT_NE(value, -3);
  EXPECT_FALSE(basic::ParseFlag("+0x31", &value, &err));
  EXPECT_NE(value, 49);
  ...
  EXPECT_FALSE(basic::ParseFlag(" ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("  ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("70000000000", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("--1", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\n", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\t", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("2U", &value, &err));
...
TEST(MarshallingTest, TestUint32Parsing) {
  ...
  EXPECT_TRUE(basic::ParseFlag("0X234", &value, &err));
  EXPECT_EQ(value, 564);
  EXPECT_TRUE(basic::ParseFlag("0xFFFFFFFD", &value, &err));
  ...
  EXPECT_FALSE(basic::ParseFlag(" ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("  ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("140000000000", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("-1", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("--1", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\n", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\t", &value, &err));
...
TEST(MarshallingTest, TestInt64Parsing) {
  ...
  EXPECT_TRUE(basic::ParseFlag("0x10", &value, &err));
  EXPECT_EQ(value, 16);
  EXPECT_TRUE(basic::ParseFlag("0XFFFAAABBBCCCDDD", &value, &err));
  EXPECT_EQ(value, 1152827684197027293);
  // TODO(rogeeff): fix below validation
  EXPECT_FALSE(basic::ParseFlag("-0x7FFFFFFFFFFFFFFE", &value, &err));
  EXPECT_NE(value, -2);
  EXPECT_FALSE(basic::ParseFlag("+0x31", &value, &err));
  EXPECT_NE(value, 49);
  ...
  EXPECT_TRUE(basic::ParseFlag("  012  ", &value, &err));
  EXPECT_EQ(value, 12);
  EXPECT_TRUE(basic::ParseFlag(" 0x7F    ", &value, &err));
  EXPECT_EQ(value, 127);
  ...
  EXPECT_FALSE(basic::ParseFlag(" ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("  ", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("0xFFFFFFFFFFFFFFFFFF", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("--1", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\n", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\t", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("2U", &value, &err));
...
TEST(MarshallingTest, TestUInt64Parsing) {
  ...
  EXPECT_TRUE(basic::ParseFlag("123", &value, &err));
  EXPECT_EQ(value, 123);
  EXPECT_TRUE(basic::ParseFlag("+13", &value, &err));
  EXPECT_EQ(value, 13);
  ...
  EXPECT_TRUE(basic::ParseFlag("001", &value, &err));
  EXPECT_EQ(value, 1);
  EXPECT_TRUE(basic::ParseFlag("0000300", &value, &err));
  EXPECT_EQ(value, 300);

  // Hex values.
  EXPECT_TRUE(basic::ParseFlag("0x10", &value, &err));
  EXPECT_EQ(value, 16);
  EXPECT_TRUE(basic::ParseFlag("0XFFFF", &value, &err));
...
TEST(MarshallingTest, TestFloatParsing) {
  ...
  // Ordinary values.
  EXPECT_TRUE(basic::ParseFlag("1.3", &value, &err));
  EXPECT_FLOAT_EQ(value, 1.3f);
  EXPECT_TRUE(basic::ParseFlag("-0.1", &value, &err));
  EXPECT_DOUBLE_EQ(value, -0.1f);
  EXPECT_TRUE(basic::ParseFlag("+0.01", &value, &err));
  EXPECT_DOUBLE_EQ(value, 0.01f);

  // Scientific values.
  EXPECT_TRUE(basic::ParseFlag("1.2e3", &value, &err));
  EXPECT_DOUBLE_EQ(value, 1.2e3f);
  EXPECT_TRUE(basic::ParseFlag("9.8765402e-37", &value, &err));
  EXPECT_DOUBLE_EQ(value, 9.8765402e-37f);
  EXPECT_TRUE(basic::ParseFlag("0.11e+3", &value, &err));
  EXPECT_DOUBLE_EQ(value, 0.11e+3f);
  EXPECT_TRUE(basic::ParseFlag("1.e-2300", &value, &err));
  EXPECT_DOUBLE_EQ(value, 0.f);
  EXPECT_TRUE(basic::ParseFlag("1.e+2300", &value, &err));
  EXPECT_TRUE(std::isinf(value));

  // Leading zero values.
  EXPECT_TRUE(basic::ParseFlag("01.6", &value, &err));
  EXPECT_DOUBLE_EQ(value, 1.6f);
  EXPECT_TRUE(basic::ParseFlag("000.0001", &value, &err));
  EXPECT_DOUBLE_EQ(value, 0.0001f);

  // Trailing zero values.
  EXPECT_TRUE(basic::ParseFlag("-5.1000", &value, &err));
  EXPECT_DOUBLE_EQ(value, -5.1f);

  // Exceptional values.
  EXPECT_TRUE(basic::ParseFlag("NaN", &value, &err));
  EXPECT_TRUE(std::isnan(value));
  EXPECT_TRUE(basic::ParseFlag("Inf", &value, &err));
  EXPECT_TRUE(std::isinf(value));

  // Hex values
  EXPECT_TRUE(basic::ParseFlag("0x10.23p12", &value, &err));
  EXPECT_DOUBLE_EQ(value, 66096.f);
  EXPECT_TRUE(basic::ParseFlag("-0xF1.A3p-2", &value, &err));
  EXPECT_NEAR(value, -60.4092f, 5e-5f);
  EXPECT_TRUE(basic::ParseFlag("+0x0.0AAp-12", &value, &err));
  EXPECT_NEAR(value, 1.01328e-05f, 5e-11f);
  EXPECT_TRUE(basic::ParseFlag("0x.01p1", &value, &err));
  EXPECT_NEAR(value, 0.0078125f, 5e-8f);

  // Whitespace handling
  EXPECT_TRUE(basic::ParseFlag("10.1  ", &value, &err));
  EXPECT_DOUBLE_EQ(value, 10.1f);
  EXPECT_TRUE(basic::ParseFlag("  2.34", &value, &err));
  EXPECT_DOUBLE_EQ(value, 2.34f);
  EXPECT_TRUE(basic::ParseFlag("  5.7  ", &value, &err));
  EXPECT_DOUBLE_EQ(value, 5.7f);
  EXPECT_TRUE(basic::ParseFlag("  -0xE0.F3p01  ", &value, &err));
  ...
  EXPECT_FALSE(basic::ParseFlag("\n", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("\t", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("2.3xxx", &value, &err));
  EXPECT_FALSE(basic::ParseFlag("0x0.1pAA", &value, &err));
  // TODO(rogeeff): below assertion should fail
  EXPECT_TRUE(basic::ParseFlag("0x0.1", &value, &err));
}
...
TEST(MarshallingTest, TestDoubleParsing) {
  ...
  EXPECT_TRUE(basic::ParseFlag("9.00000002e-123", &value, &err));
...
TEST(MarshallingTest, TestStringParsing) {
  ...
  EXPECT_TRUE(basic::ParseFlag("", &value, &err));
  EXPECT_EQ(value, "");
  EXPECT_TRUE(basic::ParseFlag(" ", &value, &err));
  EXPECT_EQ(value, " ");
  EXPECT_TRUE(basic::ParseFlag("   ", &value, &err));
  EXPECT_EQ(value, "   ");
  EXPECT_TRUE(basic::ParseFlag("\n", &value, &err));
  EXPECT_EQ(value, "\n");
  EXPECT_TRUE(basic::ParseFlag("\t", &value, &err));
  EXPECT_EQ(value, "\t");
  EXPECT_TRUE(basic::ParseFlag("asdfg", &value, &err));
  EXPECT_EQ(value, "asdfg");
  EXPECT_TRUE(basic::ParseFlag("asdf ghjk", &value, &err));
  EXPECT_EQ(value, "asdf ghjk");
  EXPECT_TRUE(basic::ParseFlag("a\nb\nc", &value, &err));
  EXPECT_EQ(value, "a\nb\nc");
  EXPECT_TRUE(basic::ParseFlag("asd\0fgh", &value, &err));
  EXPECT_EQ(value, "asd");
  EXPECT_TRUE(basic::ParseFlag("\\\\", &value, &err));
  EXPECT_EQ(value, "\\\\");
}
...
TEST(MarshallingTest, TestVectorOfStringParsing) {
  ...
  EXPECT_TRUE(basic::ParseFlag("a,b", &value, &err));
  EXPECT_EQ(value, std::vector<std::string>({"a", "b"}));
  EXPECT_TRUE(basic::ParseFlag("a,b,c,", &value, &err));
  EXPECT_EQ(value, std::vector<std::string>({"a", "b", "c", ""}));
  EXPECT_TRUE(basic::ParseFlag("a,,", &value, &err));
...
TEST(MarshallingTest, TestBoolUnparsing) {
  EXPECT_EQ(basic::UnparseFlag(true), "true");
  EXPECT_EQ(basic::UnparseFlag(false), "false");
}
...
TEST(MarshallingTest, TestInt16Unparsing) {
  ...
  value = 1;
  EXPECT_EQ(basic::UnparseFlag(value), "1");
  value = 0;
  EXPECT_EQ(basic::UnparseFlag(value), "0");
  value = -1;
  EXPECT_EQ(basic::UnparseFlag(value), "-1");
  value = 9876;
  EXPECT_EQ(basic::UnparseFlag(value), "9876");
  value = -987;
  EXPECT_EQ(basic::UnparseFlag(value), "-987");
}
...
TEST(MarshallingTest, TestUint16Unparsing) {
  ...
  EXPECT_EQ(basic::UnparseFlag(value), "0");
  value = 19876;
  EXPECT_EQ(basic::UnparseFlag(value), "19876");
}
...
TEST(MarshallingTest, TestInt32Unparsing) {
  ...
  EXPECT_EQ(basic::UnparseFlag(value), "-1");
  value = 12345;
  EXPECT_EQ(basic::UnparseFlag(value), "12345");
  value = -987;
  EXPECT_EQ(basic::UnparseFlag(value), "-987");
}
...
TEST(MarshallingTest, TestUint32Unparsing) {
  ...
  EXPECT_EQ(basic::UnparseFlag(value), "0");
  value = 1234500;
  EXPECT_EQ(basic::UnparseFlag(value), "1234500");
}
...
TEST(MarshallingTest, TestInt64Unparsing) {
  ...
  EXPECT_EQ(basic::UnparseFlag(value), "-1");
  value = 123456789L;
  EXPECT_EQ(basic::UnparseFlag(value), "123456789");
  value = -987654321L;
  EXPECT_EQ(basic::UnparseFlag(value), "-987654321");
  value = 0x7FFFFFFFFFFFFFFF;
  EXPECT_EQ(basic::UnparseFlag(value), "9223372036854775807");
...
TEST(MarshallingTest, TestUint64Unparsing) {
  ...
  EXPECT_EQ(basic::UnparseFlag(value), "123456789");
  value = 0xFFFFFFFFFFFFFFFF;
  EXPECT_EQ(basic::UnparseFlag(value), "18446744073709551615");
}
...
TEST(MarshallingTest, TestFloatUnparsing) {
  ...
  value = 1.1f;
  EXPECT_EQ(basic::UnparseFlag(value), "1.1");
  value = 0.01f;
  EXPECT_EQ(basic::UnparseFlag(value), "0.01");
  value = 1.23e-2f;
  EXPECT_EQ(basic::UnparseFlag(value), "0.0123");
  value = -0.71f;
  EXPECT_EQ(basic::UnparseFlag(value), "-0.71");
}
...
TEST(MarshallingTest, TestDoubleUnparsing) {
  ...
  EXPECT_EQ(basic::UnparseFlag(value), "0");
  value = std::nan("");
  EXPECT_EQ(basic::UnparseFlag(value), "nan");
  value = std::numeric_limits<double>::infinity();
  EXPECT_EQ(basic::UnparseFlag(value), "inf");
}

// --------------------------------------------------------------------

TEST(MarshallingTest, TestStringUnparsing) {
  EXPECT_EQ(basic::UnparseFlag(""), "");
  EXPECT_EQ(basic::UnparseFlag(" "), " ");
  EXPECT_EQ(basic::UnparseFlag("qwerty"), "qwerty");
  EXPECT_EQ(basic::UnparseFlag("ASDFGH"), "ASDFGH");
  EXPECT_EQ(basic::UnparseFlag("\n\t  "), "\n\t  ");
}
...
void TestRoundtrip(T v) {
  T new_v;
  std::string err;
  EXPECT_TRUE(basic::ParseFlag(basic::UnparseFlag(v), &new_v, &err));
  EXPECT_EQ(new_v, v);
}
