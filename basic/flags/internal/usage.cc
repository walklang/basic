...
class XMLElement {
 public:
  XMLElement(basic::string_view tag, basic::string_view txt)
      : tag_(tag), txt_(txt) {}
 ...
 private:
  basic::string_view tag_;
  basic::string_view txt_;
};
...
class FlagHelpPrettyPrinter {
 public:
  ...
  void Write(basic::string_view str, bool wrap_line = false) {
    // Empty std::string - do nothing.
    if (str.empty()) return;

    std::vector<basic::string_view> tokens;
    if (wrap_line) {
      tokens = basic::StrSplit(str, basic::ByAnyChar(" \f\n\r\t\v"),
                              basic::SkipEmpty());
    } else {
...
void FlagHelpHumanReadable(const flags_internal::CommandLineFlag& flag,
  ...
  // Flag name.
  printer.Write(basic::StrCat("-", flag.Name()));

  // Flag help.
  printer.Write(basic::StrCat("(", flag.Help(), ");"), /*wrap_line=*/true);

  // Flag data type (for V1 flags only).
  if (!flag.IsBasicFlag() && !flag.IsRetired()) {
    printer.Write(basic::StrCat("type: ", flag.Typename(), ";"));
  }
  ...
  if (flag.IsOfType<std::string>()) {
    dflt_val = basic::StrCat("\"", dflt_val, "\"");
  }
  printer.Write(basic::StrCat("default: ", dflt_val, ";"));

  if (flag.IsModified()) {
    std::string curr_val = flag.CurrentValue();
    if (flag.IsOfType<std::string>()) {
      curr_val = basic::StrCat("\"", curr_val, "\"");
    }
    printer.Write(basic::StrCat("currently: ", curr_val, ";"));
  }
...
void FlagsHelpImpl(std::ostream& out, flags_internal::FlagKindFilter filter_cb,
                   HelpFormat format, basic::string_view program_usage_message) {
  ...
  basic::string_view file_separator;  // controls blank lines between files.
...
void FlagsHelp(std::ostream& out, basic::string_view filter, HelpFormat format,
               basic::string_view program_usage_message) {
  flags_internal::FlagKindFilter filter_cb = [&](basic::string_view filename) {
    return filter.empty() || filename.find(filter) != basic::string_view::npos;
  };
...
int HandleUsageFlags(std::ostream& out,
                     basic::string_view program_usage_message) {
  if (basic::GetFlag(FLAGS_helpshort)) {
  ...
  if (basic::GetFlag(FLAGS_helpfull)) {
    // show all options
  ...
  if (!basic::GetFlag(FLAGS_helpon).empty()) {
    flags_internal::FlagsHelp(
        out, basic::StrCat("/", basic::GetFlag(FLAGS_helpon), "."),
        HelpFormat::kHumanReadable, program_usage_message);
    return 1;
  }

  if (!basic::GetFlag(FLAGS_helpmatch).empty()) {
    flags_internal::FlagsHelp(out, basic::GetFlag(FLAGS_helpmatch),
                              HelpFormat::kHumanReadable,
                              program_usage_message);
    return 1;
  }

  if (basic::GetFlag(FLAGS_help)) {
    flags_internal::FlagsHelpImpl(
  ...
  if (basic::GetFlag(FLAGS_helppackage)) {
    flags_internal::FlagsHelpImpl(
  ...
  if (basic::GetFlag(FLAGS_version)) {
  ...
  if (basic::GetFlag(FLAGS_only_check_args)) {
    return 0;
  }
