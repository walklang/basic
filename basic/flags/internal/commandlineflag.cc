...
// before including basic/flags/flag.h
...
basic::Mutex* InitFlag(CommandLineFlag* flag) {
  ABSL_CONST_INIT static basic::Mutex init_lock(basic::kConstInit);
  basic::Mutex* mu;

  {
    basic::MutexLock lock(&init_lock);
...
basic::Mutex* CommandLineFlag::InitFlagIfNecessary() const
...
void CommandLineFlag::Destroy() const {
  // Values are heap allocated for retired and Basic Flags.
  if (IsRetired() || IsBasicFlag()) {
...
bool CommandLineFlag::IsModified() const {
  basic::MutexLock l(InitFlagIfNecessary());
  return modified;
}
...
basic::string_view CommandLineFlag::Typename() const {
  // We do not store/report type in Basic Flags, so that user do not rely on in
  // at runtime
  if (IsBasicFlag() || IsRetired()) return "";
...
bool CommandLineFlag::SetValidatorFn(FlagValidator fn) {
  ...
  if (fn != nullptr && this->validator != nullptr) {
    ABSL_INTERNAL_LOG(
        WARNING, basic::StrCat("Ignoring SetValidatorFn() for flag '", Name(),
...
void CommandLineFlag::InvokeCallback()
  ...
  // exists.
  basic::Mutex* callback_mu = &this->locks->callback_mu;
  ...
  {
    basic::MutexLock lock(callback_mu);
    this->callback();
  }
...
// in 'err'
bool TryParseLocked(CommandLineFlag* flag, void* dst, basic::string_view value,
                    std::string* err)
  ...
  if (!Parse(flag->marshalling_op, value, tentative_value, &parse_err)) {
    ...
    basic::string_view err_sep = parse_err.empty() ? "" : "; ";
    basic::string_view typename_sep = type_name.empty() ? "" : " ";
    *err = basic::StrCat("Illegal value '", value, "' specified for",
  ...
  if (!flag->InvokeValidator(tentative_value)) {
    *err = basic::StrCat("Failed validation of new value '",
...
bool CommandLineFlag::SetFromString(basic::string_view value,
  ...
  switch (set_mode) {
    ...
    case SET_FLAG_IF_DEFAULT: {
      ...
      if (!this->modified) {
        ...
        // *err = basic::StrCat(this->Name(), " is already set to ",
        //                     CurrentValue(), "\n");
        // return false;
        return true;
      }
...
void CommandLineFlag::CheckDefaultValueParsingRoundtrip() const {
  ...
  basic::MutexLock lock(InitFlagIfNecessary());
  ...
  if (!flags_internal::Parse(this->marshalling_op, v, dst, &error)) {
    ABSL_INTERNAL_LOG(
        FATAL,
        basic::StrCat("Flag ", Name(), " (from ", Filename(),
...
bool CommandLineFlag::ValidateInputValue(basic::string_view value) const {
  basic::MutexLock l(InitFlagIfNecessary());  // protect default value access
...
void CommandLineFlag::Read(void* dst,
  ...
  basic::ReaderMutexLock l(InitFlagIfNecessary());
...
void CommandLineFlag::Write(const void* src,
  ...
  if (ShouldValidateFlagValue(*this)) {
    ...
    if (!Parse(marshalling_op, src_as_str, obj, &ignored_error) ||
        !InvokeValidator(obj)) {
      ABSL_INTERNAL_LOG(ERROR, basic::StrCat("Attempt to set flag '", name,
