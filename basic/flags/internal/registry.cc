...
void DestroyFlag(CommandLineFlag* flag) NO_THREAD_SAFETY_ANALYSIS {
  ...
  // CommandLineFlag handle object is heap allocated for non Basic Flags.
  if (!flag->IsBasicFlag()) {
    delete flag;
  }
}
...
class FlagRegistry {
 public:
  ...
  CommandLineFlag* FindFlagLocked(basic::string_view name);

  // Returns the retired flag object for the specified name, or nullptr if not
  // found or not retired.  Does not emit a warning.
  CommandLineFlag* FindRetiredFlagLocked(basic::string_view name);
 ...
 private:
  ...
  using FlagMap = std::map<basic::string_view, CommandLineFlag*>;
...
CommandLineFlag* FlagRegistry::FindFlagLocked(basic::string_view name) {
  ...
  if (i->second->IsRetired()) {
    flags_internal::ReportUsageError(
        basic::StrCat("Accessing retired flag '", name, "'"), false);
  }

  return i->second;
}

CommandLineFlag* FlagRegistry::FindRetiredFlagLocked(basic::string_view name) {
  FlagConstIterator i = flags_.find(name);
 ...
 public:
  ...
  void SaveFromRegistry() {
    ...
    flags_internal::ForEachFlag([&](flags_internal::CommandLineFlag* flag) {
      ...
      {
        basic::MutexLock l(flag->InitFlagIfNecessary());
  ...
  void RestoreToRegistry() {
    ...
    for (const SavedFlag& src : backup_registry_) {
      ...
      if (restored) {
        ...
        ABSL_INTERNAL_LOG(
            INFO, basic::StrCat("Restore saved value of ", flag->Name(), ": ",
...
struct FilenameFlagnameLess {
  bool operator()(const CommandLineFlagInfo& a,
    ...
    int cmp = basic::string_view(a.filename).compare(b.filename);
...
void FillCommandLineFlagInfo(CommandLineFlag* flag,
  ...
  basic::MutexLock l(flag->InitFlagIfNecessary());
  result->flag_ptr = flag->IsBasicFlag() ? nullptr : flag->cur;
}

// --------------------------------------------------------------------

CommandLineFlag* FindCommandLineFlag(basic::string_view name) {
...
CommandLineFlag* FindRetiredFlag(basic::string_view name) {
  FlagRegistry* const registry = FlagRegistry::GlobalRegistry();
  FlagRegistryLock frl(registry);
...
bool Retire(FlagOpFn ops, FlagMarshallingOpFn marshalling_ops,
  ...
  auto* flag = new CommandLineFlag(
      name,
      /*help_text=*/basic::flags_internal::HelpText::FromStaticCString(nullptr),
...
bool IsRetiredFlag(basic::string_view name, bool* type_is_bool) {
  assert(!name.empty());
