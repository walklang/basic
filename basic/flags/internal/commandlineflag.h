...
void* FlagMarshallingOps(FlagOp op, const void* v1, void* v2, void* v3) {
  switch (op) {
    case kParse: {
      ...
      if (!basic::ParseFlag<T>(*static_cast<const basic::string_view*>(v1), &temp,
    ...
    case kUnparse:
      *static_cast<std::string*>(v2) =
          basic::UnparseFlag<T>(*static_cast<const T*>(v1));
      return nullptr;
    default:
      return nullptr;
  }
...
inline bool Parse(FlagMarshallingOpFn op, basic::string_view text, void* dst,
                  std::string* error) {
...
struct CommandLineFlagLocks {
  basic::Mutex primary_mu;   // protects several fields in CommandLineFlag
  basic::Mutex callback_mu;  // used to serialize callbacks
};
...
// the legacy registration framework.  We can't use basic::variant<const char*,
...
struct CommandLineFlag {
  ...
  // basic::nullopt otherwise.
  template <typename T>
  basic::optional<T> Get() const {
    if (IsRetired() || flags_internal::FlagOps<T> != this->op)
      return basic::nullopt;
  ...
  bool SetFromString(basic::string_view value,
  ...
  // TODO(rogeeff): implement proper validators for Basic Flags
  bool ValidateDefaultValue() const;
  bool ValidateInputValue(basic::string_view value) const;
 ...
 protected:
  ...
  basic::Mutex* InitFlagIfNecessary() const;
  ...
  friend bool TryParseLocked(CommandLineFlag* flag, void* dst,
                             basic::string_view value, std::string* err);
  friend basic::Mutex* InitFlag(CommandLineFlag* flag);
};
