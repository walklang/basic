...
static std::string NormalizeFileName(basic::string_view fname) {
#ifdef _WIN32
  ...
  auto basic_pos = fname.find("/basic/");
  if (basic_pos != basic::string_view::npos) {
    fname = fname.substr(basic_pos + 1);
  }
...
class UsageReportingTest : public testing::Test {
 protected:
  UsageReportingTest() {
    ...
    basic::FlagsUsageConfig default_config;
    default_config.normalize_filename = &NormalizeFileName;
    basic::SetFlagsUsageConfig(default_config);
  }
...
TEST_F(UsageReportingDeathTest, TestSetProgramUsageMessage) {
  EXPECT_EQ(basic::ProgramUsageMessage(), kTestUsageMessage);

#ifndef _WIN32
  // TODO(rogeeff): figure out why this does not work on Windows.
  EXPECT_DEATH(basic::SetProgramUsageMessage("custom usage message"),
...
TEST_F(UsageReportingTest, TestFlagsHelpHRF) {
  ...
  Flags from basic/flags/internal/usage_test.cc:
  ...
  basic::string_view test_out_str(test_out);
  EXPECT_TRUE(
      basic::StartsWith(test_out_str, "usage_test: Custom usage message"));
  EXPECT_TRUE(basic::StrContains(
      test_out_str, "Flags from basic/flags/internal/usage_test.cc:"));
  EXPECT_TRUE(basic::StrContains(test_out_str,
                                "Flags from basic/flags/internal/usage.cc:"));
  EXPECT_TRUE(
      basic::StrContains(test_out_str, "-usage_reporting_test_flag_01 "));
  EXPECT_TRUE(basic::StrContains(test_out_str, "-help (show help"))
      << test_out_str;
}
...
TEST_F(UsageReportingTest, TestUsageFlag_helpshort) {
  basic::SetFlag(&FLAGS_helpshort, true);
...
TEST_F(UsageReportingTest, TestUsageFlag_helppackage) {
  basic::SetFlag(&FLAGS_helppackage, true);
...
TEST_F(UsageReportingTest, TestUsageFlag_version) {
  basic::SetFlag(&FLAGS_version, true);
...
TEST_F(UsageReportingTest, TestUsageFlag_only_check_args) {
  basic::SetFlag(&FLAGS_only_check_args, true);
...
TEST_F(UsageReportingTest, TestUsageFlag_helpon) {
  basic::SetFlag(&FLAGS_helpon, "bla-bla");
  ...
  basic::SetFlag(&FLAGS_helpon, "usage_test");
...
int main(int argc, char* argv[]) {
  basic::GetFlag(FLAGS_undefok);  // Force linking of parse.cc
  flags::SetProgramInvocationName("usage_test");
  basic::SetProgramUsageMessage(kTestUsageMessage);
