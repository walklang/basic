...
bool TstContainsHelpshortFlags(basic::string_view f) {
  return basic::StartsWith(flags::Basename(f), "progname.");
}

bool TstContainsHelppackageFlags(basic::string_view f) {
  return basic::EndsWith(flags::Package(f), "aaa/");
}

bool TstContainsHelpFlags(basic::string_view f) {
  return basic::EndsWith(flags::Package(f), "zzz/");
}

std::string TstVersionString() { return "program 1.0.0"; }

std::string TstNormalizeFilename(basic::string_view filename) {
  return std::string(filename.substr(2));
}

void TstReportUsageMessage(basic::string_view msg) {}
...
TEST_F(FlagsUsageConfigTest, TestGetSetFlagsUsageConfig) {
  ...
  basic::FlagsUsageConfig empty_config;
  ...
  basic::SetFlagsUsageConfig(empty_config);
