...
bool TestConstructionFor() {
  constexpr flags::Flag<T> f1("f1", &TestHelpMsg, "file",
                              &basic::flags_internal::FlagMarshallingOps<T>,
                              &TestMakeDflt<T>);
  ...
  ABSL_CONST_INIT static flags::Flag<T> f2(
      "f2", &TestHelpMsg, "file", &basic::flags_internal::FlagMarshallingOps<T>,
      &TestMakeDflt<T>);
...
bool AbslParseFlag(basic::string_view, UDT*, std::string*) { return true; }
...
TEST(FlagTest, TestFlagDefinition) {
  basic::string_view expected_file_name = "basic/flags/flag_test.cc";

  EXPECT_EQ(FLAGS_test_flag_01.Name(), "test_flag_01");
  EXPECT_EQ(FLAGS_test_flag_01.Help(), "test flag 01");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_01.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_02.Name(), "test_flag_02");
  EXPECT_EQ(FLAGS_test_flag_02.Help(), "test flag 02");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_02.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_03.Name(), "test_flag_03");
  EXPECT_EQ(FLAGS_test_flag_03.Help(), "test flag 03");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_03.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_04.Name(), "test_flag_04");
  EXPECT_EQ(FLAGS_test_flag_04.Help(), "test flag 04");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_04.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_05.Name(), "test_flag_05");
  EXPECT_EQ(FLAGS_test_flag_05.Help(), "test flag 05");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_05.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_06.Name(), "test_flag_06");
  EXPECT_EQ(FLAGS_test_flag_06.Help(), "test flag 06");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_06.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_07.Name(), "test_flag_07");
  EXPECT_EQ(FLAGS_test_flag_07.Help(), "test flag 07");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_07.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_08.Name(), "test_flag_08");
  EXPECT_EQ(FLAGS_test_flag_08.Help(), "test flag 08");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_08.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_09.Name(), "test_flag_09");
  EXPECT_EQ(FLAGS_test_flag_09.Help(), "test flag 09");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_09.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_10.Name(), "test_flag_10");
  EXPECT_EQ(FLAGS_test_flag_10.Help(), "test flag 10");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_10.Filename(), expected_file_name));

  EXPECT_EQ(FLAGS_test_flag_11.Name(), "test_flag_11");
  EXPECT_EQ(FLAGS_test_flag_11.Help(), "test flag 11");
  EXPECT_TRUE(
      basic::EndsWith(FLAGS_test_flag_11.Filename(), expected_file_name));
}
#endif  // !ABSL_FLAGS_STRIP_NAMES
...
TEST(FlagTest, TestDefault) {
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_01), true);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_02), 1234);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_03), -34);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_04), 189);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_05), 10765);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_06), 40000);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_07), -1234567);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_08), 9876543);
  EXPECT_NEAR(basic::GetFlag(FLAGS_test_flag_09), -9.876e-50, 1e-55);
  EXPECT_NEAR(basic::GetFlag(FLAGS_test_flag_10), 1.234e12f, 1e5f);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_11), "");
}

// --------------------------------------------------------------------

TEST(FlagTest, TestGetSet) {
  basic::SetFlag(&FLAGS_test_flag_01, false);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_01), false);

  basic::SetFlag(&FLAGS_test_flag_02, 321);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_02), 321);

  basic::SetFlag(&FLAGS_test_flag_03, 67);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_03), 67);

  basic::SetFlag(&FLAGS_test_flag_04, 1);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_04), 1);

  basic::SetFlag(&FLAGS_test_flag_05, -908);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_05), -908);

  basic::SetFlag(&FLAGS_test_flag_06, 4001);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_06), 4001);

  basic::SetFlag(&FLAGS_test_flag_07, -23456);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_07), -23456);

  basic::SetFlag(&FLAGS_test_flag_08, 975310);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_08), 975310);

  basic::SetFlag(&FLAGS_test_flag_09, 1.00001);
  EXPECT_NEAR(basic::GetFlag(FLAGS_test_flag_09), 1.00001, 1e-10);

  basic::SetFlag(&FLAGS_test_flag_10, -3.54f);
  EXPECT_NEAR(basic::GetFlag(FLAGS_test_flag_10), -3.54f, 1e-6f);

  basic::SetFlag(&FLAGS_test_flag_11, "asdf");
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_11), "asdf");
}
...
ABSL_FLAG(std::string, test_flag_13, basic::StrCat("AAA", "BBB"),
          "test flag 13");

namespace {

TEST(FlagTest, TestNonConstexprDefault) {
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_12), 1);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_13), "AAABBB");
}
...
ABSL_FLAG(bool, test_flag_14, true, basic::StrCat("test ", "flag ", "14"));
...
ABSL_FLAG(int, test_flag_with_lambda_cb, 200, "").OnUpdate([]() {
  cb_test_value = basic::GetFlag(FLAGS_test_flag_with_lambda_cb) +
                  basic::GetFlag(FLAGS_test_flag_with_cb);
});

namespace {

void TestFlagCB() { cb_test_value = basic::GetFlag(FLAGS_test_flag_with_cb); }

// Tests side-effects of callback invocation.
TEST(FlagTest, CallbackInvocation) {
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_with_cb), 100);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_with_lambda_cb), 200);
  EXPECT_EQ(cb_test_value, 300);

  basic::SetFlag(&FLAGS_test_flag_with_cb, 1);
  EXPECT_EQ(cb_test_value, 1);

  basic::SetFlag(&FLAGS_test_flag_with_lambda_cb, 3);
...
bool AbslParseFlag(basic::string_view in, CustomUDT* f, std::string*) {
  std::vector<basic::string_view> parts =
      basic::StrSplit(in, ':', basic::SkipWhitespace());

  if (parts.size() != 2) return false;

  if (!basic::SimpleAtoi(parts[0], &f->a)) return false;

  if (!basic::SimpleAtoi(parts[1], &f->b)) return false;

  return true;
}
std::string AbslUnparseFlag(const CustomUDT& f) {
  return basic::StrCat(f.a, ":", f.b);
}
...
TEST(FlagTest, TestCustomUDT) {
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_15), CustomUDT(1, 1));
  basic::SetFlag(&FLAGS_test_flag_15, CustomUDT(2, 3));
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_15), CustomUDT(2, 3));
}
...
TEST(Flagtest, TestTypeMismatchValidations) {
  ...
  EXPECT_DEBUG_DEATH(
      basic::GetFlag(FLAGS_mistyped_int_flag),
      "Flag 'mistyped_int_flag' is defined as one type and declared "
      "as another");
  EXPECT_DEATH(basic::SetFlag(&FLAGS_mistyped_int_flag, 0),
               "Flag 'mistyped_int_flag' is defined as one type and declared "
               "as another");

  EXPECT_DEATH(basic::GetFlag(FLAGS_mistyped_string_flag),
               "Flag 'mistyped_string_flag' is defined as one type and "
               "declared as another");
  EXPECT_DEATH(
      basic::SetFlag(&FLAGS_mistyped_string_flag, std::vector<std::string>{}),
...
bool AbslParseFlag(basic::string_view in, ConversionTestVal* val_out,
                   std::string*) {
  if (!basic::SimpleAtoi(in, &val_out->a)) {
    return false;
  }
  return true;
}
std::string AbslUnparseFlag(const ConversionTestVal& val) {
  return basic::StrCat(val.a);
}
...
TEST(FlagTest, CanSetViaImplicitConversion) {
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_16).a, 10);
  basic::SetFlag(&FLAGS_test_flag_16,
                ConversionTestVal::ViaImplicitConv::kEleven);
  EXPECT_EQ(basic::GetFlag(FLAGS_test_flag_16).a, 11);
}
...
bool AbslParseFlag(basic::string_view in, NonDfltConstructible* ndc_out,
                   std::string*) {
  return basic::SimpleAtoi(in, &ndc_out->value);
}
std::string AbslUnparseFlag(const NonDfltConstructible& ndc) {
  return basic::StrCat(ndc.value);
}
...
TEST(FlagTest, TestNonDefaultConstructibleType) {
  EXPECT_EQ(basic::GetFlag(FLAGS_ndc_flag1).value, '1' + 100);
  EXPECT_EQ(basic::GetFlag(FLAGS_ndc_flag2).value, 0);

  basic::SetFlag(&FLAGS_ndc_flag1, NonDfltConstructible('A'));
  basic::SetFlag(&FLAGS_ndc_flag2, 25);

  EXPECT_EQ(basic::GetFlag(FLAGS_ndc_flag1).value, 'A' + 100);
  EXPECT_EQ(basic::GetFlag(FLAGS_ndc_flag2).value, 25);
}
...
ABSL_RETIRED_FLAG(std::string, old_str_flag, "", basic::StrCat("old ", "descr"));
