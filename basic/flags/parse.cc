...
#include "basic/flags/internal/registry.h"
...
ABSL_CONST_INIT basic::Mutex processing_checks_guard(basic::kConstInit);
...
class ArgsList {
 public:
  ...
  basic::string_view Front() const { return args_[next_arg_]; }
...
bool ArgsList::ReadFromFlagfile(const std::string& flag_file_name) {
  ...
  if (!flag_file) {
    flags_internal::ReportUsageError(
        basic::StrCat("Can't open flagfile ", flag_file_name), true);
  ...
  while (std::getline(flag_file, line)) {
    basic::string_view stripped = basic::StripLeadingAsciiWhitespace(line);
    ...
    flags_internal::ReportUsageError(
        basic::StrCat("Unexpected line in the flagfile ", flag_file_name, ": ",
                     line),
        true);
...
//   "--foo"     -> {"foo", "", false}.
//   "--foo="    -> {"foo", "", true}.
std::tuple<basic::string_view, basic::string_view, bool> SplitNameAndValue(
    basic::string_view arg) {
  // Allow -foo and --foo
  basic::ConsumePrefix(&arg, "-");
  ...
  basic::string_view flag_name = arg.substr(0, equal_sign_pos);

  basic::string_view value;
  bool is_empty_value = false;

  if (equal_sign_pos != basic::string_view::npos) {
...
std::tuple<CommandLineFlag*, bool> LocateFlag(basic::string_view flag_name) {
  CommandLineFlag* flag = flags_internal::FindCommandLineFlag(flag_name);
  bool is_negative = false;

  if (!flag && basic::ConsumePrefix(&flag_name, "no")) {
...
bool ReadFlagsFromEnv(const std::vector<std::string>& flag_names,
  ...
  for (const auto& flag_name : flag_names) {
    ...
    if (flag_name == "fromenv" || flag_name == "tryfromenv") {
      flags_internal::ReportUsageError(
          basic::StrCat("Infinite recursion on flag ", flag_name), true);

      success = false;
      continue;
    }

    const std::string envname = basic::StrCat("FLAGS_", flag_name);
    std::string envval;
    if (!GetEnvVar(envname.c_str(), &envval)) {
      if (fail_on_absent_in_env) {
        flags_internal::ReportUsageError(
            basic::StrCat(envname, " not found in environment"), true);
    ...
    args.push_back(basic::StrCat("--", flag_name, "=", envval));
  }
...
bool HandleGeneratorFlags(std::vector<ArgsList>* input_args,
  ...
  basic::MutexLock l(&flags_internal::processing_checks_guard);
  ...
  if (flags_internal::flagfile_needs_processing) {
    auto flagfiles = basic::GetFlag(FLAGS_flagfile);
  ...
  if (flags_internal::fromenv_needs_processing) {
    auto flags_list = basic::GetFlag(FLAGS_fromenv);
  ...
  if (flags_internal::tryfromenv_needs_processing) {
    auto flags_list = basic::GetFlag(FLAGS_tryfromenv);
...
void ResetGeneratorFlags(const std::vector<std::string>& flagfile_value) {
  ...
  if (!flagfile_value.empty()) {
    basic::SetFlag(&FLAGS_flagfile, flagfile_value);
    basic::MutexLock l(&flags_internal::processing_checks_guard);
    flags_internal::flagfile_needs_processing = false;
  }

  // fromenv/tryfromenv are set to <undefined> value.
  if (!basic::GetFlag(FLAGS_fromenv).empty()) {
    basic::SetFlag(&FLAGS_fromenv, {});
  }
  if (!basic::GetFlag(FLAGS_tryfromenv).empty()) {
    basic::SetFlag(&FLAGS_tryfromenv, {});
  }
...
std::tuple<bool, basic::string_view> DeduceFlagValue(const CommandLineFlag& flag,
                                                    basic::string_view value,
                                                    bool is_negative,
                                                    bool is_empty_value,
                                                    ArgsList* curr_list) {
  ...
  if (flag.IsOfType<bool>()) {
    if (value.empty()) {
      ...
      // "--nobool_flag=Y" case
      flags_internal::ReportUsageError(
          basic::StrCat("Negative form with assignment is not valid for the "
                       "boolean flag '",
                       flag.Name(), "'"),
          true);
      return std::make_tuple(false, "");
    ...
    // "--noint_flag=1" case
    flags_internal::ReportUsageError(
        basic::StrCat("Negative form is not valid for the flag '", flag.Name(),
                     "'"),
        true);
    return std::make_tuple(false, "");
    ...
    if (curr_list->Size() == 1) {
      // "--int_flag" case
      flags_internal::ReportUsageError(
          basic::StrCat("Missing the value for the flag '", flag.Name(), "'"),
          true);
      return std::make_tuple(false, "");
    }
    ...
    if (value[0] == '-' && flag.IsOfType<std::string>()) {
      ...
      if (maybe_flag_name.empty() ||
        ...
        ABSL_INTERNAL_LOG(
            WARNING,
            basic::StrCat("Did you really mean to set flag '", flag.Name(),
                         "' to the value '", value, "'?"));
      }
    }
  }
...
bool CanIgnoreUndefinedFlag(basic::string_view flag_name) {
  auto undefok = basic::GetFlag(FLAGS_undefok);
  if (std::find(undefok.begin(), undefok.end(), flag_name) != undefok.end()) {
    return true;
  }

  if (basic::ConsumePrefix(&flag_name, "no") &&
...
std::vector<char*> ParseCommandLineImpl(int argc, char* argv[],
  ...
  while (!input_args.empty()) {
    ...
    basic::string_view arg(curr_list.Front());
    bool arg_from_argv = input_args.size() == 1;

    // 50. If argument does not start with - or is just "-" - this is
    // positional argument.
    if (!basic::ConsumePrefix(&arg, "-") || arg.empty()) {
  ...
  for (const auto& flag_name : undefined_flag_names) {
    ...
    flags_internal::ReportUsageError(
        basic::StrCat("Unknown command line flag '", flag_name.second, "'"),
        true);
