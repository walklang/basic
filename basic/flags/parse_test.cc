...
#include "basic/base/internal/scoped_set_env.h"
...
using basic::base_internal::ScopedSetEnv;
...
bool AbslParseFlag(basic::string_view in, UDT* udt, std::string* err) {
  if (in == "A") {
    udt->value = 1;
    return true;
  }
  ...
  static std::string* temp_dir_name = []() -> std::string* {
    ...
    if (res->empty()) {
      ...
      if (len < MAX_PATH && len != 0) {
        std::string temp_dir_name = basic::StrCat(
...
struct FlagfileData {
  const basic::string_view file_name;
  const basic::Span<const char* const> file_lines;
};
...
const char* GetFlagfileFlag(const std::vector<FlagfileData>& ffd,
  ...
  *flagfile_flag = "--flagfile=";
  basic::string_view separator;
  for (const auto& flagfile_data : ffd) {
    std::string flagfile_name =
        basic::StrCat(GetTestTempDir(), flagfile_data.file_name);

    std::ofstream flagfile_out(flagfile_name);
    for (auto line : flagfile_data.file_lines) {
      flagfile_out << basic::Substitute(line, GetTestTempDir()) << "\n";
    }

    basic::StrAppend(flagfile_flag, separator, flagfile_name);
    separator = ",";
  }
...
std::vector<char*> InvokeParse(const char* (&in_argv)[N]) {
  return basic::ParseCommandLine(N, const_cast<char**>(in_argv));
}
...
void TestParse(const char* (&in_argv)[N], int int_flag_value,
               double double_flag_val, basic::string_view string_flag_val,
  ...
  EXPECT_EQ(basic::GetFlag(FLAGS_int_flag), int_flag_value);
  EXPECT_NEAR(basic::GetFlag(FLAGS_double_flag), double_flag_val, 0.0001);
  EXPECT_EQ(basic::GetFlag(FLAGS_string_flag), string_flag_val);
  EXPECT_EQ(basic::GetFlag(FLAGS_bool_flag), bool_flag_val);
}
...
TEST_F(ParseTest, TestValidUDTArg) {
  ...
  EXPECT_EQ(basic::GetFlag(FLAGS_udt_flag).value, 1);

  const char* in_args2[] = {"testbin", "--udt_flag", "AAA"};
  InvokeParse(in_args2);

  EXPECT_EQ(basic::GetFlag(FLAGS_udt_flag).value, 10);
}
...
TEST_F(ParseTest, TestSimpleValidFlagfile) {
  ...
  const char* in_args1[] = {
      "testbin",
      GetFlagfileFlag({{"parse_test.ff1", basic::MakeConstSpan(ff1_data)}},
                      &flagfile_flag),
  };
  ...
  const char* in_args2[] = {
      "testbin",
      GetFlagfileFlag({{"parse_test.ff2", basic::MakeConstSpan(ff2_data)}},
                      &flagfile_flag),
  };
...
TEST_F(ParseTest, TestValidMultiFlagfile) {
  ...
  const char* in_args1[] = {
      "testbin",
      GetFlagfileFlag({{"parse_test.ff2", basic::MakeConstSpan(ff2_data)},
                       {"parse_test.ff1", basic::MakeConstSpan(ff1_data)}},
                      &flagfile_flag),
  };
  TestParse(in_args1, -1, 0.1, "q2w2  ", true);
}
...
TEST_F(ParseTest, TestFlagfileInFlagfile) {
  ...
  const char* in_args1[] = {
      "testbin",
      GetFlagfileFlag({{"parse_test.ff3", basic::MakeConstSpan(ff3_data)}},
                      &flagfile_flag),
  };
...
TEST_F(ParseDeathTest, TestInvalidFlagfiles) {
  ...
  const char* in_args1[] = {
      ...
      GetFlagfileFlag({{"parse_test.ff4",
                        basic::MakeConstSpan(ff4_data)}}, &flagfile_flag),
  };
  EXPECT_DEATH(InvokeParse(in_args1),
  ...
  const char* in_args2[] = {
      ...
      GetFlagfileFlag({{"parse_test.ff5",
                        basic::MakeConstSpan(ff5_data)}}, &flagfile_flag),
  };
  EXPECT_DEATH(InvokeParse(in_args2),
  ...
  const char* in_args3[] = {
      "testbin",
      GetFlagfileFlag({{"parse_test.ff6", basic::MakeConstSpan(ff6_data)}},
                      &flagfile_flag),
  };
  ...
  const char* in_args5[] = {
      "testbin",
      GetFlagfileFlag({{"parse_test.ff7", basic::MakeConstSpan(ff7_data)}},
                      &flagfile_flag),
  };
...
TEST_F(ParseTest, TestKeepParsedArgs) {
  ...
  EXPECT_THAT(
      out_args1,
      ElementsAreArray({basic::string_view("testbin"), basic::string_view("arg1"),
                        basic::string_view("arg2"), basic::string_view("arg3"),
                        basic::string_view("arg4")}));
  ...
  EXPECT_THAT(
      out_args2,
      ElementsAreArray({basic::string_view("testbin"),
                        basic::string_view("--bool_flag"),
                        basic::string_view("--int_flag=211"),
                        basic::string_view("--double_flag=1.1"),
                        basic::string_view("--string_flag"),
                        basic::string_view("asd"), basic::string_view("--"),
                        basic::string_view("arg1"), basic::string_view("arg2"),
                        basic::string_view("arg3"), basic::string_view("arg4")}));
}
...
TEST_F(ParseTest, TestIgnoreUndefinedFlags) {
  ...
  EXPECT_THAT(out_args1, ElementsAreArray({basic::string_view("testbin"),
                                           basic::string_view("arg1")}));

  EXPECT_EQ(basic::GetFlag(FLAGS_int_flag), 21);
  ...
  EXPECT_THAT(
      out_args2,
      ElementsAreArray(
          {basic::string_view("testbin"), basic::string_view("--undef_flag=aa"),
           basic::string_view("--string_flag=AA"), basic::string_view("arg1")}));

  EXPECT_EQ(basic::GetFlag(FLAGS_string_flag), "AA");
}
...
TEST_F(ParseDeathTest, TestHelpFlagHandling) {
  ...
  EXPECT_EQ(basic::GetFlag(FLAGS_int_flag), 3);
}
