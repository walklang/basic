...
// primitives which might use PerThreadSem, most notably basic::Mutex.
...
class PerThreadSemTest : public testing::Test {
 public:
  ...
  void TestTiming(const char *msg, bool timeout) {
    ...
    t.timeout = timeout ?
        KernelTimeout(basic::Now() + basic::Seconds(10000))  // far in the future
        : KernelTimeout::Never();
    ...
    std::string out = StrCat(
        ...
        basic::SixDigits(static_cast<double>(total_cycles) / kNumIterations));
    printf("%s\n", out.c_str());
...
TEST_F(PerThreadSemTest, Timeouts) {
  const basic::Duration delay = basic::Milliseconds(50);
  ...
  const basic::Duration slop = basic::Microseconds(200);
  EXPECT_LE(delay - slop, elapsed)
      << "Wait returned " << delay - elapsed
      << " early (with " << slop << " slop), start time was " << start;

  basic::Time negative_timeout = basic::UnixEpoch() - basic::Milliseconds(100);
  EXPECT_FALSE(Wait(negative_timeout));
  EXPECT_LE(negative_timeout, basic::Now() + slop);  // trivially true :)
