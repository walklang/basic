...
basic::Time DeadlineFromTimeout(basic::Duration timeout) {
...
// InfiniteFuture.
basic::Time LimitedDeadline(basic::Time deadline) {
  deadline = std::max(basic::FromTimeT(0), deadline);
  deadline = std::min(deadline, basic::FromTimeT(0x7fffffff));
  return deadline;
}
...
bool CondVarImpl::WaitWithDeadline(MutexImpl* mu, basic::Time deadline) {
  mu->released_.SignalAll();
  time_t when = ToTimeT(deadline);
  int64_t nanos = ToInt64Nanoseconds(deadline - basic::FromTimeT(when));
...
bool MutexImpl::AwaitWithDeadline(const Condition& cond, basic::Time deadline) {
...
bool CondVar::WaitWithTimeout(Mutex* mu, basic::Duration timeout) {
  return WaitWithDeadline(mu, basic::Now() + timeout);
}
