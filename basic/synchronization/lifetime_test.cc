...
#include <cstdlib>
#include <thread>  // NOLINT(build/c++11), Basic test
#include <type_traits>
...
#include "basic/synchronization/notification.h"
...
void ThreadOne(basic::Mutex* mutex, basic::CondVar* condvar,
               basic::Notification* notification, bool* state) {
...
void ThreadTwo(basic::Mutex* mutex, basic::CondVar* condvar,
               basic::Notification* notification, bool* state) {
  ABSL_RAW_CHECK(*state == false, "*state not initialized");
...
void RunTests(basic::Mutex* mutex, basic::CondVar* condvar) {
  basic::Mutex default_mutex;
  basic::CondVar default_condvar;
  basic::Notification notification;
  if (!mutex) {
    mutex = &default_mutex;
  }
  if (!condvar) {
...
void TestLocals() {
  basic::Mutex mutex;
  basic::CondVar condvar;
  RunTests(&mutex, &condvar);
}

// Normal kConstInit usage
ABSL_CONST_INIT basic::Mutex const_init_mutex(basic::kConstInit);
...
// the test runner.)
extern basic::Mutex early_const_init_mutex;
...
ABSL_CONST_INIT basic::Mutex early_const_init_mutex(basic::kConstInit);
...
extern basic::Mutex const_init_sanity_mutex;
OnConstruction grab_lock([]() NO_THREAD_SAFETY_ANALYSIS {
  const_init_sanity_mutex.Lock();
});
ABSL_CONST_INIT basic::Mutex const_init_sanity_mutex(basic::kConstInit);
...
// the test runner.)
extern basic::Mutex late_const_init_mutex;
...
ABSL_CONST_INIT basic::Mutex late_const_init_mutex(basic::kConstInit);
