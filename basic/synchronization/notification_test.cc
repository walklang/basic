...
static void BasicTests(bool notify_before_waiting, Notification* notification) {
  ...
  EXPECT_FALSE(
      notification->WaitForNotificationWithTimeout(basic::Milliseconds(0)));
  EXPECT_FALSE(notification->WaitForNotificationWithDeadline(basic::Now()));
  ...
  EXPECT_TRUE(notification->WaitForNotificationWithTimeout(basic::Seconds(0)));
  EXPECT_TRUE(notification->WaitForNotificationWithDeadline(basic::Now()));
