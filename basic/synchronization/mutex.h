...
//
// Unlike a `std::mutex`, the Basic `Mutex` provides the following additional
// features:
...
#include "basic/base/const_init.h"
#include "basic/base/internal/identity.h"
#include "basic/base/internal/low_level_alloc.h"
#include "basic/base/internal/thread_identity.h"
#include "basic/base/internal/tsan_mutex_interface.h"
#include "basic/base/port.h"
#include "basic/base/thread_annotations.h"
#include "basic/synchronization/internal/kernel_timeout.h"
#include "basic/synchronization/internal/per_thread_sem.h"
...
#include "basic/synchronization/internal/mutex_nonprod.inc"
#endif
...
class LOCKABLE Mutex {
 public:
  ...
  // `Mutex::Mutex(basic::kConstInit)` below instead.
  Mutex();

  // Creates a mutex with static storage duration.  A global variable
  // constructed this way avoids the lifetime issues that can occur on program
  // startup and shutdown.  (See basic/base/const_init.h.)
  //
  ...
  // Example usage:
  //   namespace foo {
  //   ABSL_CONST_INIT Mutex mu(basic::kConstInit);
  //   }
  explicit constexpr Mutex(basic::ConstInitType);
  ...
  bool AwaitWithTimeout(const Condition &cond, basic::Duration timeout);

  bool AwaitWithDeadline(const Condition &cond, basic::Time deadline);
  ...
  //
  // Negative timeouts are equivalent to a zero timeout.
  bool LockWhenWithTimeout(const Condition &cond, basic::Duration timeout)
      EXCLUSIVE_LOCK_FUNCTION();
  bool ReaderLockWhenWithTimeout(const Condition &cond, basic::Duration timeout)
      SHARED_LOCK_FUNCTION();
  bool WriterLockWhenWithTimeout(const Condition &cond, basic::Duration timeout)
      EXCLUSIVE_LOCK_FUNCTION() {
  ...
  //
  // Deadlines in the past are equivalent to an immediate deadline.
  bool LockWhenWithDeadline(const Condition &cond, basic::Time deadline)
      EXCLUSIVE_LOCK_FUNCTION();
  bool ReaderLockWhenWithDeadline(const Condition &cond, basic::Time deadline)
      SHARED_LOCK_FUNCTION();
  bool WriterLockWhenWithDeadline(const Condition &cond, basic::Time deadline)
      EXCLUSIVE_LOCK_FUNCTION() {
...
// the face of a throwing `Condition`. (When Basic is allowed to depend
...
class Condition {
 public:
  ...
  // `object->Method()`.
  //
  // Implementation Note: `basic::internal::identity` is used to allow methods to
  // come from base classes. A simpler signature like
  // `Condition(T*, bool (T::*)())` does not suffice.
  template<typename T>
  Condition(T *object, bool (basic::internal::identity<T>::type::* method)());

  // Same as above, for const members
  template<typename T>
  Condition(const T *object,
            bool (basic::internal::identity<T>::type::* method)() const);
...
class CondVar {
 public:
  ...
  //
  // Requires and ensures that the current thread holds the `Mutex`.
  bool WaitWithTimeout(Mutex *mu, basic::Duration timeout);
  ...
  //
  // Requires and ensures that the current thread holds the `Mutex`.
  bool WaitWithDeadline(Mutex *mu, basic::Time deadline);
...
#ifdef ABSL_INTERNAL_USE_NONPROD_MUTEX
inline constexpr Mutex::Mutex(basic::ConstInitType) : impl_(basic::kConstInit) {}
...
inline constexpr Mutex::Mutex(basic::ConstInitType) : mu_(0) {}
...
inline Condition::Condition(T *object,
                            bool (basic::internal::identity<T>::type::*method)())
...
// contended.  The callback is given the basic/base/cycleclock.h timestamp when
// waiting began.
//
...
// //basic/base/internal/cycleclock.h, and which may not be real
// "cycle" counts.)
//
...
// This has the same memory ordering concerns as RegisterMutexProfiler() above.
//
// DEPRECATED: The default symbolizer function is basic::Symbolize() and the
// ability to register a different hook for symbolizing stack traces will be
// removed on or after 2023-05-01.
ABSL_DEPRECATED("basic::RegisterSymbolizer() is deprecated and will be removed "
