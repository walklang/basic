...
#include "basic/synchronization/blocking_counter.h"
...
void BM_Mutex(benchmark::State& state) {
  static basic::Mutex* mu = new basic::Mutex;
  for (auto _ : state) {
...
static void DelayNs(int64_t ns, int* data) {
  int64_t end = basic::base_internal::CycleClock::Now() +
                ns * basic::base_internal::CycleClock::Frequency() / 1e9;
  while (basic::base_internal::CycleClock::Now() < end) {
    ++(*data);
...
BENCHMARK_TEMPLATE(BM_Contended, basic::Mutex)
    ->UseRealTime()
...
BENCHMARK_TEMPLATE(BM_Contended, basic::base_internal::SpinLock)
    ->UseRealTime()
    // ThreadPerCpu poorly handles non-power-of-two CPU counts.
    ->Threads(1)
    ->Threads(2)
...
void BM_ConditionWaiters(benchmark::State& state) {
  ...
  struct Helper {
    static void Waiter(basic::BlockingCounter* init, basic::Mutex* m, int* p) {
      init->DecrementCount();
      m->LockWhen(basic::Condition(
  ...
  basic::BlockingCounter init(num_waiters);
  basic::Mutex mu;
  std::vector<int> equivalence_classes(num_classes, 1);

  // Must be declared last to be destroyed first.
  basic::synchronization_internal::ThreadPool pool(num_waiters);
