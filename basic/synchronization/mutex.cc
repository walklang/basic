...
#include "basic/base/internal/atomic_hook.h"
#include "basic/base/internal/cycleclock.h"
#include "basic/base/internal/hide_ptr.h"
...
#include "basic/synchronization/internal/graphcycles.h"
#include "basic/synchronization/internal/per_thread_sem.h"
#include "basic/time/time.h"

using basic::base_internal::CurrentThreadIdentityIfPresent;
using basic::base_internal::PerThreadSynch;
using basic::base_internal::ThreadIdentity;
using basic::synchronization_internal::GetOrCreateCurrentThreadIdentity;
using basic::synchronization_internal::GraphCycles;
using basic::synchronization_internal::GraphId;
using basic::synchronization_internal::InvalidGraphId;
using basic::synchronization_internal::KernelTimeout;
using basic::synchronization_internal::PerThreadSem;
...
static struct MutexGlobals {
  MutexGlobals() {
    ...
    num_cpus = basic::base_internal::NumCPUs();
...
ABSL_CONST_INIT basic::base_internal::AtomicHook<void (*)(int64_t wait_cycles)>
    submit_profile_data;
ABSL_CONST_INIT basic::base_internal::AtomicHook<
...
static int Delay(int32_t c, DelayMode mode) {
  ...
  if (c < limit) {
    ...
    if (c == limit) {  // yield once
      ...
      basic::SleepFor(basic::Microseconds(10));
      c = 0;
    }
...
static basic::base_internal::SpinLock deadlock_graph_mu(
    basic::base_internal::kLinkerInitialized);
...
static basic::base_internal::SpinLock synch_event_mu(
    basic::base_internal::kLinkerInitialized);
...
static void PostSynchEvent(void *obj, int ev) {
  ...
  if (e == nullptr || e->log) {
    void *pcs[40];
    int n = basic::GetStackTrace(pcs, ABSL_ARRAYSIZE(pcs), 1);
...
static basic::Time DeadlineFromTimeout(basic::Duration timeout) {
#ifndef _WIN32
  struct timeval tv;
  gettimeofday(&tv, nullptr);
  return basic::TimeFromTimeval(tv) + timeout;
#else
  return basic::Now() + timeout;
#endif
}
...
static char *CurrentStackString(char *buf, int maxlen, bool symbolize) {
  void *pcs[40];
  return StackString(pcs, basic::GetStackTrace(pcs, ABSL_ARRAYSIZE(pcs), 2), buf,
                     maxlen, symbolize);
}
...
int GetStack(void** stack, int max_depth) {
  return basic::GetStackTrace(stack, max_depth, 3);
}
}  // anonymous namespace
...
static GraphId DeadlockCheck(Mutex *mu) {
  ...
  basic::base_internal::SpinLockHolder lock(&deadlock_graph_mu);
...
bool Mutex::LockWhenWithTimeout(const Condition &cond, basic::Duration timeout) {
  return LockWhenWithDeadline(cond, DeadlineFromTimeout(timeout));
}

bool Mutex::LockWhenWithDeadline(const Condition &cond, basic::Time deadline) {
  ABSL_TSAN_MUTEX_PRE_LOCK(this, 0);
  GraphId id = DebugOnlyDeadlockCheck(this);
...
bool Mutex::AwaitWithTimeout(const Condition &cond, basic::Duration timeout) {
  return AwaitWithDeadline(cond, DeadlineFromTimeout(timeout));
}

bool Mutex::AwaitWithDeadline(const Condition &cond, basic::Time deadline) {
  if (cond.Eval()) {      // condition already true; nothing to do
    if (kDebugMode) {
      this->AssertReaderHeld();
    }
...
bool CondVar::WaitWithTimeout(Mutex *mu, basic::Duration timeout) {
  return WaitWithDeadline(mu, DeadlineFromTimeout(timeout));
}

bool CondVar::WaitWithDeadline(Mutex *mu, basic::Time deadline) {
