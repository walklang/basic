...
#include "basic/base/internal/sysinfo.h"
#include "basic/memory/memory.h"
#include "basic/synchronization/internal/thread_pool.h"
...
std::unique_ptr<basic::synchronization_internal::ThreadPool> CreatePool(
    int threads) {
  return basic::make_unique<basic::synchronization_internal::ThreadPool>(threads);
}

std::unique_ptr<basic::synchronization_internal::ThreadPool>
CreateDefaultPool() {
...
static void ScheduleAfter(basic::synchronization_internal::ThreadPool *tp,
                          basic::Duration after,
                          const std::function<void()> &func) {
  tp->Schedule([func, after] {
    basic::SleepFor(after);
    func();
  });
}
...
static void TestR20ms(TestContext *cxt, int c) {
  for (int i = 0; i != cxt->iterations; i++) {
    basic::ReaderMutexLock l(&cxt->mu);
    basic::SleepFor(basic::Milliseconds(20));
...
static void TestRW(TestContext *cxt, int c) {
  if ((c & 1) == 0) {
    for (int i = 0; i != cxt->iterations; i++) {
      basic::WriterMutexLock l(&cxt->mu);
      cxt->g0++;
      cxt->g1--;
      cxt->mu.AssertHeld();
      cxt->mu.AssertReaderHeld();
...
static void TestAwait(TestContext *cxt, int c) {
  ...
  while (cxt->g0 < cxt->iterations) {
    cxt->mu.Await(basic::Condition(&mc, &MyContext::MyTurn));
...
static void TestCVTimeout(TestContext *cxt, int c) {
  ...
  while (cxt->g0 < cxt->iterations) {
    while (cxt->g0 != target && cxt->g0 != cxt->iterations) {
      cxt->cv.WaitWithTimeout(&cxt->mu, basic::Seconds(100));
    }
    if (cxt->g0 < cxt->iterations) {
      int a = cxt->g0 + 1;
      cxt->g0 = a;
...
static void TestTime(TestContext *cxt, int c, bool use_cv) {
  ...
  basic::Condition false_cond(&kFalse);
  basic::Condition g0ge2(G0GE2, cxt);
  if (c == 0) {
    basic::MutexLock l(&cxt->mu);

    basic::Time start = basic::Now();
    if (use_cv) {
      cxt->cv.WaitWithTimeout(&cxt->mu, basic::Seconds(1));
    } else {
      ABSL_RAW_CHECK(!cxt->mu.AwaitWithTimeout(false_cond, basic::Seconds(1)),
                     "TestTime failed");
    }
    basic::Duration elapsed = basic::Now() - start;
    ABSL_RAW_CHECK(
        basic::Seconds(0.9) <= elapsed && elapsed <= basic::Seconds(2.0),
        "TestTime failed");
    ...
    start = basic::Now();
    if (use_cv) {
      cxt->cv.WaitWithTimeout(&cxt->mu, basic::Seconds(4));
    } else {
      ABSL_RAW_CHECK(!cxt->mu.AwaitWithTimeout(false_cond, basic::Seconds(4)),
                     "TestTime failed");
    }
    elapsed = basic::Now() - start;
    ABSL_RAW_CHECK(
        basic::Seconds(3.9) <= elapsed && elapsed <= basic::Seconds(6.0),
        "TestTime failed");
    ...
    elapsed = basic::Now() - start;
    ABSL_RAW_CHECK(basic::Seconds(0.9) <= elapsed &&
                   elapsed <= basic::Seconds(2.0), "TestTime failed");
    ABSL_RAW_CHECK(cxt->g0 == cxt->threads, "TestTime failed");

  } else if (c == 1) {
    basic::MutexLock l(&cxt->mu);
    const basic::Time start = basic::Now();
    if (use_cv) {
      cxt->cv.WaitWithTimeout(&cxt->mu, basic::Milliseconds(500));
    } else {
      ABSL_RAW_CHECK(
          !cxt->mu.AwaitWithTimeout(false_cond, basic::Milliseconds(500)),
          "TestTime failed");
    }
    const basic::Duration elapsed = basic::Now() - start;
    ABSL_RAW_CHECK(
        basic::Seconds(0.4) <= elapsed && elapsed <= basic::Seconds(0.9),
        "TestTime failed");
    cxt->g0++;
  } else if (c == 2) {
    basic::MutexLock l(&cxt->mu);
    if (use_cv) {
      ...
      ABSL_RAW_CHECK(cxt->mu.AwaitWithTimeout(g0ge2, basic::Seconds(100)),
                     "TestTime failed");
    }
    cxt->g0++;
  } else {
...
static void EndTest(int *c0, int *c1, basic::Mutex *mu, basic::CondVar *cv,
                    const std::function<void(int)>& cb) {
  mu->Lock();
  int c = (*c0)++;
  mu->Unlock();
  cb(c);
  basic::MutexLock l(mu);
  (*c1)++;
  cv->Signal();
}
...
static int RunTestCommon(TestContext *cxt, void (*test)(TestContext *cxt, int),
  ...
  cxt->threads = threads;
  basic::synchronization_internal::ThreadPool tp(threads);
...
static int RunTestWithInvariantDebugging(void (*test)(TestContext *cxt, int),
                                         ...
                                         int operations,
                                         void (*invariant)(void *)) {
  basic::EnableMutexInvariantDebugging(true);
  SetInvariantChecked(false);
  TestContext cxt;
  cxt.mu.EnableInvariantDebugging(invariant, &cxt);
  int ret = RunTestCommon(&cxt, test, threads, iterations, operations);
  ABSL_RAW_CHECK(GetInvariantChecked(), "Invariant not checked");
  basic::EnableMutexInvariantDebugging(false);  // Restore.
  return ret;
}
#endif
...
static void WaitForA(TimeoutBugStruct *x) {
  x->mu.LockWhen(basic::Condition(&x->a));
  x->a_waiter_count--;
  x->mu.Unlock();
}
...
TEST(Mutex, CondVarWaitSignalsAwait) {
  ...
  pool->Schedule([&state] {
    ...
    state.release_mu.Await(basic::Condition(&state.release));
    state.released_cv.Signal();
    state.release_mu.Unlock();
  });

  state.barrier_mu.LockWhen(basic::Condition(&state.barrier));
  state.barrier_mu.Unlock();
  state.release_mu.Lock();
...
TEST(Mutex, CondVarWaitWithTimeoutSignalsAwait) {
  ...
  state.release = true;
  EXPECT_TRUE(
      !state.released_cv.WaitWithTimeout(&state.release_mu, basic::Seconds(10)))
      << "; Unrecoverable test failure: CondVar::WaitWithTimeout did not "
         "unblock the basic::Mutex::Await call in another thread.";
...
TEST(Mutex, MutexTimeoutBug) {
  ...
  tp->Schedule(std::bind(&WaitForA, &x));
  basic::SleepFor(basic::Seconds(1));  // Allow first two threads to hang.
  ...
  x.mu.LockWhenWithTimeout(basic::Condition(&always_false),
                           basic::Milliseconds(500));

  // if we get here, the bug is not present.   Cleanup the state.

  x.a = true;                                    // wakeup the two waiters on A
  x.mu.Await(basic::Condition(&NoAWaiters, &x));  // wait for them to exit
  x.mu.Unlock();
}
...
struct CondVarWaitDeadlock : testing::TestWithParam<int> {
  ...
  void Waiter2() {
    if (read_lock2) {
      mu.ReaderLockWhen(basic::Condition(&cond2));
      mu.ReaderUnlock();
    } else {
      mu.LockWhen(basic::Condition(&cond2));
      mu.Unlock();
    }
  }
};
...
TEST_P(CondVarWaitDeadlock, Test) {
  ...
  basic::SleepFor(basic::Milliseconds(100));
...
struct DequeueAllWakeableBugStruct {
  basic::Mutex mu;
  basic::Mutex mu2;       // protects all fields below
...
static void AcquireAsReader(DequeueAllWakeableBugStruct *x) {
  ...
  basic::SleepFor(basic::Seconds(2));
  x->mu.ReaderUnlock();
...
TEST(Mutex, MutexReaderWakeupBug) {
  ...
  tp->Schedule(std::bind(&AcquireAsReader, &x));
  basic::SleepFor(basic::Seconds(1));  // give time for reader threads to block
  x.mu.Unlock();                     // wake them up

  // both readers should finish promptly
  EXPECT_TRUE(
      x.mu2.LockWhenWithTimeout(basic::Condition(&x.done1), basic::Seconds(10)));
  x.mu2.Unlock();

  EXPECT_TRUE(
      x.mu2.LockWhenWithTimeout(basic::Condition(&x.done2), basic::Seconds(10)));
  x.mu2.Unlock();
}
...
static void LockWhenTestWaitForIsCond(LockWhenTestStruct* s) {
  s->mu1.LockWhen(basic::Condition(&LockWhenTestIsCond, s));
  s->mu1.Unlock();
}
...
TEST(Mutex, LockWhen) {
  ...
  s.mu2.LockWhen(basic::Condition(&s.waiting));
  s.mu2.Unlock();
...
static void WaitForCond(ReaderDecrementBugStruct *x) {
  basic::Mutex dummy;
  basic::MutexLock l(&dummy);
  x->mu.LockWhen(basic::Condition(&IsCond, x));
  x->done--;
  x->mu.Unlock();
}
...
static void GetReadLock(ReaderDecrementBugStruct *x) {
  ...
  x->mu2.Await(basic::Condition(&x->complete));
  x->mu2.Unlock();
  x->mu.ReaderUnlock();
  x->mu.Lock();
  x->done--;
...
TEST(Mutex, MutexReaderDecrementBug) NO_THREAD_SAFETY_ANALYSIS {
  ...
  x.mu2.LockWhen(basic::Condition(&x.waiting_on_cond));
  x.mu2.Unlock();

  // Run GetReadLock(), and wait for it to get the read lock
  std::thread thread2(GetReadLock, &x);
  x.mu2.LockWhen(basic::Condition(&x.have_reader_lock));
  x.mu2.Unlock();
  ...
  x.mu.Lock();
  x.cond = true;
  x.mu.Await(basic::Condition(&AllDone, &x));
  x.mu.Unlock();
...
TEST(Mutex, LockedMutexDestructionBug) NO_THREAD_SAFETY_ANALYSIS {
  ...
  for (int i = 0; i != 10; i++) {
    ...
    auto mu = basic::make_unique<basic::Mutex[]>(kNumLocks);
...
static void ReaderForReaderOnCondVar(basic::Mutex *mu, basic::CondVar *cv,
                                     int *running) {
  ...
  mu->ReaderLock();
  while (*running == 3) {
    basic::SleepFor(basic::Milliseconds(random_millis(gen)));
    cv->WaitWithTimeout(mu, basic::Milliseconds(random_millis(gen)));
  }
  mu->ReaderUnlock();
  mu->Lock();
  (*running)--;
...
TEST(Mutex, FunctorCondition) {
  {  // Variadic
    True f;
    EXPECT_TRUE(basic::Condition(&f).Eval());
  }

  {  // Inherited
    DerivedTrue g;
    EXPECT_TRUE(basic::Condition(&g).Eval());
  }

  {  // lambda
    int value = 3;
    auto is_zero = [&value] { return value == 0; };
    basic::Condition c(&is_zero);
    EXPECT_FALSE(c.Eval());
    value = 0;
    EXPECT_TRUE(c.Eval());
  }
...
TEST(Mutex, TestReaderOnCondVar) {
  ...
  mu.Lock();
  running--;
  mu.Await(basic::Condition(&IntIsZero, &running));
  mu.Unlock();
}

// --------------------------------------------------------
struct AcquireFromConditionStruct {
  basic::Mutex mu0;   // protects value, done
  int value;         // times condition function is called; under mu0,
  bool done;         // done with test?  under mu0
  basic::Mutex mu1;   // used to attempt to mess up state of mu0
  basic::CondVar cv;  // so the condition function can be invoked from
                     // CondVar::Wait().
};
...
static bool ConditionWithAcquire(AcquireFromConditionStruct *x) {
  ...
  if (x->value == 2 || x->value == 3) {
    ...
    bool always_false = false;
    x->mu1.LockWhenWithTimeout(basic::Condition(&always_false),
                               basic::Milliseconds(100));
    x->mu1.Unlock();
  }
...
static void WaitForCond2(AcquireFromConditionStruct *x) {
  ...
  x->mu0.LockWhen(basic::Condition(&ConditionWithAcquire, x));
  x->done = true;
  x->mu0.Unlock();
}
...
TEST(Mutex, AcquireFromCondition) {
  ...
  // return false.
  basic::SleepFor(basic::Milliseconds(500));  // allow T time to hang

  x.mu0.Lock();
  x.cv.WaitWithTimeout(&x.mu0, basic::Milliseconds(500));  // wake T
  ...
  x.mu0.LockWhen(basic::Condition(&x.done));
  x.mu0.Unlock();
}
...
TEST(Mutex, DeadlockDetector) {
  basic::SetMutexDeadlockDetectionMode(basic::OnDeadlockCycle::kAbort);
...
#endif
  basic::SetMutexDeadlockDetectionMode(basic::OnDeadlockCycle::kReport);
...
TEST(Mutex, DeadlockDetectorStessTest) NO_THREAD_SAFETY_ANALYSIS {
  ...
  auto array_of_locks = basic::make_unique<basic::Mutex[]>(n_locks);
...
TEST(Mutex, DeadlockIdBug) NO_THREAD_SAFETY_ANALYSIS {
  ...
  basic::SetMutexDeadlockDetectionMode(basic::OnDeadlockCycle::kIgnore);
  delete a;
  basic::SetMutexDeadlockDetectionMode(basic::OnDeadlockCycle::kAbort);
...
// basic::Duration and basic::Time.  For each waiting function we test with
...
static basic::Duration TimeoutTestAllowedSchedulingDelay() {
  // Note: we use a function here because Microsoft Visual Studio fails to
  // properly initialize constexpr static basic::Duration variables.
  return basic::Milliseconds(150);
}
...
static bool DelayIsWithinBounds(basic::Duration expected_delay,
                                basic::Duration actual_delay) {
  bool pass = true;
  ...
  // different clock than basic::Now(), but these cases should be handled by the
  // the retry mechanism in each TimeoutTest.
  if (actual_delay < expected_delay) {
    ABSL_RAW_LOG(WARNING,
                 "Actual delay %s was too short, expected %s (difference %s)",
                 basic::FormatDuration(actual_delay).c_str(),
                 basic::FormatDuration(expected_delay).c_str(),
                 basic::FormatDuration(actual_delay - expected_delay).c_str());
    pass = false;
  }
  ...
  basic::Duration tolerance = expected_delay <= basic::ZeroDuration()
                                 ? basic::Milliseconds(10)
...
struct TimeoutTestParam {
  ...
  // Should the absolute deadline API based on basic::Time be tested?  If false,
  // the relative deadline API based on basic::Duration is tested.
  bool use_absolute_deadline;

  // The deadline/timeout used when calling the API being tested
  // (e.g. Mutex::LockWhenWithDeadline).
  basic::Duration wait_timeout;
  ...
  // true.
  basic::Duration satisfy_condition_delay;
...
static void RunAfterDelay(basic::Duration delay,
                          basic::synchronization_internal::ThreadPool *pool,
                          const std::function<void()> &callback) {
  if (delay <= basic::ZeroDuration()) {
    callback();  // immediate
  } else if (delay != basic::InfiniteDuration()) {
...
std::vector<TimeoutTestParam> MakeTimeoutTestParamValues() {
  ...
  const basic::Duration finite = 3 * TimeoutTestAllowedSchedulingDelay();
  const basic::Duration never = basic::InfiniteDuration();
  const basic::Duration negative = -basic::InfiniteDuration();
  const basic::Duration immediate = basic::ZeroDuration();
...
TEST_P(TimeoutTest, Await) {
  ...
  for (int attempt = 1;; ++attempt) {
    ...
    std::unique_ptr<basic::synchronization_internal::ThreadPool> pool =
        CreateDefaultPool();
    ...
    basic::MutexLock lock(&mu);
    basic::Time start_time = basic::Now();
    basic::Condition cond(&value);
    bool result =
    ...
    if (DelayIsWithinBounds(params.expected_delay, basic::Now() - start_time)) {
...
TEST_P(TimeoutTest, ReaderLockWhen) {
  ...
  for (int attempt = 0;; ++attempt) {
    ...
    bool result =
        params.use_absolute_deadline
            ? mu.ReaderLockWhenWithDeadline(basic::Condition(&value),
                                            start_time + params.wait_timeout)
            : mu.ReaderLockWhenWithTimeout(basic::Condition(&value),
                                           params.wait_timeout);
    mu.ReaderUnlock();
...
TEST_P(TimeoutTest, Wait) {
  ...
  for (int attempt = 0;; ++attempt) {
    ...
    basic::Mutex mu;
    bool value = false;  // condition value (under mu)
    basic::CondVar cv;    // signals a change of `value`
    ...
    basic::MutexLock lock(&mu);
    basic::Time start_time = basic::Now();
    basic::Duration timeout = params.wait_timeout;
    basic::Time deadline = start_time + timeout;
    while (!value) {
      ...
      timeout = deadline - basic::Now();  // recompute
    }
...
TEST(Mutex, Logging) {
  ...
  basic::Mutex logged_mutex;
  logged_mutex.EnableDebugLog("fido_mutex");
  basic::CondVar logged_cv;
  logged_cv.EnableDebugLog("rover_cv");
  logged_mutex.Lock();
  logged_cv.WaitWithTimeout(&logged_mutex, basic::Milliseconds(20));
  logged_mutex.Unlock();
