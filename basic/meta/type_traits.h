...
// Basic provides any guarantee that APIs are stable in the face of template
...
struct IsTriviallyMoveAssignableReference<T&>
    : basic::is_trivially_move_assignable<T>::type {};
...
//
// NOTE: `basic::void_t` does not use the standard-specified implementation so
...
struct is_trivially_copy_assignable
    : std::integral_constant<
          bool, __has_trivial_assign(typename std::remove_reference<T>::type) &&
                    basic::is_copy_assignable<T>::value> {
#ifdef ABSL_HAVE_STD_IS_TRIVIALLY_ASSIGNABLE
 private:
  static constexpr bool compliant =
...
//
// We expose this metafunction only for internal use within basic.
template <typename T>
class is_trivially_copyable_impl {
  ...
  static constexpr bool kIsCopyOrMoveAssignable =
      basic::is_copy_assignable<ExtentsRemoved>::value ||
      basic::is_move_assignable<ExtentsRemoved>::value;
...
struct IsHashable<
    Key,
    basic::enable_if_t<std::is_convertible<
...
struct AssertHashEnabledHelper {
 private:
  ...
  static std::nullptr_t DoIt() {
    ...
    static_assert(basic::is_copy_assignable<std::hash<Key>>::value,
...
// This declaration prevents global `swap` and `basic::swap` overloads from being
...
template <class T>
struct IsSwappable
    : basic::type_traits_internal::is_detected<IsSwappableImpl, T> {};
...
struct IsNothrowSwappable
    : basic::type_traits_internal::is_detected<IsNothrowSwappableImpl, T> {};

// Swap()
//
// Performs the swap idiom from a namespace where valid candidates may only be
// found in `std` or via ADL.
template <class T, basic::enable_if_t<IsSwappable<T>::value, int> = 0>
