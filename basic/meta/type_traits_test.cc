...
struct TypeWithBarFunction {
  template <class T,
            basic::enable_if_t<std::is_same<T&&, StructA&>::value, int> = 0>
...
using BarIsCallable =
    basic::type_traits_internal::is_detected<BarIsCallableImpl, Class, T...>;

template <class Class, class... T>
using BarIsCallableConv = basic::type_traits_internal::is_detected_convertible<
...
TEST(VoidTTest, BasicUsage) {
  StaticAssertTypeEq<void, basic::void_t<Dummy>>();
  StaticAssertTypeEq<void, basic::void_t<Dummy, Dummy, Dummy>>();
}

TEST(ConjunctionTest, BasicBooleanLogic) {
  EXPECT_TRUE(basic::conjunction<>::value);
  EXPECT_TRUE(basic::conjunction<std::true_type>::value);
  EXPECT_TRUE((basic::conjunction<std::true_type, std::true_type>::value));
  EXPECT_FALSE((basic::conjunction<std::true_type, std::false_type>::value));
  EXPECT_FALSE((basic::conjunction<std::false_type, std::true_type>::value));
  EXPECT_FALSE((basic::conjunction<std::false_type, std::false_type>::value));
}
...
TEST(ConjunctionTest, ShortCircuiting) {
  EXPECT_FALSE(
      (basic::conjunction<std::true_type, std::false_type, Dummy>::value));
  EXPECT_TRUE((std::is_base_of<MyFalseType,
                               basic::conjunction<std::true_type, MyFalseType,
                                                 std::false_type>>::value));
  EXPECT_TRUE(
      (std::is_base_of<MyTrueType,
                       basic::conjunction<std::true_type, MyTrueType>>::value));
}

TEST(DisjunctionTest, BasicBooleanLogic) {
  EXPECT_FALSE(basic::disjunction<>::value);
  EXPECT_FALSE(basic::disjunction<std::false_type>::value);
  EXPECT_TRUE((basic::disjunction<std::true_type, std::true_type>::value));
  EXPECT_TRUE((basic::disjunction<std::true_type, std::false_type>::value));
  EXPECT_TRUE((basic::disjunction<std::false_type, std::true_type>::value));
  EXPECT_FALSE((basic::disjunction<std::false_type, std::false_type>::value));
}

TEST(DisjunctionTest, ShortCircuiting) {
  EXPECT_TRUE(
      (basic::disjunction<std::false_type, std::true_type, Dummy>::value));
  EXPECT_TRUE((
      std::is_base_of<MyTrueType, basic::disjunction<std::false_type, MyTrueType,
                                                    std::true_type>>::value));
  EXPECT_TRUE((
      std::is_base_of<MyFalseType,
                      basic::disjunction<std::false_type, MyFalseType>>::value));
}

TEST(NegationTest, BasicBooleanLogic) {
  EXPECT_FALSE(basic::negation<std::true_type>::value);
  EXPECT_FALSE(basic::negation<MyTrueType>::value);
  EXPECT_TRUE(basic::negation<std::false_type>::value);
  EXPECT_TRUE(basic::negation<MyFalseType>::value);
}
...
// nontrivial destructor. Since basic::is_trivially_default_constructible and
// basic::is_trivially_copy_constructible both follows Clang/GCC's interpretation
...
TEST(TypeTraitsTest, TestTrivialDestructor) {
  ...
  EXPECT_TRUE(basic::is_trivially_destructible<bool>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<char>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<unsigned char>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<signed char>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<wchar_t>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<int>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<unsigned int>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<int16_t>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<uint16_t>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<int64_t>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<uint64_t>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<float>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<double>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<long double>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<const std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<const Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<std::string**>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<Trivial**>::value);

  // classes with destructors
  EXPECT_TRUE(basic::is_trivially_destructible<Trivial>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<TrivialDestructor>::value);

  // Verify that types with a nontrivial or deleted destructor
  // are marked as such.
  EXPECT_FALSE(basic::is_trivially_destructible<NontrivialDestructor>::value);
#ifdef ABSL_TRIVIALLY_DESTRUCTIBLE_CONSIDER_DELETED_DESTRUCTOR_NOT_TRIVIAL
  EXPECT_FALSE(basic::is_trivially_destructible<DeletedDestructor>::value);
#endif

  // simple_pair of such types is trivial
  EXPECT_TRUE((basic::is_trivially_destructible<simple_pair<int, int>>::value));
  EXPECT_TRUE((basic::is_trivially_destructible<
               simple_pair<Trivial, TrivialDestructor>>::value));

  // Verify that types without trivial destructors are correctly marked as such.
  EXPECT_FALSE(basic::is_trivially_destructible<std::string>::value);
  EXPECT_FALSE(basic::is_trivially_destructible<std::vector<int>>::value);

  // Verify that simple_pairs of types without trivial destructors
  // are not marked as trivial.
  EXPECT_FALSE((basic::is_trivially_destructible<
  ...
  using int10 = int[10];
  EXPECT_TRUE(basic::is_trivially_destructible<int10>::value);
  using Trivial10 = Trivial[10];
  EXPECT_TRUE(basic::is_trivially_destructible<Trivial10>::value);
  using TrivialDestructor10 = TrivialDestructor[10];
  EXPECT_TRUE(basic::is_trivially_destructible<TrivialDestructor10>::value);

  // Conversely, the opposite also holds.
  using NontrivialDestructor10 = NontrivialDestructor[10];
  EXPECT_FALSE(basic::is_trivially_destructible<NontrivialDestructor10>::value);
}

TEST(TypeTraitsTest, TestTrivialDefaultCtor) {
  // arithmetic types and pointers have trivial default constructors.
  EXPECT_TRUE(basic::is_trivially_default_constructible<bool>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<char>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<unsigned char>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<signed char>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<wchar_t>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<int>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<unsigned int>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<int16_t>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<uint16_t>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<int64_t>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<uint64_t>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<float>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<double>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<long double>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<Trivial*>::value);
  EXPECT_TRUE(
      basic::is_trivially_default_constructible<const std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<const Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<std::string**>::value);
  EXPECT_TRUE(basic::is_trivially_default_constructible<Trivial**>::value);
  ...
  EXPECT_TRUE(basic::is_trivially_default_constructible<Trivial>::value);
  EXPECT_TRUE(
      basic::is_trivially_default_constructible<TrivialDefaultCtor>::value);

  // Verify that types without them are not.
  EXPECT_FALSE(
      basic::is_trivially_default_constructible<NontrivialDefaultCtor>::value);
  EXPECT_FALSE(
      basic::is_trivially_default_constructible<DeletedDefaultCtor>::value);

#ifdef ABSL_TRIVIALLY_CONSTRUCTIBLE_VERIFY_TRIVIALLY_DESTRUCTIBLE
  // types with nontrivial destructor are nontrivial
  EXPECT_FALSE(
      basic::is_trivially_default_constructible<NontrivialDestructor>::value);
#endif

  // types with vtables
  EXPECT_FALSE(basic::is_trivially_default_constructible<Base>::value);

  // Verify that simple_pair has trivial constructors where applicable.
  EXPECT_TRUE((basic::is_trivially_default_constructible<
  ...
  EXPECT_FALSE(basic::is_trivially_default_constructible<std::string>::value);
  EXPECT_FALSE(
      basic::is_trivially_default_constructible<std::vector<int>>::value);

  // Verify that simple_pairs of types without trivial constructors
  // are not marked as trivial.
  EXPECT_FALSE((basic::is_trivially_default_constructible<
                simple_pair<int, std::string>>::value));
  EXPECT_FALSE((basic::is_trivially_default_constructible<
                simple_pair<std::string, int>>::value));
  ...
  using int10 = int[10];
  EXPECT_TRUE(basic::is_trivially_default_constructible<int10>::value);
  using Trivial10 = Trivial[10];
  EXPECT_TRUE(basic::is_trivially_default_constructible<Trivial10>::value);
  using TrivialDefaultCtor10 = TrivialDefaultCtor[10];
  EXPECT_TRUE(
      basic::is_trivially_default_constructible<TrivialDefaultCtor10>::value);

  // Conversely, the opposite also holds.
#ifdef ABSL_GCC_BUG_TRIVIALLY_CONSTRUCTIBLE_ON_ARRAY_OF_NONTRIVIAL
  using NontrivialDefaultCtor10 = NontrivialDefaultCtor[10];
  EXPECT_FALSE(
      basic::is_trivially_default_constructible<NontrivialDefaultCtor10>::value);
#endif
}
...
TEST(TypeTraitsTest, TestTrivialMoveCtor) {
  ...
  // constructors.
  EXPECT_TRUE(basic::is_trivially_move_constructible<bool>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<char>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<unsigned char>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<signed char>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<wchar_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<int>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<unsigned int>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<int16_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<uint16_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<int64_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<uint64_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<float>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<double>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<long double>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<const std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<const Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<std::string**>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<Trivial**>::value);
  ...
  EXPECT_TRUE(basic::is_trivially_move_constructible<Trivial>::value);
  EXPECT_TRUE(basic::is_trivially_move_constructible<TrivialMoveCtor>::value);

  // Verify that types without them (i.e. nontrivial or deleted) are not.
  EXPECT_FALSE(
      basic::is_trivially_move_constructible<NontrivialCopyCtor>::value);
  EXPECT_FALSE(basic::is_trivially_move_constructible<DeletedCopyCtor>::value);
  EXPECT_FALSE(
      basic::is_trivially_move_constructible<NonCopyableOrMovable>::value);

#ifdef ABSL_TRIVIALLY_CONSTRUCTIBLE_VERIFY_TRIVIALLY_DESTRUCTIBLE
  // type with nontrivial destructor are nontrivial move construbtible
  EXPECT_FALSE(
      basic::is_trivially_move_constructible<NontrivialDestructor>::value);
#endif

  // types with vtables
  EXPECT_FALSE(basic::is_trivially_move_constructible<Base>::value);

  // Verify that simple_pair of such types is trivially move constructible
  EXPECT_TRUE(
      (basic::is_trivially_move_constructible<simple_pair<int, char*>>::value));
  EXPECT_TRUE((
      basic::is_trivially_move_constructible<simple_pair<int, Trivial>>::value));
  EXPECT_TRUE((basic::is_trivially_move_constructible<
               simple_pair<int, TrivialMoveCtor>>::value));

  // Verify that types without trivial move constructors are
  // correctly marked as such.
  EXPECT_FALSE(basic::is_trivially_move_constructible<std::string>::value);
  EXPECT_FALSE(basic::is_trivially_move_constructible<std::vector<int>>::value);

  // Verify that simple_pairs of types without trivial move constructors
  // are not marked as trivial.
  EXPECT_FALSE((basic::is_trivially_move_constructible<
                simple_pair<int, std::string>>::value));
  EXPECT_FALSE((basic::is_trivially_move_constructible<
                simple_pair<std::string, int>>::value));
  ...
  using int10 = int[10];
  EXPECT_FALSE(basic::is_trivially_move_constructible<int10>::value);
}

TEST(TypeTraitsTest, TestTrivialCopyCtor) {
  // Verify that arithmetic types and pointers have trivial copy
  // constructors.
  EXPECT_TRUE(basic::is_trivially_copy_constructible<bool>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<char>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<unsigned char>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<signed char>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<wchar_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<int>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<unsigned int>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<int16_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<uint16_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<int64_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<uint64_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<float>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<double>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<long double>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<const std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<const Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<std::string**>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<Trivial**>::value);

  // Reference types
  EXPECT_TRUE(basic::is_trivially_copy_constructible<int&>::value);
  EXPECT_FALSE(basic::is_trivially_copy_constructible<int&&>::value);

  // types with compiler generated copy ctors
  EXPECT_TRUE(basic::is_trivially_copy_constructible<Trivial>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<TrivialCopyCtor>::value);

  // Verify that types without them (i.e. nontrivial or deleted) are not.
  EXPECT_FALSE(
      basic::is_trivially_copy_constructible<NontrivialCopyCtor>::value);
  EXPECT_FALSE(basic::is_trivially_copy_constructible<DeletedCopyCtor>::value);
  EXPECT_FALSE(
      basic::is_trivially_copy_constructible<MovableNonCopyable>::value);
  EXPECT_FALSE(
      basic::is_trivially_copy_constructible<NonCopyableOrMovable>::value);

#ifdef ABSL_TRIVIALLY_CONSTRUCTIBLE_VERIFY_TRIVIALLY_DESTRUCTIBLE
  // type with nontrivial destructor are nontrivial copy construbtible
  EXPECT_FALSE(
      basic::is_trivially_copy_constructible<NontrivialDestructor>::value);
#endif

  // types with vtables
  EXPECT_FALSE(basic::is_trivially_copy_constructible<Base>::value);

  // Verify that simple_pair of such types is trivially copy constructible
  EXPECT_TRUE(
      (basic::is_trivially_copy_constructible<simple_pair<int, char*>>::value));
  EXPECT_TRUE((
      basic::is_trivially_copy_constructible<simple_pair<int, Trivial>>::value));
  EXPECT_TRUE((basic::is_trivially_copy_constructible<
               simple_pair<int, TrivialCopyCtor>>::value));

  // Verify that types without trivial copy constructors are
  // correctly marked as such.
  EXPECT_FALSE(basic::is_trivially_copy_constructible<std::string>::value);
  EXPECT_FALSE(basic::is_trivially_copy_constructible<std::vector<int>>::value);

  // Verify that simple_pairs of types without trivial copy constructors
  // are not marked as trivial.
  EXPECT_FALSE((basic::is_trivially_copy_constructible<
                simple_pair<int, std::string>>::value));
  EXPECT_FALSE((basic::is_trivially_copy_constructible<
                simple_pair<std::string, int>>::value));
  ...
  // Verify that arrays are not
  using int10 = int[10];
  EXPECT_FALSE(basic::is_trivially_copy_constructible<int10>::value);
}

TEST(TypeTraitsTest, TestTrivialMoveAssign) {
  // Verify that arithmetic types and pointers have trivial move
  // assignment operators.
  EXPECT_TRUE(basic::is_trivially_move_assignable<bool>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<char>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<unsigned char>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<signed char>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<wchar_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<int>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<unsigned int>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<int16_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<uint16_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<int64_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<uint64_t>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<float>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<double>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<long double>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<const std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<const Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<std::string**>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<Trivial**>::value);

  // const qualified types are not assignable
  EXPECT_FALSE(basic::is_trivially_move_assignable<const int>::value);

  // types with compiler generated move assignment
  EXPECT_TRUE(basic::is_trivially_move_assignable<Trivial>::value);
  EXPECT_TRUE(basic::is_trivially_move_assignable<TrivialMoveAssign>::value);

  // Verify that types without them (i.e. nontrivial or deleted) are not.
  EXPECT_FALSE(basic::is_trivially_move_assignable<NontrivialCopyAssign>::value);
  EXPECT_FALSE(basic::is_trivially_move_assignable<DeletedCopyAssign>::value);
  EXPECT_FALSE(basic::is_trivially_move_assignable<NonCopyableOrMovable>::value);

  // types with vtables
  EXPECT_FALSE(basic::is_trivially_move_assignable<Base>::value);

  // Verify that simple_pair is trivially assignable
  EXPECT_TRUE(
      (basic::is_trivially_move_assignable<simple_pair<int, char*>>::value));
  EXPECT_TRUE(
      (basic::is_trivially_move_assignable<simple_pair<int, Trivial>>::value));
  EXPECT_TRUE((basic::is_trivially_move_assignable<
               simple_pair<int, TrivialMoveAssign>>::value));

  // Verify that types not trivially move assignable are
  // correctly marked as such.
  EXPECT_FALSE(basic::is_trivially_move_assignable<std::string>::value);
  EXPECT_FALSE(basic::is_trivially_move_assignable<std::vector<int>>::value);

  // Verify that simple_pairs of types not trivially move assignable
  // are not marked as trivial.
  EXPECT_FALSE((basic::is_trivially_move_assignable<
                simple_pair<int, std::string>>::value));
  EXPECT_FALSE((basic::is_trivially_move_assignable<
                simple_pair<std::string, int>>::value));
  ...
  using int10 = int[10];
  EXPECT_FALSE(basic::is_trivially_move_assignable<int10>::value);
...
TEST(TypeTraitsTest, TestTrivialCopyAssign) {
  // Verify that arithmetic types and pointers have trivial copy
  // assignment operators.
  EXPECT_TRUE(basic::is_trivially_copy_assignable<bool>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<char>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<unsigned char>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<signed char>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<wchar_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<int>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<unsigned int>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<int16_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<uint16_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<int64_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<uint64_t>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<float>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<double>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<long double>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<const std::string*>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<const Trivial*>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<std::string**>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<Trivial**>::value);

  // const qualified types are not assignable
  EXPECT_FALSE(basic::is_trivially_copy_assignable<const int>::value);

  // types with compiler generated copy assignment
  EXPECT_TRUE(basic::is_trivially_copy_assignable<Trivial>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<TrivialCopyAssign>::value);

  // Verify that types without them (i.e. nontrivial or deleted) are not.
  EXPECT_FALSE(basic::is_trivially_copy_assignable<NontrivialCopyAssign>::value);
  EXPECT_FALSE(basic::is_trivially_copy_assignable<DeletedCopyAssign>::value);
  EXPECT_FALSE(basic::is_trivially_copy_assignable<MovableNonCopyable>::value);
  EXPECT_FALSE(basic::is_trivially_copy_assignable<NonCopyableOrMovable>::value);

  // types with vtables
  EXPECT_FALSE(basic::is_trivially_copy_assignable<Base>::value);

  // Verify that simple_pair is trivially assignable
  EXPECT_TRUE(
      (basic::is_trivially_copy_assignable<simple_pair<int, char*>>::value));
  EXPECT_TRUE(
      (basic::is_trivially_copy_assignable<simple_pair<int, Trivial>>::value));
  EXPECT_TRUE((basic::is_trivially_copy_assignable<
               simple_pair<int, TrivialCopyAssign>>::value));

  // Verify that types not trivially copy assignable are
  // correctly marked as such.
  EXPECT_FALSE(basic::is_trivially_copy_assignable<std::string>::value);
  EXPECT_FALSE(basic::is_trivially_copy_assignable<std::vector<int>>::value);

  // Verify that simple_pairs of types not trivially copy assignable
  // are not marked as trivial.
  EXPECT_FALSE((basic::is_trivially_copy_assignable<
                simple_pair<int, std::string>>::value));
  EXPECT_FALSE((basic::is_trivially_copy_assignable<
                simple_pair<std::string, int>>::value));
  ...
  using int10 = int[10];
  EXPECT_FALSE(basic::is_trivially_copy_assignable<int10>::value);
...
TEST(TypeTraitsTest, TestTriviallyCopyable) {
  ...
  EXPECT_TRUE(basic::type_traits_internal::is_trivially_copyable<bool>::value);
  EXPECT_TRUE(basic::type_traits_internal::is_trivially_copyable<char>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<unsigned char>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<signed char>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<wchar_t>::value);
  EXPECT_TRUE(basic::type_traits_internal::is_trivially_copyable<int>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<unsigned int>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<int16_t>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<uint16_t>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<int64_t>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<uint64_t>::value);
  EXPECT_TRUE(basic::type_traits_internal::is_trivially_copyable<float>::value);
  EXPECT_TRUE(basic::type_traits_internal::is_trivially_copyable<double>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<long double>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<std::string*>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<Trivial*>::value);
  EXPECT_TRUE(basic::type_traits_internal::is_trivially_copyable<
              const std::string*>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<const Trivial*>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<std::string**>::value);
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<Trivial**>::value);
  ...
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<const int>::value);

  // Trivial copy constructor/assignment and destructor.
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<Trivial>::value);
  // Trivial copy assignment, but non-trivial copy constructor/destructor.
  EXPECT_FALSE(basic::type_traits_internal::is_trivially_copyable<
  ...
  // types with vtables
  EXPECT_FALSE(basic::type_traits_internal::is_trivially_copyable<Base>::value);
  ...
  using int10 = int[10];
  EXPECT_TRUE(basic::type_traits_internal::is_trivially_copyable<int10>::value);
  using int10x10 = int[10][10];
  EXPECT_TRUE(
      basic::type_traits_internal::is_trivially_copyable<int10x10>::value);
...
#define ABSL_INTERNAL_EXPECT_ALIAS_EQUIVALENCE(trait_name, ...)          \
  EXPECT_TRUE((std::is_same<typename std::trait_name<__VA_ARGS__>::type, \
                            basic::trait_name##_t<__VA_ARGS__>>::value))
...
struct GetTypeT {
  template <typename T,
            basic::enable_if_t<std::is_same<T, TypeA>::value, int> = 0>
  TypeEnum operator()(Wrap<T>) const {
    return TypeEnum::A;
  }

  template <typename T,
            basic::enable_if_t<std::is_same<T, TypeB>::value, int> = 0>
  TypeEnum operator()(Wrap<T>) const {
    return TypeEnum::B;
  }

  template <typename T,
            basic::enable_if_t<std::is_same<T, TypeC>::value, int> = 0>
  TypeEnum operator()(Wrap<T>) const {
    return TypeEnum::C;
  }
...
struct GetTypeExtT {
  template <typename T>
  basic::result_of_t<const GetTypeT&(T)> operator()(T&& arg) const {
...
template <typename T>
bool TestCopyAssign() {
  return basic::is_copy_assignable<T>::value ==
...
template <typename T>
bool TestMoveAssign() {
  return basic::is_move_assignable<T>::value ==
...
TEST(TypeTraitsTest, IsSwappable) {
  using basic::type_traits_internal::IsSwappable;
  using basic::type_traits_internal::StdSwapIsUnconstrained;
...
TEST(TypeTraitsTest, IsNothrowSwappable) {
  using basic::type_traits_internal::IsNothrowSwappable;
  using basic::type_traits_internal::StdSwapIsUnconstrained;
