...
// -----------------------------------------------------------------------------
//
// This header file define the `basic::any` type for holding a type-safe value
// of any type. The 'basic::any` type is useful for providing a way to hold
...
// `basic::any_cast()`. Note that when casting an `basic::any`, you must cast it
...
//   basic::any_cast<char>(a);        // throws basic::bad_any_cast
//   basic::any_cast<std::string>(a); // throws basic::bad_any_cast
//
// `basic::any` is a C++11 compatible version of the C++17 `std::any` abstraction
...
// other unspecified type. `basic::any` provides an "owning" version of `void *`
// that avoids issues of pointer management.
//
// Note: just as in the case of `void *`, use of `basic::any` (and its C++17
...
// and `basic::any`, like `std::any`, is difficult to use properly. Before using
// this abstraction, make sure that you should not instead be rewriting your
// code to be more specific.
//
// Basic expects to release an `basic::variant` type shortly (a C++11 compatible
...
#include "basic/types/bad_any_cast.h"
...
// swap()
//
// Swaps two `basic::any` values. Equivalent to `x.swap(y) where `x` and `y` are
// `basic::any` types.
void swap(any& x, any& y) noexcept;

// make_any()
//
// Constructs an `basic::any` of type `T` with the given arguments.
template <typename T, typename... Args>
any make_any(Args&&... args);

// Overload of `basic::make_any()` for constructing an `basic::any` type from an
// initializer list.
...
// any_cast()
//
// Statically casts the value of a `const basic::any` type to the given type.
// This function will throw `basic::bad_any_cast` if the stored value type of the
// `basic::any` does not match the cast.
//
...
// Example:
//
//   basic::any my_any = std::vector<int>();
//   basic::any_cast<std::vector<int>&>(my_any).push_back(42);
template <typename ValueType>
ValueType any_cast(const any& operand);

// Overload of `any_cast()` to statically cast the value of a non-const
// `basic::any` type to the given type. This function will throw
// `basic::bad_any_cast` if the stored value type of the `basic::any` does not
// match the cast.
template <typename ValueType>
ValueType any_cast(any& operand);  // NOLINT(runtime/references)

// Overload of `any_cast()` to statically cast the rvalue of an `basic::any`
// type. This function will throw `basic::bad_any_cast` if the stored value type
// of the `basic::any` does not match the cast.
template <typename ValueType>
ValueType any_cast(any&& operand);

// Overload of `any_cast()` to statically cast the value of a const pointer
// `basic::any` type to the given pointer type, or `nullptr` if the stored value
// type of the `basic::any` does not match the cast.
template <typename ValueType>
...
// -----------------------------------------------------------------------------
//
// An `basic::any` object provides the facility to either store an instance of a
// type, known as the "contained object", or no value. An `basic::any` is used to
// store values of types that are unknown at compile time. The `basic::any`
// object, when containing a value, must contain a value type; storing a
// reference type is neither desired nor supported.
//
// An `basic::any` can only store a type that is copy-constructable; move-only
// types are not allowed within an `any` object.
//
// Example:
//
//   auto a = basic::any(65);                 // Literal, copyable
//   auto b = basic::any(std::vector<int>()); // Default-initialized, copyable
//   std::unique_ptr<Foo> my_foo;
//   auto c = basic::any(std::move(my_foo));  // Error, not copy-constructable
//
// Note that `basic::any` makes use of decayed types (`basic::decay_t` in this
// context) to remove const-volatile qualifiers (known as "cv qualifiers"),
// decay functions to function pointers, etc. We essentially "decay" a given
// type into its essential type.
//
// `basic::any` makes use of decayed types when determining the basic type `T` of
...
//
//   const int a = 4;
//   basic::any foo(a);  // Decay ensures we store an "int", not a "const int&".
//
//   void my_function() {}
//   basic::any bar(my_function);  // Decay ensures we store a function pointer.
//
// `basic::any` is a C++11 compatible version of the C++17 `std::any` abstraction
// and is designed to be a drop-in replacement for code compliant with C++17.
class any {
 ...
 public:
  ...
  // Constructs an empty `basic::any` object (`any::has_value()` will return
  // `false`).
  constexpr any() noexcept;

  // Copy constructs an `basic::any` object with a "contained object" of the
  // passed type of `other` (or an empty `basic::any` if `other.has_value()` is
  // `false`.
  any(const any& other)
      : obj_(other.has_value() ? other.obj_->Clone()
                               : std::unique_ptr<ObjInterface>()) {}

  // Move constructs an `basic::any` object with a "contained object" of the
  // passed type of `other` (or an empty `basic::any` if `other.has_value()` is
  // `false`).
  any(any&& other) noexcept = default;

  // Constructs an `basic::any` object with a "contained object" of the decayed
  ...
  template <
      typename T, typename VT = basic::decay_t<T>,
      basic::enable_if_t<!basic::disjunction<
          std::is_same<any, VT>, IsInPlaceType<VT>,
          basic::negation<std::is_copy_constructible<VT> > >::value>* = nullptr>
  any(T&& value) : obj_(new Obj<VT>(in_place, std::forward<T>(value))) {}

  // Constructs an `basic::any` object with a "contained object" of the decayed
  // type of `T`, which is initialized via `std::forward<T>(value)`.
  template <typename T, typename... Args, typename VT = basic::decay_t<T>,
  ...
  // Constructs an `basic::any` object with a "contained object" of the passed
  // type `VT` as a decayed type of `T`. `VT` is initialized as if
  // direct-non-list-initializing an object of type `VT` with the arguments
  // `initializer_list, std::forward<Args>(args)...`.
  template <
      typename T, typename U, typename... Args, typename VT = basic::decay_t<T>,
      basic::enable_if_t<
          basic::conjunction<std::is_copy_constructible<VT>,
  ...
  // Copy assigns an `basic::any` object with a "contained object" of the
  // passed type.
  ...
  // Move assigns an `basic::any` object with a "contained object" of the
  ...
  // Assigns an `basic::any` object with a "contained object" of the passed type.
  template <typename T, typename VT = basic::decay_t<T>,
            basic::enable_if_t<basic::conjunction<
                basic::negation<std::is_same<VT, any>>,
  ...
  // any::emplace()
  //
  // Emplaces a value within an `basic::any` object by calling `any::reset()`,
  ...
  // been destroyed.
  template <
      typename T, typename... Args, typename VT = basic::decay_t<T>,
      basic::enable_if_t<std::is_copy_constructible<VT>::value &&
                        std::is_constructible<VT, Args...>::value>* = nullptr>
  ...
  // Overload of `any::emplace()` to emplace a value within an `basic::any`
  ...
  // any::reset()
  //
  // Resets the state of the `basic::any` object, destroying the contained object
  // if present.
  void reset() noexcept { obj_ = nullptr; }

  // any::swap()
  //
  // Swaps the passed value and the value of this `basic::any` object.
