...
class optional_data_dtor_base {
 ...
 protected:
  ...
  constexpr explicit optional_data_dtor_base(in_place_t, Args&&... args)
      : engaged_(true), data_(basic::forward<Args>(args)...) {}
...
class optional_data_base : public optional_data_dtor_base<T> {
 protected:
  ...
  constexpr explicit optional_data_base(in_place_t t, Args&&... args)
      : base(t, basic::forward<Args>(args)...) {}
#endif
...
// TODO(basic-team): Add another class using
...
template <typename T,
          bool unused = basic::is_trivially_copy_constructible<T>::value&&
              basic::is_trivially_copy_assignable<typename std::remove_cv<
...
class optional_data<T, true> : public optional_data_base<T> {
 protected:
  ...
  constexpr explicit optional_data(in_place_t t, Args&&... args)
      : optional_data_base<T>(t, basic::forward<Args>(args)...) {}
#endif
};
...
struct assign_copy_traits {
  static constexpr copy_traits traits =
      basic::is_copy_assignable<T>::value && std::is_copy_constructible<T>::value
          ? copy_traits::copyable
          : basic::is_move_assignable<T>::value &&
...
// Base class for std::hash<basic::optional<T>>:
...
template <typename T>
struct optional_hash_base<T, decltype(std::hash<basic::remove_const_t<T> >()(
                                 std::declval<basic::remove_const_t<T> >()))> {
  using argument_type = basic::optional<T>;
  using result_type = size_t;
  size_t operator()(const basic::optional<T>& opt) const {
    basic::type_traits_internal::AssertHashEnabled<basic::remove_const_t<T>>();
    if (opt) {
      return std::hash<basic::remove_const_t<T> >()(*opt);
    } else {
