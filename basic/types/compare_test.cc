...
struct WeakOrderingLess {
  template <typename T>
  basic::weak_ordering operator()(const T &a, const T &b) const {
    return a < b ? basic::weak_ordering::less
                 : a == b ? basic::weak_ordering::equivalent
                          : basic::weak_ordering::greater;
  }
};

TEST(CompareResultAsLessThan, SanityTest) {
  EXPECT_FALSE(basic::compare_internal::compare_result_as_less_than(false));
  EXPECT_TRUE(basic::compare_internal::compare_result_as_less_than(true));

  EXPECT_TRUE(
      basic::compare_internal::compare_result_as_less_than(weak_ordering::less));
  EXPECT_FALSE(basic::compare_internal::compare_result_as_less_than(
...
TEST(DoLessThanComparison, SanityTest) {
  ...
  EXPECT_TRUE(basic::compare_internal::do_less_than_comparison(less, -1, 0));
  EXPECT_TRUE(basic::compare_internal::do_less_than_comparison(weak, -1, 0));

  EXPECT_FALSE(basic::compare_internal::do_less_than_comparison(less, 10, 10));
  EXPECT_FALSE(basic::compare_internal::do_less_than_comparison(weak, 10, 10));

  EXPECT_FALSE(basic::compare_internal::do_less_than_comparison(less, 10, 5));
  EXPECT_FALSE(basic::compare_internal::do_less_than_comparison(weak, 10, 5));
}

TEST(CompareResultAsOrdering, SanityTest) {
  EXPECT_TRUE(Identity(
      basic::compare_internal::compare_result_as_ordering(-1) < 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::compare_result_as_ordering(-1) == 0));
  EXPECT_FALSE(
      Identity(basic::compare_internal::compare_result_as_ordering(-1) > 0));
  EXPECT_TRUE(Identity(basic::compare_internal::compare_result_as_ordering(
                           weak_ordering::less) < 0));
  EXPECT_FALSE(Identity(basic::compare_internal::compare_result_as_ordering(
                            weak_ordering::less) == 0));
  EXPECT_FALSE(Identity(basic::compare_internal::compare_result_as_ordering(
                            weak_ordering::less) > 0));

  EXPECT_FALSE(Identity(
      basic::compare_internal::compare_result_as_ordering(0) < 0));
  EXPECT_TRUE(Identity(
      basic::compare_internal::compare_result_as_ordering(0) == 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::compare_result_as_ordering(0) > 0));
...
TEST(DoThreeWayComparison, SanityTest) {
  ...
  EXPECT_TRUE(Identity(
      basic::compare_internal::do_three_way_comparison(less, -1, 0) < 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(less, -1, 0) == 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(less, -1, 0) > 0));
  EXPECT_TRUE(Identity(
      basic::compare_internal::do_three_way_comparison(weak, -1, 0) < 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(weak, -1, 0) == 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(weak, -1, 0) > 0));
  ...
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(less, 10, 10) < 0));
  EXPECT_TRUE(Identity(
      basic::compare_internal::do_three_way_comparison(less, 10, 10) == 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(less, 10, 10) > 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(weak, 10, 10) < 0));
  EXPECT_TRUE(Identity(
      basic::compare_internal::do_three_way_comparison(weak, 10, 10) == 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(weak, 10, 10) > 0));
  ...
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(less, 10, 5) < 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(less, 10, 5) == 0));
  EXPECT_TRUE(Identity(
      basic::compare_internal::do_three_way_comparison(less, 10, 5) > 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(weak, 10, 5) < 0));
  EXPECT_FALSE(Identity(
      basic::compare_internal::do_three_way_comparison(weak, 10, 5) == 0));
  EXPECT_TRUE(Identity(
      basic::compare_internal::do_three_way_comparison(weak, 10, 5) > 0));
