...
#define ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(...)                 \
  ABSL_VARIANT_TEST_EXPECT_FAIL((void)(__VA_ARGS__), basic::bad_variant_access, \
                                "Bad variant access")
...
void ToValuelessByException(basic::variant<H, T...>& v) {  // NOLINT
  try {
...
TEST(VariantTest, TestDefaultConstructor) {
...

  {
    ...
    ASSERT_FALSE(x.valueless_by_exception());
    ASSERT_EQ(0, x.index());
    EXPECT_EQ(5, basic::get<0>(x).value);
...
TYPED_TEST(VariantTypesTest, TestCopyCtor) {
  ...
  using value_type1 = basic::variant_alternative_t<0, Variant>;
  using value_type2 = basic::variant_alternative_t<1, Variant>;
  using value_type3 = basic::variant_alternative_t<2, Variant>;
  using value_type4 = basic::variant_alternative_t<3, Variant>;
  const TypeParam value(TypeParam::kIndex);
  Variant original(value);
  Variant copied(original);
  EXPECT_TRUE(basic::holds_alternative<value_type1>(copied) ||
              TypeParam::kIndex != 1);
  EXPECT_TRUE(basic::holds_alternative<value_type2>(copied) ||
              TypeParam::kIndex != 2);
  EXPECT_TRUE(basic::holds_alternative<value_type3>(copied) ||
              TypeParam::kIndex != 3);
  EXPECT_TRUE(basic::holds_alternative<value_type4>(copied) ||
              TypeParam::kIndex != 4);
  EXPECT_TRUE((basic::get_if<value_type1>(&original) ==
               basic::get_if<value_type1>(&copied)) ||
              TypeParam::kIndex == 1);
  EXPECT_TRUE((basic::get_if<value_type2>(&original) ==
               basic::get_if<value_type2>(&copied)) ||
              TypeParam::kIndex == 2);
  EXPECT_TRUE((basic::get_if<value_type3>(&original) ==
               basic::get_if<value_type3>(&copied)) ||
              TypeParam::kIndex == 3);
  EXPECT_TRUE((basic::get_if<value_type4>(&original) ==
               basic::get_if<value_type4>(&copied)) ||
              TypeParam::kIndex == 4);
  EXPECT_TRUE((basic::get_if<value_type1>(&original) ==
               basic::get_if<value_type1>(&copied)) ||
              TypeParam::kIndex == 1);
  ...
  EXPECT_TRUE((basic::get_if<value_type4>(&original) ==
               basic::get_if<value_type4>(&copied)) ||
              TypeParam::kIndex == 4);
  const TypeParam* ovalptr = basic::get_if<TypeParam>(&original);
  const TypeParam* cvalptr = basic::get_if<TypeParam>(&copied);
  ASSERT_TRUE(ovalptr != nullptr);
  ASSERT_TRUE(cvalptr != nullptr);
  EXPECT_EQ(*ovalptr, *cvalptr);
  TypeParam* mutable_ovalptr = basic::get_if<TypeParam>(&original);
  TypeParam* mutable_cvalptr = basic::get_if<TypeParam>(&copied);
...
TEST(VariantTest, TestMoveConstruct) {
  ...
  V v2 = basic::move(v);
  EXPECT_EQ(10, basic::get<1>(v2).value);
}
...
struct is_trivially_move_assignable
    : basic::is_move_assignable<SingleUnion<T>>::type {};
...
TYPED_TEST(VariantTypesTest, TestValueCtor) {
  ...
  const TypeParam value(TypeParam::kIndex);
  Variant v(value);
  EXPECT_TRUE(basic::holds_alternative<value_type1>(v) ||
              TypeParam::kIndex != 1);
  EXPECT_TRUE(basic::holds_alternative<value_type2>(v) ||
              TypeParam::kIndex != 2);
  EXPECT_TRUE(basic::holds_alternative<value_type3>(v) ||
              TypeParam::kIndex != 3);
  EXPECT_TRUE(basic::holds_alternative<value_type4>(v) ||
              TypeParam::kIndex != 4);
  EXPECT_TRUE(nullptr != basic::get_if<value_type1>(&v) ||
              TypeParam::kIndex != 1);
  EXPECT_TRUE(nullptr != basic::get_if<value_type2>(&v) ||
              TypeParam::kIndex != 2);
  EXPECT_TRUE(nullptr != basic::get_if<value_type3>(&v) ||
              TypeParam::kIndex != 3);
  EXPECT_TRUE(nullptr != basic::get_if<value_type4>(&v) ||
              TypeParam::kIndex != 4);
  EXPECT_TRUE(nullptr != basic::get_if<value_type1>(&v) ||
              TypeParam::kIndex != 1);
  EXPECT_TRUE(nullptr != basic::get_if<value_type2>(&v) ||
  ...
  EXPECT_TRUE(nullptr != basic::get_if<value_type4>(&v) ||
              TypeParam::kIndex != 4);
  const TypeParam* valptr = basic::get_if<TypeParam>(&v);
  ASSERT_TRUE(nullptr != valptr);
  EXPECT_EQ(value.value, valptr->value);
  const TypeParam* mutable_valptr = basic::get_if<TypeParam>(&v);
  ASSERT_TRUE(nullptr != mutable_valptr);
  EXPECT_EQ(value.value, mutable_valptr->value);
}

TEST(VariantTest, AmbiguousValueConstructor) {
  EXPECT_FALSE((std::is_convertible<int, basic::variant<int, int>>::value));
  EXPECT_FALSE((std::is_constructible<basic::variant<int, int>, int>::value));
}
...
TEST(VariantTest, InPlaceType) {
  ...
  ASSERT_TRUE(basic::holds_alternative<int>(v1));
  EXPECT_EQ(7, basic::get<int>(v1));

  Var v2(in_place_type_t<std::string>(), "ABC");
  ASSERT_TRUE(basic::holds_alternative<std::string>(v2));
  EXPECT_EQ("ABC", basic::get<std::string>(v2));

  Var v3(in_place_type_t<std::string>(), "ABC", 2);
  ASSERT_TRUE(basic::holds_alternative<std::string>(v3));
  EXPECT_EQ("AB", basic::get<std::string>(v3));

  Var v4(in_place_type_t<NonCopyable>{});
  ASSERT_TRUE(basic::holds_alternative<NonCopyable>(v4));

  Var v5(in_place_type_t<std::vector<int>>(), {1, 2, 3});
  ASSERT_TRUE(basic::holds_alternative<std::vector<int>>(v5));
  EXPECT_THAT(basic::get<std::vector<int>>(v5), ::testing::ElementsAre(1, 2, 3));
}
...
TEST(VariantTest, InPlaceTypeInitializerList) {
  ...
  ASSERT_TRUE(basic::holds_alternative<MoveOnlyWithListConstructor>(v1));
  EXPECT_EQ(6, basic::get<MoveOnlyWithListConstructor>(v1).value);
}
...
TEST(VariantTest, InPlaceIndex) {
  ...
  EXPECT_TRUE(basic::holds_alternative<NonCopyable>(v4));

  // Verify that a variant with only non-copyables can still be constructed.
  EXPECT_TRUE(basic::holds_alternative<NonCopyable>(
...
// See comment in basic/base/config.h
...
TEST(VariantTest, TestSelfAssignment) {
  ...
  int counter = 0;
  IncrementInDtor counter_adjuster(&counter);
  basic::variant_alternative_t<0, Variant> value(counter_adjuster);
  ...
  EXPECT_EQ(long_str, basic::get<1>(so));
  so = *&so;
...
TYPED_TEST(VariantTypesTest, TestAssignmentCopiesValueSameTypes) {
  ...
  ASSERT_TRUE(basic::holds_alternative<TypeParam>(source));
  ASSERT_TRUE(basic::holds_alternative<TypeParam>(target));
  ASSERT_NE(basic::get<TypeParam>(source), basic::get<TypeParam>(target));
  target = source;
  ASSERT_TRUE(basic::holds_alternative<TypeParam>(source));
  ASSERT_TRUE(basic::holds_alternative<TypeParam>(target));
  EXPECT_EQ(basic::get<TypeParam>(source), basic::get<TypeParam>(target));
}
...
TYPED_TEST(VariantTypesTest, TestAssignmentCopiesValuesVaryingSourceType) {
  ...
  ASSERT_TRUE(basic::holds_alternative<value_type1>(target));
  target = source;
  EXPECT_TRUE(basic::holds_alternative<TypeParam>(source));
  EXPECT_TRUE(basic::holds_alternative<TypeParam>(target));
  EXPECT_EQ(basic::get<TypeParam>(source), basic::get<TypeParam>(target));
}
...
TYPED_TEST(VariantTypesTest, TestAssignmentCopiesValuesVaryingTargetType) {
  ...
  ASSERT_TRUE(basic::holds_alternative<value_type1>(source));
  const TypeParam value(TypeParam::kIndex);
  Variant target(value);
  ASSERT_TRUE(basic::holds_alternative<TypeParam>(target));
  target = source;
  EXPECT_TRUE(basic::holds_alternative<value_type1>(target));
  EXPECT_TRUE(basic::holds_alternative<value_type1>(source));
  EXPECT_EQ(basic::get<value_type1>(source), basic::get<value_type1>(target));
}
...
TEST(VariantTest, TestAssign) {
  ...
  for (int i = 0; i != kSize; i++) {
    counter[i] = 0;
    counter_adjustor[i] = basic::make_unique<IncrementInDtor>(&counter[i]);
  }
...
TEST(VariantTest, TestEmplaceBasic) {
  ...
  Variant v(basic::in_place_index<0>, 0);

  {
    char& emplace_result = v.emplace<char>();
    ASSERT_TRUE(basic::holds_alternative<char>(v));
    EXPECT_EQ(basic::get<char>(v), 0);
    EXPECT_EQ(&emplace_result, &basic::get<char>(v));
  }
  ...
  v.emplace<char>('b');
  ASSERT_TRUE(basic::holds_alternative<char>(v));
  EXPECT_EQ(basic::get<char>(v), 'b');

  {
    int& emplace_result = v.emplace<int>();
    EXPECT_TRUE(basic::holds_alternative<int>(v));
    EXPECT_EQ(basic::get<int>(v), 0);
    EXPECT_EQ(&emplace_result, &basic::get<int>(v));
  }
}
...
TEST(VariantTest, TestEmplaceInitializerList) {
  ...
  Var v1(basic::in_place_index<0>, 555);
  MoveOnlyWithListConstructor& emplace_result =
      v1.emplace<MoveOnlyWithListConstructor>({1, 2, 3, 4, 5}, 6);
  ASSERT_TRUE(basic::holds_alternative<MoveOnlyWithListConstructor>(v1));
  EXPECT_EQ(6, basic::get<MoveOnlyWithListConstructor>(v1).value);
  EXPECT_EQ(&emplace_result, &basic::get<MoveOnlyWithListConstructor>(v1));
}

TEST(VariantTest, TestEmplaceIndex) {
  using Variant = variant<int, char>;

  Variant v(basic::in_place_index<0>, 555);
...
TEST(VariantTest, IndexValuelessByException) {
  ...
  Var v(basic::in_place_index<0>);
  EXPECT_EQ(0, v.index());
  ToValuelessByException(v);
  EXPECT_EQ(basic::variant_npos, v.index());
  v = "str";
  EXPECT_EQ(1, v.index());
}
...
TEST(VariantTest, MemberSwap) {
  ...
  EXPECT_TRUE(basic::get<SpecialSwap>(a).special_swap);
...
TEST(VariantTest, VariantSize) {
  {
    using Size1Variant = basic::variant<int>;
    EXPECT_EQ(1, basic::variant_size<Size1Variant>::value);
    EXPECT_EQ(1, basic::variant_size<const Size1Variant>::value);
    EXPECT_EQ(1, basic::variant_size<volatile Size1Variant>::value);
    EXPECT_EQ(1, basic::variant_size<const volatile Size1Variant>::value);
  }

  {
    using Size3Variant = basic::variant<int, float, int>;
    EXPECT_EQ(3, basic::variant_size<Size3Variant>::value);
    EXPECT_EQ(3, basic::variant_size<const Size3Variant>::value);
    EXPECT_EQ(3, basic::variant_size<volatile Size3Variant>::value);
    EXPECT_EQ(3, basic::variant_size<const volatile Size3Variant>::value);
  }
}

TEST(VariantTest, VariantAlternative) {
  {
    using V = basic::variant<float, int, const char*>;
    EXPECT_TRUE(
        (std::is_same<float, basic::variant_alternative_t<0, V>>::value));
    EXPECT_TRUE((std::is_same<const float,
                              basic::variant_alternative_t<0, const V>>::value));
    EXPECT_TRUE(
        (std::is_same<volatile float,
                      basic::variant_alternative_t<0, volatile V>>::value));
    EXPECT_TRUE((
        std::is_same<const volatile float,
                     basic::variant_alternative_t<0, const volatile V>>::value));

    EXPECT_TRUE((std::is_same<int, basic::variant_alternative_t<1, V>>::value));
    EXPECT_TRUE((std::is_same<const int,
                              basic::variant_alternative_t<1, const V>>::value));
    EXPECT_TRUE(
        (std::is_same<volatile int,
                      basic::variant_alternative_t<1, volatile V>>::value));
    EXPECT_TRUE((
        std::is_same<const volatile int,
                     basic::variant_alternative_t<1, const volatile V>>::value));

    EXPECT_TRUE(
        (std::is_same<const char*, basic::variant_alternative_t<2, V>>::value));
    EXPECT_TRUE((std::is_same<const char* const,
                              basic::variant_alternative_t<2, const V>>::value));
    EXPECT_TRUE(
        (std::is_same<const char* volatile,
                      basic::variant_alternative_t<2, volatile V>>::value));
    EXPECT_TRUE((
        std::is_same<const char* const volatile,
                     basic::variant_alternative_t<2, const volatile V>>::value));
  }

  {
    using V = basic::variant<float, volatile int, const char*>;
    EXPECT_TRUE(
        (std::is_same<float, basic::variant_alternative_t<0, V>>::value));
    EXPECT_TRUE((std::is_same<const float,
                              basic::variant_alternative_t<0, const V>>::value));
    ...
    EXPECT_TRUE(
        (std::is_same<volatile int, basic::variant_alternative_t<1, V>>::value));
...
TEST(VariantTest, HoldsAlternative) {
  ...
  Var v = 1;
  EXPECT_TRUE(basic::holds_alternative<int>(v));
  EXPECT_FALSE(basic::holds_alternative<std::string>(v));
  EXPECT_FALSE(basic::holds_alternative<double>(v));
  v = "str";
  EXPECT_FALSE(basic::holds_alternative<int>(v));
  EXPECT_TRUE(basic::holds_alternative<std::string>(v));
  EXPECT_FALSE(basic::holds_alternative<double>(v));
  v = 0.;
  EXPECT_FALSE(basic::holds_alternative<int>(v));
  EXPECT_FALSE(basic::holds_alternative<std::string>(v));
  EXPECT_TRUE(basic::holds_alternative<double>(v));

  Var v2 = v;
  EXPECT_FALSE(basic::holds_alternative<int>(v2));
  EXPECT_FALSE(basic::holds_alternative<std::string>(v2));
  EXPECT_TRUE(basic::holds_alternative<double>(v2));
  v2.emplace<int>(3);
  EXPECT_TRUE(basic::holds_alternative<int>(v2));
  EXPECT_FALSE(basic::holds_alternative<std::string>(v2));
  EXPECT_FALSE(basic::holds_alternative<double>(v2));
}
...
TEST(VariantTest, GetIndex) {
  ...
  {
    Var v(basic::in_place_index<0>, 0);

    using LValueGetType = decltype(basic::get<0>(v));
    using RValueGetType = decltype(basic::get<0>(basic::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, int&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, int&&>::value));
    EXPECT_EQ(basic::get<0>(v), 0);
    EXPECT_EQ(basic::get<0>(basic::move(v)), 0);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(basic::get<0>(const_v));
    using ConstRValueGetType = decltype(basic::get<0>(basic::move(const_v)));
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const int&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const int&&>::value));
    EXPECT_EQ(basic::get<0>(const_v), 0);
    EXPECT_EQ(basic::get<0>(basic::move(const_v)), 0);
  }

  {
    Var v = std::string("Hello");

    using LValueGetType = decltype(basic::get<1>(v));
    using RValueGetType = decltype(basic::get<1>(basic::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, std::string&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, std::string&&>::value));
    EXPECT_EQ(basic::get<1>(v), "Hello");
    EXPECT_EQ(basic::get<1>(basic::move(v)), "Hello");

    const Var& const_v = v;
    using ConstLValueGetType = decltype(basic::get<1>(const_v));
    using ConstRValueGetType = decltype(basic::get<1>(basic::move(const_v)));
    ...
    EXPECT_EQ(basic::get<1>(const_v), "Hello");
    EXPECT_EQ(basic::get<1>(basic::move(const_v)), "Hello");
  }

  {
    Var v = 2.0;

    using LValueGetType = decltype(basic::get<2>(v));
    using RValueGetType = decltype(basic::get<2>(basic::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, double&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, double&&>::value));
    EXPECT_EQ(basic::get<2>(v), 2.);
    EXPECT_EQ(basic::get<2>(basic::move(v)), 2.);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(basic::get<2>(const_v));
    using ConstRValueGetType = decltype(basic::get<2>(basic::move(const_v)));
    ...
    EXPECT_EQ(basic::get<2>(const_v), 2.);
    EXPECT_EQ(basic::get<2>(basic::move(const_v)), 2.);
  }

  {
    ...
    using LValueGetType = decltype(basic::get<3>(v));
    using RValueGetType = decltype(basic::get<3>(basic::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, int&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, int&&>::value));
    EXPECT_EQ(basic::get<3>(v), 1);
    EXPECT_EQ(basic::get<3>(basic::move(v)), 1);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(basic::get<3>(const_v));
    using ConstRValueGetType = decltype(basic::get<3>(basic::move(const_v)));
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const int&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const int&&>::value));
    EXPECT_EQ(basic::get<3>(const_v), 1);
    EXPECT_EQ(basic::get<3>(basic::move(const_v)), 1);  // NOLINT
  }
}
...
TEST(VariantTest, BadGetIndex) {
...

  {
    ...
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<1>(v));
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<1>(std::move(v)));

    const Var& const_v = v;
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<1>(const_v));
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
        basic::get<1>(std::move(const_v)));  // NOLINT
  }

  {
    Var v = std::string("Hello");

    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<0>(v));
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<0>(std::move(v)));

    const Var& const_v = v;
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<0>(const_v));
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
        basic::get<0>(std::move(const_v)));  // NOLINT
  }
}
...
TEST(VariantTest, GetType) {
...

  {
    ...
    using LValueGetType = decltype(basic::get<int>(v));
    using RValueGetType = decltype(basic::get<int>(basic::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, int&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, int&&>::value));
    EXPECT_EQ(basic::get<int>(v), 1);
    EXPECT_EQ(basic::get<int>(basic::move(v)), 1);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(basic::get<int>(const_v));
    using ConstRValueGetType = decltype(basic::get<int>(basic::move(const_v)));
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const int&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const int&&>::value));
    EXPECT_EQ(basic::get<int>(const_v), 1);
    EXPECT_EQ(basic::get<int>(basic::move(const_v)), 1);
  }

  {
    ...
    EXPECT_TRUE((std::is_same<LValueGetType, std::string&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, std::string&&>::value));
    EXPECT_EQ(basic::get<std::string>(v), "Hello");
    EXPECT_EQ(basic::get<std::string>(basic::move(v)), "Hello");
    ...
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const std::string&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const std::string&&>::value));
    EXPECT_EQ(basic::get<std::string>(const_v), "Hello");
    EXPECT_EQ(basic::get<std::string>(basic::move(const_v)), "Hello");
  }

  {
    ...
    EXPECT_TRUE((std::is_same<LValueGetType, double&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, double&&>::value));
    EXPECT_EQ(basic::get<double>(v), 2.);
    EXPECT_EQ(basic::get<double>(basic::move(v)), 2.);
    ...
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const double&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const double&&>::value));
    EXPECT_EQ(basic::get<double>(const_v), 2.);
    EXPECT_EQ(basic::get<double>(basic::move(const_v)), 2.);
  }
}
...
TEST(VariantTest, BadGetType) {
...

  {
    ...
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<std::string>(v));
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
        basic::get<std::string>(std::move(v)));

    const Var& const_v = v;
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
        basic::get<std::string>(const_v));
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
        basic::get<std::string>(std::move(const_v)));  // NOLINT
  }

  {
    Var v = std::string("Hello");

    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<int>(v));
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<int>(std::move(v)));

    const Var& const_v = v;
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(basic::get<int>(const_v));
    ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
        basic::get<int>(std::move(const_v)));  // NOLINT
  }
}
...
TEST(VariantTest, GetIfIndex) {
...

  {
    ...
    const Var& const_v = v;
    EXPECT_TRUE(noexcept(basic::get_if<0>(&const_v)));

    {
      auto* elem = basic::get_if<0>(&const_v);
      EXPECT_TRUE((std::is_same<decltype(elem), const int*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, 0);
      {
        auto* bad_elem = basic::get_if<1>(&const_v);
        EXPECT_TRUE(
            (std::is_same<decltype(bad_elem), const std::string*>::value));
        EXPECT_EQ(bad_elem, nullptr);
      }
      {
        auto* bad_elem = basic::get_if<2>(&const_v);
        EXPECT_TRUE((std::is_same<decltype(bad_elem), const double*>::value));
        EXPECT_EQ(bad_elem, nullptr);
      }
      {
        auto* bad_elem = basic::get_if<3>(&const_v);
        EXPECT_EQ(bad_elem, nullptr);
  ...
  {
    Var v = std::string("Hello");
    EXPECT_TRUE(noexcept(basic::get_if<1>(&v)));

    {
      auto* elem = basic::get_if<1>(&v);
      EXPECT_TRUE((std::is_same<decltype(elem), std::string*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, "Hello");
      {
        auto* bad_elem = basic::get_if<0>(&v);
        EXPECT_TRUE((std::is_same<decltype(bad_elem), int*>::value));
        EXPECT_EQ(bad_elem, nullptr);
      }
      {
    ...
    const Var& const_v = v;
    EXPECT_TRUE(noexcept(basic::get_if<1>(&const_v)));

    {
      auto* elem = basic::get_if<1>(&const_v);
      EXPECT_TRUE((std::is_same<decltype(elem), const std::string*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, "Hello");
      {
        auto* bad_elem = basic::get_if<0>(&const_v);
        EXPECT_TRUE((std::is_same<decltype(bad_elem), const int*>::value));
        EXPECT_EQ(bad_elem, nullptr);
      }
      {
  ...
  {
    Var v = 2.0;
    EXPECT_TRUE(noexcept(basic::get_if<2>(&v)));

    {
      auto* elem = basic::get_if<2>(&v);
    ...
    const Var& const_v = v;
    EXPECT_TRUE(noexcept(basic::get_if<2>(&const_v)));

    {
      auto* elem = basic::get_if<2>(&const_v);
...

  {
    Var v(basic::in_place_index<0>, 0);
    v.emplace<3>(1);
    EXPECT_TRUE(noexcept(basic::get_if<3>(&v)));

    {
      auto* elem = basic::get_if<3>(&v);
      EXPECT_TRUE((std::is_same<decltype(elem), int*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, 1);
      {
    ...
    const Var& const_v = v;
    EXPECT_TRUE(noexcept(basic::get_if<3>(&const_v)));

    {
      auto* elem = basic::get_if<3>(&const_v);
      EXPECT_TRUE((std::is_same<decltype(elem), const int*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, 1);
      {
...
TEST(VariantTest, ValuelessOperatorEquals) {
  variant<MoveCanThrow, std::string> int_v(1), string_v("Hello"),
      valueless(basic::in_place_index<0>),
      other_valueless(basic::in_place_index<0>);
  ToValuelessByException(valueless);
...
TEST(VariantTest, VisitSimple) {
  ...
  std::string str = basic::visit(ConvertTo<std::string>{}, v);
  EXPECT_EQ("A", str);

  v = std::string("B");

  basic::string_view piece = basic::visit(ConvertTo<basic::string_view>{}, v);
  EXPECT_EQ("B", piece);
  ...
  v = "SomeStr";
  EXPECT_EQ(7, basic::visit(StrLen{}, v));
  v = std::string("VeryLargeThisTime");
  EXPECT_EQ(17, basic::visit(StrLen{}, v));
}
...
TEST(VariantTest, VisitRValue) {
  ...
  EXPECT_FALSE(basic::visit(Visitor{}, v));
  EXPECT_TRUE(basic::visit(Visitor{}, basic::move(v)));

  // Also test the variadic overload.
  EXPECT_EQ(0, basic::visit(Visitor{}, v, v));
  EXPECT_EQ(1, basic::visit(Visitor{}, v, basic::move(v)));
  EXPECT_EQ(2, basic::visit(Visitor{}, basic::move(v), v));
  EXPECT_EQ(3, basic::visit(Visitor{}, basic::move(v), basic::move(v)));
}
...
TEST(VariantTest, VisitRValueVisitor) {
  ...
  Visitor visitor;
  EXPECT_FALSE(basic::visit(visitor, v));
  EXPECT_TRUE(basic::visit(Visitor{}, v));
}
...
TEST(VariantTest, VisitResultTypeDifferent) {
  ...
  EXPECT_TRUE(
      (std::is_same<LValue_LValue, decltype(basic::visit(visitor, v))>::value));
  EXPECT_TRUE(
      (std::is_same<RValue_LValue,
                    decltype(basic::visit(visitor, basic::move(v)))>::value));
  EXPECT_TRUE((
      std::is_same<LValue_RValue, decltype(basic::visit(Visitor{}, v))>::value));
  EXPECT_TRUE(
...
TEST(VariantTest, VisitVariadic) {
  ...
  using B = variant<std::unique_ptr<int>, basic::string_view>;

  struct Visitor {
    std::pair<int, int> operator()(int a, std::unique_ptr<int> b) const {
      return {a, *b};
    }
    std::pair<int, int> operator()(basic::string_view a,
                                   std::unique_ptr<int> b) const {
      return {static_cast<int>(a.size()), static_cast<int>(*b)};
    }
    std::pair<int, int> operator()(int a, basic::string_view b) const {
      return {a, static_cast<int>(b.size())};
    }
    std::pair<int, int> operator()(basic::string_view a,
                                   basic::string_view b) const {
      return {static_cast<int>(a.size()), static_cast<int>(b.size())};
    }
  };

  EXPECT_THAT(basic::visit(Visitor(), A(1), B(std::unique_ptr<int>(new int(7)))),
              ::testing::Pair(1, 7));
  EXPECT_THAT(basic::visit(Visitor(), A(1), B(basic::string_view("ABC"))),
              ::testing::Pair(1, 3));
  EXPECT_THAT(basic::visit(Visitor(), A(std::string("BBBBB")),
                          B(std::unique_ptr<int>(new int(7)))),
              ::testing::Pair(5, 7));
  EXPECT_THAT(basic::visit(Visitor(), A(std::string("BBBBB")),
                          B(basic::string_view("ABC"))),
              ::testing::Pair(5, 3));
}

TEST(VariantTest, VisitNoArgs) {
  EXPECT_EQ(5, basic::visit([] { return 5; }));
}
...
TEST(VariantTest, VisitReferenceWrapper) {
  ConstFunctor cf;
  MutableFunctor mf;
  basic::variant<int> three = 3;
  basic::variant<int> two = 2;

  EXPECT_EQ(1, basic::visit(std::cref(cf), three, two));
  EXPECT_EQ(1, basic::visit(std::ref(cf), three, two));
  EXPECT_EQ(1, basic::visit(std::ref(mf), three, two));
}

// libstdc++ std::variant doesn't support the INVOKE semantics.
#if !(defined(ABSL_HAVE_STD_VARIANT) && defined(__GLIBCXX__))
TEST(VariantTest, VisitMemberFunction) {
  basic::variant<std::unique_ptr<Class>> p(basic::make_unique<Class>());
  basic::variant<std::unique_ptr<const Class>> cp(
      basic::make_unique<const Class>());
  basic::variant<int> three = 3;
  basic::variant<int> two = 2;

  EXPECT_EQ(1, basic::visit(&Class::Method, p, three, two));
  EXPECT_EQ(1, basic::visit(&Class::ConstMethod, p, three, two));
  EXPECT_EQ(1, basic::visit(&Class::ConstMethod, cp, three, two));
}

TEST(VariantTest, VisitDataMember) {
  basic::variant<std::unique_ptr<Class>> p(basic::make_unique<Class>(Class{42}));
  basic::variant<std::unique_ptr<const Class>> cp(
      basic::make_unique<const Class>(Class{42}));
  EXPECT_EQ(42, basic::visit(&Class::member, p));

  basic::visit(&Class::member, p) = 5;
  EXPECT_EQ(5, basic::visit(&Class::member, p));

  EXPECT_EQ(42, basic::visit(&Class::member, cp));
}
...
TEST(VariantTest, MonostateBasic) {
  basic::monostate mono;
  (void)mono;

  // TODO(mattcalabrese) Expose move triviality metafunctions in basic.
  EXPECT_TRUE(basic::is_trivially_default_constructible<basic::monostate>::value);
  EXPECT_TRUE(is_trivially_move_constructible<basic::monostate>::value);
  EXPECT_TRUE(basic::is_trivially_copy_constructible<basic::monostate>::value);
  EXPECT_TRUE(is_trivially_move_assignable<basic::monostate>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<basic::monostate>::value);
  EXPECT_TRUE(basic::is_trivially_destructible<basic::monostate>::value);
}

TEST(VariantTest, VariantMonostateDefaultConstruction) {
  basic::variant<basic::monostate, NonDefaultConstructible> var;
...
TEST(VariantTest, MonostateComparisons) {
  ...
  EXPECT_TRUE(noexcept(std::declval<basic::monostate>() ==
                       std::declval<basic::monostate>()));
  EXPECT_TRUE(noexcept(std::declval<basic::monostate>() !=
                       std::declval<basic::monostate>()));
  EXPECT_TRUE(noexcept(std::declval<basic::monostate>() <
                       std::declval<basic::monostate>()));
...
TEST(VariantTest, NonmemberSwap) {
  ...
  EXPECT_FALSE(basic::get<SpecialSwap>(a).special_swap);
#endif

  swap(a, b);
  EXPECT_THAT(a, VariantWith<SpecialSwap>(v1));
  EXPECT_THAT(b, VariantWith<SpecialSwap>(v2));
  EXPECT_TRUE(basic::get<SpecialSwap>(b).special_swap);
}
...
TEST(VariantTest, BadAccess) {
  EXPECT_TRUE(noexcept(basic::bad_variant_access()));
  basic::bad_variant_access exception_obj;
...
TEST(VariantTest, MonostateHash) {
  basic::monostate mono, other_mono;
  std::hash<basic::monostate> const hasher{};
...
// result on the same value as different alternative.
...

  {
    ...
    std::hash<basic::variant<const int>> c_hash;
    for (int i = 0; i < 100; ++i) {
...
TEST(VariantTest, TestConvertingSet) {
  ...
  v = two;
  EXPECT_TRUE(basic::holds_alternative<double>(v));
  ASSERT_TRUE(nullptr != basic::get_if<double>(&v));
  EXPECT_DOUBLE_EQ(2, basic::get<double>(v));
}
...
TEST(VariantTest, TestVariantWithNonCopyableType) {
  ...
  const int kValue = 1;
  Variant v(kValue);
  ASSERT_TRUE(basic::holds_alternative<int>(v));
  EXPECT_EQ(kValue, basic::get<int>(v));
}
...
TEST(VariantTest, TestEmplace) {
  ...
  ASSERT_TRUE(basic::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(0, basic::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(basic::holds_alternative<int>(v));
  ...
  ASSERT_TRUE(basic::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(1, basic::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(basic::holds_alternative<int>(v));
  ...
  ASSERT_TRUE(basic::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(3, basic::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(basic::holds_alternative<int>(v));
  ...
  ASSERT_TRUE(basic::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(6, basic::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(basic::holds_alternative<int>(v));
  ...
  ASSERT_TRUE(basic::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(10, basic::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(basic::holds_alternative<int>(v));
}
...
TEST(VariantTest, TestEmplaceDestroysCurrentValue) {
  ...
  ASSERT_TRUE(basic::holds_alternative<IncrementInDtor>(v));
  ASSERT_EQ(0, counter);
  v.emplace<NonCopyable>();
  ASSERT_TRUE(basic::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(1, counter);
...
TEST(VariantTest, TestMoveSemantics) {
  ...
  Variant v(basic::WrapUnique(new int(10)));
  EXPECT_TRUE(basic::holds_alternative<std::unique_ptr<int>>(v));

  // Construct a variant by moving from another variant.
  Variant v2(basic::move(v));
  ASSERT_TRUE(basic::holds_alternative<std::unique_ptr<int>>(v2));
  ASSERT_NE(nullptr, basic::get<std::unique_ptr<int>>(v2));
  EXPECT_EQ(10, *basic::get<std::unique_ptr<int>>(v2));

  // Moving from a variant object leaves it holding moved-from value of the
  // same element type.
  EXPECT_TRUE(basic::holds_alternative<std::unique_ptr<int>>(v));
  ASSERT_NE(nullptr, basic::get_if<std::unique_ptr<int>>(&v));
  EXPECT_EQ(nullptr, basic::get<std::unique_ptr<int>>(v));

  // Assign a variant from an element value by move.
  v = basic::make_unique<std::string>("foo");
  ASSERT_TRUE(basic::holds_alternative<std::unique_ptr<std::string>>(v));
  EXPECT_EQ("foo", *basic::get<std::unique_ptr<std::string>>(v));

  // Move-assign a variant.
  v2 = basic::move(v);
  ASSERT_TRUE(basic::holds_alternative<std::unique_ptr<std::string>>(v2));
  EXPECT_EQ("foo", *basic::get<std::unique_ptr<std::string>>(v2));
  EXPECT_TRUE(basic::holds_alternative<std::unique_ptr<std::string>>(v));
}
...
TEST(VariantTest, TestImplicitConversion) {
  EXPECT_TRUE(basic::holds_alternative<int>(PassThrough(0)));

  // We still need the explicit cast for std::string, because C++ won't apply
  // two user-defined implicit conversions in a row.
  EXPECT_TRUE(
      basic::holds_alternative<std::string>(PassThrough(std::string("foo"))));
}
...
TEST(VariantTest, TestRvalueConversion) {
  ...
  ASSERT_TRUE(basic::holds_alternative<double>(var));
  EXPECT_EQ(0.0, basic::get<double>(var));

  var = ConvertVariantTo<variant<double, std::string>>(
      variant<const char*, float>("foo"));
  ASSERT_TRUE(basic::holds_alternative<std::string>(var));
  EXPECT_EQ("foo", basic::get<std::string>(var));

  variant<double> singleton(
      ConvertVariantTo<variant<double>>(variant<int, float>(42)));
  ASSERT_TRUE(basic::holds_alternative<double>(singleton));
  EXPECT_EQ(42.0, basic::get<double>(singleton));

  singleton = ConvertVariantTo<variant<double>>(variant<int, float>(3.14f));
  ASSERT_TRUE(basic::holds_alternative<double>(singleton));
  EXPECT_FLOAT_EQ(3.14f, static_cast<float>(basic::get<double>(singleton)));

  singleton = ConvertVariantTo<variant<double>>(variant<int>(0));
  ASSERT_TRUE(basic::holds_alternative<double>(singleton));
  EXPECT_EQ(0.0, basic::get<double>(singleton));

  variant<int32_t, uint32_t> variant2(
      ConvertVariantTo<variant<int32_t, uint32_t>>(variant<int32_t>(42)));
  ASSERT_TRUE(basic::holds_alternative<int32_t>(variant2));
  EXPECT_EQ(42, basic::get<int32_t>(variant2));

  variant2 = ConvertVariantTo<variant<int32_t, uint32_t>>(variant<uint32_t>(42));
  ASSERT_TRUE(basic::holds_alternative<uint32_t>(variant2));
  EXPECT_EQ(42, basic::get<uint32_t>(variant2));
#endif  // !ABSL_HAVE_STD_VARIANT
  ...
  ASSERT_TRUE(basic::holds_alternative<Convertible1>(variant3));

  variant3 = ConvertVariantTo<variant<Convertible1, Convertible2>>(
      variant<Convertible2, Convertible1>(Convertible2()));
  ASSERT_TRUE(basic::holds_alternative<Convertible2>(variant3));
}
...
TEST(VariantTest, TestLvalueConversion) {
  ...
  ASSERT_TRUE(basic::holds_alternative<double>(destination));
  EXPECT_EQ(0.0, basic::get<double>(destination));

  variant<const char*, float> source2 = "foo";
  destination = ConvertVariantTo<variant<double, std::string>>(source2);
  ASSERT_TRUE(basic::holds_alternative<std::string>(destination));
  EXPECT_EQ("foo", basic::get<std::string>(destination));
...
TEST(VariantTest, TestMoveConversion) {
  ...
  Variant var(
      ConvertVariantTo<Variant>(OtherVariant{basic::make_unique<int>(0)}));
  ASSERT_TRUE(basic::holds_alternative<std::unique_ptr<const int>>(var));
  ASSERT_NE(basic::get<std::unique_ptr<const int>>(var), nullptr);
  EXPECT_EQ(0, *basic::get<std::unique_ptr<const int>>(var));

  var = ConvertVariantTo<Variant>(
      OtherVariant(basic::make_unique<std::string>("foo")));
  ASSERT_TRUE(basic::holds_alternative<std::unique_ptr<const std::string>>(var));
  EXPECT_EQ("foo", *basic::get<std::unique_ptr<const std::string>>(var));
}
...
TEST(VariantTest, DoesNotMoveFromLvalues) {
  ...
  Variant v2(v1);
  EXPECT_EQ(basic::get<std::shared_ptr<const int>>(v1),
            basic::get<std::shared_ptr<const int>>(v2));

  // Test copy-assignment operator
  v1 = std::make_shared<const std::string>("foo");
  v2 = v1;
  EXPECT_EQ(basic::get<std::shared_ptr<const std::string>>(v1),
            basic::get<std::shared_ptr<const std::string>>(v2));

  // Test converting copy constructor
  OtherVariant other(std::make_shared<int>(0));
  Variant v3(ConvertVariantTo<Variant>(other));
  EXPECT_EQ(basic::get<std::shared_ptr<int>>(other),
            basic::get<std::shared_ptr<const int>>(v3));

  other = std::make_shared<std::string>("foo");
  v3 = ConvertVariantTo<Variant>(other);
  EXPECT_EQ(basic::get<std::shared_ptr<std::string>>(other),
            basic::get<std::shared_ptr<const std::string>>(v3));
}
...
TEST(VariantTest, TestRvalueConversionViaConvertVariantTo) {
  ...
  EXPECT_THAT(basic::get_if<double>(&var), Pointee(3.0));

  var = ConvertVariantTo<variant<double, std::string>>(
      variant<const char*, float>("foo"));
  EXPECT_THAT(basic::get_if<std::string>(&var), Pointee(std::string("foo")));

  variant<double> singleton(
      ConvertVariantTo<variant<double>>(variant<int, float>(42)));
  EXPECT_THAT(basic::get_if<double>(&singleton), Pointee(42.0));

  singleton = ConvertVariantTo<variant<double>>(variant<int, float>(3.14f));
  EXPECT_THAT(basic::get_if<double>(&singleton), Pointee(DoubleEq(3.14f)));

  singleton = ConvertVariantTo<variant<double>>(variant<int>(3));
  EXPECT_THAT(basic::get_if<double>(&singleton), Pointee(3.0));

  variant<int32_t, uint32_t> variant2(
      ConvertVariantTo<variant<int32_t, uint32_t>>(variant<int32_t>(42)));
  EXPECT_THAT(basic::get_if<int32_t>(&variant2), Pointee(42));

  variant2 = ConvertVariantTo<variant<int32_t, uint32_t>>(variant<uint32_t>(42));
  EXPECT_THAT(basic::get_if<uint32_t>(&variant2), Pointee(42));
#endif
...
TEST(VariantTest, TestLvalueConversionViaConvertVariantTo) {
  ...
  variant<double, std::string> destination(
      ConvertVariantTo<variant<double, std::string>>(source1));
  EXPECT_THAT(basic::get_if<double>(&destination), Pointee(3.0));

  variant<const char*, float> source2 = "foo";
  destination = ConvertVariantTo<variant<double, std::string>>(source2);
  EXPECT_THAT(basic::get_if<std::string>(&destination),
...
TEST(VariantTest, TestMoveConversionViaConvertVariantTo) {
  ...
  Variant var(
      ConvertVariantTo<Variant>(OtherVariant{basic::make_unique<int>(3)}));
  EXPECT_THAT(basic::get_if<std::unique_ptr<const int>>(&var),
              Pointee(Pointee(3)));

  var = ConvertVariantTo<Variant>(
      OtherVariant(basic::make_unique<std::string>("foo")));
  EXPECT_THAT(basic::get_if<std::unique_ptr<const std::string>>(&var),
...
TEST(VariantTest, TestCopyAndMoveTypeTraits) {
  ...
  EXPECT_TRUE(basic::is_copy_assignable<variant<std::string>>::value);
  EXPECT_TRUE(std::is_move_constructible<variant<std::string>>::value);
  EXPECT_TRUE(basic::is_move_assignable<variant<std::string>>::value);
  EXPECT_TRUE(std::is_move_constructible<variant<std::unique_ptr<int>>>::value);
  EXPECT_TRUE(basic::is_move_assignable<variant<std::unique_ptr<int>>>::value);
  EXPECT_FALSE(
      std::is_copy_constructible<variant<std::unique_ptr<int>>>::value);
  EXPECT_FALSE(basic::is_copy_assignable<variant<std::unique_ptr<int>>>::value);

  EXPECT_FALSE(
      basic::is_trivially_copy_constructible<variant<std::string>>::value);
  EXPECT_FALSE(basic::is_trivially_copy_assignable<variant<std::string>>::value);
#if ABSL_VARIANT_PROPAGATE_COPY_MOVE_TRIVIALITY
  EXPECT_TRUE(basic::is_trivially_copy_constructible<variant<int>>::value);
  EXPECT_TRUE(basic::is_trivially_copy_assignable<variant<int>>::value);
...
TEST(VariantTest, TestVectorOfMoveonlyVariant) {
  ...
  vec.push_back(basic::make_unique<int>(42));
  vec.emplace_back("Hello");
  vec.reserve(3);
  auto another_vec = basic::move(vec);
  // As a sanity check, verify vector contents.
  ASSERT_EQ(2, another_vec.size());
  EXPECT_EQ(42, *basic::get<std::unique_ptr<int>>(another_vec[0]));
  EXPECT_EQ("Hello", basic::get<std::string>(another_vec[1]));
}

TEST(VariantTest, NestedVariant) {
#if ABSL_VARIANT_PROPAGATE_COPY_MOVE_TRIVIALITY
  static_assert(basic::is_trivially_copy_constructible<variant<int>>(), "");
  static_assert(basic::is_trivially_copy_assignable<variant<int>>(), "");
  static_assert(is_trivially_move_constructible<variant<int>>(), "");
  static_assert(is_trivially_move_assignable<variant<int>>(), "");

  static_assert(basic::is_trivially_copy_constructible<variant<variant<int>>>(),
                "");
  static_assert(basic::is_trivially_copy_assignable<variant<variant<int>>>(),
                "");
  ...
  EXPECT_TRUE(basic::holds_alternative<variant<int>>(z));
  EXPECT_EQ(x, basic::get<variant<int>>(z));
}
...
TEST(VariantTest, TestTriviality) {
  {
    using TrivDestVar = basic::variant<TriviallyDestructible>;

    EXPECT_FALSE(is_trivially_move_constructible<TrivDestVar>::value);
    EXPECT_FALSE(basic::is_trivially_copy_constructible<TrivDestVar>::value);
    EXPECT_FALSE(is_trivially_move_assignable<TrivDestVar>::value);
    EXPECT_FALSE(basic::is_trivially_copy_assignable<TrivDestVar>::value);
    EXPECT_TRUE(basic::is_trivially_destructible<TrivDestVar>::value);
  }

  {
    using TrivMoveVar = basic::variant<TriviallyMovable>;

    EXPECT_TRUE(is_trivially_move_constructible<TrivMoveVar>::value);
    EXPECT_FALSE(basic::is_trivially_copy_constructible<TrivMoveVar>::value);
    EXPECT_FALSE(is_trivially_move_assignable<TrivMoveVar>::value);
    EXPECT_FALSE(basic::is_trivially_copy_assignable<TrivMoveVar>::value);
    EXPECT_TRUE(basic::is_trivially_destructible<TrivMoveVar>::value);
  }

  {
    using TrivCopyVar = basic::variant<TriviallyCopyable>;

    EXPECT_TRUE(is_trivially_move_constructible<TrivCopyVar>::value);
    EXPECT_TRUE(basic::is_trivially_copy_constructible<TrivCopyVar>::value);
    EXPECT_FALSE(is_trivially_move_assignable<TrivCopyVar>::value);
    EXPECT_FALSE(basic::is_trivially_copy_assignable<TrivCopyVar>::value);
    EXPECT_TRUE(basic::is_trivially_destructible<TrivCopyVar>::value);
  }

  {
    using TrivMoveAssignVar = basic::variant<TriviallyMoveAssignable>;

    EXPECT_TRUE(is_trivially_move_constructible<TrivMoveAssignVar>::value);
    EXPECT_FALSE(
        basic::is_trivially_copy_constructible<TrivMoveAssignVar>::value);
    EXPECT_TRUE(is_trivially_move_assignable<TrivMoveAssignVar>::value);
    EXPECT_FALSE(basic::is_trivially_copy_assignable<TrivMoveAssignVar>::value);
    EXPECT_TRUE(basic::is_trivially_destructible<TrivMoveAssignVar>::value);
  }

  {
    using TrivCopyAssignVar = basic::variant<TriviallyCopyAssignable>;

    EXPECT_TRUE(is_trivially_move_constructible<TrivCopyAssignVar>::value);
    EXPECT_TRUE(
        basic::is_trivially_copy_constructible<TrivCopyAssignVar>::value);
    EXPECT_TRUE(is_trivially_move_assignable<TrivCopyAssignVar>::value);
    EXPECT_TRUE(basic::is_trivially_copy_assignable<TrivCopyAssignVar>::value);
    EXPECT_TRUE(basic::is_trivially_destructible<TrivCopyAssignVar>::value);
  }
}
#endif  // ABSL_VARIANT_PROPAGATE_COPY_MOVE_TRIVIALITY

// To verify that basic::variant correctly use the nontrivial move ctor of its
...
TEST(VariantTest, MoveCtorBug) {
  ...
  {
    using V = basic::variant<TrivialCopyNontrivialMove, int>;
    V v1(basic::in_place_index<0>);
    // this should invoke the move ctor, rather than the trivial copy ctor.
    V v2(std::move(v1));
    EXPECT_TRUE(basic::get<0>(v2).called);
  }
  {
    // this case failed to compile before our fix due to a GCC bug.
    using V = basic::variant<int, TrivialCopyNontrivialMove>;
    V v1(basic::in_place_index<1>);
    // this should invoke the move ctor, rather than the trivial copy ctor.
    V v2(std::move(v1));
    EXPECT_TRUE(basic::get<1>(v2).called);
  }
}
