...
using Optional = basic::optional<Thrower>;

using MoveThrower = testing::ThrowingValue<testing::TypeSpec::kNoThrowMove>;
using MoveOptional = basic::optional<MoveThrower>;
...
} catch (const basic::bad_optional_access&) {
  return true;
}
...
TEST(OptionalExceptionSafety, ThrowingConstructors) {
  ...
  auto integer_nonempty = basic::optional<int>(kInitialInteger);
  ...
  testing::TestThrowingCtor<basic::optional<ThrowerVec>>(
      basic::in_place,
...
TEST(OptionalExceptionSafety, EverythingThrowsSwap) {
  ...
  auto swap_nonempty = [](Optional* optional_ptr) {
    auto nonempty =
        Optional(basic::in_place, kUpdatedInteger, testing::nothrow_ctor);
