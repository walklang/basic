...
// -----------------------------------------------------------------------------
//
// This header file defines the `basic::weak_equality`, `basic::strong_equality`,
// `basic::partial_ordering`, `basic::weak_ordering`, and `basic::strong_ordering`
// types for storing the results of three way comparisons.
//
// Example:
//   basic::weak_ordering compare(const std::string& a, const std::string& b);
//
...
namespace compare_internal {
// We also provide these comparator adapter functions for internal basic use.

// Helper functions to do a boolean comparison of two keys given a boolean
// or three-way comparator.
// SFINAE prevents implicit conversions to bool (such as from int).
template <typename Bool,
          basic::enable_if_t<std::is_same<bool, Bool>::value, int> = 0>
constexpr bool compare_result_as_less_than(const Bool r) { return r; }
constexpr bool compare_result_as_less_than(const basic::weak_ordering r) {
  return r < 0;
}
...
template <typename Int,
          basic::enable_if_t<std::is_same<int, Int>::value, int> = 0>
constexpr basic::weak_ordering compare_result_as_ordering(const Int c) {
  return c < 0 ? basic::weak_ordering::less
               : c == 0 ? basic::weak_ordering::equivalent
                        : basic::weak_ordering::greater;
}
constexpr basic::weak_ordering compare_result_as_ordering(
    const basic::weak_ordering c) {
  return c;
}

template <
    typename Compare, typename K, typename LK,
    basic::enable_if_t<!std::is_same<bool, basic::result_of_t<Compare(
                                              const K &, const LK &)>>::value,
                      int> = 0>
constexpr basic::weak_ordering do_three_way_comparison(const Compare &compare,
...
constexpr basic::weak_ordering do_three_way_comparison(const Compare &compare,
                                                      const K &x, const LK &y) {
  return compare(x, y) ? basic::weak_ordering::less
                       : compare(y, x) ? basic::weak_ordering::greater
                                       : basic::weak_ordering::equivalent;
}
