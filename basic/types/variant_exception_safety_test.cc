...
using ThrowingVariant =
    basic::variant<Thrower, CopyNothrow, MoveNothrow, ThrowerVec>;
...
testing::AssertionResult VariantInvariants(ThrowingVariant* v) {
  ...
  if (basic::holds_alternative<Thrower>(*v)) {
    auto& t = basic::get<Thrower>(*v);
    t = Thrower{-100};
    if (t.Get() != -100) {
      return AssertionFailure() << "Thrower should be assigned -100";
    }
  } else if (basic::holds_alternative<ThrowerVec>(*v)) {
    auto& tv = basic::get<ThrowerVec>(*v);
    tv.clear();
    tv.emplace_back(-100);
    if (tv.size() != 1 || tv[0].Get() != -100) {
      return AssertionFailure() << "ThrowerVec should be {Thrower{-100}}";
    }
  } else if (basic::holds_alternative<CopyNothrow>(*v)) {
    auto& t = basic::get<CopyNothrow>(*v);
    t = CopyNothrow{-100};
    if (t.Get() != -100) {
      return AssertionFailure() << "CopyNothrow should be assigned -100";
    }
  } else if (basic::holds_alternative<MoveNothrow>(*v)) {
    auto& t = basic::get<MoveNothrow>(*v);
    t = MoveNothrow{-100};
    if (t.Get() != -100) {
  ...
  try {
    auto unused = basic::get<Thrower>(*v);
    static_cast<void>(unused);
    return AssertionFailure() << "Variant should not contain Thrower";
  } catch (const basic::bad_variant_access&) {
  } catch (...) {
    return AssertionFailure() << "Unexpected exception throw from basic::get";
  }

  // Try using the variant
  v->emplace<Thrower>(100);
  if (!basic::holds_alternative<Thrower>(*v) ||
      basic::get<Thrower>(*v) != Thrower(100)) {
    return AssertionFailure() << "Variant should contain Thrower(100)";
  }
  v->emplace<ThrowerVec>({Thrower(100)});
  if (!basic::holds_alternative<ThrowerVec>(*v) ||
      basic::get<ThrowerVec>(*v)[0] != Thrower(100)) {
...
TEST(VariantExceptionSafetyTest, InPlaceTypeConstructor) {
  TestThrowingCtor<ThrowingVariant>(basic::in_place_type_t<Thrower>{},
                                    ExpectedThrower());
  TestThrowingCtor<ThrowingVariant>(basic::in_place_type_t<ThrowerVec>{},
                                    ExpectedThrowerVec());
}

TEST(VariantExceptionSafetyTest, InPlaceIndexConstructor) {
  TestThrowingCtor<ThrowingVariant>(basic::in_place_index_t<0>{},
                                    ExpectedThrower());
  TestThrowingCtor<ThrowingVariant>(basic::in_place_index_t<3>{},
                                    ExpectedThrowerVec());
}
