...
void BM_FixedArray(benchmark::State& state) {
  ...
  for (auto _ : state) {
    basic::FixedArray<C, stack_size> fa(size);
    benchmark::DoNotOptimize(fa.data());
  }
}
BENCHMARK_TEMPLATE(BM_FixedArray, char, basic::kFixedArrayUseDefault)
    ->Range(0, 1 << 16);
...
BENCHMARK_TEMPLATE(BM_FixedArray, SimpleClass, basic::kFixedArrayUseDefault)
    ->Range(0, 1 << 16);
...
BENCHMARK_TEMPLATE(BM_FixedArray, std::string, basic::kFixedArrayUseDefault)
    ->Range(0, 1 << 16);
