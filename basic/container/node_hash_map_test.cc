...
#include "basic/container/node_hash_map.h"

#include "basic/container/internal/tracked.h"
...
using MapTypes = ::testing::Types<
    basic::node_hash_map<int, int, StatefulTestingHash, StatefulTestingEqual,
                        Alloc<std::pair<const int, int>>>,
    basic::node_hash_map<std::string, std::string, StatefulTestingHash,
...
using M = basic::node_hash_map<std::string, Tracked<int>>;
...
TEST(NodeHashMap, AssignRecursive) {
  struct Tree {
    ...
    basic::node_hash_map<int, Tree> children;
  };
  Tree root;
...
TEST(FlatHashMap, MoveOnlyKey) {
  ...
  basic::node_hash_map<Key, int, Hash, Eq> m;
  m[Key()];
}
...
TEST(NodeHashMap, MergeExtractInsert) {
  basic::node_hash_map<NonMovableKey, int, NonMovableKeyHash, NonMovableKeyEq>
      set1, set2;
