...
#include "basic/container/internal/hash_generator_testing.h"
#include "basic/container/internal/unordered_set_constructor_test.h"
#include "basic/container/internal/unordered_set_lookup_test.h"
#include "basic/container/internal/unordered_set_members_test.h"
#include "basic/container/internal/unordered_set_modifiers_test.h"
...
using ::basic::container_internal::hash_internal::Enum;
using ::basic::container_internal::hash_internal::EnumClass;
...
template <class T>
using Set =
    basic::flat_hash_set<T, StatefulTestingHash, StatefulTestingEqual, Alloc<T>>;
...
TEST(FlatHashSet, EmplaceString) {
  ...
  basic::flat_hash_set<basic::string_view> hs(v.begin(), v.end());
...
TEST(FlatHashSet, MergeExtractInsert) {
  ...
  basic::flat_hash_set<std::unique_ptr<int>, Hash, Eq> set1, set2;
  set1.insert(basic::make_unique<int>(7));
  set1.insert(basic::make_unique<int>(17));

  set2.insert(basic::make_unique<int>(7));
  set2.insert(basic::make_unique<int>(19));
  ...
  auto node = set1.extract(basic::make_unique<int>(7));
  EXPECT_TRUE(node);
  ...
  node = set1.extract(basic::make_unique<int>(17));
  EXPECT_TRUE(node);
  EXPECT_THAT(node.value(), Pointee(17));
  EXPECT_THAT(set1, UnorderedElementsAre(Pointee(19)));

  node.value() = basic::make_unique<int>(23);
