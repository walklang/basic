...
// -----------------------------------------------------------------------------
//
// An `basic::node_hash_map<K, V>` is an unordered associative container of
// unique keys and associated values designed to be a more efficient replacement
// for `std::unordered_map`. Like `unordered_map`, search, insertion, and
// deletion of map elements can be done as an `O(1)` operation. However,
// `node_hash_map` (and other unordered associative containers known as the
// collection of Basic "Swiss tables") contain other optimizations that result
...
#include "basic/container/internal/container_memory.h"
#include "basic/container/internal/hash_function_defaults.h"  // IWYU pragma: export
#include "basic/container/internal/node_hash_policy.h"
#include "basic/container/internal/raw_hash_map.h"  // IWYU pragma: export
...
// -----------------------------------------------------------------------------
//
// An `basic::node_hash_map<K, V>` is an unordered associative container which
...
//
// By default, `node_hash_map` uses the `basic::Hash` hashing framework.
// All fundamental and Basic types that support the `basic::Hash` framework have
// a compatible equality operator for comparing insertions into `node_hash_map`.
// If your type is not yet supported by the `basic::Hash` framework, see
// basic/hash/hash.h for information on extending Basic hashing to user-defined
// types.
//
// Example:
//
//   // Create a node hash map of three strings (that map to strings)
//   basic::node_hash_map<std::string, std::string> ducks =
...
//  }
template <class Key, class Value,
          class Hash = basic::container_internal::hash_default_hash<Key>,
          class Eq = basic::container_internal::hash_default_eq<Key>,
          class Alloc = std::allocator<std::pair<const Key, Value>>>
class node_hash_map
    : public basic::container_internal::raw_hash_map<
          basic::container_internal::NodeHashMapPolicy<Key, Value>, Hash, Eq,
          Alloc> {
 ...
 public:
  ...
  //    basic::node_hash_map<int, std::string> map1;
  //
  // * Initializer List constructor
  //
  //   basic::node_hash_map<int, std::string> map2 =
  //       {{1, "huey"}, {2, "dewey"}, {3, "louie"},};
  //
  // * Copy constructor
  //
  //   basic::node_hash_map<int, std::string> map3(map2);
  //
  // * Copy assignment operator
  //
  //  // Hash functor and Comparator are copied as well
  //  basic::node_hash_map<int, std::string> map4;
  //  map4 = map3;
  //
  // * Move constructor
  //
  //   // Move is guaranteed efficient
  //   basic::node_hash_map<int, std::string> map5(std::move(map4));
  //
  // * Move assignment operator
  //
  //   // May be efficient if allocators are compatible
  //   basic::node_hash_map<int, std::string> map6;
  //   map6 = std::move(map5);
  //
  // * Range constructor
  //
  //   std::vector<std::pair<int, std::string>> v = {{1, "a"}, {2, "b"}};
  //   basic::node_hash_map<int, std::string> map7(v.begin(), v.end());
  node_hash_map() {}
  using Base::Base;
  ...
  //
  // NOTE: this member function is particular to `basic::node_hash_map` and is
...
template <class Key, class Value>
class NodeHashMapPolicy
    : public basic::container_internal::node_hash_policy<
 ...
 public:
  ...
  static value_type* new_element(Allocator* alloc, Args&&... args) {
    using PairAlloc = typename basic::allocator_traits<
        Allocator>::template rebind_alloc<value_type>;
    PairAlloc pair_alloc(*alloc);
    value_type* res =
        basic::allocator_traits<PairAlloc>::allocate(pair_alloc, 1);
    basic::allocator_traits<PairAlloc>::construct(pair_alloc, res,
  ...
  static void delete_element(Allocator* alloc, value_type* pair) {
    using PairAlloc = typename basic::allocator_traits<
        Allocator>::template rebind_alloc<value_type>;
    PairAlloc pair_alloc(*alloc);
    basic::allocator_traits<PairAlloc>::destroy(pair_alloc, pair);
    basic::allocator_traits<PairAlloc>::deallocate(pair_alloc, pair, 1);
  }

  template <class F, class... Args>
  static decltype(basic::container_internal::DecomposePair(
      std::declval<F>(), std::declval<Args>()...))
  apply(F&& f, Args&&... args) {
    return basic::container_internal::DecomposePair(std::forward<F>(f),
                                                   std::forward<Args>(args)...);
  }
...
// Specialization of trait in basic/algorithm/container.h
template <class Key, class T, class Hash, class KeyEqual, class Allocator>
struct IsUnorderedContainer<
    basic::node_hash_map<Key, T, Hash, KeyEqual, Allocator>> : std::true_type {};
