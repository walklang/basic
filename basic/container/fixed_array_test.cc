...
// Helper routine to determine if a basic::FixedArray used stack allocation.
...
TEST(FixedArrayTest, CopyCtor) {
  basic::FixedArray<int, 10> on_stack(5);
  std::iota(on_stack.begin(), on_stack.end(), 0);
  basic::FixedArray<int, 10> stack_copy = on_stack;
  EXPECT_THAT(stack_copy, ElementsAreArray(on_stack));
  EXPECT_TRUE(IsOnStack(stack_copy));

  basic::FixedArray<int, 10> allocated(15);
  std::iota(allocated.begin(), allocated.end(), 0);
  basic::FixedArray<int, 10> alloced_copy = allocated;
  EXPECT_THAT(alloced_copy, ElementsAreArray(allocated));
  EXPECT_FALSE(IsOnStack(alloced_copy));
}

TEST(FixedArrayTest, MoveCtor) {
  basic::FixedArray<std::unique_ptr<int>, 10> on_stack(5);
  for (int i = 0; i < 5; ++i) {
    on_stack[i] = basic::make_unique<int>(i);
  }

  basic::FixedArray<std::unique_ptr<int>, 10> stack_copy = std::move(on_stack);
  for (int i = 0; i < 5; ++i) EXPECT_EQ(*(stack_copy[i]), i);
  EXPECT_EQ(stack_copy.size(), on_stack.size());

  basic::FixedArray<std::unique_ptr<int>, 10> allocated(15);
  for (int i = 0; i < 15; ++i) {
    allocated[i] = basic::make_unique<int>(i);
  }

  basic::FixedArray<std::unique_ptr<int>, 10> alloced_copy =
      std::move(allocated);
...
TEST(FixedArrayTest, SmallObjects) {
...

  {
    ...
    basic::FixedArray<int> array1(0);
    basic::FixedArray<char> array2(0);
...

  {
    ...
    basic::FixedArray<std::vector<int>> array(2);
    EXPECT_EQ(0, array[0].size());
    EXPECT_EQ(0, array[1].size());
  }

  {
    // Regardless of basic::FixedArray implementation, check that a type with a
    // low alignment requirement and a non power-of-two size is initialized
    // correctly.
    ThreeInts::counter = 1;
    basic::FixedArray<ThreeInts> array(2);
...
TEST(FixedArrayRelationalsTest, EqualArrays) {
  for (int i = 0; i < 10; ++i) {
    basic::FixedArray<int, 5> a1(i);
    std::iota(a1.begin(), a1.end(), 0);
    basic::FixedArray<int, 5> a2(a1.begin(), a1.end());
...
static void TestArray(int n) {
  ...
  {
    basic::FixedArray<ConstructionTester, stack_elements> array(n);
...
static void TestArrayOfArrays(int n) {
  ...
  ConstructionTester::destructions = 0;
  {
    ...
    auto array_ptr =
        basic::make_unique<basic::FixedArray<InnerArray, inline_elements>>(n);
...
TEST(IteratorConstructorTest, NonInline) {
  ...
  basic::FixedArray<int, ABSL_ARRAYSIZE(kInput) - 1> const fixed(
...
TEST(IteratorConstructorTest, Inline) {
  int const kInput[] = {2, 3, 5, 7, 11, 13, 17};
  basic::FixedArray<int, ABSL_ARRAYSIZE(kInput)> const fixed(
      kInput, kInput + ABSL_ARRAYSIZE(kInput));
  ASSERT_EQ(ABSL_ARRAYSIZE(kInput), fixed.size());
  for (size_t i = 0; i < ABSL_ARRAYSIZE(kInput); ++i) {
    ASSERT_EQ(kInput[i], fixed[i]);
...
TEST(IteratorConstructorTest, NonPod) {
  char const* kInput[] = {"red",  "orange", "yellow", "green",
                          "blue", "indigo", "violet"};
  basic::FixedArray<std::string> const fixed(kInput,
...
TEST(IteratorConstructorTest, FromEmptyVector) {
  ...
  basic::FixedArray<int> const fixed(empty.begin(), empty.end());
...
TEST(IteratorConstructorTest, FromNonEmptyVector) {
  ...
  basic::FixedArray<int> const fixed(items.begin(), items.end());
...
TEST(FixedArrayTest, AvoidParanoidDiagnostics) {
  basic::FixedArray<char, 32> buf(32);
...
TEST(FixedArrayTest, TooBigInlinedSpace) {
  ...
  // Simulate the data members of basic::FixedArray, a pointer and a size_t.
  struct Data {
    TooBig* p;
    size_t size;
  };
  ...
  static_assert(sizeof(basic::FixedArray<TooBig, 0>) == sizeof(Data),
                "0-sized basic::FixedArray should have same size as Data.");
  static_assert(alignof(basic::FixedArray<TooBig, 0>) == alignof(Data),
                "0-sized basic::FixedArray should have same alignment as Data.");
  static_assert(sizeof(basic::FixedArray<TooBig>) == sizeof(Data),
                "default-sized basic::FixedArray should have same size as Data");
  static_assert(
      alignof(basic::FixedArray<TooBig>) == alignof(Data),
      "default-sized basic::FixedArray should have same alignment as Data.");
}
...
TEST(FixedArrayTest, UsesGlobalAlloc) { basic::FixedArray<PickyDelete, 0> a(5); }

TEST(FixedArrayTest, Data) {
  static const int kInput[] = {2, 3, 5, 7, 11, 13, 17};
  basic::FixedArray<int> fa(std::begin(kInput), std::end(kInput));
  EXPECT_EQ(fa.data(), &*fa.begin());
  EXPECT_EQ(fa.data(), &fa[0]);

  const basic::FixedArray<int>& cfa = fa;
  EXPECT_EQ(cfa.data(), &*cfa.begin());
  EXPECT_EQ(cfa.data(), &cfa[0]);
}

TEST(FixedArrayTest, Empty) {
  basic::FixedArray<int> empty(0);
  basic::FixedArray<int> inline_filled(1);
  basic::FixedArray<int, 0> heap_filled(1);
...
TEST(FixedArrayTest, FrontAndBack) {
  basic::FixedArray<int, 3 * sizeof(int)> inlined = {1, 2, 3};
  EXPECT_EQ(inlined.front(), 1);
  EXPECT_EQ(inlined.back(), 3);

  basic::FixedArray<int, 0> allocated = {1, 2, 3};
  EXPECT_EQ(allocated.front(), 1);
  EXPECT_EQ(allocated.back(), 3);

  basic::FixedArray<int> one_element = {1};
  EXPECT_EQ(one_element.front(), one_element.back());
}

TEST(FixedArrayTest, ReverseIteratorInlined) {
  basic::FixedArray<int, 5 * sizeof(int)> a = {0, 1, 2, 3, 4};

  int counter = 5;
  for (basic::FixedArray<int>::reverse_iterator iter = a.rbegin();
       iter != a.rend(); ++iter) {
    counter--;
  ...
  counter = 5;
  for (basic::FixedArray<int>::const_reverse_iterator iter = a.rbegin();
       iter != a.rend(); ++iter) {
    counter--;
    EXPECT_EQ(counter, *iter);
  }
...
TEST(FixedArrayTest, ReverseIteratorAllocated) {
  basic::FixedArray<int, 0> a = {0, 1, 2, 3, 4};
...
TEST(FixedArrayTest, Fill) {
  basic::FixedArray<int, 5 * sizeof(int)> inlined(5);
  int fill_val = 42;
  inlined.fill(fill_val);
  for (int i : inlined) EXPECT_EQ(i, fill_val);

  basic::FixedArray<int, 0> allocated(5);
...
TEST(FixedArrayTest, DefaultCtorDoesNotValueInit) {
  ...
  using FixedArrType = basic::FixedArray<T, capacity>;
  using FixedArrBuffType =
      basic::aligned_storage_t<sizeof(FixedArrType), alignof(FixedArrType)>;
...
class CountingAllocator : public std::allocator<T> {
 public:
  ...
  void construct(pointer p, Args&&... args) {
    Alloc::construct(p, basic::forward<Args>(args)...);
    if (instance_count_) {
      *instance_count_ += 1;
    }
  }
...
TEST(AllocatorSupportTest, CountInlineAllocations) {
  ...
  using AllocFxdArr = basic::FixedArray<int, inlined_size, Alloc>;
...
TEST(FixedArrayTest, AddressSanitizerAnnotations3) {
  basic::FixedArray<uint64_t, 20> a(20);
  uint64_t* raw = a.data();
  raw[0] = 0;
  raw[19] = 0;
  EXPECT_DEATH(raw[-1] = 0, "container-overflow");
...
TEST(FixedArrayTest, AddressSanitizerAnnotations4) {
  basic::FixedArray<ThreeInts> a(10);
...
TEST(FixedArrayTest, AbslHashValueWorks) {
  ...
  EXPECT_TRUE(basic::VerifyTypeImplementsAbslHashCorrectly(cases));
}
