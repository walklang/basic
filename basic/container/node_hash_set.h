...
// -----------------------------------------------------------------------------
//
// An `basic::node_hash_set<T>` is an unordered associative container designed to
...
// associative containers known as the collection of Basic "Swiss tables")
...
#include "basic/container/internal/raw_hash_set.h"  // IWYU pragma: export
...
// -----------------------------------------------------------------------------
//
// An `basic::node_hash_set<T>` is an unordered associative container which
...
//
// By default, `node_hash_set` uses the `basic::Hash` hashing framework.
...
//  }
template <class T, class Hash = basic::container_internal::hash_default_hash<T>,
          class Eq = basic::container_internal::hash_default_eq<T>,
          class Alloc = std::allocator<T>>
class node_hash_set
    : public basic::container_internal::raw_hash_set<
          basic::container_internal::NodeHashSetPolicy<T>, Hash, Eq, Alloc> {
 ...
 public:
  ...
  //    basic::node_hash_set<std::string> set1;
  //
  // * Initializer List constructor
  //
  //   basic::node_hash_set<std::string> set2 =
  //       {{"huey"}, {"dewey"}, {"louie"},};
  //
  // * Copy constructor
  //
  //   basic::node_hash_set<std::string> set3(set2);
  //
  // * Copy assignment operator
  //
  //  // Hash functor and Comparator are copied as well
  //  basic::node_hash_set<std::string> set4;
  //  set4 = set3;
  //
  // * Move constructor
  //
  //   // Move is guaranteed efficient
  //   basic::node_hash_set<std::string> set5(std::move(set4));
  //
  // * Move assignment operator
  //
  //   // May be efficient if allocators are compatible
  //   basic::node_hash_set<std::string> set6;
  //   set6 = std::move(set5);
  //
  // * Range constructor
  //
  //   std::vector<std::string> v = {"a", "b"};
  //   basic::node_hash_set<std::string> set7(v.begin(), v.end());
  node_hash_set() {}
  using Base::Base;
  ...
  //
  // NOTE: this member function is particular to `basic::node_hash_set` and is
...
template <class T>
struct NodeHashSetPolicy
    : basic::container_internal::node_hash_policy<T&, NodeHashSetPolicy<T>> {
  using key_type = T;
  using init_type = T;
  ...
  static T* new_element(Allocator* alloc, Args&&... args) {
    using ValueAlloc =
        typename basic::allocator_traits<Allocator>::template rebind_alloc<T>;
    ValueAlloc value_alloc(*alloc);
    T* res = basic::allocator_traits<ValueAlloc>::allocate(value_alloc, 1);
    basic::allocator_traits<ValueAlloc>::construct(value_alloc, res,
  ...
  static void delete_element(Allocator* alloc, T* elem) {
    using ValueAlloc =
        typename basic::allocator_traits<Allocator>::template rebind_alloc<T>;
    ValueAlloc value_alloc(*alloc);
    basic::allocator_traits<ValueAlloc>::destroy(value_alloc, elem);
    basic::allocator_traits<ValueAlloc>::deallocate(value_alloc, elem, 1);
  }

  template <class F, class... Args>
  static decltype(basic::container_internal::DecomposeValue(
      std::declval<F>(), std::declval<Args>()...))
  apply(F&& f, Args&&... args) {
    return basic::container_internal::DecomposeValue(
...
struct IsUnorderedContainer<basic::node_hash_set<Key, Hash, KeyEqual, Allocator>>
    : std::true_type {};
