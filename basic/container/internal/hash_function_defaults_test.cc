...
using StringTypes = ::testing::Types<std::string, basic::string_view>;
...
TYPED_TEST(EqString, Works) {
  auto eq = this->key_eq;
  EXPECT_TRUE(eq("a", "a"));
  EXPECT_TRUE(eq("a", basic::string_view("a")));
  EXPECT_TRUE(eq("a", std::string("a")));
  EXPECT_FALSE(eq("a", "b"));
  EXPECT_FALSE(eq("a", basic::string_view("b")));
...
TYPED_TEST(HashString, Works) {
  ...
  auto h = hash("a");
  EXPECT_EQ(h, hash(basic::string_view("a")));
  EXPECT_EQ(h, hash(std::string("a")));
  EXPECT_NE(h, hash(basic::string_view("b")));
...
// Cartesian product of (std::string, basic::string_view)
// with (std::string, basic::string_view, const char*).
using StringTypesCartesianProduct = Types<
    // clang-format off

    std::pair<basic::string_view, std::string>,
    std::pair<basic::string_view, basic::string_view>,
    std::pair<basic::string_view, const char*>>;
// clang-format on
