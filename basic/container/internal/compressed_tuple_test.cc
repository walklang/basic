...
TEST(CompressedTupleTest, MoveOnlyElements) {
  CompressedTuple<std::unique_ptr<std::string>> str_tup(
      basic::make_unique<std::string>("str"));

  CompressedTuple<CompressedTuple<std::unique_ptr<std::string>>,
                  std::unique_ptr<int>>
  x(std::move(str_tup), basic::make_unique<int>(5));
...
TEST(CompressedTupleTest, AnyElements) {
  ...
  EXPECT_EQ(basic::any_cast<int>(x.get<0>()), 5);
  EXPECT_EQ(basic::any_cast<std::string>(x.get<1>()), "str");

  a = 0.5f;
  EXPECT_EQ(basic::any_cast<float>(x.get<1>()), 0.5);

  // Ensure copy construction work in the face of a type with a universal
  // implicit constructor;
  CompressedTuple<basic::any> c{}, d(c);  // NOLINT
}
...
TEST(CompressedTupleTest, Constexpr) {
  ...
  constexpr int x2m = basic::move(x.get<2>()).get<0>();
  constexpr CallType x3m = basic::move(x).get<3>().value();
  EXPECT_EQ(x2m, 5);
