...
namespace {
using ::basic::synchronization_internal::ThreadPool;
...
TEST(HashtablezInfoTest, PrepareForSampling) {
  basic::Time test_start = basic::Now();
  HashtablezInfo info;
  basic::MutexLock l(&info.init_mu);
  ...
  info.create_time = test_start - basic::Hours(20);
...
TEST(HashtablezSamplerTest, MultiThreaded) {
    ...
    pool.Schedule([&sampler, &stop]() {
      ...
      while (!stop.HasBeenNotified()) {
        ...
        switch (std::uniform_int_distribution<>(0, 2)(gen)) {
          ...
          case 2: {
            basic::Duration oldest = basic::ZeroDuration();
            sampler.Iterate([&](const HashtablezInfo& info) {
              oldest = std::max(oldest, basic::Now() - info.create_time);
            });
            ASSERT_GE(oldest, basic::ZeroDuration());
            break;
          }
        }
      }
  ...
  // spot errors.
  basic::SleepFor(basic::Seconds(3));
  stop.Notify();
}
