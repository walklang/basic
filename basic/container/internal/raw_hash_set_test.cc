...
#include "basic/container/internal/raw_hash_set.h"
...
#include "basic/container/internal/hash_function_defaults.h"
#include "basic/container/internal/hash_policy_testing.h"
#include "basic/container/internal/hashtable_debug.h"
...
class StringPolicy {
  template <class F, class K, class V,
            class = typename std::enable_if<
                std::is_convertible<const K&, basic::string_view>::value>::type>
  decltype(std::declval<F>()(
      std::declval<const basic::string_view&>(), std::piecewise_construct,
      std::declval<std::tuple<K>>(),
      std::declval<V>())) static apply_impl(F&& f,
                                            std::pair<std::tuple<K>, V> p) {
    const basic::string_view& key = std::get<0>(p.first);
...
struct StringHash : basic::Hash<basic::string_view> {
  using is_transparent = void;
};
struct StringEq : std::equal_to<basic::string_view> {
  using is_transparent = void;
};
...
TEST(Table, EmptyFunctorOptimization) {
  static_assert(std::is_empty<std::equal_to<basic::string_view>>::value, "");
  ...
  struct StatelessHash {
    size_t operator()(basic::string_view) const { return 0; }
  };
  ...
  EXPECT_EQ(
      ...
      sizeof(
          raw_hash_set<StringPolicy, StatelessHash,
                       std::equal_to<basic::string_view>, std::allocator<int>>));
...
TEST(Table, Prefetch) {
  ...
  const auto now = [] { return basic::base_internal::CycleClock::Now(); };
...
TEST(Table, NoThrowMoveConstruct) {
  ASSERT_TRUE(
      std::is_nothrow_copy_constructible<basic::Hash<basic::string_view>>::value);
  ASSERT_TRUE(std::is_nothrow_copy_constructible<
              std::equal_to<basic::string_view>>::value);
...
TEST(Table, NoThrowMoveAssign) {
  ASSERT_TRUE(
      std::is_nothrow_move_assignable<basic::Hash<basic::string_view>>::value);
  ASSERT_TRUE(
      std::is_nothrow_move_assignable<std::equal_to<basic::string_view>>::value);
  ASSERT_TRUE(std::is_nothrow_move_assignable<std::allocator<int>>::value);
  ASSERT_TRUE(
      basic::allocator_traits<std::allocator<int>>::is_always_equal::value);
  EXPECT_TRUE(std::is_nothrow_move_assignable<StringTable>::value);
}

TEST(Table, NoThrowSwappable) {
  ASSERT_TRUE(
      container_internal::IsNoThrowSwappable<basic::Hash<basic::string_view>>());
  ASSERT_TRUE(container_internal::IsNoThrowSwappable<
              std::equal_to<basic::string_view>>());
...
struct VerifyResultOf<C, Table, basic::void_t<C<Table>>> : std::true_type {};

TEST(Table, HeterogeneousLookupOverloads) {
  using NonTransparentTable =
      raw_hash_set<StringPolicy, basic::Hash<basic::string_view>,
                   std::equal_to<basic::string_view>, std::allocator<int>>;
  ...
  using TransparentTable = raw_hash_set<
      StringPolicy,
      basic::container_internal::hash_default_hash<basic::string_view>,
      basic::container_internal::hash_default_eq<basic::string_view>,
      std::allocator<int>>;
