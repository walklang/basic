...
void HashtablezInfo::PrepareForSampling() {
  ...
  create_time = basic::Now();
  // The inliner makes hardcoded skip_count difficult (especially when combined
  // with LTO).  We use the ability to exclude stacks by regex when encoding
  // instead.
  depth = basic::GetStackTrace(stack, HashtablezInfo::kMaxStackDepth,
                              /* skip_count= */ 0);
  dead = nullptr;
}
...
HashtablezSampler::HashtablezSampler()
  ...
  basic::MutexLock l(&graveyard_.init_mu);
...
void HashtablezSampler::PushDead(HashtablezInfo* sample) {
  ...
  basic::MutexLock graveyard_lock(&graveyard_.init_mu);
  basic::MutexLock sample_lock(&sample->init_mu);
