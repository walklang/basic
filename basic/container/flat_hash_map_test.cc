...
#include "basic/container/internal/unordered_map_constructor_test.h"
#include "basic/container/internal/unordered_map_lookup_test.h"
#include "basic/container/internal/unordered_map_members_test.h"
#include "basic/container/internal/unordered_map_modifiers_test.h"
...
TEST(FlatHashMap, IteratesMsan) {
  ...
  std::vector<basic::flat_hash_map<int, balast>> garbage;
  for (int i = 0; i < 100; ++i) {
    basic::flat_hash_map<int, balast> t;
    for (int j = 0; j < 100; ++j) {
      t[j];
...
TEST(FlatHashMap, MergeExtractInsert) {
  ...
  basic::flat_hash_map<int, int> m = {{1, 7}, {2, 9}};
...
TEST(FlatHashMap, Any) {
  basic::flat_hash_map<int, basic::any> m;
  m.emplace(1, 7);
  auto it = m.find(1);
  ASSERT_NE(it, m.end());
  EXPECT_EQ(7, basic::any_cast<int>(it->second));

  m.emplace(std::piecewise_construct, std::make_tuple(2), std::make_tuple(8));
  it = m.find(2);
  ASSERT_NE(it, m.end());
  EXPECT_EQ(8, basic::any_cast<int>(it->second));

  m.emplace(std::piecewise_construct, std::make_tuple(3),
            std::make_tuple(basic::any(9)));
  it = m.find(3);
  ASSERT_NE(it, m.end());
  EXPECT_EQ(9, basic::any_cast<int>(it->second));

  struct H {
    size_t operator()(const basic::any&) const { return 0; }
  };
  struct E {
    bool operator()(const basic::any&, const basic::any&) const { return true; }
  };
  basic::flat_hash_map<basic::any, int, H, E> m2;
  m2.emplace(1, 7);
  auto it2 = m2.find(1);
