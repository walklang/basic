...
#include "basic/container/internal/counting_allocator.h"
...
using basic::container_internal::CountingAllocator;
using basic::test_internal::CopyableMovableInstance;
...
using IntVec = basic::InlinedVector<int, 8>;
...
using RefCountedVec = basic::InlinedVector<RefCounted, 8>;
...
using DynamicVec = basic::InlinedVector<Dynamic, 8>;
...
TEST(InlinedVectorTest, NoDefaultCtor) {
  basic::InlinedVector<NoDefaultCtor, 1> v(10, NoDefaultCtor(2));
  (void)v;
}
TEST(InlinedVectorTest, NoCopy) {
  basic::InlinedVector<NoCopy, 1> v(10);
  (void)v;
}
TEST(InlinedVectorTest, NoAssign) {
  basic::InlinedVector<NoAssign, 1> v(10);
  (void)v;
}
TEST(InlinedVectorTest, MoveOnly) {
  basic::InlinedVector<MoveOnly, 2> v;
...
TEST(InlinedVectorTest, Noexcept) {
  ...
  EXPECT_TRUE((std::is_nothrow_move_constructible<
               basic::InlinedVector<MoveOnly, 2>>::value));

  struct MoveCanThrow {
    MoveCanThrow(MoveCanThrow&&) {}
  };
  EXPECT_EQ(basic::default_allocator_is_nothrow::value,
            (std::is_nothrow_move_constructible<
                basic::InlinedVector<MoveCanThrow, 2>>::value));
}

TEST(InlinedVectorTest, EmplaceBack) {
  basic::InlinedVector<std::pair<std::string, int>, 1> v;
...
TEST(InlinedVectorTest, ShrinkToFitEdgeCases) {
  ...
  {
    basic::InlinedVector<std::string, 2> v(100);
    v.resize(0);
    v.shrink_to_fit();
...
class NotTriviallyDestructible {
 public:
  ...
  NotTriviallyDestructible& operator=(const NotTriviallyDestructible& other) {
    p_ = basic::make_unique<int>(*other.p_);
    return *this;
  }
...
TEST(AliasingTest, Emplace) {
  for (int i = 2; i < 20; ++i) {
    basic::InlinedVector<NotTriviallyDestructible, 10> vec;
    for (int j = 0; j < i; ++j) {
...
TEST(OverheadTest, Storage) {
  ...
  EXPECT_EQ(2 * sizeof(int*),
            sizeof(basic::InlinedVector<int*, 1>) - 1 * sizeof(int*));
  EXPECT_EQ(1 * sizeof(int*),
            sizeof(basic::InlinedVector<int*, 2>) - 2 * sizeof(int*));
  EXPECT_EQ(1 * sizeof(int*),
            sizeof(basic::InlinedVector<int*, 3>) - 3 * sizeof(int*));
  EXPECT_EQ(1 * sizeof(int*),
            sizeof(basic::InlinedVector<int*, 4>) - 4 * sizeof(int*));
  EXPECT_EQ(1 * sizeof(int*),
            sizeof(basic::InlinedVector<int*, 5>) - 5 * sizeof(int*));
  EXPECT_EQ(1 * sizeof(int*),
            sizeof(basic::InlinedVector<int*, 6>) - 6 * sizeof(int*));
  EXPECT_EQ(1 * sizeof(int*),
            sizeof(basic::InlinedVector<int*, 7>) - 7 * sizeof(int*));
  EXPECT_EQ(1 * sizeof(int*),
            sizeof(basic::InlinedVector<int*, 8>) - 8 * sizeof(int*));
}
...
TEST(StringVec, SelfRefPushBack) {
  ...
  basic::InlinedVector<std::string, 4> v;
...
TEST(StringVec, SelfMove) {
  ...
  for (int len = 0; len < 20; len++) {
    SCOPED_TRACE(len);
    basic::InlinedVector<std::string, 8> v;
    for (int i = 0; i < len; ++i) {
      SCOPED_TRACE(i);
      v.push_back(s);
    }
...
TYPED_TEST_P(InstanceTest, Swap) {
  ...
  using InstanceVec = basic::InlinedVector<Instance, 8>;
  for (int l1 = 0; l1 < 20; l1++) {
    SCOPED_TRACE(l1);
    for (int l2 = 0; l2 < 20; l2++) {
      SCOPED_TRACE(l2);
...
TEST(CountElemAssign, SimpleTypeWithInlineBacking) {
  for (size_t original_size = 0; original_size <= 5; ++original_size) {
    ...
    basic::InlinedVector<int, 2> v(original_contents.begin(),
...
TYPED_TEST_P(InstanceTest, CountElemAssignInlineBacking) {
  ...
  for (size_t original_size = 0; original_size <= 5; ++original_size) {
    ...
    basic::InlinedVector<Instance, 2> v(original_contents.begin(),
                                       original_contents.end());
...
TEST(RangedConstructor, SimpleType) {
  ...
  basic::InlinedVector<int, 4> v(source_v.begin(), source_v.end());
  EXPECT_EQ(3, v.size());
  ...
  basic::InlinedVector<int, 2> realloc_v(source_v.begin(), source_v.end());
...
void InstanceRangedConstructorTestForContainer() {
  ...
  tracker.ResetCopiesMovesSwaps();
  basic::InlinedVector<Instance, inlined_capacity> v(source_v.begin(),
                                                    source_v.end());
  EXPECT_EQ(2, v.size());
...
TEST(RangedConstructor, ElementsAreConstructed) {
  ...
  basic::InlinedVector<std::string, 1> v(source_v.begin(), source_v.end());
  EXPECT_EQ("cat", v[0]);
  EXPECT_EQ("dog", v[1]);
}
...
TEST(RangedAssign, SimpleType) {
  ...
  for (size_t original_size = 0; original_size <= 5; ++original_size) {
    ...
    for (size_t target_size = 0; target_size <= 5; ++target_size) {
      ...
      basic::InlinedVector<int, 3> v(original_contents.begin(),
                                    original_contents.end());
...
void InstanceRangedAssignTestForContainer() {
  ...
  for (size_t original_size = 0; original_size <= 5; ++original_size) {
    ...
    for (size_t target_size = 0; target_size <= 5; ++target_size) {
      ...
      basic::InlinedVector<Instance, 3> v(original_contents.begin(),
                                         original_contents.end());
      v.assign(new_contents.begin(), new_contents.end());
...
TEST(InitializerListConstructor, SimpleTypeWithInlineBacking) {
  EXPECT_THAT((basic::InlinedVector<int, 4>{4, 5, 6}),
              AllOf(SizeIs(3), CapacityIs(4), ElementsAre(4, 5, 6)));
}

TEST(InitializerListConstructor, SimpleTypeWithReallocationRequired) {
  EXPECT_THAT((basic::InlinedVector<int, 2>{4, 5, 6}),
              AllOf(SizeIs(3), CapacityIs(Gt(2)), ElementsAre(4, 5, 6)));
}

TEST(InitializerListConstructor, DisparateTypesInList) {
  EXPECT_THAT((basic::InlinedVector<int, 2>{-7, 8ULL}), ElementsAre(-7, 8));

  EXPECT_THAT((basic::InlinedVector<std::string, 2>{"foo", std::string("bar")}),
              ElementsAre("foo", "bar"));
}

TEST(InitializerListConstructor, ComplexTypeWithInlineBacking) {
  EXPECT_THAT((basic::InlinedVector<CopyableMovableInstance, 1>{
...
TEST(InitializerListConstructor, ComplexTypeWithReallocationRequired) {
  EXPECT_THAT(
      (basic::InlinedVector<CopyableMovableInstance, 1>{
...
TYPED_TEST_P(InstanceTest, InitializerListAssign) {
  ...
  for (size_t original_size = 0; original_size <= 4; ++original_size) {
    SCOPED_TRACE(original_size);
    basic::InlinedVector<Instance, 2> v(original_size, Instance(12345));
...
TEST(AllocatorSupportTest, Constructors) {
  ...
  using AllocVec = basic::InlinedVector<int, 4, MyAlloc>;
...
TEST(AllocatorSupportTest, ScopedAllocatorWorks) {
  ...
  using AllocVec = basic::InlinedVector<StdVector, 4, MyAlloc>;
  ...
  // The basic::InlinedVector does not allocate any memory.
  ...
  // The basic::InlinedVector will now allocate.
...
TEST(AllocatorSupportTest, SizeAllocConstructor) {
  ...
  using AllocVec = basic::InlinedVector<int, inlined_size, Alloc>;
...
TEST(InlinedVectorTest, AbslHashValueWorks) {
  using V = basic::InlinedVector<int, 4>;
  std::vector<V> cases;
