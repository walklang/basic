...
//
// An `basic::InlinedVector<T, N>` specifies the default capacity `N` as one of
// its template parameters. Instances where `size() <= N` hold contained
// elements in inline space. Typically `N` is very small so that sequences that
// are expected to be short do not require allocations.
//
// An `basic::InlinedVector` does not usually require a specific allocator. If
// the inlined vector grows beyond its initial constraints, it will need to
// allocate (as any normal `std::vector` would). This is usually performed with
// the default allocator (defined as `std::allocator<T>`). Optionally, a custom
// allocator type may be specified as `A` in `basic::InlinedVector<T, N, A>`.
...
#include "basic/container/internal/inlined_vector.h"
...
// -----------------------------------------------------------------------------
//
// An `basic::InlinedVector` is designed to be a drop-in replacement for
// `std::vector` for use cases where the vector's size is sufficiently small
// that it can be inlined. If the inlined vector does grow beyond its estimated
// capacity, it will trigger an initial allocation on the heap, and will behave
// as a `std:vector`. The API of the `basic::InlinedVector` within this file is
...
class InlinedVector {
  ...
  template <typename Iterator>
  using EnableIfAtLeastForwardIterator = basic::enable_if_t<
      inlined_vector_internal::IsAtLeastForwardIterator<Iterator>::value>;

  template <typename Iterator>
  using DisableIfAtLeastForwardIterator = basic::enable_if_t<
      !inlined_vector_internal::IsAtLeastForwardIterator<Iterator>::value>;
 ...
 public:
  ...
  InlinedVector(InlinedVector&& other) noexcept(
      basic::allocator_is_nothrow<allocator_type>::value ||
 ...
 private:
  ...
  friend H AbslHashValue(H h, const basic::InlinedVector<TheT, TheN, TheA>& a);
  ...
  reference Construct(pointer p, Args&&... args) {
    basic::allocator_traits<allocator_type>::construct(
  ...
  void Destroy(pointer from, pointer to) {
    for (pointer cur = from; cur != to; ++cur) {
      basic::allocator_traits<allocator_type>::destroy(*storage_.GetAllocPtr(),
                                                      cur);
    }
#if !defined(NDEBUG)
...
void swap(basic::InlinedVector<T, N, A>& a,
          basic::InlinedVector<T, N, A>& b) noexcept(noexcept(a.swap(b))) {
  a.swap(b);
}
...
template <typename T, size_t N, typename A>
bool operator==(const basic::InlinedVector<T, N, A>& a,
                const basic::InlinedVector<T, N, A>& b) {
  auto a_data = a.data();
  auto a_size = a.size();
  auto b_data = b.data();
  auto b_size = b.size();
  return basic::equal(a_data, a_data + a_size, b_data, b_data + b_size);
}
...
// `AbslHashValue()`
//
// Provides `basic::Hash` support for `basic::InlinedVector`. You do not normally
// call this function directly.
template <typename H, typename TheT, size_t TheN, typename TheA>
H AbslHashValue(H h, const basic::InlinedVector<TheT, TheN, TheA>& a) {
  auto a_data = a.data();
  auto a_size = a.size();
