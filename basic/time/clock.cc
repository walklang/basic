...
#include "basic/base/internal/unscaledcycleclock.h"
...
Time Now() {
  ...
  int64_t n = basic::GetCurrentTimeNanos();
  if (n >= 0) {
    return time_internal::FromUnixDuration(
        time_internal::MakeDuration(n / 1000000000, n % 1000000000 * 4));
  }
  return time_internal::FromUnixDuration(basic::Nanoseconds(n));
}
}  // namespace basic
...
#include "basic/time/internal/get_current_time_chrono.inc"
#else
#include "basic/time/internal/get_current_time_posix.inc"
#endif

// Allows override by test.
#ifndef GET_CURRENT_TIME_NANOS_FROM_SYSTEM
#define GET_CURRENT_TIME_NANOS_FROM_SYSTEM() \
  ::basic::time_internal::GetCurrentTimeNanosFromSystem()
#endif
...
#define GET_CURRENT_TIME_NANOS_CYCLECLOCK_NOW() \
  ::basic::time_internal::UnscaledCycleClockWrapperForGetCurrentTime::Now()
#endif
...
static basic::base_internal::SpinLock lock(
...
//
// TODO(basic-team): Remove this attribute when our compiler is smart enough
...
constexpr basic::Duration MaxSleep() {
#ifdef _WIN32
  // Windows Sleep() takes unsigned long argument in milliseconds.
  return basic::Milliseconds(
      std::numeric_limits<unsigned long>::max());  // NOLINT(runtime/int)
#else
  return basic::Seconds(std::numeric_limits<time_t>::max());
#endif
}

// Sleeps for the given duration.
// REQUIRES: to_sleep <= MaxSleep().
void SleepOnce(basic::Duration to_sleep) {
#ifdef _WIN32
  Sleep(to_sleep / basic::Milliseconds(1));
#else
  struct timespec sleep_time = basic::ToTimespec(to_sleep);
...
ABSL_ATTRIBUTE_WEAK void AbslInternalSleepFor(basic::Duration duration) {
  while (duration > basic::ZeroDuration()) {
    basic::Duration to_sleep = std::min(duration, basic::MaxSleep());
    basic::SleepOnce(to_sleep);
    duration -= to_sleep;
  }
}
