...
TEST(Time, Now) {
  const basic::Time before = basic::FromUnixNanos(basic::GetCurrentTimeNanos());
  const basic::Time now = basic::Now();
  const basic::Time after = basic::FromUnixNanos(basic::GetCurrentTimeNanos());
...
bool SleepForBounded(basic::Duration d, basic::Duration lower_bound,
                     basic::Duration upper_bound, basic::Duration timeout,
                     AlarmPolicy alarm_policy, int* attempts) {
  const basic::Time deadline = basic::Now() + timeout;
  while (basic::Now() < deadline) {
#if defined(ABSL_HAVE_ALARM)
    sig_t old_alarm = SIG_DFL;
    if (alarm_policy == AlarmPolicy::kWithAlarm) {
      alarm_handler_invoked = false;
      old_alarm = signal(SIGALRM, AlarmHandler);
      alarm(basic::ToInt64Seconds(d / 2));
    }
#else
    ...
    basic::SleepFor(d);
    basic::Duration actual = basic::Now() - start;
#if defined(ABSL_HAVE_ALARM)
    if (alarm_policy == AlarmPolicy::kWithAlarm) {
...
testing::AssertionResult AssertSleepForBounded(basic::Duration d,
                                               basic::Duration early,
                                               basic::Duration late,
                                               basic::Duration timeout,
                                               AlarmPolicy alarm_policy) {
  const basic::Duration lower_bound = d - early;
  const basic::Duration upper_bound = d + late;
  int attempts = 0;
...
TEST(SleepFor, Bounded) {
  const basic::Duration d = basic::Milliseconds(2500);
  const basic::Duration early = basic::Milliseconds(100);
  const basic::Duration late = basic::Milliseconds(300);
  const basic::Duration timeout = 48 * d;
