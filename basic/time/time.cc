...
// The implementation of the basic::Time class, which is declared in
// //basic/time.h.
//
// The representation for an basic::Time is an basic::Duration offset from the
...
//
// tz: An basic::TimeZone
// ci: An basic::TimeZone::CivilInfo
// ti: An basic::TimeZone::TimeInfo
// cd: An basic::CivilDay or a cctz::civil_day
// cs: An basic::CivilSecond or a cctz::civil_second
// bd: An basic::Time::Breakdown
...
inline int64_t FloorToUnit(basic::Duration d, basic::Duration unit) {
  basic::Duration rem;
  int64_t q = basic::IDivDuration(d, unit, &rem);
  return (q > 0 ||
          rem >= ZeroDuration() ||
          q == std::numeric_limits<int64_t>::min()) ? q : q - 1;
}

inline basic::Time::Breakdown InfiniteFutureBreakdown() {
  basic::Time::Breakdown bd;
  ...
  bd.minute = 59;
  bd.second = 59;
  bd.subsecond = basic::InfiniteDuration();
  bd.weekday = 4;
  bd.yearday = 365;
  bd.offset = 0;
  bd.is_dst = false;
...
inline basic::Time::Breakdown InfinitePastBreakdown() {
  Time::Breakdown bd;
...
inline basic::TimeZone::CivilInfo InfiniteFutureCivilInfo() {
...
inline basic::TimeZone::CivilInfo InfinitePastCivilInfo() {
  TimeZone::CivilInfo ci;
...
inline basic::TimeConversion InfiniteFutureTimeConversion() {
  basic::TimeConversion tc;
  tc.pre = tc.trans = tc.post = basic::InfiniteFuture();
  tc.kind = basic::TimeConversion::UNIQUE;
  tc.normalized = true;
  return tc;
}
...
inline TimeConversion InfinitePastTimeConversion() {
  basic::TimeConversion tc;
  tc.pre = tc.trans = tc.post = basic::InfinitePast();
  tc.kind = basic::TimeConversion::UNIQUE;
  tc.normalized = true;
  return tc;
}
...
basic::Time::Breakdown Time::In(basic::TimeZone tz) const {
  if (*this == basic::InfiniteFuture()) return InfiniteFutureBreakdown();
  if (*this == basic::InfinitePast()) return InfinitePastBreakdown();
...
basic::Time FromUDate(double udate) {
  return time_internal::FromUnixDuration(basic::Milliseconds(udate));
}

basic::Time FromUniversal(int64_t universal) {
  return basic::UniversalEpoch() + 100 * basic::Nanoseconds(universal);
}
...
int64_t ToUnixNanos(Time t) {
  ...
  return FloorToUnit(time_internal::ToUnixDuration(t), basic::Nanoseconds(1));
}
...
int64_t ToUnixMicros(Time t) {
  ...
  return FloorToUnit(time_internal::ToUnixDuration(t), basic::Microseconds(1));
}
...
int64_t ToUnixMillis(Time t) {
  ...
  return FloorToUnit(time_internal::ToUnixDuration(t), basic::Milliseconds(1));
}
...
time_t ToTimeT(Time t) { return basic::ToTimespec(t).tv_sec; }

double ToUDate(Time t) {
  return basic::FDivDuration(time_internal::ToUnixDuration(t),
                            basic::Milliseconds(1));
}

int64_t ToUniversal(basic::Time t) {
  return basic::FloorToUnit(t - basic::UniversalEpoch(), basic::Nanoseconds(100));
}

basic::Time TimeFromTimespec(timespec ts) {
  return time_internal::FromUnixDuration(basic::DurationFromTimespec(ts));
}

basic::Time TimeFromTimeval(timeval tv) {
  return time_internal::FromUnixDuration(basic::DurationFromTimeval(tv));
}

timespec ToTimespec(Time t) {
  timespec ts;
  basic::Duration d = time_internal::ToUnixDuration(t);
...
timeval ToTimeval(Time t) {
  timeval tv;
  timespec ts = basic::ToTimespec(t);
  tv.tv_sec = ts.tv_sec;
...
std::chrono::system_clock::time_point ToChronoTime(basic::Time t) {
...
basic::TimeZone::CivilInfo TimeZone::At(Time t) const {
  if (t == basic::InfiniteFuture()) return InfiniteFutureCivilInfo();
  if (t == basic::InfinitePast()) return InfinitePastCivilInfo();
...
basic::TimeZone::TimeInfo TimeZone::At(CivilSecond ct) const {
...
basic::TimeConversion ConvertDateTime(int64_t year, int mon, int day, int hour,
...
basic::Time FromTM(const struct tm& tm, basic::TimeZone tz) {
...
struct tm ToTM(basic::Time t, basic::TimeZone tz) {
  struct tm tm = {};
