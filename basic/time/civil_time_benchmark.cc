...
void BM_Difference_Days(benchmark::State& state) {
  const basic::CivilDay c(2014, 8, 22);
  const basic::CivilDay epoch(1970, 1, 1);
  while (state.KeepRunning()) {
    const basic::civil_diff_t n = c - epoch;
...
void BM_Step_Days(benchmark::State& state) {
  const basic::CivilDay kStart(2014, 8, 22);
  basic::CivilDay c = kStart;
  while (state.KeepRunning()) {
...
void BM_Format(benchmark::State& state) {
  const basic::CivilSecond c(2014, 1, 2, 3, 4, 5);
  while (state.KeepRunning()) {
    const std::string s = basic::FormatCivilTime(c);
...
void BM_CivilTimeAbslHash(benchmark::State& state) {
  ...
  basic::Hash<T> basic_hasher;
  while (state.KeepRunningBatch(kSize)) {
    for (const T civil_time : civil_times) {
      benchmark::DoNotOptimize(basic_hasher(civil_time));
    }
  }
}
void BM_CivilYearAbslHash(benchmark::State& state) {
  BM_CivilTimeAbslHash<basic::CivilYear>(state);
}
void BM_CivilMonthAbslHash(benchmark::State& state) {
  BM_CivilTimeAbslHash<basic::CivilMonth>(state);
}
void BM_CivilDayAbslHash(benchmark::State& state) {
  BM_CivilTimeAbslHash<basic::CivilDay>(state);
}
void BM_CivilHourAbslHash(benchmark::State& state) {
  BM_CivilTimeAbslHash<basic::CivilHour>(state);
}
void BM_CivilMinuteAbslHash(benchmark::State& state) {
  BM_CivilTimeAbslHash<basic::CivilMinute>(state);
}
void BM_CivilSecondAbslHash(benchmark::State& state) {
  BM_CivilTimeAbslHash<basic::CivilSecond>(state);
}
