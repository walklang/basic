...
// The implementation of the basic::Duration class, which is declared in
// //basic/time.h.  This class behaves like a numeric type; it has no public
// methods and is used only through the operators defined here.
//
// Implementation notes:
//
// An basic::Duration is represented as
//
...
inline uint64_t EncodeTwosComp(int64_t v) {
  return basic::bit_cast<uint64_t>(v);
}
inline int64_t DecodeTwosComp(uint64_t v) { return basic::bit_cast<int64_t>(v); }
...
Duration Floor(const Duration d, const Duration unit) {
  const basic::Duration td = Trunc(d, unit);
