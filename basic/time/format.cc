...
basic::Time Join(const cctz_parts& parts) {
...
std::string FormatTime(const std::string& format, basic::Time t,
                       basic::TimeZone tz) {
  if (t == basic::InfiniteFuture()) return kInfiniteFutureStr;
  if (t == basic::InfinitePast()) return kInfinitePastStr;
  const auto parts = Split(t);
  return cctz::detail::format(format, parts.sec, parts.fem,
                              cctz::time_zone(tz));
}

std::string FormatTime(basic::Time t, basic::TimeZone tz) {
  return FormatTime(RFC3339_full, t, tz);
}

std::string FormatTime(basic::Time t) {
  return basic::FormatTime(RFC3339_full, t, basic::LocalTimeZone());
}

bool ParseTime(const std::string& format, const std::string& input,
               basic::Time* time, std::string* err) {
  return basic::ParseTime(format, input, basic::UTCTimeZone(), time, err);
}

// If the input string does not contain an explicit UTC offset, interpret
// the fields with respect to the given TimeZone.
bool ParseTime(const std::string& format, const std::string& input,
               basic::TimeZone tz, basic::Time* time, std::string* err) {
...
// Functions required to support basic::Time flags.
bool ParseFlag(const std::string& text, basic::Time* t, std::string* error) {
  return basic::ParseTime(RFC3339_full, text, basic::UTCTimeZone(), t, error);
}

std::string UnparseFlag(basic::Time t) {
  return basic::FormatTime(RFC3339_full, t, basic::UTCTimeZone());
}
