...
TEST(TimeZone, NamedTimeZones) {
  ...
  const time_zone fixed0 = fixed_time_zone(basic::time_internal::cctz::seconds::zero());
...
TEST(TimeZone, Equality) {
  ...
  const time_zone fixed_zero = fixed_time_zone(basic::time_internal::cctz::seconds::zero());
...
TEST(BreakTime, TimePointResolution) {
  ...
  ExpectTime(chrono::time_point_cast<chrono::seconds>(t0), utc,
             1970, 1, 1, 0, 0, 0, 0, false, "UTC");
  ExpectTime(chrono::time_point_cast<basic::time_internal::cctz::seconds>(t0), utc,
             1970, 1, 1, 0, 0, 0, 0, false, "UTC");
...
TEST(TimeZoneImpl, LocalTimeInFixed) {
  const basic::time_internal::cctz::seconds offset =
...
TEST(MakeTime, TimePointResolution) {
  ...
  const time_point<basic::time_internal::cctz::seconds> tp_s64 =
      convert(civil_second(2015, 1, 2, 3, 4, 5), utc);
...
TEST(MakeTime, SysSecondsLimits) {
  ...
  time_point<basic::time_internal::cctz::seconds> tp;
  ...
  EXPECT_EQ(time_point<basic::time_internal::cctz::seconds>::max(), tp);
  ...
  EXPECT_EQ(time_point<basic::time_internal::cctz::seconds>::min(), tp);
    ...
    for (auto tp = zi.lookup(civil_second()).trans;
      ...
      if (fcl.kind == time_zone::civil_lookup::UNIQUE) {
        if (tcl.kind == time_zone::civil_lookup::UNIQUE) {
          ...
          const auto tprev = trans - basic::time_internal::cctz::seconds(1);
...
TEST(NextTransition, UTC) {
  ...
  auto tp = time_point<basic::time_internal::cctz::seconds>::min();
  EXPECT_FALSE(tz.next_transition(tp, &trans));

  tp = time_point<basic::time_internal::cctz::seconds>::max();
  EXPECT_FALSE(tz.next_transition(tp, &trans));
}
...
TEST(PrevTransition, UTC) {
  ...
  auto tp = time_point<basic::time_internal::cctz::seconds>::max();
  EXPECT_FALSE(tz.prev_transition(tp, &trans));

  tp = time_point<basic::time_internal::cctz::seconds>::min();
  EXPECT_FALSE(tz.prev_transition(tp, &trans));
}
...
TEST(TimeZoneEdgeCase, AmericaNewYork) {
  ...
  tp += basic::time_internal::cctz::seconds(1);
