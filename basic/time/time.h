...
//
//  * `basic::Time` defines an absolute, specific instance in time
//  * `basic::Duration` defines a signed, fixed-length span of time
//  * `basic::TimeZone` defines geopolitical time zone regions (as collected
...
// (`basic::TimeZone` within this API). That is:
//
...
//   if (!basic::LoadTimeZone("America/New_York", &nyc)) {
//      // handle error case
//   }
//
//   // My flight leaves NYC on Jan 2, 2017 at 03:04:05
//   basic::CivilSecond cs(2017, 1, 2, 3, 4, 5);
//   basic::Time takeoff = basic::FromCivil(cs, nyc);
//
//   basic::Duration flight_duration = basic::Hours(21) + basic::Minutes(35);
//   basic::Time landing = takeoff + flight_duration;
//
//   basic::TimeZone syd;
//   if (!basic::LoadTimeZone("Australia/Sydney", &syd)) {
//      // handle error case
//   }
//   std::string s = basic::FormatTime(
...
// potential to break Basic users.
//
...
// Duration
//
// The `basic::Duration` class represents a signed, fixed-length span of time.
// A `Duration` is generated using a unit-specific factory function, or is
// the result of subtracting one `basic::Time` from another. Durations behave
...
// Examples:
//
//   constexpr basic::Duration ten_ns = basic::Nanoseconds(10);
//   constexpr basic::Duration min = basic::Minutes(1);
//   constexpr basic::Duration hour = basic::Hours(1);
//   basic::Duration dur = 60 * min;  // dur == hour
//   basic::Duration half_sec = basic::Milliseconds(500);
//   basic::Duration quarter_sec = 0.25 * basic::Seconds(1);
//
...
// Example:
//
//   constexpr basic::Duration dur = basic::Milliseconds(1500);
//   int64_t ns = dur / basic::Nanoseconds(1);   // ns == 1500000000
//   int64_t ms = dur / basic::Milliseconds(1);  // ms == 1500
//   int64_t sec = dur / basic::Seconds(1);    // sec == 1 (subseconds truncated)
//   int64_t min = dur / basic::Minutes(1);    // min == 0
//
...
// Example:
//
//   constexpr basic::Duration a =
//       basic::Seconds(std::numeric_limits<int64_t>::max());  // big
//   constexpr basic::Duration b = basic::Nanoseconds(1);       // small
//
//   basic::Duration rem = a % b;
//   // rem == basic::ZeroDuration()
//
//   // Here, q would overflow int64_t, so rem accounts for the difference.
//   int64_t q = basic::IDivDuration(a, b, &rem);
...
// Example:
//
//   double d = basic::FDivDuration(basic::Milliseconds(1500), basic::Seconds(1));
//   // d == 1.5
...
// Example:
//
//   basic::Duration d = basic::Nanoseconds(123456789);
//   basic::Duration a = basic::Trunc(d, basic::Microseconds(1));  // 123456us
...
//
//   basic::Duration d = basic::Nanoseconds(123456789);
//   basic::Duration b = basic::Floor(d, basic::Microseconds(1));  // 123456us
...
//
//   basic::Duration d = basic::Nanoseconds(123456789);
//   basic::Duration c = basic::Ceil(d, basic::Microseconds(1));   // 123457us
...
//   const basic::Duration d = ... any finite duration ...
//
//   inf == inf + inf
//   inf == inf + d
//   inf == inf - inf
...
//   inf == d / 0
//   INT64_MAX == d / basic::ZeroDuration()
//
...
// `basic::Hours(24)`. (If you actually want a civil day, use basic::CivilDay
// from civil_time.h.)
//
// Example:
//
//   basic::Duration a = basic::Seconds(60);
//   basic::Duration b = basic::Minutes(1);  // b == a
...
// Example:
//
//   auto a = basic::Seconds(1.5);        // OK
//   auto b = basic::Milliseconds(1500);  // BETTER
...
// Example:
//
//   basic::Duration d = basic::Milliseconds(1500);
//   int64_t isec = basic::ToInt64Seconds(d);  // isec == 1
...
//
//   basic::Duration d = basic::Milliseconds(1500);
//   double dsec = basic::ToDoubleSeconds(d);  // dsec == 1.5
...
// FromChrono()
//
// Converts any of the pre-defined std::chrono durations to an basic::Duration.
//
// Example:
//
//   std::chrono::milliseconds ms(123);
//   basic::Duration d = basic::FromChrono(ms);
...
// ToChronoHours()
//
// Converts an basic::Duration to any of the pre-defined std::chrono durations.
// If overflow would occur, the returned value will saturate at the min/max
// chrono duration value instead.
//
// Example:
//
//   basic::Duration d = basic::Microseconds(123);
//   auto x = basic::ToChronoMicroseconds(d);
//   auto y = basic::ToChronoNanoseconds(d);  // x == y
//   auto z = basic::ToChronoSeconds(basic::InfiniteDuration());
...
// in a format that is valid input for basic::ParseDuration().
bool ParseFlag(const std::string& text, Duration* dst, std::string* error);
std::string UnparseFlag(Duration d);

// Time
//
// An `basic::Time` represents a specific instant in time. Arithmetic operators
// are provided for naturally expressing time calculations. Instances are
// created using `basic::Now()` and the `basic::From*()` factory functions that
// accept the gamut of other time representations. Formatting and parsing
// functions are provided for conversion to and from strings.  `basic::Time`
// should be passed by value rather than const reference.
//
// `basic::Time` assumes there are 60 seconds in a minute, which means the
// underlying time scales must be "smeared" to eliminate leap seconds.
// See https://developers.google.com/time/smear.
//
// Even though `basic::Time` supports a wide range of timestamps, exercise
// caution when using values in the distant past. `basic::Time` uses the
...
// governments.
//
// The `basic::Time` class represents an instant in time as a count of clock
// ticks of some granularity (resolution) from some starting point (epoch).
//
// `basic::Time` uses a resolution that is high enough to avoid loss in
...
//
//   basic::Time t1 = ...;
//   basic::Time t2 = t1 + basic::Minutes(2);
//   basic::Duration d = t2 - t1;  // == basic::Minutes(2)
//
class Time {
 public:
  // Value semantics.
  ...
  //   basic::Time t = basic::Now();
  //   basic::Time t = basic::TimeFromTimeval(tv);
  //   basic::Time t = basic::InfinitePast();
  ...
  // `basic::Time` in a certain `basic::TimeZone`. This struct is not
  // intended to represent an instant in time. So, rather than passing
  // a `Time::Breakdown` to a function, pass an `basic::Time` and an
  // `basic::TimeZone`.
  //
  // Deprecated. Use `basic::TimeZone::CivilInfo`.
  struct
      Breakdown {
  ...
  //
  // Deprecated. Use `basic::TimeZone::At(Time)`.
...
// UnixEpoch()
//
// Returns the `basic::Time` representing "1970-01-01 00:00:00.0 +0000".
constexpr Time UnixEpoch() { return Time(); }

// UniversalEpoch()
//
// Returns the `basic::Time` representing "0001-01-01 00:00:00.0 +0000", the
...
// InfiniteFuture()
//
// Returns an `basic::Time` that is infinitely far in the future.
...
// InfinitePast()
//
// Returns an `basic::Time` that is infinitely far in the past.
...
// FromUniversal()
//
// Creates an `basic::Time` from a variety of other representations.
...
// ToUniversal()
//
// Converts an `basic::Time` to a variety of other representations.  Note that
...
// FromChrono()
//
// Converts a std::chrono::system_clock::time_point to an basic::Time.
//
// Example:
//
//   auto tp = std::chrono::system_clock::from_time_t(123);
//   basic::Time t = basic::FromChrono(tp);
//   // t == basic::FromTimeT(123)
Time FromChrono(const std::chrono::system_clock::time_point& tp);

// ToChronoTime()
//
// Converts an basic::Time to a std::chrono::system_clock::time_point. If
// overflow would occur, the returned value will saturate at the min/max time
// point value instead.
//
// Example:
//
//   basic::Time t = basic::FromTimeT(123);
//   auto tp = basic::ToChronoTime(t);
//   // tp == std::chrono::system_clock::from_time_t(123);
std::chrono::system_clock::time_point ToChronoTime(Time);

// Support for flag values of type Time. Time flags must be specified in a
// format that matches basic::RFC3339_full. For example:
//
...
// seconds/milliseconds/etc from the Unix epoch, use an basic::Duration flag
// and add that duration to basic::UnixEpoch() to get an basic::Time.
bool ParseFlag(const std::string& text, Time* t, std::string* error);
std::string UnparseFlag(Time t);

// TimeZone
//
// The `basic::TimeZone` is an opaque, small, value-type class representing a
// geo-political region within which particular rules are used for converting
// between absolute and civil times (see https://git.io/v59Ly). `basic::TimeZone`
// values are named using the TZ identifiers from the IANA Time Zone Database,
// such as "America/Los_Angeles" or "Australia/Sydney". `basic::TimeZone` values
// are created from factory functions such as `basic::LoadTimeZone()`. Note:
...
// Examples:
//
//   basic::TimeZone utc = basic::UTCTimeZone();
//   basic::TimeZone pst = basic::FixedTimeZone(-8 * 60 * 60);
//   basic::TimeZone loc = basic::LocalTimeZone();
//   basic::TimeZone lax;
//   if (!basic::LoadTimeZone("America/Los_Angeles", &lax)) {
//     // handle error case
//   }
//
// See also:
...
class TimeZone {
 public:
  ...
  // than passing a `TimeZone::CivilInfo` to a function, pass an `basic::Time`
  // and an `basic::TimeZone`.
  struct CivilInfo {
    CivilSecond cs;
    Duration subsecond;
  ...
  // TimeZone::At(Time)
  //
  // Returns the civil time for this TimeZone at a certain `basic::Time`.
  ...
  // Example:
  //
  //   const auto epoch = lax.At(basic::UnixEpoch());
  //   // epoch.cs == 1969-12-31 16:00:00
  //   // epoch.subsecond == basic::ZeroDuration()
  //   // epoch.offset == -28800
  //   // epoch.is_dst == false
  //   // epoch.abbr == "PST"
  ...
  // To account for these possibilities, `basic::TimeZone::TimeInfo` is
  // richer than just a single `basic::Time`.
  struct TimeInfo {
    enum CivilKind {
  ...
  //
  // Returns an `basic::TimeInfo` containing the absolute time(s) for this
  // TimeZone at an `basic::CivilSecond`. When the civil time is skipped or
  ...
  //
  //   // A unique civil time
  //   const auto jan01 = lax.At(basic::CivilSecond(2011, 1, 1, 0, 0, 0));
  ...
  //   const auto mar13 = lax.At(basic::CivilSecond(2011, 3, 13, 2, 15, 0));
  ...
  //   const auto nov06 = lax.At(basic::CivilSecond(2011, 11, 6, 1, 15, 0));
...
//   basic::Time t = ...;
//   basic::TimeZone tz = ...;
//   const auto cd = basic::ToCivilDay(t, tz);
...
// TimeConversion
//
// An `basic::TimeConversion` represents the conversion of year, month, day,
// hour, minute, and second values (i.e., a civil time), in a particular
// `basic::TimeZone`, to a time instant (an absolute time), as returned by
// `basic::ConvertDateTime()`. Lecacy version of `basic::TimeZone::TimeInfo`.
//
// Deprecated. Use `basic::TimeZone::TimeInfo`.
struct
    TimeConversion {
  Time pre;    // time calculated using the pre-transition offset
  Time trans;  // when the civil-time discontinuity occurred
...
// ConvertDateTime()
//
// Legacy version of `basic::TimeZone::At(basic::CivilSecond)` that takes
...
//   basic::TimeConversion tc =
//       basic::ConvertDateTime(2013, 10, 32, 8, 30, 0, lax);
//   // tc.kind == TimeConversion::UNIQUE && tc.normalized == true
//   // basic::ToCivilDay(tc.pre, tz).month() == 11
//   // basic::ToCivilDay(tc.pre, tz).day() == 1
//
// Deprecated. Use `basic::TimeZone::At(CivilSecond)`.
TimeConversion ConvertDateTime(int64_t year, int mon, int day, int hour,
                               int min, int sec, TimeZone tz);

// FromDateTime()
//
// A convenience wrapper for `basic::ConvertDateTime()` that simply returns
// the "pre" `basic::Time`.  That is, the unique result, or the instant that
// is correct using the pre-transition offset (as if the transition never
// happened).
//
// Example:
//
//   basic::Time t = basic::FromDateTime(2017, 9, 26, 9, 30, 0, lax);
//   // t = 2017-09-26 09:30:00 -0700
//
// Deprecated. Use `basic::FromCivil(CivilSecond, TimeZone)`. Note that the
// behavior of `FromCivil()` differs from `FromDateTime()` for skipped civil
// times. If you care about that see `basic::TimeZone::At(basic::CivilSecond)`.
...
// `tm_sec` fields to an `basic::Time` using the given time zone. See ctime(3)
// for a description of the expected values of the tm fields. If the indicated
// time instant is not unique (see `basic::TimeZone::At(basic::CivilSecond)`
...
// ToTM()
//
// Converts the given `basic::Time` to a struct tm using the given time zone.
...
// FormatTime()
//
// Formats the given `basic::Time` in the `basic::TimeZone` according to the
...
// Example:
//
//   basic::CivilSecond cs(2013, 1, 2, 3, 4, 5);
//   basic::Time t = basic::FromCivil(cs, lax);
//   std::string f = basic::FormatTime("%H:%M:%S", t, lax);  // "03:04:05"
//   f = basic::FormatTime("%H:%M:%E3S", t, lax);  // "03:04:05.000"
//
// Note: If the given `basic::Time` is `basic::InfiniteFuture()`, the returned
// string will be exactly "infinite-future". If the given `basic::Time` is
// `basic::InfinitePast()`, the returned string will be exactly "infinite-past".
// In both cases the given format string and `basic::TimeZone` are ignored.
//
...
// returns the corresponding `basic::Time`. Uses strftime()-like formatting
...
//
// For example, parsing a string of "15:45" (%H:%M) will return an basic::Time
...
// %Ez, or %E*z).
//
// Note also that `basic::ParseTime()` only heeds the fields year, month, day,
// hour, minute, (fractional) second, and UTC offset.  Other fields, like
// weekday (%a or %A), while parsed for syntactic validity, are ignored
// in the conversion.
//
// Date and time fields that are out-of-range will be treated as errors
// rather than normalizing them like `basic::CivilSecond` does.  For example,
...
// `basic::Time` will be `basic::InfiniteFuture()` and `true` will be returned.
// If the input string is "infinite-past", the returned `basic::Time` will be
// `basic::InfinitePast()` and `true` will be returned.
//
...
// Converts a std::chrono::duration to an basic::Duration.
...
// Converts an basic::Duration to a chrono duration of type T.
template <typename T>
