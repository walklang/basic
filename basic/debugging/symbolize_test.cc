...
#include "basic/base/internal/per_thread_tls.h"
#include "basic/base/internal/raw_logging.h"
#include "basic/base/optimization.h"
#include "basic/debugging/internal/stack_consumption.h"
...
// A wrapper function for basic::Symbolize() to make the unit test simple.  The
// limit must be < sizeof(try_symbolize_buffer).  Returns null if
// basic::Symbolize() returns false, otherwise returns try_symbolize_buffer with
// the result of basic::Symbolize().
static const char *TrySymbolizeWithLimit(void *pc, int limit) {
  ...
  auto heap_buffer = basic::make_unique<char[]>(sizeof(try_symbolize_buffer));
  bool found = basic::Symbolize(pc, heap_buffer.get(), limit);
  if (found) {
    ABSL_RAW_CHECK(strnlen(heap_buffer.get(), limit) < limit,
                   "basic::Symbolize() did not properly terminate the string");
...
static void SymbolizeSignalHandler(int signo) {
  if (basic::Symbolize(g_pc_to_symbolize, g_symbolize_buffer,
...
static const char *SymbolizeStackConsumption(void *pc, int *stack_consumed) {
  ...
  *stack_consumed = basic::debugging_internal::GetSignalHandlerStackConsumption(
  ...
  for (int i = 0; i < info->dlpi_phnum; i++) {
    if (info->dlpi_phdr[i].p_type == PT_LOAD &&
      ...
      const void *const vaddr =
          basic::bit_cast<void *>(info->dlpi_addr + info->dlpi_phdr[i].p_vaddr);
      ...
      basic::debugging_internal::RegisterFileMappingHint(
...
TEST(Symbolize, SymbolizeWithMultipleMaps) {
  ...
  basic::Symbolize(kPadding0, buf, sizeof(buf));
  EXPECT_STREQ("kPadding0", buf);

  memset(buf, 0, sizeof(buf));
  basic::Symbolize(kPadding1, buf, sizeof(buf));
  ...
  basic::Symbolize(nullptr, buf, sizeof(buf));
  ...
  for (int i = 0; i < 2; i++) {
    for (size_t offset : offsets) {
      memset(buf, 0, sizeof(buf));
      basic::Symbolize(ptrs[i] + offset, buf, sizeof(buf));
...
static void DummySymbolDecorator(
    const basic::debugging_internal::SymbolDecoratorArgs *args) {
...
TEST(Symbolize, InstallAndRemoveSymbolDecorators) {
  ...
  EXPECT_GE(ticket_a = basic::debugging_internal::InstallSymbolDecorator(
                DummySymbolDecorator, &a_message),
            0);

  int ticket_b;
  std::string b_message("b");
  EXPECT_GE(ticket_b = basic::debugging_internal::InstallSymbolDecorator(
                DummySymbolDecorator, &b_message),
            0);

  int ticket_c;
  std::string c_message("c");
  EXPECT_GE(ticket_c = basic::debugging_internal::InstallSymbolDecorator(
  ...
  EXPECT_TRUE(basic::debugging_internal::RemoveSymbolDecorator(ticket_b));

  EXPECT_STREQ("ac", TrySymbolize(address++));

  // Cleanup: remove all remaining decorators so other stack traces don't
  // get mystery "ac" decoration.
  EXPECT_TRUE(basic::debugging_internal::RemoveSymbolDecorator(ticket_a));
  EXPECT_TRUE(basic::debugging_internal::RemoveSymbolDecorator(ticket_c));
}
...
TEST(Symbolize, ForEachSection) {
  ...
  ASSERT_TRUE(basic::debugging_internal::ForEachSection(
...
TEST(Symbolize, Unimplemented) {
  char buf[64];
  EXPECT_FALSE(basic::Symbolize((void *)(&nonstatic_func), buf, sizeof(buf)));
  EXPECT_FALSE(basic::Symbolize((void *)(&static_func), buf, sizeof(buf)));
  EXPECT_FALSE(basic::Symbolize((void *)(&Foo::func), buf, sizeof(buf)));
}
