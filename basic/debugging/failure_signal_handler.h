...
// -----------------------------------------------------------------------------
//
// This file configures the Basic *failure signal handler* to capture and dump
// useful debugging information (such as a stacktrace) upon program failure.
//
// To use the failure signal handler, call `basic::InstallFailureSignalHandler()`
...
//
// Note that you should *not* install the Basic failure signal handler more
// than once. You may, of course, have another (non-Basic) failure signal
// handler installed (which would be triggered if Basic's failure signal
...
//
// Struct for holding `basic::InstallFailureSignalHandler()` configuration
// options.
struct FailureSignalHandlerOptions {
  ...
  // the Basic library into a state from which they cannot recover.
