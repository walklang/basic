...
#include "basic/debugging/leak_check.h"

namespace {

TEST(LeakCheckTest, DetectLeakSanitizer) {
#ifdef ABSL_EXPECT_LEAK_SANITIZER
  EXPECT_TRUE(basic::HaveLeakSanitizer());
#else
  EXPECT_FALSE(basic::HaveLeakSanitizer());
#endif
}

TEST(LeakCheckTest, IgnoreLeakSuppressesLeakedMemoryErrors) {
  auto foo = basic::IgnoreLeak(new std::string("some ignored leaked string"));
  ABSL_RAW_LOG(INFO, "Ignoring leaked std::string %s", foo->c_str());
}

TEST(LeakCheckTest, LeakCheckDisablerIgnoresLeak) {
  basic::LeakCheckDisabler disabler;
