...
//
// Note: stack traces in Basic that do not utilize a symbolizer will result in
...
// for the `basic::GetStackFrames()` routine itself is also skipped.)
// routine itself.
//
// Example:
//
...
//        void* result[10];
//        int sizes[10];
//        int depth = basic::GetStackFrames(result, sizes, 10, 1);
//      }
//
...
// `basic::GetStackFramesWithContext()` routine itself is also skipped.)
//
...
// Records program counter values for up to `max_depth` frames, skipping the
// most recent `skip_count` stack frames, and stores their corresponding values
// in `results`. Note that this function is similar to `basic::GetStackFrames()`
...
//      bar() {
//        void* result[10];
//        int depth = basic::GetStackTrace(result, 10, 1);
//      }
//
// This produces:
//
...
// arguments passed to `basic::GetStackFramesWithContext()` except that sizes
...
// *****************************************************************************
//
// basic::SetStackUnwinder is not suitable for general purpose use.  It is
// provided for custom runtimes.
// Some things to watch out for when calling `basic::SetStackUnwinder()`:
//
...
// case where `basic::SetStackUnwinder()` was never called. Equivalents are:
//
