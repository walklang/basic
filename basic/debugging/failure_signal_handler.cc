...
#include "basic/debugging/internal/examine_stack.h"
...
static void WriteToStderr(const char* data) {
  int old_errno = errno;
  basic::raw_logging_internal::SafeWriteToStderr(data, strlen(data));
  errno = old_errno;
}
...
// Many of the basic::debugging_internal::Dump* functions in
...
ABSL_ATTRIBUTE_NOINLINE static void WriteStackTrace(
  ...
  int depth = basic::GetStackFramesWithContext(
      stack, frame_sizes, kNumStackFrames,
      1,  // Do not include this function in stack trace.
      ucontext, &min_dropped_frames);
  basic::debugging_internal::DumpPCAndFrameSizesAndStackTrace(
      basic::debugging_internal::GetProgramCounter(ucontext), stack, frame_sizes,
...
// basic::SleepFor() can't be used here since AbslInternalSleepFor()
...
// basic::base_internal::GetTID() returns pid_t on most platforms, but
// returns basic::base_internal::pid_t on Windows.
using GetTidType = decltype(basic::base_internal::GetTID());
...
static void AbslFailureSignalHandler(int signo, siginfo_t*, void* ucontext) {
  ...
  const GetTidType this_tid = basic::base_internal::GetTID();
  ...
  if (!failed_tid.compare_exchange_strong(
    ...
    ABSL_RAW_LOG(
        ...
        signo, basic::debugging_internal::GetProgramCounter(ucontext));
