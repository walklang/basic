...
template <typename T>
void BM_basic_equal_benchmark(benchmark::State& state) {
  std::vector<T> xs(state.range(0), T(0));
  std::vector<T> ys = xs;
  while (state.KeepRunning()) {
    const bool same = basic::equal(xs.begin(), xs.end(), ys.begin(), ys.end());
...
template <typename T>
void BM_basic_equal_self_benchmark(benchmark::State& state) {
  std::vector<T> xs(state.range(0), T(0));
  while (state.KeepRunning()) {
    const bool same = basic::equal(xs.begin(), xs.end(), xs.begin(), xs.end());
    benchmark::DoNotOptimize(same);
  }
}

BENCHMARK_TEMPLATE(BM_basic_equal_benchmark, uint8_t)
...
BENCHMARK_TEMPLATE(BM_memcmp_benchmark, uint8_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_basic_equal_self_benchmark, uint8_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);

BENCHMARK_TEMPLATE(BM_basic_equal_benchmark, uint16_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_std_equal_benchmark, uint16_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_memcmp_benchmark, uint16_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_basic_equal_self_benchmark, uint16_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);

BENCHMARK_TEMPLATE(BM_basic_equal_benchmark, uint32_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_std_equal_benchmark, uint32_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_memcmp_benchmark, uint32_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_basic_equal_self_benchmark, uint32_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);

BENCHMARK_TEMPLATE(BM_basic_equal_benchmark, uint64_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_std_equal_benchmark, uint64_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_memcmp_benchmark, uint64_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_basic_equal_self_benchmark, uint64_t)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);

BENCHMARK_TEMPLATE(BM_basic_equal_benchmark, EightBits)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_std_equal_benchmark, EightBits)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_memcmp_benchmark, EightBits)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
BENCHMARK_TEMPLATE(BM_basic_equal_self_benchmark, EightBits)
    ->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
