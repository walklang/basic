"""basic specific copts.

This file simply selects the correct options from the generated files.  To
change Basic copts, edit basic/copts/copts.py
"""

load(
    "//basic:copts/GENERATED_copts.bzl",
...
ABSL_DEFAULT_COPTS = select({
    "//basic:windows": ABSL_MSVC_FLAGS,
    "//basic:llvm_compiler": ABSL_LLVM_FLAGS,
    "//conditions:default": ABSL_GCC_FLAGS,
})

# in absence of modules (--compiler=gcc or -c opt), cc_tests leak their copts
# to their (included header) dependencies and fail to build outside basic
ABSL_TEST_COPTS = ABSL_DEFAULT_COPTS + select({
    "//basic:windows": ABSL_MSVC_TEST_FLAGS,
    "//basic:llvm_compiler": ABSL_LLVM_TEST_FLAGS,
    "//conditions:default": ABSL_GCC_TEST_FLAGS,
})

ABSL_EXCEPTIONS_FLAG = select({
    "//basic:windows": ABSL_MSVC_EXCEPTIONS_FLAGS,
    "//basic:llvm_compiler": ABSL_LLVM_EXCEPTIONS_FLAGS,
...
ABSL_DEFAULT_LINKOPTS = select({
    "//basic:windows": ABSL_MSVC_LINKOPTS,
    "//conditions:default": [],
})
...
def basic_random_randen_copts_init():
