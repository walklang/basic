...
TEST(WrapUniqueTest, WrapUnique) {
  ...
  // destructor for its payload gets called at the proper time.
  {
    ...
    std::unique_ptr<DestructorVerifier> ptr = basic::WrapUnique(dv);
    EXPECT_EQ(1, DestructorVerifier::instance_count());
  }
  EXPECT_EQ(0, DestructorVerifier::instance_count());
}
TEST(MakeUniqueTest, Basic) {
  std::unique_ptr<std::string> p = basic::make_unique<std::string>();
  EXPECT_EQ("", *p);
  p = basic::make_unique<std::string>("hi");
  EXPECT_EQ("hi", *p);
}
...
TEST(Initialization, MakeUnique) {
  auto p = basic::make_unique<InitializationVerifier>();
...
TEST(Initialization, MakeUniqueArray) {
  auto p = basic::make_unique<InitializationVerifier[]>(2);
...
TEST(MakeUniqueTest, MoveOnlyTypeAndValue) {
  ...
  {
    auto p = basic::make_unique<MoveOnly>();
  ...
  {
    auto p = basic::make_unique<MoveOnly>(1);
    static_assert(std::is_same<decltype(p), ExpectedType>::value,
                  "unexpected return type");
  ...
  {
    auto p = basic::make_unique<MoveOnly>(1, 2);
    static_assert(std::is_same<decltype(p), ExpectedType>::value,
                  "unexpected return type");
    EXPECT_TRUE(p->ip1 && *p->ip1 == 1);
...
TEST(MakeUniqueTest, AcceptMoveOnly) {
  auto p = basic::make_unique<AcceptMoveOnly>(MoveOnly());
...
TEST(Make_UniqueTest, Array) {
  ...
  auto p = basic::make_unique<ArrayWatch[]>(5);
...
TEST(Make_UniqueTest, NotAmbiguousWithStdMakeUnique) {
  // Ensure that basic::make_unique is not ambiguous with std::make_unique.
  // In C++14 mode, the below call to make_unique has both types as candidates.
  struct TakesStdType {
    explicit TakesStdType(const std::vector<int> &vec) {}
  };
  using basic::make_unique;
...
TEST(MakeUniqueTestNC, AcceptMoveOnlyLvalue) {
  auto m = MoveOnly();
  auto p = basic::make_unique<AcceptMoveOnly>(m);
}
...
TEST(RawPtrTest, RawPointer) {
  int i = 5;
  EXPECT_EQ(&i, basic::RawPtr(&i));
}

TEST(RawPtrTest, SmartPointer) {
  int* o = new int(5);
  std::unique_ptr<int> p(o);
  EXPECT_EQ(o, basic::RawPtr(p));
}
...
TEST(RawPtrTest, NullValuedRawPointer) {
  int* p = nullptr;
  EXPECT_EQ(nullptr, basic::RawPtr(p));
}
...
TEST(ShareUniquePtrTest, Share) {
  auto up = basic::make_unique<int>();
  int* rp = up.get();
  auto sp = basic::ShareUniquePtr(std::move(up));
  EXPECT_EQ(sp.get(), rp);
}
...
TEST(PointerTraits, Types) {
  using TraitsWith = basic::pointer_traits<PointerWith>;
  ...
  using TraitsWithout = basic::pointer_traits<PointerWithout<double, int>>;
  ...
  using TraitsRawPtr = basic::pointer_traits<char*>;
...
TEST(PointerTraits, Functions) {
  int i;
  EXPECT_EQ(&i, basic::pointer_traits<PointerWith>::pointer_to(i).ptr);
  EXPECT_EQ(&i, basic::pointer_traits<int*>::pointer_to(i));
}
...
TEST(AllocatorTraits, Typedefs) {
  ...
  EXPECT_TRUE((
      std::is_same<A,
                   typename basic::allocator_traits<A>::allocator_type>::value));
  EXPECT_TRUE(
      (std::is_same<A::value_type,
                    typename basic::allocator_traits<A>::value_type>::value));
  ...
  EXPECT_TRUE((std::is_same<SmartPointer<X>, typename basic::allocator_traits<
                                                 HasPointer>::pointer>::value));
  EXPECT_TRUE(
      (std::is_same<A::value_type*,
                    typename basic::allocator_traits<A>::pointer>::value));

  EXPECT_TRUE(
      (std::is_same<
          SmartPointer<const X>,
          typename basic::allocator_traits<HasPointer>::const_pointer>::value));
  EXPECT_TRUE(
      (std::is_same<const A::value_type*,
                    typename basic::allocator_traits<A>::const_pointer>::value));
  ...
  EXPECT_TRUE((std::is_same<HasVoidPointer::void_pointer,
                            typename basic::allocator_traits<
                                HasVoidPointer>::void_pointer>::value));
  EXPECT_TRUE(
      (std::is_same<SmartPointer<void>, typename basic::allocator_traits<
  ...
  EXPECT_TRUE(
      (std::is_same<int, typename basic::allocator_traits<
                             HasDifferenceType>::difference_type>::value));
  EXPECT_TRUE((std::is_same<char, typename basic::allocator_traits<
  ...
  EXPECT_TRUE((std::is_same<unsigned int, typename basic::allocator_traits<
                                              HasSizeType>::size_type>::value));
  EXPECT_TRUE((std::is_same<unsigned char, typename basic::allocator_traits<
  ...
  EXPECT_TRUE(
      (std::is_same<HasPropagateOnCopy::propagate_on_container_copy_assignment,
                    typename basic::allocator_traits<HasPropagateOnCopy>::
  ...
  EXPECT_TRUE(
      (std::is_same<HasPropagateOnMove::propagate_on_container_move_assignment,
                    typename basic::allocator_traits<HasPropagateOnMove>::
  ...
  EXPECT_TRUE(
      (std::is_same<HasPropagateOnSwap::propagate_on_container_swap,
                    typename basic::allocator_traits<HasPropagateOnSwap>::
                        propagate_on_container_swap>::value));
  EXPECT_TRUE(
      (std::is_same<std::false_type, typename basic::allocator_traits<A>::
                                         propagate_on_container_swap>::value));
  ...
  EXPECT_TRUE((std::is_same<std::true_type, typename basic::allocator_traits<
  ...
  EXPECT_TRUE(
      (std::is_same<std::false_type,
                    basic::allocator_traits<NonEmpty>::is_always_equal>::value));
}
...
TEST(AllocatorTraits, RebindWithPrivateInheritance) {
  ...
  EXPECT_TRUE(
      (std::is_same<AllocWithPrivateInheritance<int>,
                    basic::allocator_traits<AllocWithPrivateInheritance<char>>::
...
TEST(AllocatorTraits, Rebind) {
  ...
  EXPECT_TRUE(
      (std::is_same<basic::allocator_traits<Rebound<int>>,
                    typename basic::allocator_traits<
                        AllocWithRebind>::template rebind_traits<int>>::value));

  EXPECT_TRUE(
      (std::is_same<AllocWithoutRebind<double, char>,
                    typename basic::allocator_traits<AllocWithoutRebind<
                        int, char>>::template rebind_alloc<double>>::value));
  EXPECT_TRUE(
      (std::is_same<basic::allocator_traits<AllocWithoutRebind<double, char>>,
                    typename basic::allocator_traits<AllocWithoutRebind<
...
TEST(AllocatorTraits, FunctionsMinimal) {
  ...
  using Traits = basic::allocator_traits<MinimalMockAllocator>;
...
TEST(AllocatorTraits, FunctionsFull) {
  ...
  using Traits = basic::allocator_traits<FullMockAllocator>;
  EXPECT_CALL(mock, allocate(7)).WillRepeatedly(Return(&x));
...
TEST(AllocatorNoThrowTest, DefaultAllocator) {
  ...
  EXPECT_TRUE(basic::default_allocator_is_nothrow::value);
#else
  EXPECT_FALSE(basic::default_allocator_is_nothrow::value);
#endif
}

TEST(AllocatorNoThrowTest, StdAllocator) {
#if defined(ABSL_ALLOCATOR_NOTHROW) && ABSL_ALLOCATOR_NOTHROW
  EXPECT_TRUE(basic::allocator_is_nothrow<std::allocator<int>>::value);
#else
  EXPECT_FALSE(basic::allocator_is_nothrow<std::allocator<int>>::value);
#endif
}
...
TEST(AllocatorNoThrowTest, CustomAllocator) {
  ...
  EXPECT_TRUE(basic::allocator_is_nothrow<NoThrowAllocator>::value);
  EXPECT_FALSE(basic::allocator_is_nothrow<CanThrowAllocator>::value);
  EXPECT_FALSE(basic::allocator_is_nothrow<UnspecifiedAllocator>::value);
}
