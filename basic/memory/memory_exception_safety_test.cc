...
using ThrowerStorage =
    basic::aligned_storage_t<sizeof(Thrower), alignof(Thrower)>;
...
TEST(MakeUnique, CheckForLeaks) {
  ...
  EXPECT_TRUE(tester.Test([](Thrower* thrower) {
    static_cast<void>(basic::make_unique<Thrower>(*thrower));
  }));

  EXPECT_TRUE(tester.Test([](Thrower* thrower) {
    static_cast<void>(basic::make_unique<Thrower>(std::move(*thrower)));
  }));

  // Test T[n] overload
  EXPECT_TRUE(tester.Test([&](Thrower*) {
    static_cast<void>(basic::make_unique<Thrower[]>(kLength));
  }));
}
