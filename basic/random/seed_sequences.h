...
#include "basic/random/internal/salted_seed_seq.h"
#include "basic/random/internal/seed_material.h"
#include "basic/random/seed_gen_exception.h"
...
// -----------------------------------------------------------------------------
//
// `basic::SeedSeq` constructs a seed sequence according to [rand.req.seedseq]
// for use within bit generators. `basic::SeedSeq`, unlike `std::seed_seq`
// additionally salts the generated seeds with extra implementation-defined
// entropy. For that reason, you can use `basic::SeedSeq` in combination with
// standard library bit generators (e.g. `std::mt19937`) to introduce
// non-determinism in your seeds.
//
// Example:
//
//   basic::SeedSeq my_seed_seq({a, b, c});
//   std::mt19937 my_bitgen(my_seed_seq);
//
using SeedSeq = random_internal::SaltedSeedSeq<std::seed_seq>;

// -----------------------------------------------------------------------------
// basic::CreateSeedSeqFrom(bitgen*)
// -----------------------------------------------------------------------------
//
...
// Example:
//
//   basic::BitGen my_bitgen;
//   auto seed_seq = basic::CreateSeedSeqFrom(&my_bitgen);
//   basic::BitGen new_engine(seed_seq); // derived from my_bitgen, but not
//                                      // correlated.
//
...
SeedSeq CreateSeedSeqFrom(URBG* urbg) {
  ...
  if (!random_internal::ReadSeedMaterialFromURBG(
          urbg, basic::MakeSpan(seed_material))) {
...
// -----------------------------------------------------------------------------
//
// Constructs an `basic::SeedSeq` salting the generated values using
...
// Example:
//
//   auto my_seed_seq = basic::MakeSeedSeq();
