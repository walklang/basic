...
TYPED_TEST(GaussianDistributionInterfaceTest, SerializeTest) {
  using param_type =
      typename basic::gaussian_distribution<TypeParam>::param_type;
  ...
  for (const auto mod : {0, 1, 2, 3}) {
    for (const auto x : kParams) {
      ...
      for (const auto y : kParams) {
        ...
        basic::gaussian_distribution<TypeParam> before(mean, stddev);
        EXPECT_EQ(before.mean(), param.mean());
        EXPECT_EQ(before.stddev(), param.stddev());

        {
          basic::gaussian_distribution<TypeParam> via_param(param);
        ...
        if (!std::is_same<TypeParam, long double>::value) {
          ABSL_INTERNAL_LOG(
              INFO, basic::StrFormat("Range{%f, %f}: %f, %f", mean, stddev,
        ...
        basic::gaussian_distribution<TypeParam> after(-0.53f, 2.3456f);
...
class GaussianModel {
 public:
  ...
  double InverseCDF(double p) {
    ABSL_ASSERT(p >= 0.0);
    ABSL_ASSERT(p < 1.0);
    return mean() + stddev() * -basic::random_internal::InverseNormalSurvival(p);
  }
...
double GaussianDistributionTests::SingleChiSquaredTest() {
  ...
  if (chi_square > threshold) {
    ...
    ABSL_INTERNAL_LOG(
        INFO, basic::StrCat("mean=", mean(), " stddev=", stddev(), "\n",   //
                           " expected ", expected, "\n",                  //
...
TEST_P(GaussianDistributionTests, ZTest) {
  // TODO(basic-team): Run these tests against std::normal_distribution<double>
  ...
  for (int i = 0; i < param.trials; i++) {
    failures +=
        SingleZTest<basic::gaussian_distribution<double>>(p, kSamples) ? 0 : 1;
  }
...
TEST_P(GaussianDistributionTests, ChiSquaredTest) {
  ...
  for (int i = 0; i < kTrials; i++) {
    double p_value =
        SingleChiSquaredTest<basic::gaussian_distribution<double>>();
...
std::string ParamName(const ::testing::TestParamInfo<Param>& info) {
  ...
  std::string name = basic::StrCat("mean_", basic::SixDigits(p.mean), "__stddev_",
                                  basic::SixDigits(p.stddev));
...
// NOTE: basic::gaussian_distribution is not guaranteed to be stable.
TEST(GaussianDistributionTest, StabilityTest) {
  // basic::gaussian_distribution stability relies on the underlying zignor
  // data, basic::random_interna::RandU64ToDouble, std::exp, std::log, and
  // std::abs.
  ...
  {
    basic::gaussian_distribution<double> dist;
  ...
  urbg.reset();
  {
    basic::gaussian_distribution<float> dist;
    std::generate(std::begin(output), std::end(output),
