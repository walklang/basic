...
// basic::discrete_distribution takes O(N) time to precompute the probabilities
  ...
  class param_type {
   ...
   private:
    ...
    static_assert(std::is_integral<result_type>::value,
                  "Class-template basic::discrete_distribution<> must be "
...
discrete_distribution<IntType>::operator()(
    ...
    const param_type& p) {
  const auto idx = basic::uniform_int_distribution<result_type>(0, p.n())(g);
  const auto& q = p.q_[idx];
  const bool selected = basic::bernoulli_distribution(q.first)(g);
