...
// Benchmarks for basic random distributions as well as a selection of the
...
#include "basic/random/internal/fast_uniform_bits.h"
...
typename basic::enable_if_t<!use_default_initialization<Engine>::value, Engine>
make_engine() {
...
#define BM_BASIC(Engine)                                                       \
  ...
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     basic::random_internal::FastUniformBits<uint32_t>);        \
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     basic::random_internal::FastUniformBits<uint64_t>);        \
  ...
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     basic::uniform_int_distribution<int32_t>);                 \
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     basic::uniform_int_distribution<int64_t>);                 \
  ...
  BENCHMARK_TEMPLATE(BM_Dist, Engine, basic::uniform_real_distribution<float>); \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, basic::uniform_real_distribution<double>)
...
#define BM_THREAD(Engine)                                           \
  BENCHMARK_TEMPLATE(BM_Thread, Engine,                             \
      ...
      ->ThreadPerCpu();                                             \
  BENCHMARK_TEMPLATE(BM_Thread, Engine,                             \
                     basic::uniform_real_distribution<double>)       \
      ->ThreadPerCpu();                                             \
...
#define BM_EXTENDED(Engine)                                                    \
  ...
  BENCHMARK_TEMPLATE(BM_Small, Engine,                                         \
                     basic::uniform_real_distribution<float>);                  \
  BENCHMARK_TEMPLATE(BM_Small, Engine,                                         \
                     basic::uniform_real_distribution<double>);                 \
  /* -------------- Other -----------------------*/                            \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, std::normal_distribution<double>);       \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, basic::gaussian_distribution<double>);    \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, std::exponential_distribution<double>);  \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, basic::exponential_distribution<double>); \
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, std::poisson_distribution<int64_t>,   \
                     100);                                                     \
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, basic::poisson_distribution<int64_t>,  \
                     100);                                                     \
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, std::poisson_distribution<int64_t>,   \
                     10 * 100);                                                \
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, basic::poisson_distribution<int64_t>,  \
  ...
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, basic::poisson_distribution<int64_t>,  \
                     13 * 100);                                                \
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     basic::log_uniform_int_distribution<int32_t>);             \
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     basic::log_uniform_int_distribution<int64_t>);             \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, std::geometric_distribution<int64_t>);   \
  BENCHMARK_TEMPLATE(BM_Zipf, Engine, basic::zipf_distribution<uint64_t>);      \
  BENCHMARK_TEMPLATE(BM_Zipf, Engine, basic::zipf_distribution<uint64_t>, 3,    \
                     2);                                                       \
  BENCHMARK_TEMPLATE(BM_Bernoulli, Engine, std::bernoulli_distribution,        \
                     257305);                                                  \
  BENCHMARK_TEMPLATE(BM_Bernoulli, Engine, basic::bernoulli_distribution,       \
                     257305);                                                  \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, basic::beta_distribution<double>, 65,     \
                     41);                                                      \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, basic::beta_distribution<double>, 99,     \
                     330);                                                     \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, basic::beta_distribution<double>, 150,    \
                     150);                                                     \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, basic::beta_distribution<double>, 410,    \
                     580);                                                     \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, basic::beta_distribution<float>, 65, 41); \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, basic::beta_distribution<float>, 99,      \
                     330);                                                     \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, basic::beta_distribution<float>, 150,     \
                     150);                                                     \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, basic::beta_distribution<float>, 410,     \
                     580);                                                     \
...
BM_BASIC(basic::InsecureBitGen);  // === pcg64_2018_engine
BM_BASIC(basic::BitGen);    // === randen_engine<uint64_t>.
BM_THREAD(basic::BitGen);
BM_EXTENDED(basic::BitGen);

// Instantiate benchmarks for multiple engines.
using randen_engine_64 = basic::random_internal::randen_engine<uint64_t>;
using randen_engine_32 = basic::random_internal::randen_engine<uint32_t>;
