...
#include "basic/container/flat_hash_map.h"
...
// basic::poisson_distribution conforms to the interface and serialization
...
TYPED_TEST(PoissonDistributionInterfaceTest, SerializeTest) {
  using param_type = typename basic::poisson_distribution<TypeParam>::param_type;
  const double kMax =
  ...
  for (const double m : kParams) {
    ...
    // Validate parameters.
    basic::poisson_distribution<TypeParam> before(mean);
    EXPECT_EQ(before.mean(), param.mean());

    {
      basic::poisson_distribution<TypeParam> via_param(param);
    ...
    ABSL_INTERNAL_LOG(INFO, basic::StrCat("Range {", param.mean(), "}: ",
    ...
    basic::poisson_distribution<TypeParam> after(3.8);
...
class PoissonModel {
 public:
  ...
  void LogCDF() {
    ABSL_INTERNAL_LOG(INFO, basic::StrCat("CDF (mean = ", mean_, ")"));
    for (const auto c : cdf_) {
      ABSL_INTERNAL_LOG(INFO,
                        basic::StrCat(c.index, ": pmf=", c.pmf, " cdf=", c.cdf));
    }
  }
...
bool PoissonDistributionZTest::SingleZTest(const double p,
  ...
  basic::flat_hash_map<int32_t, int> buckets;
...
TEST_P(PoissonDistributionZTest, AbslPoissonDistribution) {
  ...
  for (int i = 0; i < param.trials; i++) {
    failures +=
        SingleZTest<basic::poisson_distribution<int32_t>>(p, param.samples) ? 0
                                                                           : 1;
  }
...
std::string ZParamName(const ::testing::TestParamInfo<ZParam>& info) {
  ...
  std::string name = basic::StrCat("mean_", basic::SixDigits(p.mean));
...
double PoissonDistributionChiSquaredTest::ChiSquaredTestImpl() {
  ...
  const double p = basic::random_internal::ChiSquarePValue(chi_square, dof);

  // Log if the chi_squared value is above the threshold.
  if (chi_square > threshold) {
    LogCDF();

    ABSL_INTERNAL_LOG(INFO, basic::StrCat("VALUES  buckets=", counts.size(),
                                         "  samples=", kSamples));
    for (size_t i = 0; i < counts.size(); i++) {
      ABSL_INTERNAL_LOG(
          INFO, basic::StrCat(cutoffs_[i], ": ", counts[i], " vs. E=", e[i]));
    }

    ABSL_INTERNAL_LOG(
        INFO,
        basic::StrCat(kChiSquared, "(data, dof=", dof, ") = ", chi_square, " (",
...
TEST_P(PoissonDistributionChiSquaredTest, AbslPoissonDistribution) {
  ...
  for (int i = 0; i < kTrials; i++) {
    double p_value = ChiSquaredTestImpl<basic::poisson_distribution<int32_t>>();
    if (p_value < 0.005) {
      failures++;
    }
  }
...
// NOTE: basic::poisson_distribution is not guaranteed to be stable.
TEST(PoissonDistributionTest, StabilityTest) {
  using testing::ElementsAre;
  // basic::poisson_distribution stability relies on stability of
  // std::exp, std::log, std::sqrt, std::ceil, std::floor, and
  // basic::FastUniformBits, basic::StirlingLogFactorial, basic::RandU64ToDouble.
  ...
  // Method 1.
  {
    basic::poisson_distribution<int> dist(5);
  ...
  {
    urbg.reset();
    basic::poisson_distribution<int> dist(25);
    std::generate(std::begin(output), std::end(output),
                  [&] { return dist(urbg); });
  }
  ...
  {
    urbg.reset();
    basic::poisson_distribution<int> dist(121);
    std::generate(std::begin(output), std::end(output),
                  [&] { return dist(urbg); });
  }
...
TEST(PoissonDistributionTest, AlgorithmExpectedValue_1) {
  ...
  basic::random_internal::sequence_urbg urbg({0x8000000000000001ull});
  basic::poisson_distribution<int> dist(5);
