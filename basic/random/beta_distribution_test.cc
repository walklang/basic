...
TYPED_TEST(BetaDistributionInterfaceTest, SerializeTest) {
  ...
  using param_type = typename basic::beta_distribution<TypeParam>::param_type;
  ...
  for (TypeParam alpha : kValues) {
    for (TypeParam beta : kValues) {
      ABSL_INTERNAL_LOG(
          INFO, basic::StrFormat("Smoke test for Beta(%f, %f)", alpha, beta));

      param_type param(alpha, beta);
      basic::beta_distribution<TypeParam> before(alpha, beta);
      EXPECT_EQ(before.alpha(), param.alpha());
      EXPECT_EQ(before.beta(), param.beta());

      {
        basic::beta_distribution<TypeParam> via_param(param);
      ...
      std::stringstream ss;
      ss << before;
      basic::beta_distribution<TypeParam> after(3.8f, 1.43f);
...
TYPED_TEST(BetaDistributionInterfaceTest, DegenerateCases) {
  ...
  };
  {
    ...
    for (TypeParam alpha : kSmallValues) {
      for (TypeParam beta : kSmallValues) {
        int zeros = 0;
        int ones = 0;
        basic::beta_distribution<TypeParam> d(alpha, beta);
        for (int i = 0; i < kCount; ++i) {
          TypeParam x = d(gen);
          if (x == 0.0) {
            zeros++;
  ...
  }
  {
    ...
    basic::beta_distribution<TypeParam> d(std::numeric_limits<TypeParam>::max(),
                                         std::numeric_limits<TypeParam>::max());
    for (int i = 0; i < kCount; ++i) {
      EXPECT_EQ(d(gen), 0.5);
    }
  }
  {
    ...
    basic::beta_distribution<TypeParam> d(
        std::numeric_limits<TypeParam>::max(),
...
bool BetaDistributionTest::SingleZTestOnMeanAndVariance(double p,
  ...
  const double z_mean = basic::random_internal::ZScore(Mean(), m);
  const bool pass =
      basic::random_internal::Near("z", z_mean, 0.0, max_err) &&
      basic::random_internal::Near("z_variance", z_variance, 0.0, max_err);
  if (!pass) {
    ABSL_INTERNAL_LOG(
        INFO,
        basic::StrFormat(
...
bool BetaDistributionTest::SingleChiSquaredTest(double p, size_t samples,
  ...
  for (; i < buckets; ++i) {
    ...
    const double boundary =
        basic::random_internal::BetaIncompleteInv(alpha_, beta_, p);
  ...
  const double chi_square = basic::random_internal::ChiSquare(
      counts.begin(), counts.end(), expected.begin(), expected.end());
  const bool pass =
      (basic::random_internal::ChiSquarePValue(chi_square, dof) >= p);
  if (!pass) {
    for (int i = 0; i < cutoffs.size(); i++) {
      ABSL_INTERNAL_LOG(
          INFO, basic::StrFormat("cutoff[%d] = %f, actual count %d, expected %d",
                                i, cutoffs[i], counts[i],
                                static_cast<int>(expected[i])));
    }

    ABSL_INTERNAL_LOG(
        INFO, basic::StrFormat(
                  "Beta(%f, %f) %s %f, p = %f", alpha_, beta_,
                  basic::random_internal::kChiSquared, chi_square,
                  basic::random_internal::ChiSquarePValue(chi_square, dof)));
  }
  return pass;
}
...
TEST_P(BetaDistributionTest, TestSampleStatistics) {
  ...
  const double p =
      basic::random_internal::RequiredSuccessProbability(kPFail, kRuns);
  static constexpr int kSampleCount = 10000;
  static constexpr int kBucketCount = 100;
  int failed = 0;
  for (int i = 0; i < kRuns; ++i) {
    if (!SingleZTestOnMeanAndVariance<basic::beta_distribution<double>>(
            p, kSampleCount)) {
      failed++;
    }
    if (!SingleChiSquaredTest<basic::beta_distribution<double>>(
...
std::string ParamName(
  ...
  std::string name = basic::StrCat("alpha_", ::testing::get<0>(info.param),
...
// NOTE: basic::beta_distribution is not guaranteed to be stable.
TEST(BetaDistributionTest, StabilityTest) {
  // basic::beta_distribution stability relies on the stability of
  // basic::random_interna::RandU64ToDouble, std::exp, std::log, std::pow,
  // and std::sqrt.
  //
  ...
  std::vector<uint64_t> output(20);
  {
    ...
    basic::beta_distribution<float> dist(0.1f, 0.2f);
  ...
  urbg.reset();
  {
    ...
    basic::beta_distribution<double> dist(0.1, 0.2);
    std::generate(std::begin(output), std::end(output),
  ...
  {
    // Algorithm Cheng 1
    basic::beta_distribution<double> dist(0.9, 2.0);
    std::generate(std::begin(output), std::end(output),
                  [&] { return double_to_u64(dist(urbg)); });
  ...
  {
    // Algorithm Cheng 2
    basic::beta_distribution<double> dist(1.5, 2.5);
    std::generate(std::begin(output), std::end(output),
                  [&] { return double_to_u64(dist(urbg)); });
...
TEST(BetaDistributionTest, AlgorithmBounds) {
  {
    ...
    // u=0.499, v=0.5
    basic::beta_distribution<double> dist(1e-4, 1e-4);
    double a = dist(urbg);
  ...
  {
    // 1/alpha = 1/beta = 2.
    basic::beta_distribution<float> dist(0.5, 0.5);
    ...
    {
      double y = basic::beta_distribution<double>(0.5, 0.5)(urbg);
    ...
    urbg.reset();
    {
      float x = basic::beta_distribution<float>(0.5, 0.5)(urbg);
      EXPECT_EQ(4, urbg.invocations());
