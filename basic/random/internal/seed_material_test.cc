...
TEST(SeedBitsToBlocks, VerifyCases) {
  EXPECT_EQ(0, basic::random_internal::SeedBitsToBlocks(0));
  EXPECT_EQ(1, basic::random_internal::SeedBitsToBlocks(1));
  EXPECT_EQ(1, basic::random_internal::SeedBitsToBlocks(31));
  EXPECT_EQ(1, basic::random_internal::SeedBitsToBlocks(32));
  EXPECT_EQ(2, basic::random_internal::SeedBitsToBlocks(33));
  EXPECT_EQ(4, basic::random_internal::SeedBitsToBlocks(127));
  EXPECT_EQ(4, basic::random_internal::SeedBitsToBlocks(128));
  EXPECT_EQ(5, basic::random_internal::SeedBitsToBlocks(129));
}
...
TEST(ReadSeedMaterialFromOSEntropy, SuccessiveReadsAreDistinct) {
  ...
  EXPECT_TRUE(basic::random_internal::ReadSeedMaterialFromOSEntropy(
      basic::Span<uint32_t>(seed_material_1, kSeedMaterialSize)));
  EXPECT_TRUE(basic::random_internal::ReadSeedMaterialFromOSEntropy(
      basic::Span<uint32_t>(seed_material_2, kSeedMaterialSize)));
...
TEST(ReadSeedMaterialFromOSEntropy, ReadZeroBytesIsNoOp) {
  ...
  EXPECT_TRUE(basic::random_internal::ReadSeedMaterialFromOSEntropy(
      basic::Span<uint32_t>(seed_material, 0)));
...
#ifdef NDEBUG
  EXPECT_FALSE(basic::random_internal::ReadSeedMaterialFromOSEntropy(
      basic::Span<uint32_t>(nullptr, 32)));
#else
  bool result;
  ABSL_EXPECT_DEATH_IF_SUPPORTED(
      result = basic::random_internal::ReadSeedMaterialFromOSEntropy(
          basic::Span<uint32_t>(nullptr, 32)),
      "!= nullptr");
...
TEST(ReadSeedMaterialFromURBG, SeedMaterialEqualsVariateSequence) {
  ...
  EXPECT_TRUE(basic::random_internal::ReadSeedMaterialFromURBG(
      &urbg_1, basic::Span<uint32_t>(seed_material, kSeedMaterialSize)));
...
TEST(ReadSeedMaterialFromURBG, ReadZeroBytesIsNoOp) {
  ...
  std::memset(seed_material, 0xAA, sizeof(seed_material));
  EXPECT_TRUE(basic::random_internal::ReadSeedMaterialFromURBG(
      &urbg, basic::Span<uint32_t>(seed_material, 0)));
...
TEST(ReadSeedMaterialFromURBG, NullUrbgArgument) {
  ...
  uint32_t seed_material[kSeedMaterialSize];
#ifdef NDEBUG
  EXPECT_FALSE(basic::random_internal::ReadSeedMaterialFromURBG<std::mt19937_64>(
      nullptr, basic::Span<uint32_t>(seed_material, kSeedMaterialSize)));
#else
  bool result;
  ABSL_EXPECT_DEATH_IF_SUPPORTED(
      result = basic::random_internal::ReadSeedMaterialFromURBG<std::mt19937_64>(
          nullptr, basic::Span<uint32_t>(seed_material, kSeedMaterialSize)),
      "!= nullptr");
  (void)result;  // suppress unused-variable warning
#endif
...
TEST(ReadSeedMaterialFromURBG, NullPtrVectorArgument) {
  std::mt19937_64 urbg;
#ifdef NDEBUG
  EXPECT_FALSE(basic::random_internal::ReadSeedMaterialFromURBG(
      &urbg, basic::Span<uint32_t>(nullptr, 32)));
#else
  bool result;
  ABSL_EXPECT_DEATH_IF_SUPPORTED(
      result = basic::random_internal::ReadSeedMaterialFromURBG(
          &urbg, basic::Span<uint32_t>(nullptr, 32)),
      "!= nullptr");
  (void)result;  // suppress unused-variable warning
#endif
...
TEST(MixSequenceIntoSeedMaterial, AvalancheEffectTestOneBitLong) {
  ...
  for (uint32_t v = 1; v != 0; v <<= 1) {
    ...
    basic::random_internal::MixIntoSeedMaterial(
        basic::Span<uint32_t>(&v, 1),
        basic::Span<uint32_t>(seed_material_copy.data(),
