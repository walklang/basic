...
// the basic random distributions.
...
template <typename T>
typename basic::enable_if_t<!std::is_base_of<std::ios_base, T>::value,
                           null_state_saver<T>>
...
// stream_u128_helper allows us to write out either basic::uint128 or
...
template <>
struct stream_u128_helper<basic::uint128> {
  template <typename IStream>
  inline basic::uint128 read(IStream& in) {
    uint64_t h = 0;
    uint64_t l = 0;
    in >> h >> l;
    return basic::MakeUint128(h, l);
  }

  template <typename OStream>
  inline void write(basic::uint128 val, OStream& out) {
