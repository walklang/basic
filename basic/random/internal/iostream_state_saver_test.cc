...
using basic::random_internal::make_istream_state_saver;
using basic::random_internal::make_ostream_state_saver;
using basic::random_internal::stream_precision_helper;

template <typename T>
typename basic::enable_if_t<std::is_integral<T>::value, T>  //
StreamRoundTrip(T t) {
  std::stringstream ss;
  {
...
template <typename T>
typename basic::enable_if_t<std::is_floating_point<T>::value, T>  //
StreamRoundTrip(T t) {
  std::stringstream ss;
  {
    auto saver = make_ostream_state_saver(ss);
  ...
  {
    auto saver = make_istream_state_saver(ss);
    result = basic::random_internal::read_floating_point<T>(ss);
  }
  EXPECT_FALSE(ss.fail())            //
      << ss.str() << " "             //
      << (ss.good() ? "good " : "")  //
