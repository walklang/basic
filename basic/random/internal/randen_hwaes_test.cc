...
#include "basic/random/internal/randen_detect.h"
#include "basic/random/internal/randen_traits.h"
#include "basic/strings/str_format.h"

namespace {

using basic::random_internal::RandenHwAes;
using basic::random_internal::RandenTraits;
...
TEST(RandenHwAesTest, Default) {
  EXPECT_TRUE(basic::random_internal::CPUSupportsRandenHwAes());
  ...
  for (const auto& elem : kGolden) {
    auto a = basic::StrFormat("%#x", elem);
    auto b = basic::StrFormat("%#x", *id++);
    EXPECT_EQ(a, b);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  int x = basic::random_internal::HasRandenHwAesImplementation();
  ABSL_RAW_LOG(INFO, "HasRandenHwAesImplementation = %d", x);

  int y = basic::random_internal::CPUSupportsRandenHwAes();
