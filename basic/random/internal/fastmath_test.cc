...
TEST(DistributionImplTest, LeadingSetBit) {
  using basic::random_internal::LeadingSetBit;
...
TEST(FastMathTest, IntLog2FloorTest) {
  using basic::random_internal::IntLog2Floor;
  constexpr uint64_t kZero = 0;
...
TEST(FastMathTest, IntLog2CeilTest) {
  using basic::random_internal::IntLog2Ceil;
  constexpr uint64_t kZero = 0;
...
TEST(FastMathTest, StirlingLogFactorial) {
  using basic::random_internal::StirlingLogFactorial;
