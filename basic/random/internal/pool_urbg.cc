...
using basic::base_internal::SpinLock;
using basic::base_internal::SpinLockHolder;
...
class RandenPoolEntry {
 public:
  ...
  void Init(basic::Span<const uint32_t> data) {
...
// Shared pool entries.
static basic::once_flag pool_once;
...
template <typename T>
void RandenPool<T>::Fill(basic::Span<result_type> data) {
  auto* pool = GetPoolForCurrentThread();
