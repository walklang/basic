...
DistributionMoments ComputeDistributionMoments(
    basic::Span<const double> data_points) {
...
std::ostream& operator<<(std::ostream& os, const DistributionMoments& moments) {
  return os << basic::StrFormat("mean=%f, stddev=%f, skewness=%f, kurtosis=%f",
...
double InverseNormalSurvival(double x) {
  ...
  return -kSqrt2 * basic::random_internal::erfinv(2 * x - 1.0);
}

bool Near(basic::string_view msg, double actual, double expected, double bound) {
  assert(bound > 0.0);
  ...
  std::string formatted = basic::StrCat(
      msg, " actual=", actual, " expected=", expected, " err=", delta / bound);
  ABSL_RAW_LOG(INFO, "%s", formatted.c_str());
  return false;
}

// TODO(basic-team): Replace with an "ABSL_HAVE_SPECIAL_MATH" and try
