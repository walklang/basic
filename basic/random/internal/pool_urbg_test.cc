...
using basic::random_internal::PoolURBG;
using basic::random_internal::RandenPool;

namespace {

// is_randen_pool trait is true when parameterized by an RandenPool
template <typename T>
using is_randen_pool = typename basic::disjunction<  //
...
typename basic::enable_if_t<basic::negation<is_randen_pool<T>>::value, void>  //
MyFill(T& rng, basic::Span<V> data) {  // NOLINT(runtime/references)
  std::generate(std::begin(data), std::end(data), rng);
}

template <typename T, typename V>
typename basic::enable_if_t<is_randen_pool<T>::value, void>  //
MyFill(T& rng, basic::Span<V> data) {  // NOLINT(runtime/references)
  rng.Fill(data);
}
