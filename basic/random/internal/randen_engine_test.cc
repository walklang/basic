...
#include "basic/random/internal/explicit_seed_seq.h"
#include "basic/strings/str_cat.h"
#include "basic/time/clock.h"

#define UPDATE_GOLDEN 0

using randen_u64 = basic::random_internal::randen_engine<uint64_t>;
using randen_u32 = basic::random_internal::randen_engine<uint32_t>;
using basic::random_internal::ExplicitSeedSeq;
...
TYPED_TEST(RandenEngineTypedTest, VerifyReseedChangesAllValues) {
  using randen = typename basic::random_internal::randen_engine<TypeParam>;
...
TEST(RandenTest, IsFastOrSlow) {
  ...
  auto start = basic::GetCurrentTimeNanos();
  for (int i = 0; i < kCount; i++) {
    sum += engine();
  }
  auto duration = basic::GetCurrentTimeNanos() - start;

  ABSL_INTERNAL_LOG(INFO, basic::StrCat(static_cast<double>(duration) /
