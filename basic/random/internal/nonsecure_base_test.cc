...
using ExampleNonsecureURBG =
    basic::random_internal::NonsecureURBGBase<std::mt19937>;
...
TEST(RecommendedTemplates, CanBeConstructed) {
  basic::BitGen default_generator;
  basic::InsecureBitGen insecure_generator;
}
...
TEST(NonsecureURBGBase, StandardInterface) {
  ...
  using E = basic::random_internal::NonsecureURBGBase<std::minstd_rand>;
...
TEST(NonsecureURBGBase, CompatibleWithDistributionUtils) {
  ...
  basic::Uniform(rbg, 0, 100);
  basic::Uniform(rbg, 0.5, 0.7);
  basic::Poisson<uint32_t>(rbg);
  basic::Exponential<float>(rbg);
}
...
struct SeederTestEngine {
  ...
  template <class SeedSequence,
            typename = typename basic::enable_if_t<
...
TEST(NonsecureURBGBase, SeederWorksForU32) {
  using U32 =
      basic::random_internal::NonsecureURBGBase<SeederTestEngine<uint32_t>>;
  U32 x;
  EXPECT_NE(0, x());
}
...
TEST(NonsecureURBGBase, SeederWorksForU64) {
  using U64 =
      basic::random_internal::NonsecureURBGBase<SeederTestEngine<uint64_t>>;
