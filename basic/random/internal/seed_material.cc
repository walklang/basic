...
// TODO(basic-team): Possibly place a cap on how much entropy may be read at a
// time.

#if defined(ABSL_RANDOM_USE_BCRYPT)

// On Windows potentially use the BCRYPT CNG API to read available entropy.
bool ReadSeedMaterialFromOSEntropyImpl(basic::Span<uint32_t> values) {
...
bool ReadSeedMaterialFromOSEntropy(basic::Span<uint32_t> values) {
...
void MixIntoSeedMaterial(basic::Span<const uint32_t> sequence,
                         basic::Span<uint32_t> seed_material) {
