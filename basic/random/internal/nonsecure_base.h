...
class NonsecureURBGBase {
 public:
  ...
  template <class SSeq, typename = typename basic::enable_if_t<
 ...
 private:
  ...
  struct Seeder {
    ...
    void generate_impl(std::integral_constant<bool, true>,
      ...
      auto buffer = basic::MakeSpan(begin, end);
      auto target = basic::MakeSpan(reinterpret_cast<uint32_t*>(buffer.data()),
                                   buffer.size());
    ...
    void generate_impl(std::integral_constant<bool, false>,
      ...
      RandenPool<uint32_t>::Fill(basic::MakeSpan(data.begin(), data.end()));
