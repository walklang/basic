...
TYPED_TEST(ZipfDistributionTypedTest, SerializeTest) {
  using param_type = typename basic::zipf_distribution<TypeParam>::param_type;
  ...
  for (const auto& param : {
    ...
    basic::zipf_distribution<TypeParam> before(k, q, v);
    EXPECT_EQ(before.k(), param.k());
    EXPECT_EQ(before.q(), param.q());
    EXPECT_EQ(before.v(), param.v());

    {
      basic::zipf_distribution<TypeParam> via_param(param);
    ...
    std::stringstream ss;
    ss << before;
    basic::zipf_distribution<TypeParam> after(4, 5.5, 4.4);
    ...
    ABSL_INTERNAL_LOG(INFO,
                      basic::StrCat("Range: ", +sample_min, ", ", +sample_max));
  }
}
...
using zipf_u64 = basic::zipf_distribution<uint64_t>;
...
TEST_P(ZipfTest, ChiSquaredTest) {
  ...
  const double threshold = basic::random_internal::ChiSquareValue(dof, 0.9999);
  ...
  if (chi_square > threshold) {
    ...
    for (size_t i = 0; i < expected.size(); i++) {
      ABSL_INTERNAL_LOG(INFO, basic::StrCat(points[i], ": ", buckets[i],
                                           " vs. E=", expected[i]));
    }
    ABSL_INTERNAL_LOG(INFO, basic::StrCat("trials ", trials));
    ABSL_INTERNAL_LOG(INFO,
                      basic::StrCat("mean ", avg, " vs. expected ", mean()));
    ABSL_INTERNAL_LOG(INFO, basic::StrCat(kChiSquared, "(data, ", dof, ") = ",
                                         chi_square, " (", p_actual, ")"));
    ABSL_INTERNAL_LOG(INFO,
                      basic::StrCat(kChiSquared, " @ 0.9995 = ", threshold));
...
std::string ParamName(
  ...
  std::string name = basic::StrCat("k_", p.k(), "__q_", basic::SixDigits(p.q()),
                                  "__v_", basic::SixDigits(p.v()));
...
// NOTE: basic::zipf_distribution is not guaranteed to be stable.
TEST(ZipfDistributionTest, StabilityTest) {
  // basic::zipf_distribution stability relies on
  // basic::uniform_real_distribution, std::log, std::exp, std::log1p
  ...
  {
    basic::zipf_distribution<int32_t> dist;
  ...
  urbg.reset();
  {
    basic::zipf_distribution<int32_t> dist(std::numeric_limits<int32_t>::max(),
                                          3.3);
    std::generate(std::begin(output), std::end(output),
                  [&] { return dist(urbg); });
...
TEST(ZipfDistributionTest, AlgorithmBounds) {
  ...
  // Small values from basic::uniform_real_distribution map to larger Zipf
  // distribution values.
  ...
  for (const auto& instance : kInputs) {
    basic::random_internal::sequence_urbg urbg({instance.first});
