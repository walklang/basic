...
TYPED_TEST(UniformIntDistributionTest, ParamSerializeTest) {
  ...
  using param_type =
      typename basic::uniform_int_distribution<TypeParam>::param_type;
  ...
  for (const auto& param : {
    ...
    basic::uniform_int_distribution<TypeParam> before(a, b);
    EXPECT_EQ(before.a(), param.a());
    EXPECT_EQ(before.b(), param.b());

    {
      // Initialize via param_type
      basic::uniform_int_distribution<TypeParam> via_param(param);
    ...
    basic::uniform_int_distribution<TypeParam> after(Limits::min() + 3,
                                                    Limits::max() - 5);
    ...
    std::string msg = basic::StrCat("Range: ", +sample_min, ", ", +sample_max);
...
TYPED_TEST(UniformIntDistributionTest, ViolatesPreconditionsDeathTest) {
  ...
  // Hi < Lo
  EXPECT_DEBUG_DEATH({ basic::uniform_int_distribution<TypeParam> dist(10, 1); },
                     "");
  ...
  basic::InsecureBitGen gen;
  basic::uniform_int_distribution<TypeParam> dist(10, 1);
  auto x = dist(gen);

  // Any value will generate a non-empty std::string.
  EXPECT_FALSE(basic::StrCat(+x).empty()) << x;
#endif  // NDEBUG
}
...
TYPED_TEST(UniformIntDistributionTest, TestMoments) {
  ...
  for (const auto& param :
    ...
    basic::uniform_int_distribution<TypeParam> dist(param);
    ...
    auto moments = basic::random_internal::ComputeDistributionMoments(values);
...
TYPED_TEST(UniformIntDistributionTest, ChiSquaredTest50) {
  ...
  const int kThreshold =
      basic::random_internal::ChiSquareValue(kBuckets, 0.999999);

  const TypeParam min = std::is_unsigned<TypeParam>::value ? 37 : -37;
  const TypeParam max = min + kBuckets;

  basic::InsecureBitGen rng;
  basic::uniform_int_distribution<TypeParam> dist(min, max);
...
TEST(UniformIntDistributionTest, StabilityTest) {
  // basic::uniform_int_distribution stability relies only on integer operations.
  ...
  {
    basic::uniform_int_distribution<int32_t> dist(0, 4);
    for (auto& v : output) {
      v = dist(urbg);
    }
  }
  ...
  {
    urbg.reset();
    basic::uniform_int_distribution<int32_t> dist(0, 100);
    for (auto& v : output) {
      v = dist(urbg);
    }
  }
  ...
  {
    urbg.reset();
    basic::uniform_int_distribution<int32_t> dist(0, 10000);
    for (auto& v : output) {
      v = dist(urbg);
    }
  }
