...
//
// * basic::uniform_real_distribution<float> generates between 2^28 and 2^29
//   distinct floating point values in the range [0, 1).
//
// * basic::uniform_real_distribution<float> generates at least 2^23 distinct
...
//
// * basic::uniform_real_distribution<double> generates more than 2^52 distinct
...
TYPED_TEST(UniformRealDistributionTest, ParamSerializeTest) {
  using param_type =
      typename basic::uniform_real_distribution<TypeParam>::param_type;
  ...
  for (const auto& param : {
    ...
    basic::uniform_real_distribution<TypeParam> before(a, b);
    EXPECT_EQ(before.a(), param.a());
    EXPECT_EQ(before.b(), param.b());

    {
      basic::uniform_real_distribution<TypeParam> via_param(param);
      EXPECT_EQ(via_param, before);
    }

    std::stringstream ss;
    ss << before;
    basic::uniform_real_distribution<TypeParam> after(TypeParam(1.0),
                                                     TypeParam(3.1));
    ...
    if (!std::is_same<TypeParam, long double>::value) {
      ...
      std::string msg = basic::StrCat("Range: ", static_cast<double>(sample_min),
...
TYPED_TEST(UniformRealDistributionTest, ViolatesPreconditionsDeathTest) {
  ...
  // Hi < Lo
  EXPECT_DEBUG_DEATH(
      { basic::uniform_real_distribution<TypeParam> dist(10.0, 1.0); }, "");

  // Hi - Lo > numeric_limits<>::max()
  EXPECT_DEBUG_DEATH(
      {
        basic::uniform_real_distribution<TypeParam> dist(
...
TYPED_TEST(UniformRealDistributionTest, ChiSquaredTest50) {
  ...
  const int kThreshold =
      basic::random_internal::ChiSquareValue(kBuckets - 1, 0.999999);
  ...
  for (const auto& param : {param_type(0, 1), param_type(5, 12),
    ...
    basic::uniform_real_distribution<TypeParam> dist(param);
    ...
    if (chi_square > kThreshold) {
      double p_value =
          basic::random_internal::ChiSquarePValue(chi_square, kBuckets);

      // Chi-squared test failed. Output does not appear to be uniform.
      std::string msg;
      for (const auto& a : counts) {
        basic::StrAppend(&msg, a, "\n");
      }
      basic::StrAppend(&msg, kChiSquared, " p-value ", p_value, "\n");
      basic::StrAppend(&msg, "High ", kChiSquared, " value: ", chi_square, " > ",
                      kThreshold);
      ABSL_RAW_LOG(INFO, "%s", msg.c_str());
      FAIL() << msg;
    }
...
TYPED_TEST(UniformRealDistributionTest, StabilityTest) {
  // basic::uniform_real_distribution stability relies only on
...
TEST(UniformRealDistributionTest, AlgorithmBounds) {
  basic::uniform_real_distribution<double> dist;
  ...
  {
    // This returns a value very near 0.5 from basic::uniform_real_distribution.
    basic::random_internal::sequence_urbg urbg({0x8000000000000000ull});
    double a = dist(urbg);
    EXPECT_EQ(a, 0.5);
  }
