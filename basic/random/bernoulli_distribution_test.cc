...
TEST_P(BernoulliTest, Serialize) {
  ...
  basic::bernoulli_distribution before(d);

  {
    basic::bernoulli_distribution via_param{
        basic::bernoulli_distribution::param_type(d)};
    EXPECT_EQ(via_param, before);
  }

  std::stringstream ss;
  ss << before;
  basic::bernoulli_distribution after(0.6789);
...
TEST_P(BernoulliTest, Accuracy) {
  ...
  size_t yes = 0;
  basic::bernoulli_distribution dist(p);
...
// NOTE: basic::bernoulli_distribution is not guaranteed to be stable.
TEST(BernoulliTest, StabilityTest) {
  // basic::bernoulli_distribution stability relies on FastUniformBits and
  // integer arithmetic.
  ...
  auto generate = [&urbg](basic::bernoulli_distribution& dist) {
    std::string output;
    output.reserve(36);
    urbg.reset();
    for (int i = 0; i < 35; i++) {
...
TEST(BernoulliTest, StabilityTest2) {
  ...
  {
    basic::bernoulli_distribution dist(b0);
    auto v = generate(dist);
    EXPECT_EQ(12, urbg.invocations());
    EXPECT_EQ(v, "000011100101") << dist;
  }
  {
    basic::bernoulli_distribution dist(b1);
    auto v = generate(dist);
    EXPECT_EQ(12, urbg.invocations());
    EXPECT_EQ(v, "001111101101") << dist;
  }
  {
    basic::bernoulli_distribution dist(b3);
    auto v = generate(dist);
    EXPECT_EQ(12, urbg.invocations());
