  ...
  class param_type {
   ...
   private:
    ...
    static_assert(std::is_integral<IntType>::value,
                  "Class-template basic::log_uniform_int_distribution<> must be "
...
log_uniform_int_distribution<IntType>::Generate(
  ...
  // 2 => [b, (b^2)-1]
  // n => [b^(n-1)..(b^n)-1]
  const int e = basic::uniform_int_distribution<int>(0, p.log_range())(g);
  if (e == 0) {
    return 0;
  }
  const int d = e - 1;
  ...
  return basic::uniform_int_distribution<result_type>(lo, hi)(g);
}
