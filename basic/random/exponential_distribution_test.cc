...
#include "basic/random/internal/chi_square.h"
#include "basic/random/internal/distribution_test_util.h"
#include "basic/random/internal/sequence_urbg.h"
...
using basic::random_internal::kChiSquared;
...
TYPED_TEST(ExponentialDistributionTypedTest, SerializeTest) {
  using param_type =
      typename basic::exponential_distribution<TypeParam>::param_type;
  ...
  for (const TypeParam lambda : kParams) {
    ...
    basic::exponential_distribution<TypeParam> before(lambda);
    EXPECT_EQ(before.lambda(), param.lambda());

    {
      basic::exponential_distribution<TypeParam> via_param(param);
    ...
    if (!std::is_same<TypeParam, long double>::value) {
      ABSL_INTERNAL_LOG(INFO,
                        basic::StrFormat("Range {%f}: %f, %f, lambda=%f", lambda,
    ...
    basic::exponential_distribution<TypeParam> after(34.56f);
...
bool ExponentialDistributionTests::SingleZTest(const double p,
  ...
  const auto m = basic::random_internal::ComputeDistributionMoments(data);
  const double max_err = basic::random_internal::MaxErrorTolerance(p);
  const double z = basic::random_internal::ZScore(mean(), m);
  const bool pass = basic::random_internal::Near("z", z, 0.0, max_err);

  if (!pass) {
    ABSL_INTERNAL_LOG(
        INFO, basic::StrFormat("p=%f max_err=%f\n"
                              " lambda=%f\n"
                              " mean=%f vs. %f\n"
                              " stddev=%f vs. %f\n"
                              " skewness=%f vs. %f\n"
...
double ExponentialDistributionTests::SingleChiSquaredTest() {
  ...
  const double threshold = basic::random_internal::ChiSquareValue(dof, 0.98);

  const double expected =
      static_cast<double>(kSamples) / static_cast<double>(counts.size());

  double chi_square = basic::random_internal::ChiSquareWithExpected(
      std::begin(counts), std::end(counts), expected);
  double p = basic::random_internal::ChiSquarePValue(chi_square, dof);

  if (chi_square > threshold) {
    for (int i = 0; i < cutoffs.size(); i++) {
      ABSL_INTERNAL_LOG(
          INFO, basic::StrFormat("%d : (%f) = %d", i, cutoffs[i], counts[i]));
    }

    ABSL_INTERNAL_LOG(INFO,
                      basic::StrCat("lambda ", lambda(), "\n",     //
                                   " expected ", expected, "\n",  //
...
TEST_P(ExponentialDistributionTests, ZTest) {
  ...
  const double p = basic::random_internal::RequiredSuccessProbability(
      param.p_fail, param.trials);

  int failures = 0;
  for (int i = 0; i < param.trials; i++) {
    failures += SingleZTest<basic::exponential_distribution<double>>(p, kSamples)
                    ? 0
                    : 1;
  }
...
TEST_P(ExponentialDistributionTests, ChiSquaredTest) {
  ...
  for (int i = 0; i < kTrials; i++) {
    double p_value =
        SingleChiSquaredTest<basic::exponential_distribution<double>>();
...
std::string ParamName(const ::testing::TestParamInfo<Param>& info) {
  ...
  std::string name = basic::StrCat("lambda_", basic::SixDigits(p.lambda));
  return basic::StrReplaceAll(name, {{"+", "_"}, {"-", "_"}, {".", "_"}});
}

INSTANTIATE_TEST_CASE_P(, ExponentialDistributionTests,
                        ::testing::ValuesIn(GenParams()), ParamName);

// NOTE: basic::exponential_distribution is not guaranteed to be stable.
TEST(ExponentialDistributionTest, StabilityTest) {
  // basic::exponential_distribution stability relies on std::log1p and
  // basic::uniform_real_distribution.
  basic::random_internal::sequence_urbg urbg(
  ...
  {
    basic::exponential_distribution<double> dist;
  ...
  urbg.reset();
  {
    basic::exponential_distribution<float> dist;
    std::generate(std::begin(output), std::end(output),
...
TEST(ExponentialDistributionTest, AlgorithmBounds) {
  // Relies on basic::uniform_real_distribution, so some of these comments
  // reference that.
  basic::exponential_distribution<double> dist;

  {
    // This returns the smallest value >0 from basic::uniform_real_distribution.
    basic::random_internal::sequence_urbg urbg({0x0000000000000001ull});
    double a = dist(urbg);
    EXPECT_EQ(a, 5.42101086242752217004e-20);
  }

  {
    // This returns a value very near 0.5 from basic::uniform_real_distribution.
    basic::random_internal::sequence_urbg urbg({0x7fffffffffffffefull});
    double a = dist(urbg);
    EXPECT_EQ(a, 0.693147180559945175204);
  }

  {
    // This returns the largest value <1 from basic::uniform_real_distribution.
    // WolframAlpha: ~39.1439465808987766283058547296341915292187253
    basic::random_internal::sequence_urbg urbg({0xFFFFFFFFFFFFFFeFull});
    double a = dist(urbg);
    EXPECT_EQ(a, 36.7368005696771007251);
  }
  {
    // This *ALSO* returns the largest value <1.
    basic::random_internal::sequence_urbg urbg({0xFFFFFFFFFFFFFFFFull});
    double a = dist(urbg);
    EXPECT_EQ(a, 36.7368005696771007251);
  }
}
