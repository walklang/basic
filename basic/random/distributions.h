...
// combination with an Basic random bit generator to produce random values
// according to the rules of that distribution.
//
// The Basic random library defines the following distributions within this
// file:
//
//   * `basic::Uniform` for uniform (constant) distributions having constant
//     probability
//   * `basic::Bernoulli` for discrete distributions having exactly two outcomes
//   * `basic::Beta` for continuous distributions parameterized through two
//     free parameters
//   * `basic::Exponential` for discrete distributions of events occurring
//     continuously and independently at a constant average rate
//   * `basic::Gaussian` (also known as "normal distributions") for continuous
//     distributions using an associated quadratic function
//   * `basic::LogUniform` for continuous uniform distributions where the log
//     to the given base of all values is uniform
//   * `basic::Poisson` for discrete probability distributions that express the
//     probability of a given number of events occurring within a fixed interval
//   * `basic::Zipf` for discrete probability distributions commonly used for
...
#include "basic/random/distribution_format_traits.h"
#include "basic/random/exponential_distribution.h"
#include "basic/random/gaussian_distribution.h"
#include "basic/random/internal/distributions.h"  // IWYU pragma: export
#include "basic/random/internal/uniform_helper.h"  // IWYU pragma: export
...
// -----------------------------------------------------------------------------
// basic::Uniform<T>(tag, bitgen, lo, hi)
// -----------------------------------------------------------------------------
//
// `basic::Uniform()` produces random values of type `T` uniformly distributed in
...
//
//   * `basic::IntervalOpenOpen`
//   * `basic::IntervalOpenClosed`
//   * `basic::IntervalClosedOpen`
//   * `basic::IntervalClosedClosed`
//
...
//
// In the absence of an explicit return type `T`, `basic::Uniform()` will deduce
...
//   auto x = basic::Uniform(basic::IntervalClosedClosed, bitgen, 0.0f, 1.0f);
//
//   // The most common interval of `basic::IntervalClosedOpen` is available by
//   // default:
//
//   auto x = basic::Uniform(bitgen, 0.0f, 1.0f);
//
...
//
//   auto x = basic::Uniform<float>(bitgen, 0, 1);
//
template <typename R = void, typename TagType, typename URBG>
typename basic::enable_if_t<!std::is_same<R, void>::value, R>  //
Uniform(TagType tag,
        URBG&& urbg,  // NOLINT(runtime/references)
        R lo, R hi) {
  using gen_t = basic::decay_t<URBG>;
  return random_internal::UniformImpl<R, TagType, gen_t>(tag, urbg, lo, hi);
}

// basic::Uniform<T>(bitgen, lo, hi)
//
...
Uniform(URBG&& urbg,  // NOLINT(runtime/references)
        R lo, R hi) {
  constexpr auto tag = basic::IntervalClosedOpen;
...
// basic::Uniform(tag, bitgen, lo, hi)
//
...
template <typename R = void, typename TagType, typename URBG, typename A,
          typename B>
typename basic::enable_if_t<std::is_same<R, void>::value,
...
// basic::Uniform<unsigned T>(bitgen)
//
...
typename basic::enable_if_t<!std::is_signed<R>::value, R>  //
Uniform(URBG&& urbg) {  // NOLINT(runtime/references)
  constexpr auto tag = basic::IntervalClosedClosed;
...
// -----------------------------------------------------------------------------
// basic::Bernoulli(bitgen, p)
// -----------------------------------------------------------------------------
//
// `basic::Bernoulli` produces a random boolean value, with probability `p`
// (where 0.0 <= p <= 1.0) equaling `true`.
//
// Prefer `basic::Bernoulli` to produce boolean values over other alternatives
// such as comparing an `basic::Uniform()` value to a specific output.
//
...
//   basic::BitGen bitgen;
//   ...
//   if (basic::Bernoulli(bitgen, 1.0/3721.0)) {
...
// -----------------------------------------------------------------------------
// basic::Beta<T>(bitgen, alpha, beta)
// -----------------------------------------------------------------------------
//
// `basic::Beta` produces a floating point number distributed in the closed
...
//   basic::BitGen bitgen;
//   ...
//   double sample = basic::Beta(bitgen, 3.0, 2.0);
//
...
RealType Beta(URBG&& urbg,  // NOLINT(runtime/references)
  ...
  static_assert(
      ...
      "basic::Beta<RealType, URBG>(...)");

  using gen_t = basic::decay_t<URBG>;
  using distribution_t = typename basic::beta_distribution<RealType>;
  using format_t = random_internal::DistributionFormatTraits<distribution_t>;
...
// -----------------------------------------------------------------------------
// basic::Exponential<T>(bitgen, lambda = 1)
// -----------------------------------------------------------------------------
//
// `basic::Exponential` produces a floating point number for discrete
...
//   basic::BitGen bitgen;
//   ...
//   double call_length = basic::Exponential(bitgen, 7.0);
//
template <typename RealType, typename URBG>
RealType Exponential(URBG&& urbg,  // NOLINT(runtime/references)
  ...
  static_assert(
      std::is_floating_point<RealType>::value,
      "Template-argument 'RealType' must be a floating-point type, in "
      "basic::Exponential<RealType, URBG>(...)");

  using gen_t = basic::decay_t<URBG>;
  using distribution_t = typename basic::exponential_distribution<RealType>;
  using format_t = random_internal::DistributionFormatTraits<distribution_t>;
...
// -----------------------------------------------------------------------------
// basic::Gaussian<T>(bitgen, mean = 0, stddev = 1)
// -----------------------------------------------------------------------------
//
// `basic::Gaussian` produces a floating point number selected from the Gaussian
...
//   basic::BitGen bitgen;
//   ...
//   double giraffe_height = basic::Gaussian(bitgen, 16.3, 3.3);
//
template <typename RealType, typename URBG>
RealType Gaussian(URBG&& urbg,  // NOLINT(runtime/references)
  ...
  static_assert(
      std::is_floating_point<RealType>::value,
      "Template-argument 'RealType' must be a floating-point type, in "
      "basic::Gaussian<RealType, URBG>(...)");

  using gen_t = basic::decay_t<URBG>;
  using distribution_t = typename basic::gaussian_distribution<RealType>;
  using format_t = random_internal::DistributionFormatTraits<distribution_t>;
...
// -----------------------------------------------------------------------------
// basic::LogUniform<T>(bitgen, lo, hi, base = 2)
// -----------------------------------------------------------------------------
//
// `basic::LogUniform` produces random values distributed where the log to a
...
//   basic::BitGen bitgen;
//   ...
//   int v = basic::LogUniform(bitgen, 0, 1000);
//
...
IntType LogUniform(URBG&& urbg,  // NOLINT(runtime/references)
  ...
  static_assert(std::is_integral<IntType>::value,
                ...
                "basic::LogUniform<IntType, URBG>(...)");

  using gen_t = basic::decay_t<URBG>;
  using distribution_t = typename basic::log_uniform_int_distribution<IntType>;
  using format_t = random_internal::DistributionFormatTraits<distribution_t>;
...
// -----------------------------------------------------------------------------
// basic::Poisson<T>(bitgen, mean = 1)
// -----------------------------------------------------------------------------
//
// `basic::Poisson` produces discrete probabilities for a given number of events
...
//   basic::BitGen bitgen;
//   ...
//   int requests_per_minute = basic::Poisson<int>(bitgen, 3.2);
//
template <typename IntType, typename URBG>
IntType Poisson(URBG&& urbg,  // NOLINT(runtime/references)
  ...
  static_assert(std::is_integral<IntType>::value,
                "Template-argument 'IntType' must be an integral type, in "
                "basic::Poisson<IntType, URBG>(...)");

  using gen_t = basic::decay_t<URBG>;
  using distribution_t = typename basic::poisson_distribution<IntType>;
  using format_t = random_internal::DistributionFormatTraits<distribution_t>;
...
// -----------------------------------------------------------------------------
// basic::Zipf<T>(bitgen, hi = max, q = 2, v = 1)
// -----------------------------------------------------------------------------
//
// `basic::Zipf` produces discrete probabilities commonly used for modelling of
...
//   basic::BitGen bitgen;
//   ...
//   int term_rank = basic::Zipf<int>(bitgen);
//
template <typename IntType, typename URBG>
IntType Zipf(URBG&& urbg,  // NOLINT(runtime/references)
  ...
  static_assert(std::is_integral<IntType>::value,
                "Template-argument 'IntType' must be an integral type, in "
                "basic::Zipf<IntType, URBG>(...)");

  using gen_t = basic::decay_t<URBG>;
  using distribution_t = typename basic::zipf_distribution<IntType>;
  using format_t = random_internal::DistributionFormatTraits<distribution_t>;
