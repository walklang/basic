...
TYPED_TEST(LogUniformIntDistributionTypeTest, SerializeTest) {
  using param_type =
      typename basic::log_uniform_int_distribution<TypeParam>::param_type;
  ...
  for (const auto& param : {
    ...
    basic::log_uniform_int_distribution<TypeParam> before(min, max, base);
    EXPECT_EQ(before.min(), param.min());
    EXPECT_EQ(before.max(), param.max());
    EXPECT_EQ(before.base(), param.base());

    {
      basic::log_uniform_int_distribution<TypeParam> via_param(param);
    ...
    basic::log_uniform_int_distribution<TypeParam> after(3, 6, 17);
...
using log_uniform_i32 = basic::log_uniform_int_distribution<int32_t>;
...
double LogUniformIntChiSquaredTest::ChiSquaredTestImpl() {
  ...
  if (chi_square > threshold) {
    ...
    for (size_t i = 0; i < buckets.size(); i++) {
      ABSL_INTERNAL_LOG(INFO, basic::StrCat(i, ": ", buckets[i]));
    }
    ABSL_INTERNAL_LOG(INFO,
                      basic::StrFormat("trials=%d\n"
                                      "%s(data, %d) = %f (%f)\n"
                                      "%s @ 0.98 = %f",
...
std::string ParamName(
  ...
  std::string name =
      basic::StrCat("min_", p.min(), "__max_", p.max(), "__base_", p.base());
...
// NOTE: basic::log_uniform_int_distribution is not guaranteed to be stable.
TEST(LogUniformIntDistributionTest, StabilityTest) {
  using testing::ElementsAre;
  // basic::uniform_int_distribution stability relies on
  // basic::random_internal::LeadingSetBit, std::log, std::pow.
  ...
  {
    basic::log_uniform_int_distribution<int32_t> dist(0, 256);
  ...
  urbg.reset();
  {
    basic::log_uniform_int_distribution<int32_t> dist(0, 256, 10);
    std::generate(std::begin(output), std::end(output),
                  [&] { return dist(urbg); });
