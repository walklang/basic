...
#include "basic/random/internal/distribution_test_util.h"
...
TEST_F(RandomDistributionsTest, UniformBoundFunctions) {
  using basic::IntervalClosedClosed;
  using basic::IntervalClosedOpen;
  using basic::IntervalOpenClosed;
  using basic::IntervalOpenOpen;
  using basic::random_internal::uniform_lower_bound;
  using basic::random_internal::uniform_upper_bound;

  // basic::uniform_int_distribution natively assumes IntervalClosedClosed
  // basic::uniform_real_distribution natively assumes IntervalClosedOpen
...
auto InferredUniformReturnT(int)
    -> decltype(basic::Uniform(std::declval<basic::InsecureBitGen&>(),
...
auto InferredTaggedUniformReturnT(int)
    -> decltype(basic::Uniform(std::declval<TagType>(),
                              std::declval<basic::InsecureBitGen&>(),
                              std::declval<A>(), std::declval<B>()));
...
//
//   basic::Uniform(gen, A{}, B{})
//
// returns the type "Expect".
//
// This interface can also be used to assert that a given basic::Uniform()
// overload does not exist / will not compile. Given types <A, B>, the
// expression
//
//   decltype(basic::Uniform(..., std::declval<A>(), std::declval<B>()))
//
...
//
//   basic::Uniform(gen, 1.23f, std::numeric_limits<int>::max() - 1)
//
      ...
      basic::conjunction<
          std::is_same<Expect,
                       decltype(InferredTaggedUniformReturnT<
                                basic::random_internal::IntervalOpenOpenT, A, B>(
                           0))>,
          std::is_same<Expect,
                       decltype(InferredTaggedUniformReturnT<
                                basic::random_internal::IntervalOpenOpenT, B, A>(
                           0))>>::value,
      "");
}
...
auto ExplicitUniformReturnT(int) -> decltype(
    basic::Uniform<ExplicitRet>(*std::declval<basic::InsecureBitGen*>(),
                               std::declval<A>(), std::declval<B>()));
...
auto ExplicitTaggedUniformReturnT(int) -> decltype(basic::Uniform<ExplicitRet>(
    std::declval<TagType>(), *std::declval<basic::InsecureBitGen*>(),
    std::declval<A>(), std::declval<B>()));
...
TEST_F(RandomDistributionsTest, UniformTypeInference) {
  ...
  EXPECT_NE(1, basic::Uniform(gen, static_cast<uint16_t>(0), 1.0f));
  EXPECT_NE(1, basic::Uniform(gen, 0, 1.0));
  EXPECT_NE(1, basic::Uniform(basic::IntervalOpenOpen, gen,
                             static_cast<uint16_t>(0), 1.0f));
  EXPECT_NE(1, basic::Uniform(basic::IntervalOpenOpen, gen, 0, 1.0));
  EXPECT_NE(1, basic::Uniform(basic::IntervalOpenOpen, gen, -1, 1.0));
  EXPECT_NE(1, basic::Uniform<double>(basic::IntervalOpenOpen, gen, -1, 1));
  EXPECT_NE(1, basic::Uniform<float>(basic::IntervalOpenOpen, gen, 0, 1));
  EXPECT_NE(1, basic::Uniform<float>(gen, 0, 1));
}

TEST_F(RandomDistributionsTest, UniformNoBounds) {
  basic::InsecureBitGen gen;

  basic::Uniform<uint8_t>(gen);
  basic::Uniform<uint16_t>(gen);
  basic::Uniform<uint32_t>(gen);
  basic::Uniform<uint64_t>(gen);
}
...
TEST_F(RandomDistributionsTest, UniformReal) {
  ...
  for (int i = 0; i < kSize; i++) {
    values[i] = basic::Uniform(gen, 0, 1.0);
  }

  const auto moments =
      basic::random_internal::ComputeDistributionMoments(values);
...
TEST_F(RandomDistributionsTest, UniformInt) {
  ...
  for (int i = 0; i < kSize; i++) {
    ...
    int64_t j = basic::Uniform(basic::IntervalClosedClosed, gen, 0, kMax);
    // convert to double.
  ...
  /*
  // NOTE: These are not supported by basic::Uniform, which is specialized
  ...
  basic::Uniform(gen, E0, E1);
  basic::Uniform(gen, S0, S1);
  basic::Uniform(gen, U0, U1);
  */
}
...
TEST_F(RandomDistributionsTest, Exponential) {
  ...
  basic::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = basic::Exponential<double>(gen);
  }
...
TEST_F(RandomDistributionsTest, PoissonDefault) {
  ...
  basic::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = basic::Poisson<int64_t>(gen);
  }
...
TEST_F(RandomDistributionsTest, PoissonLarge) {
  ...
  basic::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = basic::Poisson<int64_t>(gen, kMean);
  }
...
TEST_F(RandomDistributionsTest, Bernoulli) {
  ...
  basic::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = basic::Bernoulli(gen, kP);
  }
...
TEST_F(RandomDistributionsTest, Beta) {
  ...
  basic::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = basic::Beta(gen, kAlpha, kBeta);
  }
...
TEST_F(RandomDistributionsTest, Zipf) {
  ...
  basic::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = basic::Zipf<int64_t>(gen, 100);
  }
...
TEST_F(RandomDistributionsTest, Gaussian) {
  ...
  basic::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = basic::Gaussian<double>(gen);
  }
...
TEST_F(RandomDistributionsTest, LogUniform) {
  ...
  basic::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = basic::LogUniform<int64_t>(gen, 0, (1 << 10) - 1);
  }
