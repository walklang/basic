...
#include "basic/strings/internal/charconv_bigint.h"
#include "basic/strings/internal/charconv_parse.h"
...
struct FloatTraits<double> {
  ...
  static double Make(uint64_t mantissa, int exponent, bool sign) {
    ...
    dbl += mantissa;
    return basic::bit_cast<double>(dbl);
...
struct FloatTraits<float> {
  ...
  static float Make(uint32_t mantissa, int exponent, bool sign) {
    ...
    flt += mantissa;
    return basic::bit_cast<float>(flt);
#endif  // ABSL_BIT_PACK_FLOATS
  }
};
...
void EncodeResult(const CalculatedFloat& calculated, bool negative,
                  basic::from_chars_result* result, FloatType* value) {
...
bool MustRoundUp(uint64_t guess_mantissa, int guess_exponent,
  ...
  basic::strings_internal::BigUnsigned<84> exact_mantissa;
  ...
  basic::strings_internal::BigUnsigned<84>& lhs = exact_mantissa;
  int comparison;
  if (exact_exponent >= 0) {
    lhs.MultiplyByFiveToTheNth(exact_exponent);
    basic::strings_internal::BigUnsigned<84> rhs(guess_mantissa);
    ...
    basic::strings_internal::BigUnsigned<84> rhs =
        basic::strings_internal::BigUnsigned<84>::FiveToTheNth(-exact_exponent);
