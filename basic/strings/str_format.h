...
// Example:
//
//   std::string s = basic::StrFormat(
//                      "%s %s You have $%d!", "Hello", name, dollars);
//
// The library consists of the following basic utilities:
//
//   * `basic::StrFormat()`, a type-safe replacement for `std::sprintf()`, to
//     write a format string to a `string` value.
//   * `basic::StrAppendFormat()` to append a format string to a `string`
//   * `basic::StreamFormat()` to more efficiently write a format string to a
//     stream, such as`std::cout`.
//   * `basic::PrintF()`, `basic::FPrintF()` and `basic::SNPrintF()` as
...
#include "basic/strings/internal/str_format/arg.h"  // IWYU pragma: export
#include "basic/strings/internal/str_format/bind.h"  // IWYU pragma: export
#include "basic/strings/internal/str_format/checker.h"  // IWYU pragma: export
#include "basic/strings/internal/str_format/extension.h"  // IWYU pragma: export
#include "basic/strings/internal/str_format/parser.h"  // IWYU pragma: export
...
// Example:
//
//   basic::UntypedFormatSpec format("%d");
//   std::string out;
//   CHECK(basic::FormatUntyped(&out, format, {basic::FormatArg(1)}));
...
// Example:
//
//   basic::StrFormat("%s", basic::FormatStreamed(obj));
template <typename T>
...
//
//   int n = 0;
//   std::string s = basic::StrFormat("%s%d%n", "hello", 123,
//                       basic::FormatCountCapture(&n));
//   EXPECT_EQ(8, n);
...
// either:
//
// * A `constexpr` literal or `basic::string_view`, which is how it most often
//   used.
...
//   basic::StrFormat("Welcome to %s, Number %d!", "The Village", 6);
//
//   // Provided as a constexpr basic::string_view.
//   constexpr basic::string_view formatString = "Welcome to %s, Number %d!";
//   basic::StrFormat(formatString, "The Village", 6);
//
//   // Provided as a pre-compiled ParsedFormat object.
//   // Note that this example is useful only for illustration purposes.
//   basic::ParsedFormat<'s', 'd'> formatString("Welcome to %s, Number %d!");
//   basic::StrFormat(formatString, "TheVillage", 6);
//
...
//     int n = 0;
//     std::string s = basic::StrFormat(
//         "%s%d%n", "hello", 123, basic::FormatCountCapture(&n));
//     EXPECT_EQ(8, n);
//
...
// any string-like argument, so `std::string`, `basic::string_view`, and
...
//
//   // Verified at runtime.
//   auto format_runtime = basic::ParsedFormat<'d'>::New(format_string);
//   if (format_runtime) {
//     value = basic::StrFormat(*format_runtime, i);
//   } else {
//     ... error case ...
//   }
template <char... Conv>
...
// `basic:: StrFormat()`. The returned object must be streamed before the full
// expression ends.
//
// Example:
//
...
// prefer `basic::PrintF()` over `std::printf()`.
//
// Example:
//
//   std::string_view s = "Ulaanbaatar";
//   basic::PrintF("The capital of Mongolia is %s", s);
//
...
// prefer `basic::FPrintF()` over `std::fprintf()`.
//
// Example:
//
//   std::string_view s = "Ulaanbaatar";
//   basic::FPrintF(stdout, "The capital of Mongolia is %s", s);
//
//   Outputs: "The capital of Mongolia is Ulaanbaatar"
//
template <typename... Args>
...
// type-safe); prefer `basic::SNPrintF()` over `std::snprintf()`.
//
// Example:
//
//   std::string_view s = "Ulaanbaatar";
//   char output[128];
//   basic::SNPrintF(output, sizeof(output),
...
// `basic::FormatRawSink` interface), using a format string and zero or more
...
//
// `basic::Format()` is a generic version of `basic::StrFormat(), for custom
...
//
// Writes a formatted string to an arbitrary sink object (implementing the
// `basic::FormatRawSink` interface), using an `UntypedFormatSpec` and zero or
...
// unspecified.
//
// The arguments are provided in an `basic::Span<const basic::FormatArg>`.
// Each `basic::FormatArg` object binds to a single argument and keeps a
...
//     std::string out;
//     std::vector<basic::FormatArg> args;
...
//       args.emplace_back(v);
//     }
//     basic::UntypedFormatSpec format(in_format);
//     if (!basic::FormatUntyped(&out, format, args)) {
//       return std::nullopt;
//     }
//     return std::move(out);
//   }
...
ABSL_MUST_USE_RESULT inline bool FormatUntyped(
    ...
    basic::Span<const FormatArg> args) {
  return str_format_internal::FormatUntyped(
      str_format_internal::FormatRawSinkImpl::Extract(raw_sink),
