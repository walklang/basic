...
// (they use basic::ascii_tolower instead of tolower).
//
...
#include "basic/base/port.h"  // disable some warnings on Windows
#include "basic/strings/ascii.h"  // for basic::ascii_tolower
...
const char* int_memmatch(const char* haystack, size_t haylen,
  ...
  for (; haystack < hayend; ++haystack) {
    char hay = case_sensitive
                   ? *haystack
                   : basic::ascii_tolower(static_cast<unsigned char>(*haystack));
    char nee = case_sensitive
                   ? *needle
                   : basic::ascii_tolower(static_cast<unsigned char>(*needle));
    if (hay == nee) {
      if (++needle == needleend) {
