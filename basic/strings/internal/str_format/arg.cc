...
#include "basic/strings/internal/str_format/float_conversion.h"
...
template <>
struct MakeUnsigned<basic::uint128> {
  using type = basic::uint128;
};

template <typename T>
struct IsSigned : std::is_signed<T> {};
template <>
struct IsSigned<basic::uint128> : std::false_type {};
...
IntegralConvertResult FormatConvertImpl(basic::uint128 v,
                                        const ConversionSpec conv,
                                        FormatSinkImpl *sink) {
  return {ConvertIntArg(v, conv, sink)};
}
