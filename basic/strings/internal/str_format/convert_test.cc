...
#include "basic/strings/internal/str_format/bind.h"
...
void TestStringConvert(const T& str) {
  ...
  for (const Expectation &e : kExpect) {
    ...
    EXPECT_EQ(e.out, FormatPack(format, basic::MakeSpan(args)));
  }
}
...
TEST_F(FormatConvertTest, Pointer) {
  ...
  for (const Expectation &e : kExpect) {
    UntypedFormatSpecImpl format(e.fmt);
    EXPECT_EQ(e.out, FormatPack(format, basic::MakeSpan(args))) << e.fmt;
  }
}
...
TEST_F(FormatConvertTest, Enum) {
  ...
  EXPECT_EQ("3", FormatPack(format, basic::MakeSpan(args)));
  EXPECT_EQ("-3", FormatPack(format2, basic::MakeSpan(args)));
}
      ...
      for (size_t fsi = 0; fsi < flag_sets.size(); ++fsi) {
        ...
        for (size_t wi = 0; wi < ArraySize(kWid); ++wi) {
          ...
          for (size_t pi = 0; pi < ArraySize(kPrec); ++pi) {
            ...
            EXPECT_EQ(old_result, FormatPack(format, basic::MakeSpan(args)));
          }
        }
      }
    }
...
TYPED_TEST_P(TypedFormatConvertTest, Char) {
  ...
  for (const T &c : kVals) {
    ...
    EXPECT_EQ(StrPrint("%c", c), FormatPack(format, basic::MakeSpan(args)));
  }
}
...
TEST_F(FormatConvertTest, VectorBool) {
  ...
  EXPECT_EQ("1,0,1,0",
            FormatPack(UntypedFormatSpecImpl("%d,%d,%d,%d"),
                       basic::Span<const FormatArgImpl>(
                           {FormatArgImpl(v[0]), FormatArgImpl(v[1]),
                            FormatArgImpl(cv[0]), FormatArgImpl(cv[1])})));
}

TEST_F(FormatConvertTest, Uint128) {
  basic::uint128 v = static_cast<basic::uint128>(0x1234567890abcdef) * 1979;
  basic::uint128 max = basic::Uint128Max();
  ...
  for (auto c : cases) {
    ...
    EXPECT_EQ(c.expected, FormatPack(format, basic::MakeSpan(args)));
  }
}
    ...
    for (char f : {'f', 'F',  //
      ...
      for (double d : doubles) {
        ...
        ASSERT_EQ(StrPrint(fmt_str.c_str(), d, i),
                  FormatPack(format, basic::MakeSpan(args)))
...
bool FormatFails(const char* test_format, T value) {
  ...
  EXPECT_EQ(FormatPack(format, basic::MakeSpan(args)), "")
      << "format=" << test_format << " value=" << value;
  return FormatPack(format, basic::MakeSpan(args)).empty();
}
