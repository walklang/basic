...
inline bool BindFromPosition(int position, int* value,
                             basic::Span<const FormatArgImpl> pack) {
  assert(position > 0);
...
class ArgContext {
 public:
  explicit ArgContext(basic::Span<const FormatArgImpl> pack) : pack_(pack) {}
 ...
 private:
  basic::Span<const FormatArgImpl> pack_;
};
...
class ConverterConsumer {
 public:
  ConverterConsumer(Converter converter, basic::Span<const FormatArgImpl> pack)
...
bool ConvertAll(const UntypedFormatSpecImpl format,
                basic::Span<const FormatArgImpl> args, Converter converter) {
...
std::string Summarize(const UntypedFormatSpecImpl format,
                      basic::Span<const FormatArgImpl> args) {
