...
// elements to be joined, using the same formatting that `basic::StrCat()` uses.
...
// objects. The separator string is specified as an `basic::string_view`.
//
// Because the default formatter uses the `basic::AlphaNum` class,
// `basic::StrJoin()`, like `basic::StrCat()`, will work out-of-the-box on
// collections of strings, ints, floats, doubles, etc.
//
// Example:
//
//   std::vector<std::string> v = {"foo", "bar", "baz"};
//   std::string s = basic::StrJoin(v, "-");
//   EXPECT_EQ("foo-bar-baz", s);
//
// See comments on the `basic::StrJoin()` function for more examples.
...
#include "basic/strings/internal/str_join_internal.h"
...
// functions. You may provide your own Formatter to enable `basic::StrJoin()` to
...
// argument to `basic::StrJoin()`:
//
//   std::vector<int> v = {1, 2, 3, 4};
//   std::string s = basic::StrJoin(v, "-", MyFormatter());
//   EXPECT_EQ("1-2-3-4", s);
//
...
// AlphaNumFormatter()
//
// Default formatter used if none is specified. Uses `basic::AlphaNum` to convert
...
// Function Template: PairFormatter(Formatter, basic::string_view, Formatter)
//
...
PairFormatter(FirstFormatter f1, basic::string_view sep, SecondFormatter f2) {
...
inline strings_internal::PairFormatterImpl<
    ...
    strings_internal::AlphaNumFormatterImpl>
PairFormatter(basic::string_view sep) {
...
// `basic::StrJoin()` takes a range, a separator string to use between the
...
//   // of `basic::string_view` or even `const char*`.
//   std::vector<std::string> v = {"foo", "bar", "baz"};
//   std::string s = basic::StrJoin(v, "-");
//   EXPECT_EQ("foo-bar-baz", s);
//
...
//   // of ints or `basic::string_view` -- any `AlphaNum`-compatible type.
//   std::string s = basic::StrJoin({"foo", "bar", "baz"}, "-");
//   EXPECT_EQ("foo-bar-baz", s);
//
// Example 3:
...
//       std::make_pair("b", 2),
//       std::make_pair("c", 3)};
//   std::string s = basic::StrJoin(m, ",", basic::PairFormatter("="));
//   EXPECT_EQ("a=1,b=2,c=3", s);
//
// Example 7:
//   // These examples show how `basic::StrJoin()` handles a few common edge
//   // cases:
//   std::vector<std::string> v_empty;
//   EXPECT_EQ("", basic::StrJoin(v_empty, "-"));
//
//   std::vector<std::string> v_one_item = {"foo"};
//   EXPECT_EQ("foo", basic::StrJoin(v_one_item, "-"));
//
//   std::vector<std::string> v_empty_string = {""};
//   EXPECT_EQ("", basic::StrJoin(v_empty_string, "-"));
//
//   std::vector<std::string> v_one_item_empty_string = {"a", ""};
//   EXPECT_EQ("a-", basic::StrJoin(v_one_item_empty_string, "-"));
//
//   std::vector<std::string> v_two_empty_string = {"", ""};
//   EXPECT_EQ("-", basic::StrJoin(v_two_empty_string, "-"));
//
// Example 8:
//   // Joins a `std::tuple<T...>` of heterogeneous types, converting each to
//   // a std::string using the `basic::AlphaNum` class.
//   std::string s = basic::StrJoin(std::make_tuple(123, "abc", 0.456), "-");
//   EXPECT_EQ("123-abc-0.456", s);

template <typename Iterator, typename Formatter>
std::string StrJoin(Iterator start, Iterator end, basic::string_view sep,
                    Formatter&& fmt) {
  return strings_internal::JoinAlgorithm(start, end, sep, fmt);
}

template <typename Range, typename Formatter>
std::string StrJoin(const Range& range, basic::string_view separator,
                    Formatter&& fmt) {
  return strings_internal::JoinRange(range, separator, fmt);
}

template <typename T, typename Formatter>
std::string StrJoin(std::initializer_list<T> il, basic::string_view separator,
                    Formatter&& fmt) {
  return strings_internal::JoinRange(il, separator, fmt);
}

template <typename... T, typename Formatter>
std::string StrJoin(const std::tuple<T...>& value, basic::string_view separator,
                    Formatter&& fmt) {
  return strings_internal::JoinAlgorithm(value, separator, fmt);
}

template <typename Iterator>
std::string StrJoin(Iterator start, Iterator end, basic::string_view separator) {
