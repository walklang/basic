...
#include "basic/base/dynamic_annotations.h"  // for RunningOnValgrind
...
TEST(Split, TraitsTest) {
  static_assert(!basic::strings_internal::SplitterIsConvertibleTo<int>::value,
                "");
  static_assert(
      !basic::strings_internal::SplitterIsConvertibleTo<std::string>::value, "");
  static_assert(basic::strings_internal::SplitterIsConvertibleTo<
                    std::vector<std::string>>::value,
                "");
  static_assert(
      !basic::strings_internal::SplitterIsConvertibleTo<std::vector<int>>::value,
      "");
  static_assert(basic::strings_internal::SplitterIsConvertibleTo<
                    std::vector<basic::string_view>>::value,
                "");
  static_assert(basic::strings_internal::SplitterIsConvertibleTo<
                    std::map<std::string, std::string>>::value,
                "");
  static_assert(basic::strings_internal::SplitterIsConvertibleTo<
                    std::map<basic::string_view, basic::string_view>>::value,
                "");
  static_assert(!basic::strings_internal::SplitterIsConvertibleTo<
...
// This tests the overall split API, which is made up of the basic::StrSplit()
// function and the Delimiter objects in the basic:: namespace.
...
TEST(Split, APIExamples) {
  {
    ...
    std::vector<std::string> v = basic::StrSplit("a,b,c", ",");  // NOLINT
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));

    // Equivalent to...
    using basic::ByString;
    v = basic::StrSplit("a,b,c", ByString(","));
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));

    // Equivalent to...
    EXPECT_THAT(basic::StrSplit("a,b,c", ByString(",")),
                ElementsAre("a", "b", "c"));
  }

  {
    // Same as above, but using a single character as the delimiter.
    std::vector<std::string> v = basic::StrSplit("a,b,c", ',');
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));

    // Equivalent to...
    using basic::ByChar;
    v = basic::StrSplit("a,b,c", ByChar(','));
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));
  }

  {
    // Uses the Literal std::string "=>" as the delimiter.
    const std::vector<std::string> v = basic::StrSplit("a=>b=>c", "=>");
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));
  }

  {
    // The substrings are returned as string_views, eliminating copying.
    std::vector<basic::string_view> v = basic::StrSplit("a,b,c", ',');
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));
  }
...

  {
    ...
    std::vector<std::string> v = basic::StrSplit("abc", ',');
...

  {
    ...
    // say that's the empty std::string when constructing the basic::string_view
    ...
    std::vector<std::string> v = basic::StrSplit(embedded_nulls, null_delim);
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));
  }

  {
    // Stores first two split strings as the members in a std::pair.
    std::pair<std::string, std::string> p = basic::StrSplit("a,b,c", ',');
    EXPECT_EQ("a", p.first);
...

  {
    ...
    std::set<std::string> v = basic::StrSplit("a,b,c,a,b,c,a,b,c", ',');
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));
  }

  {
    ...
    char a[] = ",";
    char* d = a + 0;
    std::vector<std::string> v = basic::StrSplit("a,b,c", d);
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));
  }

  {
    // Results split using either of , or ;
    using basic::ByAnyChar;
    std::vector<std::string> v = basic::StrSplit("a,b;c", ByAnyChar(",;"));
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));
  }

  {
    // Uses the SkipWhitespace predicate.
    using basic::SkipWhitespace;
    std::vector<std::string> v =
        basic::StrSplit(" a , ,,b,", ',', SkipWhitespace());
    EXPECT_THAT(v, ElementsAre(" a ", "b"));
  }

  {
    // Uses the ByLength delimiter.
    using basic::ByLength;
    std::vector<std::string> v = basic::StrSplit("abcdefg", ByLength(3));
    EXPECT_THAT(v, ElementsAre("abc", "def", "g"));
  }

  {
    // Different forms of initialization / conversion.
    std::vector<std::string> v1 = basic::StrSplit("a,b,c", ',');
    EXPECT_THAT(v1, ElementsAre("a", "b", "c"));
    std::vector<std::string> v2(basic::StrSplit("a,b,c", ','));
    EXPECT_THAT(v2, ElementsAre("a", "b", "c"));
    auto v3 = std::vector<std::string>(basic::StrSplit("a,b,c", ','));
    EXPECT_THAT(v3, ElementsAre("a", "b", "c"));
    v3 = basic::StrSplit("a,b,c", ',');
    EXPECT_THAT(v3, ElementsAre("a", "b", "c"));
  }

  {
    // Results stored in a std::map.
    std::map<std::string, std::string> m = basic::StrSplit("a,1,b,2,a,3", ',');
    EXPECT_EQ(2, m.size());
    EXPECT_EQ("3", m["a"]);
    EXPECT_EQ("2", m["b"]);
  }
...

  {
    ...
    for (basic::string_view sp : basic::StrSplit(s, ',')) {
      EXPECT_EQ("x", sp);
    }
  }

  {
    ...
    for (basic::string_view sp : basic::StrSplit(s, ',', SkipWhitespace())) {
      EXPECT_EQ("x", sp);
    }
  }

  {
    ...
    // basic::StrSplit. One call to split the records, and another call to split
    // the keys and values. This also uses the Limit delimiter so that the
    // std::string "a=b=c" will split to "a" -> "b=c".
    std::map<std::string, std::string> m;
    for (basic::string_view sp : basic::StrSplit("a=b=c,d=e,f=,g", ',')) {
      m.insert(basic::StrSplit(sp, basic::MaxSplits('=', 1)));
    }
...
TEST(SplitIterator, Basics) {
  auto splitter = basic::StrSplit("a,b", ',');
...
class Skip {
 public:
  ...
  bool operator()(basic::string_view sp) { return sp != s_; }
...
TEST(SplitIterator, Predicate) {
  auto splitter = basic::StrSplit("a,b,c", ',', Skip("b"));
  auto it = splitter.begin();
  auto end = splitter.end();
...
TEST(SplitIterator, EdgeCases) {
  ...
  for (const auto& spec : specs) {
    ...
    auto splitter = basic::StrSplit(spec.in, ',');
    auto it = splitter.begin();
    auto end = splitter.end();
...
TEST(Splitter, Const) {
  const auto splitter = basic::StrSplit("a,b,c", ',');
  EXPECT_THAT(splitter, ElementsAre("a", "b", "c"));
}

TEST(Split, EmptyAndNull) {
  // Attention: Splitting a null basic::string_view is different than splitting
  // an empty basic::string_view even though both string_views are considered
  ...
  EXPECT_THAT(basic::StrSplit(basic::string_view(""), '-'), ElementsAre(""));
  EXPECT_THAT(basic::StrSplit(basic::string_view(), '-'), ElementsAre());
}

TEST(SplitIterator, EqualityAsEndCondition) {
  auto splitter = basic::StrSplit("a,b,c", ',');
  auto it = splitter.begin();
  auto it2 = it;
  ...
  std::vector<basic::string_view> v;
  for (; it != it2; ++it) {
    v.push_back(*it);
  }
  EXPECT_THAT(v, ElementsAre("a", "b"));
...
TEST(Splitter, RangeIterators) {
  auto splitter = basic::StrSplit("a,b,c", ',');
  std::vector<basic::string_view> output;
  for (const basic::string_view p : splitter) {
    output.push_back(p);
  }
...
TEST(Splitter, ConversionOperator) {
  auto splitter = basic::StrSplit("a,b,c,d", ',');

  TestConversionOperator<std::vector<basic::string_view>>(splitter);
  TestConversionOperator<std::vector<std::string>>(splitter);
  TestConversionOperator<std::list<basic::string_view>>(splitter);
  TestConversionOperator<std::list<std::string>>(splitter);
  TestConversionOperator<std::deque<basic::string_view>>(splitter);
  TestConversionOperator<std::deque<std::string>>(splitter);
  TestConversionOperator<std::set<basic::string_view>>(splitter);
  TestConversionOperator<std::set<std::string>>(splitter);
  TestConversionOperator<std::multiset<basic::string_view>>(splitter);
  TestConversionOperator<std::multiset<std::string>>(splitter);
  TestConversionOperator<std::unordered_set<std::string>>(splitter);

  // Tests conversion to map-like objects.

  TestMapConversionOperator<std::map<basic::string_view, basic::string_view>>(
      splitter);
  TestMapConversionOperator<std::map<basic::string_view, std::string>>(splitter);
  TestMapConversionOperator<std::map<std::string, basic::string_view>>(splitter);
  TestMapConversionOperator<std::map<std::string, std::string>>(splitter);
  TestMapConversionOperator<
      std::multimap<basic::string_view, basic::string_view>>(splitter);
  TestMapConversionOperator<std::multimap<basic::string_view, std::string>>(
      splitter);
  TestMapConversionOperator<std::multimap<std::string, basic::string_view>>(
      splitter);
  ...
  TestPairConversionOperator<basic::string_view, basic::string_view>(splitter);
  TestPairConversionOperator<basic::string_view, std::string>(splitter);
  TestPairConversionOperator<std::string, basic::string_view>(splitter);
...
TEST(Splitter, ToPair) {
  {
    // Empty std::string
    std::pair<std::string, std::string> p = basic::StrSplit("", ',');
    EXPECT_EQ("", p.first);
    EXPECT_EQ("", p.second);
  }
  ...
  {
    // Only first
    std::pair<std::string, std::string> p = basic::StrSplit("a", ',');
    EXPECT_EQ("a", p.first);
    EXPECT_EQ("", p.second);
  }
  ...
  {
    // Only second
    std::pair<std::string, std::string> p = basic::StrSplit(",b", ',');
    EXPECT_EQ("", p.first);
    EXPECT_EQ("b", p.second);
  }
  ...
  {
    // First and second.
    std::pair<std::string, std::string> p = basic::StrSplit("a,b", ',');
    EXPECT_EQ("a", p.first);
    EXPECT_EQ("b", p.second);
  }
...
TEST(Splitter, Predicates) {
  ...
  using basic::AllowEmpty;
  using basic::SkipEmpty;
  using basic::SkipWhitespace;

  {
    // No predicate. Does not skip empties.
    auto splitter = basic::StrSplit(kTestChars, ',');
...

  {
    ...
    auto splitter = basic::StrSplit(kTestChars, ',', AllowEmpty());
    ...
    auto splitter_nopredicate = basic::StrSplit(kTestChars, ',');
  ...
  {
    // Skips empty strings.
    auto splitter = basic::StrSplit(kTestChars, ',', SkipEmpty());
    std::vector<std::string> v = splitter;
    EXPECT_THAT(v, ElementsAre("a", " ", "b"));
  }

  {
    ...
    auto splitter = basic::StrSplit(kTestChars, ',', SkipWhitespace());
    std::vector<std::string> v = splitter;
    EXPECT_THAT(v, ElementsAre("a", "b"));
  }
}
...
TEST(Split, Basics) {
...

  {
    ...
    std::vector<std::string> v;
    v = basic::StrSplit("a,b,c", ',');

    EXPECT_THAT(v, ElementsAre("a", "b", "c"));
    std::map<std::string, std::string> m;
    m = basic::StrSplit("a,b,c", ',');
    EXPECT_EQ(2, m.size());
    std::unordered_map<std::string, std::string> hm;
    hm = basic::StrSplit("a,b,c", ',');
    EXPECT_EQ(2, hm.size());
  }
}

basic::string_view ReturnStringView() { return "Hello World"; }
...
TEST(Split, AcceptsCertainTemporaries) {
  std::vector<std::string> v;
  v = basic::StrSplit(ReturnStringView(), ' ');
  EXPECT_THAT(v, ElementsAre("Hello", "World"));
  v = basic::StrSplit(ReturnConstCharP(), ' ');
  EXPECT_THAT(v, ElementsAre("Hello", "World"));
  v = basic::StrSplit(ReturnCharP(), ' ');
  EXPECT_THAT(v, ElementsAre("Hello", "World"));
}
...
TEST(Split, Temporary) {
  ...
  auto splitter = basic::StrSplit(std::string(input), ',');
  std::string expected = "a";
  for (basic::string_view letter : splitter) {
  ...
  // This happens more often in C++11 as part of a range-based for loop.
  auto std_splitter = basic::StrSplit(std::string(input), ',');
  expected = "a";
  for (basic::string_view letter : std_splitter) {
    EXPECT_EQ(expected, letter);
    ++expected[0];
  }
  EXPECT_EQ("v", expected);
...
TEST(Split, LvalueCaptureIsCopyable) {
  ...
  auto heap_splitter = CopyToHeap(basic::StrSplit(input, ','));
...
TEST(Split, TemporaryCaptureIsCopyable) {
  auto heap_splitter = CopyToHeap(basic::StrSplit(std::string("a,b"), ','));
  auto stack_splitter = *heap_splitter;
  heap_splitter.reset();
  std::vector<std::string> result = stack_splitter;
  EXPECT_THAT(result, testing::ElementsAre("a", "b"));
...
TEST(Split, StringDelimiter) {
  {
    std::vector<basic::string_view> v = basic::StrSplit("a,b", ',');
    EXPECT_THAT(v, ElementsAre("a", "b"));
  }

  {
    std::vector<basic::string_view> v = basic::StrSplit("a,b", std::string(","));
    EXPECT_THAT(v, ElementsAre("a", "b"));
  }

  {
    std::vector<basic::string_view> v =
        basic::StrSplit("a,b", basic::string_view(","));
    EXPECT_THAT(v, ElementsAre("a", "b"));
  }
}
...
TEST(Split, UTF8) {
  ...
  std::string utf8_string = u8"\u03BA\u1F79\u03C3\u03BC\u03B5";
  {
    ...
    std::vector<basic::string_view> v = basic::StrSplit(to_split, ',');
...

  {
    ...
    std::vector<basic::string_view> v =
        basic::StrSplit(to_split, unicode_delimiter);
    EXPECT_THAT(v, ElementsAre("a", "b"));
  }

  {
    ...
    std::vector<basic::string_view> v =
        basic::StrSplit(u8"Foo h\u00E4llo th\u4E1Ere", basic::ByAnyChar(" \t"));
...
TEST(Split, EmptyStringDelimiter) {
  {
    std::vector<std::string> v = basic::StrSplit("", "");
    EXPECT_THAT(v, ElementsAre(""));
  }

  {
    std::vector<std::string> v = basic::StrSplit("a", "");
    EXPECT_THAT(v, ElementsAre("a"));
  }

  {
    std::vector<std::string> v = basic::StrSplit("ab", "");
    EXPECT_THAT(v, ElementsAre("a", "b"));
  }

  {
    std::vector<std::string> v = basic::StrSplit("a b", "");
    EXPECT_THAT(v, ElementsAre("a", " ", "b"));
  }
}

TEST(Split, SubstrDelimiter) {
  std::vector<basic::string_view> results;
  basic::string_view delim("//");

  results = basic::StrSplit("", delim);
  EXPECT_THAT(results, ElementsAre(""));

  results = basic::StrSplit("//", delim);
  EXPECT_THAT(results, ElementsAre("", ""));

  results = basic::StrSplit("ab", delim);
  ...
  results = basic::StrSplit("a/b", delim);
...
TEST(Split, EmptyResults) {
  ...
  results = basic::StrSplit("", '#');
  EXPECT_THAT(results, ElementsAre(""));

  results = basic::StrSplit("#", '#');
  EXPECT_THAT(results, ElementsAre("", ""));

  results = basic::StrSplit("#cd", '#');
  EXPECT_THAT(results, ElementsAre("", "cd"));

  results = basic::StrSplit("ab#cd#", '#');
  EXPECT_THAT(results, ElementsAre("ab", "cd", ""));

  results = basic::StrSplit("ab##cd", '#');
  EXPECT_THAT(results, ElementsAre("ab", "", "cd"));

  results = basic::StrSplit("ab##", '#');
  EXPECT_THAT(results, ElementsAre("ab", "", ""));

  results = basic::StrSplit("ab#ab#", '#');
  EXPECT_THAT(results, ElementsAre("ab", "ab", ""));

  results = basic::StrSplit("aaaa", 'a');
  EXPECT_THAT(results, ElementsAre("", "", "", "", ""));

  results = basic::StrSplit("", '#', basic::SkipEmpty());
  EXPECT_THAT(results, ElementsAre());
}

template <typename Delimiter>
static bool IsFoundAtStartingPos(basic::string_view text, Delimiter d,
                                 size_t starting_pos, int expected_pos) {
  basic::string_view found = d.Find(text, starting_pos);
...
template <typename Delimiter>
static bool IsFoundAt(basic::string_view text, Delimiter d, int expected_pos) {
...
TEST(Delimiter, ByString) {
  ...
  // There is a test below that demonstrates this for basic::string_view::find().
...
TEST(Delimiter, ByAnyChar) {
  ...
  // std::string. That is, it always returns a zero-length basic::string_view
...
TEST(Split, WorksWithLargeStrings) {
  if (sizeof(size_t) > 4) {
    ...
    s.back() = '-';
    std::vector<basic::string_view> v = basic::StrSplit(s, '-');
    EXPECT_EQ(2, v.size());
...
TEST(SplitInternalTest, TypeTraits) {
  EXPECT_FALSE(basic::strings_internal::HasMappedType<int>::value);
  EXPECT_TRUE(
      (basic::strings_internal::HasMappedType<std::map<int, int>>::value));
  EXPECT_FALSE(basic::strings_internal::HasValueType<int>::value);
  EXPECT_TRUE(
      (basic::strings_internal::HasValueType<std::map<int, int>>::value));
  EXPECT_FALSE(basic::strings_internal::HasConstIterator<int>::value);
  EXPECT_TRUE(
      (basic::strings_internal::HasConstIterator<std::map<int, int>>::value));
  EXPECT_FALSE(basic::strings_internal::IsInitializerList<int>::value);
  EXPECT_TRUE((basic::strings_internal::IsInitializerList<
