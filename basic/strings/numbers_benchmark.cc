...
void BM_FastIntToBuffer(benchmark::State& state) {
  ...
  char buf[basic::numbers_internal::kFastToBufferSize];
  // Use the unsigned type to increment to take advantage of well-defined
  // modular arithmetic.
  typename std::make_unsigned<T>::type x = 0;
  for (auto _ : state) {
    basic::numbers_internal::FastIntToBuffer(static_cast<T>(x), buf);
    x += inc;
  }
}
...
void BM_safe_strto32_string(benchmark::State& state) {
  ...
  for (auto _ : state) {
    benchmark::DoNotOptimize(
        basic::numbers_internal::safe_strto32_base(str, &value, base));
  }
...
void BM_safe_strto64_string(benchmark::State& state) {
  ...
  for (auto _ : state) {
    benchmark::DoNotOptimize(
        basic::numbers_internal::safe_strto64_base(str, &value, base));
  }
  ABSL_RAW_CHECK(value == RepeatedSevens(digits, base), "");
}
...
void BM_safe_strtou32_string(benchmark::State& state) {
  ...
  for (auto _ : state) {
    benchmark::DoNotOptimize(
        basic::numbers_internal::safe_strtou32_base(str, &value, base));
  }
  ABSL_RAW_CHECK(value == RepeatedSevens(digits, base), "");
}
...
void BM_safe_strtou64_string(benchmark::State& state) {
  ...
  for (auto _ : state) {
    benchmark::DoNotOptimize(
        basic::numbers_internal::safe_strtou64_base(str, &value, base));
  }
  ABSL_RAW_CHECK(value == RepeatedSevens(digits, base), "");
}
...
void BM_SimpleAtof(benchmark::State& state) {
  ...
  for (auto _ : state) {
    for (const T& input : inputs) {
      benchmark::DoNotOptimize(basic::SimpleAtof(input, &value));
    }
  }
}
BENCHMARK_TEMPLATE(BM_SimpleAtof, basic::string_view)
    ->ArgPair(10, 1)
    ->ArgPair(10, 2)
    ->ArgPair(10, 4)
    ->ArgPair(10, 8);
...
void BM_SimpleAtod(benchmark::State& state) {
  ...
  for (auto _ : state) {
    for (const T& input : inputs) {
      benchmark::DoNotOptimize(basic::SimpleAtod(input, &value));
    }
  }
}
BENCHMARK_TEMPLATE(BM_SimpleAtod, basic::string_view)
    ->ArgPair(10, 1)
    ->ArgPair(10, 2)
    ->ArgPair(10, 4)
    ->ArgPair(10, 8);
