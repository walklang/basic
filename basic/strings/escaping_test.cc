...
#include "basic/strings/internal/escaping_test_common.h"
...
TEST(CEscape, EscapeAndUnescape) {
  ...
  for (int kind = 0; kind < 4; kind++) {
    for (const std::string& original : inputs) {
      ...
      switch (kind) {
        case 0:
          escaped = basic::CEscape(original);
          break;
        case 1:
          escaped = basic::CHexEscape(original);
          break;
        case 2:
          escaped = basic::Utf8SafeCEscape(original);
          break;
        case 3:
          escaped = basic::Utf8SafeCHexEscape(original);
          break;
      }
      std::string unescaped_str;
      EXPECT_TRUE(basic::CUnescape(escaped, &unescaped_str));
      EXPECT_EQ(unescaped_str, original);

      // Check in-place unescaping
      std::string s = escaped;
      EXPECT_TRUE(basic::CUnescape(s, &s));
  ...
  for (int char0 = 0; char0 < 256; char0++) {
    for (int char1 = 0; char1 < 256; char1++) {
      ...
      std::string escaped = basic::CHexEscape(s);
      std::string unescaped;
      EXPECT_TRUE(basic::CUnescape(escaped, &unescaped));
...
TEST(CEscape, BasicEscaping) {
  ...
  for (const epair& val : oct_values) {
    std::string escaped = basic::CEscape(val.unescaped);
    EXPECT_EQ(escaped, val.escaped);
  }
  for (const epair& val : hex_values) {
    std::string escaped = basic::CHexEscape(val.unescaped);
    EXPECT_EQ(escaped, val.escaped);
  }
  for (const epair& val : utf8_oct_values) {
    std::string escaped = basic::Utf8SafeCEscape(val.unescaped);
    EXPECT_EQ(escaped, val.escaped);
  }
  for (const epair& val : utf8_hex_values) {
    std::string escaped = basic::Utf8SafeCHexEscape(val.unescaped);
    EXPECT_EQ(escaped, val.escaped);
  }
}
...
TEST(Unescape, BasicFunction) {
  ...
  for (const epair& val : tests) {
    std::string out;
    EXPECT_TRUE(basic::CUnescape(val.escaped, &out));
  ...
  for (const std::string& e : bad) {
    std::string error;
    std::string out;
    EXPECT_FALSE(basic::CUnescape(e, &out, &error));
...
TEST_F(CUnescapeTest, Unescapes1CharOctalNull) {
  ...
  EXPECT_TRUE(basic::CUnescape(original_string, &result_string_));
...
static struct {
  basic::string_view plaintext;
  basic::string_view cyphertext;
} const base64_tests[] = {
    // Empty std::string.
    {{"", 0}, {"", 0}},
    {{nullptr, 0},
...
void TestEscapeAndUnescape() {
  ...
  for (const auto& tc : base64_tests) {
    ...
    basic::Base64Escape(tc.plaintext, &encoded);
    EXPECT_EQ(encoded, tc.cyphertext);
    EXPECT_EQ(basic::Base64Escape(tc.plaintext), tc.cyphertext);

    StringType decoded("this junk should be ignored");
    EXPECT_TRUE(basic::Base64Unescape(encoded, &decoded));
    ...
    basic::WebSafeBase64Escape(tc.plaintext, &encoded);
    EXPECT_EQ(encoded, websafe);
    EXPECT_EQ(basic::WebSafeBase64Escape(tc.plaintext), websafe);

    // Let's try the std::string version of the decoder
    decoded = "this junk should be ignored";
    EXPECT_TRUE(basic::WebSafeBase64Unescape(websafe, &decoded));
    EXPECT_EQ(decoded, tc.plaintext);
  }

  // Now try the long strings, this tests the streaming
  for (const auto& tc : basic::strings_internal::base64_strings()) {
    StringType buffer;
    basic::WebSafeBase64Escape(tc.plaintext, &buffer);
    EXPECT_EQ(tc.cyphertext, buffer);
    EXPECT_EQ(basic::WebSafeBase64Escape(tc.plaintext), tc.cyphertext);
  }

  // Verify the behavior when decoding bad data
  {
    basic::string_view data_set[] = {"ab-/", basic::string_view("\0bcd", 4),
                                    basic::string_view("abc.\0", 5)};
    for (basic::string_view bad_data : data_set) {
      StringType buf;
      EXPECT_FALSE(basic::Base64Unescape(bad_data, &buf));
      EXPECT_FALSE(basic::WebSafeBase64Unescape(bad_data, &buf));
...
TEST(Base64, DISABLED_HugeData) {
  ...
  std::string escaped;
  basic::Base64Escape(huge, &escaped);
  ...
  std::string unescaped;
  EXPECT_TRUE(basic::Base64Unescape(escaped, &unescaped));
...
TEST(HexAndBack, HexStringToBytes_and_BytesToHexString) {
  ...
  std::string bytes_result = basic::HexStringToBytes(hex_mixed);
  EXPECT_EQ(bytes_expected, bytes_result);

  std::string prefix_valid = hex_mixed + "?";
  std::string prefix_valid_result = basic::HexStringToBytes(
      basic::string_view(prefix_valid.data(), prefix_valid.size() - 1));
  EXPECT_EQ(bytes_expected, prefix_valid_result);

  std::string infix_valid = "?" + hex_mixed + "???";
  std::string infix_valid_result = basic::HexStringToBytes(
      basic::string_view(infix_valid.data() + 1, hex_mixed.size()));
  EXPECT_EQ(bytes_expected, infix_valid_result);

  std::string hex_result = basic::BytesToHexString(bytes_expected);
