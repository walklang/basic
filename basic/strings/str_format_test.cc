...
TEST_F(FormatEntryPointTest, UntypedFormat) {
  ...
  for (const char* fmt : formats) {
    ...
    basic::Span<const FormatArgImpl> args(&arg_123, 1);
...
TEST_F(FormatEntryPointTest, StringFormat) {
  ...
  constexpr basic::string_view view("=%d=", 4);
...
// On Android platform <=21, there is a regression in hexfloat formatting.
  ...
  EXPECT_EQ(basic::StrFormat("%2$s, %3$s, %1$s!", "vici", "veni", "vidi"),
            "veni, vidi, vici!");
...
std::string WrappedFormat(const basic::FormatSpec<Args...>& format,
                          const Args&... args) {
