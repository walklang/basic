...
#include "basic/strings/internal/numbers_test_common.h"
#include "basic/strings/internal/pow10_helper.h"

namespace {

using basic::numbers_internal::kSixDigitsToBufferSize;
using basic::numbers_internal::safe_strto32_base;
using basic::numbers_internal::safe_strto64_base;
using basic::numbers_internal::safe_strtou32_base;
using basic::numbers_internal::safe_strtou64_base;
using basic::numbers_internal::SixDigitsToBuffer;
using basic::strings_internal::Itoa;
using basic::strings_internal::strtouint32_test_cases;
using basic::strings_internal::strtouint64_test_cases;
using basic::SimpleAtoi;
using testing::Eq;
...
void CheckInt32(int32_t x) {
  char buffer[basic::numbers_internal::kFastToBufferSize];
  char* actual = basic::numbers_internal::FastIntToBuffer(x, buffer);
  std::string expected = std::to_string(x);
  EXPECT_EQ(expected, std::string(buffer, actual)) << " Input " << x;

  char* generic_actual = basic::numbers_internal::FastIntToBuffer(x, buffer);
  EXPECT_EQ(expected, std::string(buffer, generic_actual)) << " Input " << x;
}

void CheckInt64(int64_t x) {
  char buffer[basic::numbers_internal::kFastToBufferSize + 3];
  buffer[0] = '*';
  buffer[23] = '*';
  buffer[24] = '*';
  char* actual = basic::numbers_internal::FastIntToBuffer(x, &buffer[1]);
  std::string expected = std::to_string(x);
  ...
  char* my_actual =
      basic::numbers_internal::FastIntToBuffer(MyInt64(x), &buffer[1]);
...
void CheckUInt64(uint64_t x) {
  char buffer[basic::numbers_internal::kFastToBufferSize + 1];
  char* actual = basic::numbers_internal::FastIntToBuffer(x, &buffer[1]);
  std::string expected = std::to_string(x);
  EXPECT_EQ(expected, std::string(&buffer[1], actual)) << " Input " << x;

  char* generic_actual = basic::numbers_internal::FastIntToBuffer(x, &buffer[1]);
  EXPECT_EQ(expected, std::string(&buffer[1], generic_actual))
      << " Input " << x;

  char* my_actual =
      basic::numbers_internal::FastIntToBuffer(MyUInt64(x), &buffer[1]);
  EXPECT_EQ(expected, std::string(&buffer[1], my_actual)) << " Input " << x;
}

void CheckHex64(uint64_t v) {
  char expected[16 + 1];
  std::string actual = basic::StrCat(basic::Hex(v, basic::kZeroPad16));
...
void VerifySimpleAtoiGood(in_val_type in_value, int_type exp_value) {
  std::string s = basic::StrCat(in_value);
...
TEST(NumbersTest, Atoi) {
  // SimpleAtoi(basic::string_view, int32_t)
  ...
  // SimpleAtoi(basic::string_view, uint32_t)
  ...
  // SimpleAtoi(basic::string_view, int64_t)
  ...
  // SimpleAtoi(basic::string_view, uint64_t)
...
void test_random_integer_parse_base(bool (*parse_func)(basic::string_view,
                                                       IntType* value,
                                                       int base)) {
  ...
  for (size_t i = 0; i < kNumRandomTests; i++) {
    ...
    // Test overflow
    EXPECT_FALSE(
        parse_func(basic::StrCat(std::numeric_limits<IntType>::max(), value),
                   &parsed_value, base));

    // Test underflow
    if (std::numeric_limits<IntType>::min() < 0) {
      EXPECT_FALSE(
          parse_func(basic::StrCat(std::numeric_limits<IntType>::min(), value),
                     &parsed_value, base));
    } else {
      EXPECT_FALSE(parse_func(basic::StrCat("-", value), &parsed_value, base));
    }
  }
}
...
TEST(stringtest, safe_strtou32_base_length_delimited) {
  for (int i = 0; strtouint32_test_cases()[i].str != nullptr; ++i) {
    ...
    EXPECT_EQ(e.expect_ok,
              safe_strtou32_base(basic::string_view(tmp.data(), strlen(e.str)),
                                 &value, e.base))
        << "str=\"" << e.str << "\" base=" << e.base;
    if (e.expect_ok) {
      EXPECT_EQ(e.expected, value) << "i=" << i << " str=" << e.str
...
TEST(stringtest, safe_strtou64_base_length_delimited) {
  for (int i = 0; strtouint64_test_cases()[i].str != nullptr; ++i) {
    ...
    uint64_t value;
    EXPECT_EQ(e.expect_ok,
              safe_strtou64_base(basic::string_view(tmp.data(), strlen(e.str)),
                                 &value, e.base))
        << "str=\"" << e.str << "\" base=" << e.base;
    if (e.expect_ok) {
      EXPECT_EQ(e.expected, value) << "str=\"" << e.str << "\" base=" << e.base;
...
TEST_F(SimpleDtoaTest, ExhaustiveDoubleToSixDigits) {
    ...
    if (strcmp(sixdigitsbuf, snprintfbuf) != 0) {
      ...
      if (mismatches.size() < 10) {
        ABSL_RAW_LOG(ERROR, "%s",
                     basic::StrCat("Six-digit failure with double.  ", "d=", d,
  ...
  if (mismatches.empty()) {
    ...
    for (int exponent = -324; exponent <= 308; ++exponent) {
      double powten = basic::strings_internal::Pow10(exponent);
      ...
      if (kFloatNumCases >= 1e9) {
        ...
        ABSL_RAW_LOG(
            INFO, "%s",
            basic::StrCat("Exp ", exponent, " powten=", powten, "(", powten,
                         ") (",
...
TEST(StrToInt32, Partial) {
  ...
  Int32TestLine int32_test_line[] = {
      ...
      {"-", false, 0},
      {"123@@@", false, 123},
      {basic::StrCat(int32_min, int32_max), false, int32_min},
      {basic::StrCat(int32_max, int32_max), false, int32_max},
  };
  ...
  for (const Int32TestLine& test_line : int32_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = -2;
    status = safe_strto32_base(basic::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToUint32, Partial) {
  ...
  Uint32TestLine uint32_test_line[] = {
      ...
      {"-", false, 0},
      {"123@@@", false, 123},
      {basic::StrCat(uint32_max, uint32_max), false, uint32_max},
  };
  ...
  for (const Uint32TestLine& test_line : uint32_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = 2;
    status = safe_strtou32_base(basic::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToInt64, Partial) {
  ...
  Int64TestLine int64_test_line[] = {
      ...
      {"-", false, 0},
      {"123@@@", false, 123},
      {basic::StrCat(int64_min, int64_max), false, int64_min},
      {basic::StrCat(int64_max, int64_max), false, int64_max},
  };
  ...
  for (const Int64TestLine& test_line : int64_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = -2;
    status = safe_strto64_base(basic::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToUint64, Partial) {
  ...
  Uint64TestLine uint64_test_line[] = {
      ...
      {"-", false, 0},
      {"123@@@", false, 123},
      {basic::StrCat(uint64_max, uint64_max), false, uint64_max},
  };
  ...
  for (const Uint64TestLine& test_line : uint64_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = 2;
    status = safe_strtou64_base(basic::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToInt32Base, PrefixOnly) {
  ...
  for (const Int32TestLine& line : int32_test_line) {
    for (const int base : base_array) {
      ...
      EXPECT_EQ(line.value, value) << line.input << " " << base;
      value = 2;
      status = safe_strto32_base(basic::string_view(line.input), &value, base);
      EXPECT_EQ(line.status, status) << line.input << " " << base;
      EXPECT_EQ(line.value, value) << line.input << " " << base;
    }
  }
...
TEST(StrToUint32Base, PrefixOnly) {
  ...
  for (const Uint32TestLine& line : uint32_test_line) {
    for (const int base : base_array) {
      ...
      EXPECT_EQ(line.value, value) << line.input << " " << base;
      value = 2;
      status = safe_strtou32_base(basic::string_view(line.input), &value, base);
      EXPECT_EQ(line.status, status) << line.input << " " << base;
      EXPECT_EQ(line.value, value) << line.input << " " << base;
    }
  }
...
TEST(StrToInt64Base, PrefixOnly) {
  ...
  for (const Int64TestLine& line : int64_test_line) {
    for (const int base : base_array) {
      ...
      EXPECT_EQ(line.value, value) << line.input << " " << base;
      value = 2;
      status = safe_strto64_base(basic::string_view(line.input), &value, base);
      EXPECT_EQ(line.status, status) << line.input << " " << base;
      EXPECT_EQ(line.value, value) << line.input << " " << base;
    }
  }
...
TEST(StrToUint64Base, PrefixOnly) {
  ...
  for (const Uint64TestLine& line : uint64_test_line) {
    for (const int base : base_array) {
      ...
      EXPECT_EQ(line.value, value) << line.input << " " << base;
      value = 2;
      status = safe_strtou64_base(basic::string_view(line.input), &value, base);
      EXPECT_EQ(line.status, status) << line.input << " " << base;
      EXPECT_EQ(line.value, value) << line.input << " " << base;
    }
  }
