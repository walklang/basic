...
#include "basic/strings/charconv.h"
...
using basic::strings_internal::Pow10;

#if ABSL_COMPILER_DOES_EXACT_ROUNDING

// Tests that the given string is accepted by basic::from_chars, and that it
// converts exactly equal to the given number.
void TestDoubleParse(basic::string_view str, double expected_number) {
  SCOPED_TRACE(str);
  double actual_number = 0.0;
  basic::from_chars_result result =
      basic::from_chars(str.data(), str.data() + str.length(), actual_number);
  EXPECT_EQ(result.ec, std::errc());
  EXPECT_EQ(result.ptr, str.data() + str.length());
  EXPECT_EQ(actual_number, expected_number);
}

void TestFloatParse(basic::string_view str, float expected_number) {
  SCOPED_TRACE(str);
...
// parsed correctly by basic::from_chars.
//
...
float ToFloat(basic::string_view s) {
  float f;
  basic::from_chars(s.data(), s.data() + s.size(), f);
  return f;
}

double ToDouble(basic::string_view s) {
  double d;
  basic::from_chars(s.data(), s.data() + s.size(), d);
  return d;
}
...
void TestHalfwayValue(const std::string& mantissa, int exponent,
  ...
  basic::StrAppend(&low_rep, std::string(1000, '9'), "e", exponent);

  FloatType actual_low = 0;
  basic::from_chars(low_rep.data(), low_rep.data() + low_rep.size(), actual_low);
  EXPECT_EQ(expected_low, actual_low);

  std::string high_rep =
      basic::StrCat(mantissa, std::string(1000, '0'), "1e", exponent);
  FloatType actual_high = 0;
  basic::from_chars(high_rep.data(), high_rep.data() + high_rep.size(),
                   actual_high);
  EXPECT_EQ(expected_high, actual_high);

  std::string halfway_rep = basic::StrCat(mantissa, "e", exponent);
  FloatType actual_half = 0;
  basic::from_chars(halfway_rep.data(), halfway_rep.data() + halfway_rep.size(),
                   actual_half);
...
TEST(FromChars, Underflow) {
  ...
  d = 100.0;
  result = basic::from_chars(begin, end, d);
  ...
  f = 100.0;
  result = basic::from_chars(begin, end, f);
  EXPECT_EQ(result.ptr, end);
  EXPECT_EQ(result.ec, std::errc::result_out_of_range);
...
TEST(FromChars, TestVersusStrtod) {
  for (int mantissa = 1000000; mantissa <= 9999999; mantissa += 501) {
    for (int exponent = -300; exponent < 300; ++exponent) {
      std::string candidate = basic::StrCat(mantissa, "e", exponent);
      double strtod_value = strtod(candidate.c_str(), nullptr);
      double basic_value = 0;
      basic::from_chars(candidate.data(), candidate.data() + candidate.size(),
                       basic_value);
      ASSERT_EQ(strtod_value, basic_value) << candidate;
    }
  }
}

// Check for a wide range of inputs that strtof() and basic::from_chars() exactly
// agree on the conversion amount.
//
...
TEST(FromChars, NaNDoubles) {
  for (std::string n_char_sequence :
    ...
    std::string input = basic::StrCat("nan(", n_char_sequence, ")");
    SCOPED_TRACE(input);
    double from_chars_double;
    basic::from_chars(input.data(), input.data() + input.size(),
                     from_chars_double);
    ...
    basic::from_chars(negative_input.data(),
...
void TestOverflowAndUnderflow(
  ...
  for (index = lower_bound, step = 1; index < upper_bound;
    ...
    Float actual;
    auto result =
        basic::from_chars(input.data(), input.data() + input.size(), actual);
    EXPECT_EQ(result.ec, std::errc());
    EXPECT_EQ(expected, actual)
        << basic::StrFormat("%a vs %a", expected, actual);
  }
...
TEST(FromChars, HexdecimalDoubleLimits) {
  auto input_gen = [](int index) { return basic::StrCat("0x1.0p", index); };
...
TEST(FromChars, DecimalDoubleLimits) {
  auto input_gen = [](int index) { return basic::StrCat("1.0e", index); };
