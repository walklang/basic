...
bool NonInlinedEq(basic::string_view a, basic::string_view b) { return a == b; }
...
ABSL_ATTRIBUTE_NOINLINE
void DoEqualityComparisons(benchmark::State& state, basic::string_view a,
                           basic::string_view b) {
  for (auto _ : state) {
...
// made with basic::string_view comparisons against constant strings. The idea is
...
void BM_CompareSame(benchmark::State& state) {
  ...
  std::string y = x;
  basic::string_view a = x;
  basic::string_view b = y;
...
struct EasyMap : public std::map<basic::string_view, uint64_t> {
...
void StringViewMapBenchmark(benchmark::State& state) {
  ...
  for (int i = 0; i < table_size; i++) {
    ...
    do {
      ...
      for (int j = 0; j < WordsPerKey; j++) {
        basic::StrAppend(&keys[i], j > 0 ? " " : "", words[uniform(rng)]);
      }
  ...
  ABSL_RAW_CHECK(
      num_cold * kNumLookupsOfColdKeys + num_hot * kNumLookupsOfHotKeys ==
          indices.size(),
      "");
  // After constructing the array we probe it with basic::string_views built from
