...
void BM_Split2StringView(benchmark::State& state) {
  ...
  for (auto _ : state) {
    std::vector<basic::string_view> result = basic::StrSplit(test, ';');
...
void BM_Split2StringViewLifted(benchmark::State& state) {
  std::string test = MakeTestString(state.range(0));
  std::vector<basic::string_view> result;
  for (auto _ : state) {
    result = basic::StrSplit(test, ';');
  }
  benchmark::DoNotOptimize(result);
}
...
void BM_Split2String(benchmark::State& state) {
  std::string test = MakeTestString(state.range(0));
  for (auto _ : state) {
    std::vector<std::string> result = basic::StrSplit(test, ';');
    benchmark::DoNotOptimize(result);
  }
}
...
void BM_Split2SplitStringUsing(benchmark::State& state) {
  ...
  for (auto _ : state) {
    std::vector<std::string> result =
        basic::StrSplit(test, ';', basic::SkipEmpty());
    benchmark::DoNotOptimize(result);
  }
}
...
void BM_SplitStringWithOneChar(benchmark::State& state) {
  ...
  std::vector<basic::string_view> pieces;
  size_t v = 0;
  for (auto _ : state) {
    pieces = basic::StrSplit("The quick brown fox jumps over the lazy dog",
                            delimiter);
    v += pieces.size();
  }
