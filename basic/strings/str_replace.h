...
// -----------------------------------------------------------------------------
//
// This file defines `basic::StrReplaceAll()`, a general-purpose string
...
// vary, `basic::Substitute()` may be a better choice.
//
// Example:
//
// std::string html_escaped = basic::StrReplaceAll(user_input, {
//                                                {"&", "&amp;"},
//                                                {"<", "&lt;"},
//                                                {">", "&gt;"},
//                                                {"\"", "&quot;"},
...
// Example:
//
//   std::string s = basic::StrReplaceAll(
//       "$who bought $count #Noun. Thanks $who!",
//       {{"$count", basic::StrCat(5)},
//        {"$who", "Bob"},
//        {"#Noun", "Apples"}});
//   EXPECT_EQ("Bob bought 5 Apples. Thanks Bob!", s);
ABSL_MUST_USE_RESULT std::string StrReplaceAll(
    basic::string_view s,
    std::initializer_list<std::pair<basic::string_view, basic::string_view>>
        replacements);
...
// Examples:
//
//   std::map<const basic::string_view, const basic::string_view> replacements;
...
//   std::vector<std::pair<const basic::string_view, std::string>> replacements;
//   replacements.push_back({"&", "&amp;"});
//   replacements.push_back({"<", "&lt;"});
//   replacements.push_back({">", "&gt;"});
//   std::string s = basic::StrReplaceAll("if (ptr < &foo)",
//                                  replacements);
//   EXPECT_EQ("if (ptr &lt; &amp;foo)", s);
template <typename StrToStrMapping>
std::string StrReplaceAll(basic::string_view s,
...
struct ViableSubstitution {
  basic::string_view old;
  basic::string_view replacement;
  size_t offset;

  ViableSubstitution(basic::string_view old_str,
                     basic::string_view replacement_str, size_t offset_val)
...
std::vector<ViableSubstitution> FindSubstitutions(
    basic::string_view s, const StrToStrMapping& replacements) {
  std::vector<ViableSubstitution> subs;
  subs.reserve(replacements.size());

  for (const auto& rep : replacements) {
    using std::get;
    basic::string_view old(get<0>(rep));
...
int ApplySubstitutions(basic::string_view s,
