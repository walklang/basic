...
void BM_WebSafeBase64Escape_string(benchmark::State& state) {
  std::string raw;
  for (int i = 0; i < 10; ++i) {
    for (const auto& test_set : basic::strings_internal::base64_strings()) {
  ...
  std::string escaped;
  for (auto _ : state) {
    basic::WebSafeBase64Escape(raw, &escaped);
  }
  ...
  basic::WebSafeBase64Unescape(escaped, &round_trip);
...
void CEscapeBenchmarkHelper(benchmark::State& state, const char* string_value,
  ...
  while (src.size() < max_len) {
    basic::StrAppend(&src, string_value);
  }
