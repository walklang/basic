...
TEST(StrJoin, APIExamples) {
  {
    ...
    EXPECT_EQ("foo-bar-baz", basic::StrJoin(v, "-"));
  }

  {
    // Collection of basic::string_view
    std::vector<basic::string_view> v = {"foo", "bar", "baz"};
    EXPECT_EQ("foo-bar-baz", basic::StrJoin(v, "-"));
  }
...

  {
    ...
    EXPECT_EQ("1-2-3--4", basic::StrJoin(v, "-"));
  }

  {
    // Literals passed as a std::initializer_list
    std::string s = basic::StrJoin({"a", "b", "c"}, "-");
    EXPECT_EQ("a-b-c", s);
  }
  {
    // Join a std::tuple<T...>.
    std::string s = basic::StrJoin(std::make_tuple(123, "abc", 0.456), "-");
...

  {
    ...
    EXPECT_EQ("1-2-3", basic::StrJoin(v, "-"));
  }

  {
    // Array of ints
    const int a[] = {1, 2, 3, -4};
    EXPECT_EQ("1-2-3--4", basic::StrJoin(a, a + ABSL_ARRAYSIZE(a), "-"));
  }
...

  {
    ...
    EXPECT_EQ("a-b", basic::StrJoin(v, "-"));
  }

  {
    // A std::map, which is a collection of std::pair<>s.
    std::map<std::string, int> m = {{"a", 1}, {"b", 2}, {"c", 3}};
    EXPECT_EQ("a=1,b=2,c=3", basic::StrJoin(m, ",", basic::PairFormatter("=")));
  }

  {
    // Shows basic::StrSplit and basic::StrJoin working together. This example is
    // equivalent to s/=/-/g.
    const std::string s = "a=b=c=d";
    EXPECT_EQ("a-b-c-d", basic::StrJoin(basic::StrSplit(s, "="), "-"));
  }
...

  {
    ...
    EXPECT_EQ("1-0-1", basic::StrJoin(v, "-"));
  }
}
...
TEST(StrJoin, CustomFormatter) {
  ...
  {
    std::string joined =
        basic::StrJoin(v, "", [](std::string* out, const std::string& in) {
          basic::StrAppend(out, "(", in, ")");
        });
  ...
  }
  {
    ...
    EXPECT_EQ("(One)(Two)(Three)", basic::StrJoin(v, "", ImmovableFormatter()));
  }
  {
    ...
    EXPECT_EQ("(One)(Two)(Three)", basic::StrJoin(v, "", OverloadedFormatter()));
    const OverloadedFormatter fmt = {};
    EXPECT_EQ("[One][Two][Three]", basic::StrJoin(v, "", fmt));
  }
}
...
TEST(AlphaNumFormatter, FormatterAPI) {
  ...
  auto f = basic::AlphaNumFormatter();
  std::string s;
  f(&s, "Testing: ");
...
TEST(PairFormatter, FormatterAPI) {
  {
    ...
    const auto f = basic::PairFormatter("=");
    std::string s;
...

  {
    ...
    auto f = basic::PairFormatter(TestingParenFormatter(), "=",
...
TEST(DereferenceFormatter, FormatterAPI) {
  {
    ...
    const basic::strings_internal::DereferenceFormatterImpl<
        basic::strings_internal::AlphaNumFormatterImpl>
        f;
    int x = 1, y = 2, z = 3;
    std::string s;
    f(&s, &x);
...

  {
    ...
    basic::strings_internal::DereferenceFormatterImpl<
        basic::strings_internal::DefaultFormatter<std::string>::Type>
        f;
...

  {
    ...
    auto f = basic::DereferenceFormatter(TestingParenFormatter());
    int x = 1, y = 2, z = 3;
    std::string s;
    f(&s, &x);
    f(&s, &y);
...
TEST(StrJoin, PublicAPIOverloads) {
  ...
  // Iterators + formatter
  EXPECT_EQ("a-b-c",
            basic::StrJoin(v.begin(), v.end(), "-", basic::AlphaNumFormatter()));
  // Range + formatter
  EXPECT_EQ("a-b-c", basic::StrJoin(v, "-", basic::AlphaNumFormatter()));
  // Iterators, no formatter
  EXPECT_EQ("a-b-c", basic::StrJoin(v.begin(), v.end(), "-"));
  // Range, no formatter
  EXPECT_EQ("a-b-c", basic::StrJoin(v, "-"));
}

TEST(StrJoin, Array) {
  const basic::string_view a[] = {"a", "b", "c"};
  EXPECT_EQ("a-b-c", basic::StrJoin(a, "-"));
}

TEST(StrJoin, InitializerList) {
  { EXPECT_EQ("a-b-c", basic::StrJoin({"a", "b", "c"}, "-")); }
  ...
  {
    std::initializer_list<basic::string_view> a = {"a", "b", "c"};
    EXPECT_EQ("a-b-c", basic::StrJoin(a, "-"));
  }

  {
    ...
    EXPECT_EQ("(a)-(b)-(c)", basic::StrJoin(a, "-", f));
  }

  {
    // initializer_list of ints
    EXPECT_EQ("1-2-3", basic::StrJoin({1, 2, 3}, "-"));
  }

  {
    ...
    auto a = {1, 2, 3};
    TestingParenFormatter f;
    EXPECT_EQ("(1)-(2)-(3)", basic::StrJoin(a, "-", f));
  }
}

TEST(StrJoin, Tuple) {
  EXPECT_EQ("", basic::StrJoin(std::make_tuple(), "-"));
  EXPECT_EQ("hello", basic::StrJoin(std::make_tuple("hello"), "-"));

  int x(10);
  std::string y("hello");
  double z(3.14);
  EXPECT_EQ("10-hello-3.14", basic::StrJoin(std::make_tuple(x, y, z), "-"));

  // Faster! Faster!!
  EXPECT_EQ("10-hello-3.14",
            basic::StrJoin(std::make_tuple(x, std::cref(y), z), "-"));
  ...
  EXPECT_EQ("0x0000000a-hell-3.",
            basic::StrJoin(std::make_tuple(x, y, z), "-", TestFormatter()));
  EXPECT_EQ(
      "0x0000000a-hell-3.",
      basic::StrJoin(std::make_tuple(x, std::cref(y), z), "-", TestFormatter()));
  EXPECT_EQ("0x0000000a-hell-3.",
            basic::StrJoin(std::make_tuple(&x, &y, &z), "-",
                          basic::DereferenceFormatter(TestFormatter())));
  EXPECT_EQ("0x0000000a-hell-3.",
            basic::StrJoin(std::make_tuple(basic::make_unique<int>(x),
                                          basic::make_unique<std::string>(y),
                                          basic::make_unique<double>(z)),
                          "-", basic::DereferenceFormatter(TestFormatter())));
  EXPECT_EQ("0x0000000a-hell-3.",
            basic::StrJoin(std::make_tuple(basic::make_unique<int>(x), &y, &z),
                          "-", basic::DereferenceFormatter(TestFormatter())));
}
