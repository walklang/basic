...
void BM_Sum_By_StrCat(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    strcpy(foo, basic::StrCat(kStringOne, i, kStringTwo, i * 65536ULL).c_str());
    int sum = 0;
...
void BM_StrCat_By_Strings(benchmark::State& state) {
  ...
  for (auto _ : state) {
    std::string result =
        std::string(kStringOne) + " " + kStringTwo + ":" + basic::StrCat(i);
...
void BM_StrCat_By_StrCat(benchmark::State& state) {
  int i = 0;
  for (auto _ : state) {
    std::string result = basic::StrCat(kStringOne, " ", kStringTwo, ":", i);
    benchmark::DoNotOptimize(result);
    i = IncrementAlternatingSign(i);
  }
}
...
void BM_HexCat_By_StrCat(benchmark::State& state) {
  ...
  for (auto _ : state) {
    std::string result =
        basic::StrCat(kStringOne, " ", basic::Hex(int64_t{i} + 0x10000000));
    benchmark::DoNotOptimize(result);
    i = IncrementAlternatingSign(i);
  }
}
...
void BM_HexCat_By_Substitute(benchmark::State& state) {
  int i = 0;
  for (auto _ : state) {
    std::string result = basic::Substitute(
...
void BM_FloatToString_By_StrCat(benchmark::State& state) {
  ...
  float foo = 0.0f;
  for (auto _ : state) {
    std::string result = basic::StrCat(foo += 1.001f, " != ", int64_t{i});
    benchmark::DoNotOptimize(result);
    i = IncrementAlternatingSign(i);
  }
}
...
void BM_DoubleToString_By_SixDigits(benchmark::State& state) {
  ...
  for (auto _ : state) {
    std::string result =
        basic::StrCat(basic::SixDigits(foo += 1.001), " != ", int64_t{i});
    benchmark::DoNotOptimize(result);
    i = IncrementAlternatingSign(i);
  }
}
