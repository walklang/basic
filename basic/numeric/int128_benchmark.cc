...
std::vector<std::pair<basic::uint128, basic::uint128>>
GetRandomClass128SampleUniformDivisor() {
  std::vector<std::pair<basic::uint128, basic::uint128>> values;
  ...
  for (size_t i = 0; i < kSampleSize; ++i) {
    ...
    values.emplace_back(std::max(a, b),
                        std::max(basic::uint128(2), std::min(a, b)));
  }
  return values;
}
...
std::vector<std::pair<basic::uint128, uint64_t>>
GetRandomClass128SampleSmallDivisor() {
  std::vector<std::pair<basic::uint128, uint64_t>> values;
  std::mt19937 random = MakeRandomEngine();
  std::uniform_int_distribution<uint64_t> uniform_uint64;
  values.reserve(kSampleSize);
  for (size_t i = 0; i < kSampleSize; ++i) {
    ...
    values.emplace_back(std::max(a, basic::uint128(b)), b);
  }
  return values;
}
...
std::vector<std::pair<basic::uint128, basic::uint128>> GetRandomClass128Sample() {
  std::vector<std::pair<basic::uint128, basic::uint128>> values;
  std::mt19937 random = MakeRandomEngine();
  std::uniform_int_distribution<uint64_t> uniform_uint64;
  values.reserve(kSampleSize);
